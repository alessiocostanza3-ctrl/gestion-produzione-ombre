            ...
            <details class="ordine-group ${b?"all-done":""}" ${b?"":"open"}>
                <summary class="ordine-group-summary">
                    <span class="og-left">
                        ...
                        <span class="og-data">${Ns(p.data)}</span>
    ...
    <div class="ordine-item ${o?"is-ordinato":""}" id="oi-${t.id_riga}" data-search="${String(t.articolo).toLowerCase()} ${String(t.fornitore).toLowerCase()}">
        ...
        <span class="oi-stato-badge ${o?"badge-ordinato-sm":"badge-attesa-sm"}">${o?'<i class="fas fa-circle-check"></i> ORDINATO':"IN ATTESA"}</span>
            ...
            <div id="lista-materiali-grid">`,d=window.innerWidth<=768;if(j.forEach((u,m)=>{let p=c[u]||[],f=s(u);l+=`
                ...
                <div class="materiale-card ${zt.card}" data-idx="${b}" data-search="${(g+" "+E+" "+A).toLowerCase().replace(/"/g,"")}">
                ...
                <div class="order-info">
                    ...
                    <div class="stato-dropdown stato-dropdown-ord" data-nordine="${T}">
                        <button type="button" class="stato-trigger" onclick="event.stopPropagation(); _toggleStatoDropdownOF(this)" title="Cambia stato">
                            ...
                            <span class="stato-label-txt">${w(C)}</span>
        ...
        </div>`}),m}function pr(t){let e=w(t.codice||"-"),o=w(hr(t.prodotto,60)),i=w(t.prodotto||"-"),n=w(t.fornitore||"-"),a=w(t.n_ordine||"-"),r=w(t.data_consegna||"-"),s=t.quantita||0,c=t.qta_evasa||0,l=t.qta_da_consegnare||0,d=br(t.importo),u=h?.nome?.toUpperCase().trim()==="ALESSIO",m=s>0?Math.round(c/s*100):0,p=m===100?"#22c55e":m>0?"#f59e0b":"#94a3b8",f=`data-codice="${e}" data-prodotto="${i}" data-fornitore="${n}" data-ordine="${a}" data-data="${r}" data-qty="${s}" data-evasa="${c}" data-daconsegnare="${l}"${u?` data-importo="${d}"`:""}`;return`<div class="item-card of-item-card${u?"":" of-item-card-no-importo"}" onclick="_apriDettaglioOF(this)" ${f}>
...
' + document.documentElement.outerHTML;
    ...
    <\/script>
        ...
        <div class="fabprod-sel-modal-box">
            ...
            ${ze(a)}
        `}catch(o){console.error("[richieste] getFabbisogni error:",o),e&&(e.innerHTML='<div class="empty-msg" style="margin:16px 0">Errore caricamento fabbisogni.</div>'),v("Errore caricamento fabbisogni.","error")}}function Uo(){document.getElementById("fabprod-archivio-modal")?.remove()}async function Br(t){let e=String(t||"").trim();if(e)try{let o=await yt({azione:"getFabbisognoById",id:e});if(o?.status!=="ok"||!o?.item)throw new Error(o?.message||"Snapshot non trovato");let i=o.item;Kt=String(i.id||e),Xt=Array.isArray(i.rows)?i.rows.map(function(n){return{codice:String(n?.codice||"").trim(),prodotto:String(n?.prodotto||"").trim(),qty:Number(n?.qtyTotale??n?.qty??0)||0,formulaQty:String(n?.formulaQty||""),ordini:Array.isArray(n?.ordini)?n.ordini:[]}}):[],W=new Set(Array.isArray(i.ordini)?i.ordini.map(function(n){return String(n||"").trim()}).filter(Boolean):[]),Uo(),Dt(),Ho(),v("Snapshot fabbisogno caricato.","success")}catch(o){console.error("[richieste] getFabbisognoById error:",o),v("Errore apertura snapshot fabbisogno.","error")}}async function Ur(t){let e=String(t||"").trim();if(!(!e||!await et("Archiviare questo fabbisogno condiviso?",{title:"Archivia snapshot",confirmText:"Archivia",cancelText:"Annulla",confirmType:"danger"})))try{let i=await yt({azione:"archiveFabbisogno",id:e});if(i?.status!=="ok")throw new Error(i?.message||"Archiviazione non riuscita");Yt=Yt.filter(function(a){return String(a.id||"")!==e});let n=document.getElementById("fabprod-archivio-body");n&&(n.innerHTML=ze(Yt)),Kt===e&&Ue(),Dt(),v("Snapshot archiviato.","success")}catch(i){console.error("[richieste] archiveFabbisogno error:",i),v("Errore archiviazione snapshot.","error")}}function Dt(){let t=document.getElementById("fabprod-list"),e=document.getElementById("fabprod-sel-badge"),o=document.getElementById("fabprod-cnt-badge");if(!t)return;let i=Array.isArray(Xt)?Xt:Cn(Ne.filter(s=>!s||!s.ordine?!1:W.has(String(s.ordine).trim())));i.length?(window._fabprodCurrentRows=i,t.innerHTML=i.map((s,c)=>{let l=s.ordini.map(u=>{let m=u.ordine.replace(/\\/g,"\\\\").replace(/'/g,"\\'"),p=(u.cliente||"").replace(/\\/g,"\\\\").replace(/'/g,"\\'");return`<span class="fabprod-order-pill fabprod-order-pill--click" onclick="event.stopPropagation();_fabprodApriModalOrdine('${m}','${p}')">ORD. ${u.ordine}${u.cliente?`<span class="fabprod-pill-cliente"> \xB7 ${u.cliente}</span>`:""}</span>`}).join(""),d=s.ordini.length>1?`<div class="fabprod-qty-breakdown">${s.ordini.map(u=>`${ht(u.qty)}\xA0(${u.ordine})`).join(" + ")}</div>`:"";return`<div class="fabprod-card" onclick="_fabprodCardClick(${c})">
        ...
        <div class="fabprod-sel-modal-box">
            ...
            <button onclick="cambiaPagina('STORICO_RICHIESTE', null)"
                ...
                &#x21bb; Riprova
                    ...
                    </div>`:'<div class="empty-msg" style="margin:16px 0 8px">Nessun articolo attivo per gli ordini selezionati.</div>':(window._fabprodCurrentRows=P,P.map((x,R)=>{let L=x.ordini.map(B=>{let nt=B.ordine.replace(/\\/g,"\\\\").replace(/'/g,"\\'"),Lt=(B.cliente||"").replace(/\\/g,"\\\\").replace(/'/g,"\\'");return`<span class="fabprod-order-pill fabprod-order-pill--click" onclick="event.stopPropagation();_fabprodApriModalOrdine('${nt}','${Lt}')">ORD. ${B.ordine}${B.cliente?`<span class="fabprod-pill-cliente"> \xB7 ${B.cliente}</span>`:""}</span>`}).join(""),M=x.ordini.length>1?`<div class="fabprod-qty-breakdown">${x.ordini.map(B=>`${ht(B.qty)}\xA0(${B.ordine})`).join(" + ")}</div>`:"";return`
                <div class="fabprod-card" onclick="_fabprodCardClick(${R})">
                    ...
                    ${M}
                    <div class="fabprod-orders">${L}</div>
                </div>`}).join("")),ct="";Object.keys(m).length===0?ct='<div class="empty-msg" style="margin:20px 0">Nessuna richiesta archiviata.</div>':Object.keys(m).reverse().forEach(P=>{ct+=Mo(m[P],a,!0)});let tt=`
            ...
            <div class="req-groups">
                ...
                <details id="rg-assegnazioni" class="req-group" ${b?"open":""}
                    ...
                    <div class="chat-inbox">${C(p,a)}</div>
                </details>
                ...
                <details id="rg-richieste" class="req-group" ${g?"open":""}
                    ...
                    <div class="chat-inbox">${C(f,a)}</div>
                </details>
        ...
        <div class="req-card${o?" archiviata":""}${s?" sollecitata":""}" data-id-riga="${String(i.id_riga||"")}" data-ordine="${String(a||"")}" data-cliente="${(r||"").toLowerCase().replace(/"/g,"")}" data-riferimento="${(i.RIFERIMENTO||"").toLowerCase().replace(/"/g,"")}">
            ...
            <div id="rc-body-${i.id_riga}" class="rc-body">
                ${t.map(y=>{let b=String(y.DA).toUpperCase().trim()===e,g=String(y.MESSAGGIO||"").includes("|")?y.MESSAGGIO.split("|")[1]:y.MESSAGGIO,E=y["DATA ORA"]?Be(y["DATA ORA"]):"";return`
    ...
    <article class="manuale-card materiale-card ${window.TW?.card||""} !p-0 overflow-hidden" data-codice="${w((t.titolo||"")+" "+(t.categoria||""))}">
        ...
        <div class="p-4">
            ...
            <div class="mt-3 text-xs text-slate-600 space-y-1">
                ...
                <p><b>Aggiornato:</b> ${w(Ve(t.updatedAt))}</p>
    ...
    </div>`}function ie(t){return`<button type="button" class="manuali-modal-close" onclick="${t}" aria-label="Chiudi" title="Chiudi"><i class="fas fa-times"></i></button>`}function Wo(t,e){let o=document.getElementById("manuali-modal-host");if(!o)return;let i=Je(e),n=i?i.schedaTecnica||[]:[],a=i?i.occorrente||[]:[],r=i?i.procedimenti||[]:Array.isArray(e?.steps)?e.steps.map(function(g){return{descrizione:g.descrizione||g.titolo||"",foto:g.foto||""}}):[],s=i&&i.disegnoTecnico?.foto||"",c={},l=[];n.forEach(function(g){Bn.includes(g.voce)?c[g.voce]=g.valore:l.push(g)});let d=0,u=Bn.map(function(g){return ic(g,c[g]||"",d++)}).join("")+l.map(function(g){return Fn(g.voce,g.valore,d++)}).join(""),m=a.length>0?a.map(function(g,E){return Vo(g,E)}).join(""):Vo(null,0),p=r.length>0?r.map(function(g,E){return Jo(g,E)}).join(""):Jo(null,0),f=K(e?.copertina||""),y=f?`<img id="manuali-copertina-preview" src="${f}" alt="copertina" style="max-width:100%;max-height:200px;border-radius:10px;border:1px solid #e2e8f0;display:block;margin-bottom:6px">`:'<div id="manuali-copertina-preview" class="text-xs text-slate-400" style="margin-bottom:6px">Nessuna copertina</div>',b=(g,E)=>`<h3 style="font-weight:700;font-size:.8rem;text-transform:uppercase;letter-spacing:.05em;color:#64748b;margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid #e2e8f0"><i class="${g}"></i> &nbsp;${E}</h3>`;o.innerHTML=`
    ...
    <div style="background:#fff;border-radius:16px;padding:28px 32px;max-width:380px;width:90%;box-shadow:0 20px 40px rgba(0,0,0,0.18);text-align:center">
        ...
        <div style="display:flex;gap:10px;justify-content:center">
            ...
            <button id="manuali-confirm-si" class="btn-modal-send" style="min-width:100px;background:#ef4444">Elimina</button>
    ...
    </div>`,document.body.appendChild(i),document.getElementById("manuali-confirm-no").onclick=function(){i.remove()},document.getElementById("manuali-confirm-si").onclick=function(){i.remove(),e()},i.addEventListener("click",function(n){n.target===i&&i.remove()})}function Vn(t,e,o,i,n){let a=document.querySelector(t);a&&a.querySelectorAll(e).forEach(function(r,s){r.setAttribute(o,String(s));let c=r.querySelector("button");if(c&&c.setAttribute("onclick",`${i}(${s})`),n){let l=r.querySelector('input[type="file"]');l&&l.setAttribute("onchange",`${n}(this, ${s})`)}})}function gc(){let t=document.getElementById("manuali-scheda-edit");if(!t)return;let e=t.querySelectorAll(".scheda-row").length;if(e>=tc){v("Numero massimo voci raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",Fn("","",e))}function vc(t){ne("Eliminare questa voce dalla scheda tecnica?",function(){let e=document.querySelector(`.scheda-row[data-row-idx="${t}"]`);e&&e.remove(),Vn("#manuali-scheda-edit",".scheda-row","data-row-idx","rimuoviSchedaRow",null)})}function hc(){let t=document.getElementById("manuali-occorrente-edit");if(!t)return;let e=t.querySelectorAll(".occorrente-item").length;if(e>=ec){v("Numero massimo elementi raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",Vo({lettera:"ABCDEFGHIJKLMNOPQRSTUVWXYZ"[e]||"",nome:"",codice:"",foto:""},e))}function bc(t){ne("Eliminare questo elemento dal materiale occorrente?",function(){let e=document.querySelector(`.occorrente-item[data-item-idx="${t}"]`);e&&e.remove(),Vn("#manuali-occorrente-edit",".occorrente-item","data-item-idx","rimuoviOccorrenteItem","cambiaFotoOccorrente")})}function yc(){let t=document.getElementById("manuali-proc-edit");if(!t)return;let e=t.querySelectorAll(".proc-step").length;if(e>=Xr){v("Numero massimo step raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",Jo(null,e))}function wc(t){ne("Eliminare questo step del procedimento?",function(){let e=document.querySelector(`.proc-step[data-step-idx="${t}"]`);e&&e.remove();let o=document.getElementById("manuali-proc-edit");o&&o.querySelectorAll(".proc-step").forEach(function(i,n){i.setAttribute("data-step-idx",String(n));let a=i.querySelector("h4");a&&(a.textContent="Step "+(n+1));let r=i.querySelector("button");r&&r.setAttribute("onclick",`rimuoviProcStep(${n})`);let s=i.querySelector('input[type="file"]');s&&s.setAttribute("onchange",`cambiaFotoProcedimento(this, ${n})`)})})}async function Sc(){let t=String(document.getElementById("manuali-titolo")?.value||"").trim(),e=String(document.getElementById("manuali-categoria")?.value||"").trim(),o=String(document.getElementById("manuali-copertina-wrap")?.getAttribute("data-copertina")||"").trim(),i=ac();if(!t){v("Inserisci un titolo manuale.","warning");return}try{v("Salvataggio manuale in corso...","info");let n;if(je?n=await Nn({id:je,titolo:t,categoria:e,copertina:o,sections:i}):n=await Mn({titolo:t,categoria:e,copertina:o,sections:i}),!n||n.status!=="ok")throw new Error(n&&(n.message||n.msg)||"Errore salvataggio manuale");await q.invalidate(jo),delete window.cacheContenuti?.MANUALI_PRODOTTI,delete window.cacheFetchTime?.MANUALI_PRODOTTI,Gn(),await Ee(null,null,!1),v("Manuale salvato correttamente.","success")}catch(n){let a=n&&n.name==="TimeoutError"?"Il salvataggio sta impiegando troppo tempo. Riprova con meno immagini.":n&&n.name==="AbortError"?"Richiesta interrotta. Controlla la connessione e riprova.":n&&n.message||"Errore durante il salvataggio.";v(a,"error")}}function _c(t){let e=Je(t),o=e?e.schedaTecnica||[]:[],i=e?e.occorrente||[]:[],n=e?e.procedimenti||[]:Array.isArray(t.steps)?t.steps.map(function(a){return{descrizione:String(a.descrizione||a.titolo||"").trim(),foto:String(a.foto||"").trim(),foto2:""}}):[];return{titolo:String(t.titolo||"(Senza titolo)"),categoria:String(t.categoria||"Generale"),versione:Number(t.version||1),aggiornato:Ve(t.updatedAt),copertina:K(t.copertina),schedaTecnica:Array.isArray(o)?o:[],occorrente:Array.isArray(i)?i:[],procedimenti:Array.isArray(n)?n:[],disegnoTecnico:K(e?.disegnoTecnico?.foto||"")}}function ee(t){return`<div class="man-print-placeholder">${w(t||"Sezione non disponibile")}</div>`}function Ec(t){let e=_c(t),o=e.copertina?`<img class="man-cover-image" src="${e.copertina}" alt="copertina manuale">`:ee("Copertina non disponibile"),i=e.schedaTecnica.map(function(s){return`<tr>
                <td>${w(s?.voce||"")}</td>
                <td>${w(s?.valore||"")}</td>
                ...
                ${l}
            </article>`}).join(""),a=e.procedimenti.map(function(s,c){let l=K(s?.foto||""),d=K(s?.foto2||""),u=[l,d].filter(Boolean),m=u.length?`<div class="man-proc-photos">${u.map(function(p){return`<img src="${p}" alt="step-${c+1}">`}).join("")}</div>`:ee("Immagini step non disponibili");return`<article class="man-proc-step">
    ...
    <div class="man-print-stage">
        ...
        <section class="man-print-page man-break">
...
</html>`}function xc(t){let e=Bt[t];if(!e){v("Manuale non trovato.","error");return}let o=window.open("","_blank");if(!o){v("Popup bloccato dal browser. Consenti le finestre popup per stampare.","warning");return}o.document.open(),o.document.write(Ec(e)),o.document.close()}function Ic(t){let e=Bt[t];if(!e)return;let o=document.getElementById("manuali-modal-host");if(!o)return;let i=K(e.copertina),n=i?`<img src="${i}" alt="copertina" style="max-width:100%;max-height:260px;border-radius:10px;border:1px solid #e2e8f0;margin-bottom:16px">`:"",a=(c,l)=>`<h3 style="font-weight:700;font-size:.8rem;text-transform:uppercase;letter-spacing:.05em;color:#64748b;margin:18px 0 8px;padding-bottom:6px;border-bottom:2px solid #e2e8f0"><i class="${c}"></i> &nbsp;${l}</h3>`,r=Je(e),s="";if(r){let c=(r.schedaTecnica||[]).map(function(m){return`<tr>
            ...
            <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden">
                ...
                <tbody>${c}</tbody>
            </table>`),He=[];let l=(r.occorrente||[]).map(function(m){let p=K(m.foto||""),f=-1;return p&&(He.push({lettera:m.lettera||"",nome:m.nome||"",foto:p}),f=He.length-1),`<div ${p?`onclick="_apriLightboxOcc_(${f})"`:""} style="${`padding:10px;border:1px solid #e2e8f0;border-radius:10px;background:#fff${p?";cursor:pointer":""}`}">
    ...
    <div id="manuali-modal" class="modal-overlay active" style="display:flex;z-index:4500">
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap">
            <div>
                ...
                <p class="text-xs text-slate-500 mb-3">${w(e.categoria||"Generale")} \xB7 v${Number(e.version||1)} \xB7 aggiornato ${w(Ve(e.updatedAt))}</p>
            </div>
        ...
        <div class="centered-msg">Caricamento storico...</div>
            ...
            <details class="border border-slate-200 rounded-xl p-3 bg-white" ${r===0?"open":""}>
                <summary class="cursor-pointer font-semibold text-slate-800">
                    v${Number(a.version||0)} \xB7 ${w(a.changeType||"UPDATE")} \xB7 ${w(Ve(a.changedAt))}
                </summary>
...
`)[0]||"",r=a.split(";").length>=a.split(",").length?";":",";i&&(i.style.display="block",i.innerHTML='<div style="display:flex;align-items:center;gap:8px;color:#64748b;font-size:0.88rem"><i class="fas fa-spinner fa-spin"></i> Import Lista di Carico in corso\u2026</div>');try{let c=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"importaListaDiCarico",csvText:n,separatore:r})})).json().catch(()=>({}));if(i)if(c.status==="ok"){let l=`<strong>\u2705 Import completato</strong><br>Nuovi: <strong>${c.nuove||0}</strong> \xB7 Aggiornati: <strong>${c.aggiornate||0}</strong> \xB7 Invariati: <strong>${c.invariate||0}</strong>`;c.corrette>0&&(l+=`<br><span style="color:#b45309"><i class="fas fa-exclamation-triangle"></i> ${c.corrette} righe corrette automaticamente (campi con separatore nel testo)</span>`),c.missingCount>0&&(l+=`<br><span style="color:#d97706">\u26A0 <strong>${c.missingCount}</strong> ordini non presenti nel CSV \u2014 vai nel tab Fornitori per archiviarli.</span>`),i.innerHTML=`<div style="background:#dcfce7;border:1px solid #86efac;border-radius:10px;padding:12px 16px;font-size:0.88rem;color:#166534">${l}</div>`,typeof window.invalidateOFCache=="function"&&window.invalidateOFCache()}else i.innerHTML=io(c.msg||c.message||"Errore sconosciuto")}catch(s){i&&(i.innerHTML=io(s.message))}t.value=""}async function Oe(){let t="_impostazioni_cache",e="_impostazioni_stati_forn_cache",i=pt(t,1/0);if(i){try{let n=typeof i=="string"?JSON.parse(i):i;if(n.stati&&n.stati.length){window.listaStati=n.stati,window.listaOperatori=n.operatori||[],window._distintaHeaderAzienda=String(n.distintaHeaderAzienda||""),Aa(n.overviewStati);let a=pt(e,1/0);if(a)try{let r=typeof a=="string"?JSON.parse(a):a;Array.isArray(r)&&r.length&&(window.listaStatiFornitori=r)}catch{}pt(t,3e5)||no().catch(r=>console.warn("[impostazioni] bg refresh:",r));return}}catch(n){console.warn("[impostazioni] cache JSON corrotta, ricarico dal server:",n)}U(t)}await no()}async function no(){let t="_impostazioni_cache",e="_impostazioni_stati_forn_cache";try{let i=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"getImpostazioni",noCache:!0})})).json();window.listaStati=i.stati&&i.stati.length?i.stati:window._defaultListaStati_(),window.listaOperatori=i.operatori||[],window._distintaHeaderAzienda=String(i.distintaHeaderAzienda||""),Aa(i.overviewStati),J(t,JSON.stringify({stati:window.listaStati,operatori:window.listaOperatori,overviewStati:i.overviewStati,distintaHeaderAzienda:window._distintaHeaderAzienda}))}catch(o){console.warn("[Boot] _fetchImpostazioniDaServer:",o),v("Impostazioni non aggiornate \u2014 uso dati locali.","warning")}try{let i=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"getStatiFornitoriConfig"})})).json();i.stati&&i.stati.length&&(window.listaStatiFornitori=i.stati,J(e,JSON.stringify(i.stati)))}catch{}}function Aa(t){t&&(Array.isArray(t.art)&&t.art.length&&(window._ovStatiArt=t.art.map(e=>e.toUpperCase().trim())),Array.isArray(t.ord)&&t.ord.length&&(window._ovStatiOrd=t.ord.map(e=>e.toUpperCase().trim())))}function xl(t,e){let o=document.getElementById(t);if(!o)return;let i=e.querySelector(".settings-row-arrow"),n=o.style.display==="block";o.style.display=n?"none":"block",i&&(i.style.transform=n?"":"rotate(180deg)"),e.classList.toggle("settings-row-active",!n),!n&&(t==="section-utenti"||t==="section-team-utenti")&&Ce(),!n&&t==="section-qr-postazioni"&&requestAnimationFrame(()=>si())}async function Ce(){let t=document.getElementById("lista-utenti-config");if(t){t.innerHTML='<div class="centered-msg small">Caricamento...</div>';try{let o=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"getUtenti"})})).json();if(!o.length){t.innerHTML='<p class="centered-msg small">Nessun utente creato. Clicca "+ Aggiungi Utente".</p>';return}t.innerHTML=o.map(i=>{let n=i.id_riga,a=(i.username||"").trim(),r=(i.email||"").trim(),s=(i.ruolo||"OPERATORE").trim().toUpperCase(),c=Number(i.max_utenti)||1;return`
        ...
        `}catch{t.innerHTML='<div style="font-size:12px;color:#b91c1c">Errore rete durante il caricamento sessioni.</div>'}}}async function Tl(){let t=(document.getElementById("session-username-target")?.value||"").trim().toUpperCase();if(!t){v("Inserisci uno username da revocare.","error");return}if(confirm("Revocare tutte le sessioni per "+t+"?"))try{let o=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"revocaSessioniUtente",usernameTarget:t})})).json();if(o&&o.status==="success"){v("Sessioni revocate: "+(o.removed||0)),gi();return}v(o&&(o.message||o.msg)||"Revoca non riuscita.","error")}catch{v("Errore rete durante revoca sessioni.","error")}}async function Rl(){if(confirm("Revocare TUTTE le sessioni (eccetto quella corrente)?"))try{let e=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"revocaTutteSessioni"})})).json();if(e&&e.status==="success"){v("Sessioni globali revocate: "+(e.removed||0)),gi();return}v(e&&(e.message||e.msg)||"Revoca globale non riuscita.","error")}catch{v("Errore rete durante revoca globale.","error")}}function Ht(){let t=document.getElementById("contenitore-dati");if(!t)return;let e=window.listaStati||[],o=window.TW||{};t.innerHTML=`
    ...
    `,z(t),requestAnimationFrame(()=>si()),h&&ql("lista-stati-config",i=>{let n=[...i.querySelectorAll("[data-idx]")],a=n.map(r=>(window.listaStati||[])[+r.dataset.idx]);window.listaStati&&(window.listaStati.length=0,a.forEach((r,s)=>{window.listaStati.push(r),n[s].dataset.idx=s})),se()})}async function kl(){let t=document.getElementById("diag-revision"),e=document.getElementById("diag-lastcheck"),o=document.getElementById("diag-online"),i=document.getElementById("diag-cache");if(!t)return;if(t.textContent=N.lastRevisionValue!==null?String(N.lastRevisionValue):"\u2014",N.lastCheckTs){let a=new Date(N.lastCheckTs);e.textContent=a.toLocaleTimeString("it-IT")}else e.textContent="\u2014";let n=N.lastOnlineList;n&&n.length>0?o.textContent=n.map(a=>a.nome+(a.pagina?" ("+a.pagina+")":"")).join(", "):o.textContent="Nessuno";try{let a=await q.listEntries();a.length?i.innerHTML=a.map(r=>{let s=Math.round((Date.now()-r.timestamp)/1e3),c=Date.now()-r.timestamp>q.TTL;return`<span style="display:block;font-family:monospace;font-size:0.78rem;color:${c?"#ef4444":"#16a34a"}">${r.chiave} <em style="color:#94a3b8">(${s}s fa${c?" \xB7 stale":""})</em></span>`}).join(""):i.textContent="Vuota"}catch{i.textContent="Errore lettura cache"}}async function Ll(){try{let e=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"forceRevisionBump",sessionToken:window._getSessionToken_()})})).json();e&&e.status==="ok"?v("\u2714 Revision bumped a "+e.nuovaRevision+" \u2014 tutti i client aggiorneranno entro 15s"):e&&e.status==="auth_error"?window._gestisciAuthError_&&window._gestisciAuthError_(e.message):v("\u26A0\uFE0F "+(e&&e.message?e.message:"Errore"),"error")}catch{v("\u26A0\uFE0F Errore di rete","error")}}async function Pl(){if(confirm("Svuota la cache IndexedDB e ricarica la pagina?")){try{await q.clear()}catch{}location.reload()}}function zl(t){confirm("Sei sicuro di voler eliminare questo stato?")&&((window.listaStati||[]).splice(t,1),se(),Ht())}function Ml(){(window.listaStati||[]).push({nome:"NUOVO",colore:"#94a3b8"}),se(),Ht()}function Nl(t){confirm("Sei sicuro di voler eliminare questo stato?")&&((window.listaStatiFornitori||[]).splice(t,1),se(),Ht())}function Dl(){window.listaStatiFornitori||(window.listaStatiFornitori=[]),window.listaStatiFornitori.push({stato:"NUOVO",colore:"#94a3b8"}),se(),Ht()}function se(){window.modifichePendenti=!0;let t=document.getElementById("btn-salva-globale");t&&(t.style.background="#ef4444",t.innerHTML="<i class='fas fa-exclamation-triangle'></i> Salva Modifiche Ora!")}function ql(t,e){let o=document.getElementById(t);if(!o)return;let i=null;o.addEventListener("dragstart",function(c){if(!(o.querySelector(".dnd-handle, .drag-handle")&&!c.target.closest(".dnd-handle, .drag-handle"))){if(i=c.target.closest('[draggable="true"]'),!i||!o.contains(i)){i=null;return}i.classList.add("dnd-dragging"),c.dataTransfer.effectAllowed="move",c.dataTransfer.setData("text/plain","")}}),o.addEventListener("dragover",function(c){if(c.preventDefault(),!i)return;let l=c.target.closest('[draggable="true"]');if(!l||l===i||!o.contains(l))return;let d=l.getBoundingClientRect();c.clientY<d.top+d.height/2?o.insertBefore(i,l):o.insertBefore(i,l.nextSibling)}),o.addEventListener("dragend",function(c){i&&(i.classList.remove("dnd-dragging"),e&&e(o)),i=null}),o.addEventListener("drop",function(c){c.preventDefault(),c.stopPropagation()});let n=null,a=null,r=0,s=0;o.addEventListener("touchstart",function(c){let l=!!o.querySelector(".dnd-handle, .drag-handle"),d=c.target.closest('[draggable="true"]');if(!d||!o.contains(d)||l&&!c.target.closest(".dnd-handle, .drag-handle"))return;n=d;let u=c.touches[0],m=We(d,u.clientX,u.clientY,{opacity:.85,borderRadius:"14px",shadow:"0 10px 30px rgba(0,0,0,0.25)"});a=m.ghost,r=m.offX,s=m.offY,d.style.opacity="0.25",d.style.transform="scale(0.97)"},{passive:!0}),o.addEventListener("touchmove",function(c){if(!n||!a)return;c.preventDefault();let l=c.touches[0];Ze(a,l.clientX,l.clientY,r,s);let d=Kn(a,l.clientX,l.clientY,'[draggable="true"]');if(d&&d!==n&&o.contains(d)){let u=d.getBoundingClientRect();o.insertBefore(n,l.clientY<u.top+u.height/2?d:d.nextSibling)}},{passive:!1}),o.addEventListener("touchend",function(){n&&(n.style.opacity="",n.style.transform="",Qe(a),a=null,e&&e(o),n=null)})}async function Bl(){try{let e=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"salva_impostazioni_globali",sessionToken:window._getSessionToken_(),stati:window.listaStati||[],operatori:[],distintaHeaderAzienda:String(window._distintaHeaderAzienda||"").trim()})})).json().catch(()=>({}));if(e.status==="success"){if(window.listaStatiFornitori&&window.listaStatiFornitori.length)try{await fetch(I,{method:"POST",body:JSON.stringify({azione:"saveStatiFornitoriConfig",stati:window.listaStatiFornitori})})}catch{}v("Impostazioni salvate correttamente!"),window.modifichePendenti=!1;let o=document.getElementById("btn-salva-globale");o&&(o.style.background="",o.innerHTML="<i class='fas fa-save'></i> Salva Impostazioni"),U("_impostazioni_cache"),U("_impostazioni_stati_forn_cache"),window.cacheContenuti&&Object.keys(window.cacheContenuti).forEach(i=>delete window.cacheContenuti[i]),Object.keys(localStorage).filter(i=>i.startsWith("_html_")).forEach(i=>localStorage.removeItem(i)),await no()}else v("Errore: "+(e.message||"risposta inattesa dal server"),"error")}catch{v("Errore nel salvataggio.","error")}}function Oa(){window._avatarStartAdd=Yc,window._avatarConfirmEdit=el,window._avatarCancelEdit=ol,window._avatarDeleteEdit=il,window._avatarRipristinaPredefiniti=nl,window._renderPredefinedSwatches=ci,window._renderCustomSwatches=Ft,window._applyAvatarColorUI=Ia,window._setAvatarColor=pi,window._avatarStartAddMob=al,window._avatarConfirmEditMob=rl,window._avatarCancelEditMob=cl,window._avatarDeleteEditMob=ll,window.toggleAccountMenu=dl,window.chiudiAccountMenu=ul,window._aggiornaPagina=pl,window.toggleAccountMenuMobile=ml,window.chiudiAccountMenuMobile=fl,window._vapidB64ToUint8_=fi,window._mostraDiagnosticaPush=gl,window._forzaRiregistraPush=vl,window._testPushNotifica=hl,window._togglePushPermission=wl,window._aggiornaUINotifiche=mi,window._onNotifPrefChange=yl,window._getNotifPrefs=oo,window.importaCSVDaFile=Sl,window.importaListaDiCaricoDaFile=El,window.caricaInterfacciaImpostazioni=Ht,window.caricaDatiIniziali=Oe,window._fetchImpostazioniDaServer=no,window.toggleSettingsSection=xl,window.caricaListaUtenti=Ce,window.salvaModificheUtente=Il,window.apriFormNuovoUtente=Al,window.salvaUtenteNuovo=Ol,window.eliminaUtente=Cl,window._caricaSessionStats_=gi,window._revocaSessioniUtenteDaUI=Tl,window._revocaTutteSessioniDaUI=Rl,window._aggiornaDiagnosticaSync=kl,window._forceRevisionBump=Ll,window._svuotaCacheLocale=Pl,window.azioneEliminaStato=zl,window.azioneAggiungiStato=Ml,window.azioneEliminaStatoFornitori=Nl,window.azioneAggiungiStatoFornitori=Dl,window.segnaModifica=se,window.salvaTutteImpostazioni=Bl,window.apriScannerQR=ea,window._chiudiScannerQR=Xe,window._processaQR=to,window._chiudiModaleQRAzione=ii,window._qrFiltroOrdini=oa,window._qrSelezionaOrdine=ia,window._qrScegliStato=na,window._qrSelezionaTutti=aa,window._qrDeselezionaTutti=sa,window._confermaSpostaPostazione=ra,window._qrApriModalNuova=ca,window._qrApriModalModifica=la,window._qrChiudiModalEdit=ni,window._qrAggiornaCodice=ai,window._qrRicalcolaCodice=ua,window._qrSalvaPostazione=pa,window._qrEliminaPostazione=ma,window._qrStampaSingola=ga,window._qrStampaSingolaIdx=va,window._qrStampaTutte=ha,window._qrAggiornaPrevQR=eo}function Ca(){Yn(),document.addEventListener("click",function(t){let e=document.getElementById("account-dropdown"),o=document.getElementById("user-avatar-btn");e&&e.classList.contains("open")&&!e.contains(t.target)&&t.target!==o&&!o.contains(t.target)&&e.classList.remove("open")}),document.addEventListener("click",function(t){let e=document.getElementById("account-dropdown-mobile"),o=document.getElementById("user-avatar-btn-mobile");!e||!o||e.classList.contains("open")&&!e.contains(t.target)&&t.target!==o&&!o.contains(t.target)&&e.classList.remove("open")}),document.addEventListener("visibilitychange",function(){document.visibilityState==="hidden"&&Xe()})}var F,$a=V(()=>{lt();Tt();ut();dt();Jt();wt();Ke();ba();F=null});function re(){return[...S.ovStatiArt,...S.ovStatiOrd]}function gt(t){let e=String(t||"").toUpperCase().trim();return e==="IMBALLATO"||e==="SPEDITO/CONSEGNATO"||e==="SPEDITO"||e==="CONSEGNATO"}var S,ao=V(()=>{S={ultimiDatiProduzione:null,pollProdTimer:null,POLL_PROD_MS:1e4,lastKanbanDragTs:0,mutationInFlight:0,mutationLastDone:0,prodCacheInvalidateTimer:null,attiviProd:[],ordiniAutocompleteCache:[],ovStatiArt:["PREPARARE","MANDA IN LAVORAZIONE","IN LAVORAZIONE","TORNATO DALLA LAVORAZIONE"],ovStatiOrd:["IN PRODUZIONE","IMBALLATO"],datiArchLazy:null}});function jt(t,e){if(!t||t.length===0)return"";let o=window.TW,i={};t.forEach(r=>{if(!e&&String(r.archiviato).toUpperCase()==="TRUE")return;let s=r.ordine||"N.D.";i[s]||(i[s]=[]),i[s].push(r)});let n="";return Object.keys(i).sort((r,s)=>{let c=i[r].some(f=>String(f.last_modified_by||"").startsWith("CSV_REVIEW")),l=i[s].some(f=>String(f.last_modified_by||"").startsWith("CSV_REVIEW"));if(c&&!l)return-1;if(!c&&l)return 1;let d=(i[r][0].cliente||"").trim().toUpperCase(),u=!d||d==="DA DEFINIRE"?(i[r][0].riferimento||r).toUpperCase():d,m=(i[s][0].cliente||"").trim().toUpperCase(),p=!m||m==="DA DEFINIRE"?(i[s][0].riferimento||s).toUpperCase():m;return u<p?-1:u>p?1:r<s?-1:r>s?1:0}).forEach(r=>{let s=i[r],c=s[0].cliente,l=s[0].riferimento||"",d=l?`<span class="riferimento-label">(${w(l)})</span>`:"",u=e?"archivio-wrapper":"",p=s.some(x=>String(x.last_modified_by||"").startsWith("CSV_REVIEW"))?" csv-review-order":"",f=e?"archivio-header":"",y=e?"#475569":"inherit",b;if(r.includes("/")){let x=r.indexOf("/"),R=r.substring(0,x),L=r.substring(x+1),M=L.length>3?L.substring(0,3)+".":L;b=`${R}/${M}`}else b=r.length>14?r.substring(0,14)+"\u2026":r;let g="";if(!e)if(window._isUtenteEsente()){let x=[...new Set(s.flatMap(M=>!gt(M.stato)&&M.assegna&&M.assegna!==""&&M.assegna!=="undefined"?M.assegna.split(",").map(B=>window._normNome(B.trim())).filter(Boolean):[]))],R=x.length?x.map(window._normNome).join(", "):"Libero",L=window.listaOperatori.map(M=>{let B=x.some(bs=>bs.toUpperCase()===window._normNome(M.nome).toUpperCase()),nt=window._getOpColor(M.nome.trim()),Lt=M.nome.trim().replace(/'/g,"\\'"),hs=r.replace(/'/g,"\\'");return`<button type="button" class="op-option${B?" is-selected":""}" onclick="selezionaOpAssegnaOrdine(this,'${hs}','${Lt}')"><span class="op-opt-dot" style="background:${nt}"></span><span>${window._normNome(M.nome)}</span>${B?'<i class="fas fa-check op-check-icon"></i>':""}</button>`}).join("");g=`<div class="op-dropdown op-dropdown-ord" data-nord="${r}" data-assegna-ord="${x.join(",").replace(/"/g,"&quot;")}"><button type="button" class="op-trigger op-trigger-ord" onclick="event.stopPropagation(); toggleOpDropdown(this)"><i class="fas fa-user-tag op-icon"></i><span class="op-trigger-label">${R}</span><i class="fas fa-chevron-down op-chevron"></i></button><div class="op-popup">${L}</div></div>`}else{let x=(h?.nome||"").toUpperCase().trim();s.some(L=>!gt(L.stato)&&L.assegna&&L.assegna.split(",").some(M=>M.trim().toUpperCase()===x))||(g=`<button class="btn-assegnami btn-assegnami-ord" onclick="event.stopPropagation(); autoAssegnamiOrdine('${r.replace(/'/g,"\\'")}')" title="Assegnami a tutto l'ordine"><i class="fas fa-user-plus"></i></button>`)}let E=r.replace(/'/g,"\\'"),A=(c||"").replace(/'/g,"\\'"),_=s[0].id_riga,$="";if(!e){let x=s.map(B=>String(B.stato||"IN ATTESA").toUpperCase().trim()).filter((B,nt,Lt)=>Lt.indexOf(B)===nt),R=x.length===1?x[0]:`${x.length} Stati`,L=window.listaStati.find(B=>B.nome===x[0])||{colore:"#e2e8f0"},M=window.listaStati.map(B=>`<button type="button" class="stato-option" onclick="event.stopPropagation(); selezionaStatoOrdine(this,'${r.replace(/'/g,"\\'")}','${B.nome}','${B.colore}')"><span class="stato-opt-dot" style="background:${B.colore}"></span><span>${B.nome}</span></button>`).join("");$=`<div class="stato-dropdown stato-dropdown-ord" data-nord="${r}"><button type="button" class="stato-trigger" onclick="event.stopPropagation(); toggleStatoDropdown(this)" title="Cambia stato tutte righe"><span class="stato-dot" style="background:${L.colore}"></span><span class="stato-label-txt">${R}</span><i class="fas fa-chevron-down stato-chevron"></i></button><div class="stato-popup">${M}</div></div>`}let k=`apriModalAiuto('${_}', 'INTERO ORDINE', '${E}', '${A}')`,T=`gestisciArchiviazione('${E}')`,C=`apriModalSollecito('','${E}','${A}','Intero Ordine')`,D=`gestisciRipristino('${E}', 'ORDINE')`,G="";e?G=`<button class="ord-menu-item" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${D}"><i class="fa-solid fa-rotate-left"></i> Ripristina</button>`:(G+=`<button class="ord-menu-item" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${k}"><i class="fa-regular fa-envelope"></i> Chiedi</button>`,G+=`<button class="ord-menu-item ord-menu-item--danger" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${T}"><i class="fa-solid fa-box-archive"></i> Archivia</button>`,(window._isCommerciale()||window._isUtenteEsente())&&(G+=`<button class="ord-menu-item ord-menu-item--warn" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${C}"><i class="fa-solid fa-calendar-alt"></i> Scadenza</button>`));let ct=e?"":`${g}${$}`,tt=`<div class="ord-azioni-menu" onclick="event.stopPropagation()">
        ...
        <div class="ordine-wrapper ${u}${p}" data-ordine="${r}" data-cliente="${(c||"").toLowerCase().replace(/"/g,"")}" data-riferimento="${(l||"").toLowerCase().replace(/"/g,"")}" data-codici="${s.map(x=>x.codice&&x.codice!=="false"?x.codice:"").join("|").toLowerCase()}">
            ...
            <div class="dettagli-container${e?" hidden":""}">
                ${s.map(x=>e?Fl(x,r):Ul(x,r,c)).join("")}
            </div>
    ...
    <div class="item-card archivio-layout ${o.card}">
        ...
        <div class="ov-stati-grid" id="ov-kanban-grid">${re().map(n=>{let a=t.filter(u=>(u.stato||"").toUpperCase().trim()===n.trim()),r=e[n]||o,s=a.length===0,c=S.ovStatiOrd.includes(n),l="",d="";if(c){let u=new Map,m=[];a.forEach(p=>{let f=String(p.ordine||"\u2014").trim();u.has(f)?u.get(f).push(p):(u.set(f,[p]),m.push({ordine:f,rows:u.get(f)}))}),m.sort((p,f)=>{let y=(p.rows[0].cliente||"").trim().toUpperCase(),b=!y||y==="DA DEFINIRE"?(p.rows[0].riferimento||p.ordine).toUpperCase():y,g=(f.rows[0].cliente||"").trim().toUpperCase(),E=!g||g==="DA DEFINIRE"?(f.rows[0].riferimento||f.ordine).toUpperCase():g;return b<E?-1:b>E?1:0}),l=m.map(({ordine:p,rows:f})=>{let y=f.map(T=>String(T.id_riga)).join(","),b=f[0];function g(T){let C=(T||"").trim().split(/\s+/).slice(0,2).join(" ");return C.length>18?C.substring(0,17)+"\u2026":C}let E=String(b.cliente||"").trim().toUpperCase(),A=!E||E==="DA DEFINIRE"?g(b.riferimento||"")||p:g(b.cliente),_=p.length>12?p.substring(0,12)+"\u2026":p,$=f.length,k=f.reduce((T,C)=>T+(parseInt(C.qty)||1),0);return`<div class="ov-stato-row ov-kanban-item"
                ...
                </div>`}).join(""),d=m.length+" ord."}else{let u=new Map,m=[];a.forEach(p=>{let f=String(p.codice&&p.codice!=="false"?p.codice:p.riferimento||"\u2014").trim();u.has(f)?u.get(f).push(p):(u.set(f,[p]),m.push({codice:f,rows:u.get(f)}))}),m.sort((p,f)=>p.codice<f.codice?-1:p.codice>f.codice?1:0),l=m.map(({codice:p,rows:f})=>{let y=p.length>24?p.substring(0,24)+"\u2026":p,b=f.map(C=>String(C.id_riga)).join(",");function g(C){let D=(C||"").trim().split(/\s+/).slice(0,2).join(" ");return D.length>14?D.substring(0,13)+"\u2026":D}function E(C){let D=String(C.cliente||"").trim().toUpperCase();return!D||D==="DA DEFINIRE"?g(C.riferimento||"")||"":g(C.cliente)}let A=new Map,_=[];f.forEach(C=>{let D=E(C);A.has(D)?A.get(D).push(C):(A.set(D,[C]),_.push(D))});let k=_.map(C=>{let G=A.get(C).map(ct=>{let tt=String(ct.ordine||"").trim();return tt.length>12?tt.substring(0,12)+"\u2026":tt}).filter(Boolean).join(" / ");return!G&&!C?"":G+(C?" <em>"+C+"</em>":"")}).filter(Boolean).join(" \xB7 "),T=f.length>1?f.map(C=>(C.qty||1)+"pz").join("+"):(f[0].qty||1)+" pz";return`<div class="ov-stato-row ov-kanban-item"
                    data-id-riga="${f[0].id_riga}"
                    data-id-righe="${b}"
                    data-count="${f.length}"
                    data-codice="${p.replace(/"/g,"&quot;")}"
                    data-ordine="${f.map(C=>C.ordine||"").join(",")}"
                    data-stato-corrente="${n}"
                    title="Doppio clic \u2192 vai all'ordine nella lista">
                    <span class="ov-drag-handle"><i class="fas fa-grip-vertical"></i></span>
                    <span class="ov-row-main">
        ...
        `,H[i]=o.innerHTML,X[i]=Date.now(),J("_html_"+i,o.innerHTML),q.set("PROGRAMMA_PRODUZIONE",t).catch(()=>{}),z(o),r&&y.size&&o.querySelectorAll(".ordine-wrapper").forEach(g=>{if(y.has(g.dataset.ordine)){let E=g.querySelector(".riga-ordine"),A=g.querySelector(".dettagli-container");E&&A&&(E.classList.add("open"),A.style.display="block")}}),window.aggiornaListaFiltrabili(),requestAnimationFrame(It),requestAnimationFrame(()=>{S.attiviProd&&S.attiviProd.forEach(g=>{if(parseFloat(g.qty_evasa)>0){let E=document.getElementById("qty-evasa-block-"+g.id_riga),A=E&&E.closest(".qty-cell")?.querySelector(".btn-qty-evasa-toggle");E&&(E.style.display="inline-flex"),A&&A.classList.add("active")}})}),fo(),S.ordiniAutocompleteCache=n.filter(g=>String(g.archiviato||"").toUpperCase()!=="TRUE").map(g=>({ordine:g.ordine||"",cliente:g.cliente||"",riferimento:g.riferimento||""}));let b=new Set;S.ordiniAutocompleteCache=S.ordiniAutocompleteCache.filter(g=>b.has(g.ordine)?!1:(b.add(g.ordine),!0)),window._ordiniAutocompleteCache=S.ordiniAutocompleteCache}async function mo(t,e=!1,o=null,i=null){let n=document.getElementById("contenitore-dati");!e&&n&&(n.innerHTML="<div class='inline-msg' id='_prod-loader'>Caricamento Dashboard...</div>",z(n));let a=e?null:setTimeout(()=>{let s=document.getElementById("_prod-loader");s&&(s.innerHTML="<i class='fas fa-spinner fa-spin'></i> Connessione lenta, sto ancora caricando...")},3500),r=e?null:setTimeout(()=>{let s=document.getElementById("_prod-loader");s&&(s.innerHTML=`\u26A0\uFE0F Server occupato o rete instabile.<br>
