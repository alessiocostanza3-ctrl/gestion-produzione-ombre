import{a as Se,b as N,c as zt}from"./chunk-chunk-VLVTJAKH.js";import{a as W,b as Ps,c as x,d as De,e as ft,f as y,g,h as z,i as Xi,j as rt,k as tn,l as gt,m as b,n as Lt,o as en,p as on,q as vt,r as Pt,s as zs,t as xt,u as ct,v as nn,w as _e}from"./chunk-chunk-NM2LGQZU.js";function ht(t,e){try{let o=localStorage.getItem(t);if(!o)return null;let i=JSON.parse(o);return Date.now()-i.ts<e?i.data:null}catch{return null}}function Z(t,e){try{let o=typeof e=="string"?e:JSON.stringify(e);if(o.length>25e5)return;localStorage.setItem(t,JSON.stringify({ts:Date.now(),data:o}))}catch{}}function U(t){try{localStorage.removeItem(t)}catch{}}function an(){try{let t=[];for(let e=0;e<localStorage.length;e++){let o=localStorage.key(e);o&&o.startsWith("_html_")&&t.push(o)}t.forEach(e=>localStorage.removeItem(e))}catch{}}var It=W(()=>{});var H,X,A,Qt=W(()=>{"use strict";H={},X={},A={dashBundle:null,dashPromise:null,rqBundle:null,rqPromise:null,matBundle:null,matPromise:null,ordiniBundle:null,ordiniPromise:null}});var Ms,Ns,qs,Ds,Bs,Us,Fs,Hs,js,sn,rn,cn=W(()=>{"use strict";Ms="_pushStato",Ns="notifPrefs",qs="mlPipQty",Ds="mlPipCaricato",Bs="mlPipPronti",Us="mlPipMovimenti",Fs="avatarColor_",Hs="avatarColorRecenti_",js="avatarColorHidden_",sn=[Ns,Ms,qs,Ds,Us,Bs],rn=[Fs,Hs,js]});function ln(t){qo=t.onRemoteChange,Do=t.onUsersOnline,Ee=t.getUtenteAttuale,qt=t.getPaginaCorrente}var qo,Do,Ee,qt,dn,M,Kt=W(()=>{_e();ft();zs();qo=null,Do=null,Ee=null,qt=null;dn={INTERVAL_MS:2e4,INTERVAL_FOCUS_MS:3e4,INTERVAL_BG_MS:3e4,PING_INTERVAL_MS:6e4,MAX_BACKOFF_MS:6e4,_timer:null,_pingTimer:null,_lastRevision:null,_lastCheck:0,_paused:!1,_errorStreak:0,_offPageTick:0,lastRevisionValue:null,lastOnlineList:[],lastCheckTs:0,start:function(){this.stop(),this._lastRevision=null,this._paused=!1,this._errorStreak=0,this._offPageTick=0,this._schedule(document.hidden?this.INTERVAL_BG_MS:this.INTERVAL_FOCUS_MS),this._schedulePing(5e3)},stop:function(){this._timer&&(clearTimeout(this._timer),this._timer=null),this._pingTimer&&(clearTimeout(this._pingTimer),this._pingTimer=null),this._lastRevision=null,this._paused=!1;var t=document.getElementById("online-indicator");t&&t.remove();var e=document.getElementById("online-indicator-mob");e&&e.remove()},pauseFor:function(t){t||(t=5e3),this._paused=!0;var e=this;setTimeout(function(){e._paused=!1},t)},_schedule:function(t){this._timer&&clearTimeout(this._timer);var e=this;this._timer=setTimeout(function(){e._tick()},t)},_tick:function(){var t=this;this._check().finally(function(){var e=qt?String(qt()||"").toUpperCase().trim():"",o=e==="PROGRAMMA PRODUZIONE DEL MESE",i=document.hidden?t.INTERVAL_BG_MS:document.hasFocus&&document.hasFocus()?t.INTERVAL_FOCUS_MS:t.INTERVAL_MS;o||(i=Math.max(i,t.INTERVAL_BG_MS));var n=i;if(t._errorStreak>0){var a=Math.min(4,1+t._errorStreak*.5);n=Math.min(t.MAX_BACKOFF_MS,Math.round(i*a))}t._schedule(n)})},_check:async function(){if(!this._paused){var t=Date.now(),e=qt?String(qt()||"").toUpperCase().trim():"",o=e==="PROGRAMMA PRODUZIONE DEL MESE";if(!(!o&&(this._offPageTick=(this._offPageTick+1)%3,this._offPageTick!==0)))try{var i=await nn();if(!i||i.status!=="ok"){this._errorStreak=Math.min(this._errorStreak+1,10),Pt("poller_check",{action:e||"UNKNOWN_PAGE",status:"invalid_payload",durationMs:Date.now()-t},{sampleRate:.5});return}var n=Number(i.revision);if(this._errorStreak=0,this._lastCheck=Date.now(),this.lastRevisionValue=n,this.lastCheckTs=Date.now(),this._lastRevision===null){this._lastRevision=n,Pt("poller_check",{action:e||"UNKNOWN_PAGE",status:"baseline",durationMs:Date.now()-t},{sampleRate:.3});return}if(n===this._lastRevision){Pt("poller_check",{action:e||"UNKNOWN_PAGE",status:"unchanged",durationMs:Date.now()-t},{sampleRate:.2});return}var a=Ee?Ee():null,s=a&&a.nome?a.nome.toUpperCase():"",r=i.utente?String(i.utente).toUpperCase():"";if(this._lastRevision=n,r===s)return;var c=i.utente||"Qualcuno";Pt("poller_check",{action:e||"UNKNOWN_PAGE",status:"remote_change",durationMs:Date.now()-t,detail:String(c||"")},{sampleRate:1}),qo&&qo(c)}catch(l){this._errorStreak=Math.min(this._errorStreak+1,10),Pt("poller_check",{action:e||"UNKNOWN_PAGE",status:"error",durationMs:Date.now()-t,error:l&&l.message?l.message:String(l||"")},{sampleRate:.7}),l&&l.name!=="AbortError"&&console.warn("[RevisionPoller]",l)}}},_schedulePing:function(t){this._pingTimer&&clearTimeout(this._pingTimer);var e=this;this._pingTimer=setTimeout(function(){e._pingServer().finally(function(){e._schedulePing(e.PING_INTERVAL_MS)})},t)},_pingServer:async function(){var t=Ee?Ee():null;if(!(!t||!t.nome))try{var e=await xt({azione:"ping",pagina:(qt?qt():"")||""});e&&e.status==="ok"&&Array.isArray(e.online)&&(Do&&Do(e.online),dn.lastOnlineList=e.online,Pt("poller_ping",{action:"ping",status:"ok",detail:String(e.online.length)},{sampleRate:.3}))}catch(o){Pt("poller_ping",{action:"ping",status:"error",error:o&&o.message?o.message:String(o||"")},{sampleRate:.7})}}},M=dn});function un(){return"ORDINI_ACQUISTI_"+(b?.nome?.toUpperCase()||"_")}function Uo(){let t=document.getElementById("contenitore-dati");t&&(pt._acq_ordini=t.innerHTML,Ct._acq_ordini=Date.now(),N.set(un(),t.innerHTML).catch(()=>{}))}function Zs(){A.ordiniBundle=null,A.ordiniPromise=null}function pn(t,e,o){t.classList.toggle("is-ordinato",o),e.classList.toggle("checked",o),e.title=o?"Segna In Attesa":"Segna Ordinato";let i=e.querySelector("i");i&&(i.className="fas "+(o?"fa-check-circle":"fa-circle"));let n=t.querySelector(".oi-stato-badge");n&&(n.className="oi-stato-badge "+(o?"badge-ordinato-sm":"badge-attesa-sm"),n.innerHTML=o?'<i class="fas fa-circle-check"></i> ORDINATO':"IN ATTESA");let a=t.querySelector(".oi-stato-dot");a&&(a.className="oi-stato-dot "+(o?"dot-ordinato":"dot-attesa"));let s=t.closest(".ordine-group");if(s){let l=s.querySelectorAll(".ordine-item").length,d=s.querySelectorAll(".ordine-item.is-ordinato").length,u=s.querySelector(".og-progress");if(u&&(u.textContent=d+"/"+l),d===l){s.classList.add("all-done");let f=s.querySelector(".og-left");f&&!f.querySelector(".og-done-badge")&&f.insertAdjacentHTML("beforeend",'<span class="og-done-badge"><i class="fas fa-check-circle"></i> Completato</span>')}else{s.classList.remove("all-done");let f=s.querySelector(".og-done-badge");f&&f.remove()}}let r=document.querySelector(".acquisti-subtitle");if(r){let c=document.querySelectorAll(".ordine-item:not(.is-ordinato)").length;r.textContent=c>0?`${c} articoli in attesa`:"Tutto ordinato \u2705"}}function mn(t,e){return fetch(x,{method:"POST",body:JSON.stringify({pagina:t}),...e?{signal:e}:{}}).then(o=>{if(!o.ok)throw new Error(`HTTP ${o.status}`);return o.json()})}function Ho(){let t=document.getElementById("acq-tab-catalogo"),e=document.getElementById("acq-tab-ordini"),o=document.getElementById("acq-tab-fornitori");t&&t.classList.toggle("active",dt==="catalogo"),e&&e.classList.toggle("active",dt==="ordini"),o&&o.classList.toggle("active",dt==="fornitori")}function Qs(t){if(t===dt)return;dt=t,window._acquistTabAttivo=t,Ho();let e=document.getElementById("contenitore-dati");if(e){if(t==="fornitori"){typeof window.caricaOrdiniFornitori=="function"&&window.caricaOrdiniFornitori(null,null,!1);return}if(t==="ordini"){let o=pt._acq_ordini,i=Ct._acq_ordini||0;if(o&&Date.now()-i<3e5){e.innerHTML=o,z(e),window.aggiornaListaFiltrabili?.();return}fn(null,null)}else{let o=pt["MATERIALE DA ORDINARE"],i=Ct["MATERIALE DA ORDINARE"]||0;if(o&&Date.now()-i<3e5){e.innerHTML=o,z(e),window.aggiornaListaFiltrabili?.();return}let n=ht("_html_MATERIALE DA ORDINARE",3e5);if(n){pt["MATERIALE DA ORDINARE"]=n,Ct["MATERIALE DA ORDINARE"]=Date.now(),e.innerHTML=n,z(e),window.aggiornaListaFiltrabili?.();return}bt(!1,null,null)}}}async function fn(t=null,e=null){if(xe)return;xe=!0;let o=document.getElementById("contenitore-dati");if(!o){xe=!1;return}let i=b?.nome?.toUpperCase().trim()==="ALESSIO",n=i?"":b?.nome||"",a=un(),s=!1;try{let r=await N.get(a);if(dt!=="ordini"){xe=!1;return}r&&r.dati&&(o.innerHTML=r.dati,pt._acq_ordini=r.dati,Ct._acq_ordini=Date.now(),z(o),window.aggiornaListaFiltrabili?.(),s=!0)}catch{}if(s||(o.innerHTML="<div class='centered-msg'><i class='fas fa-spinner fa-spin'></i> Caricamento ordini...</div>"),!(s&&t!==null))try{let r;if(A.ordiniBundle?(r=A.ordiniBundle,A.ordiniBundle=null,A.ordiniPromise=null):A.ordiniPromise?(r=await A.ordiniPromise,A.ordiniBundle=null,A.ordiniPromise=null):r=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"getOrdiniAcquisti",operatore:n}),...e?{signal:e}:{}})).json(),e?.aborted)return;if(!Array.isArray(r)||r.length===0){o.innerHTML=`<div class='empty-msg'>${i?"Nessun ordine ricevuto.":"Non hai ancora inviato ordini."}</div>`,z(o);return}let c={};r.forEach(f=>{let p=f.id_gruppo||f.data+"_"+f.operatore;c[p]||(c[p]={data:f.data,operatore:f.operatore,items:[]}),c[p].items.push(f)});let l=Object.keys(c).reverse(),d=r.filter(f=>f.stato!=="ORDINATO").length,u=`<div class="ordini-acq-page">
            <div class="acquisti-header header-flex">
                ...
                ${i?"":`<button class="btn-nuovo-fisso ${Dt.btnSuccess}" onclick="_switchAcquistiTab('catalogo')">
        ...
        <span class="oi-stato-badge ${o?"badge-ordinato-sm":"badge-attesa-sm"}">${o?'<i class="fas fa-circle-check"></i> ORDINATO':"IN ATTESA"}</span>
            ...
            <div class="acquisti-header header-flex">
                ...
                <div class="acquisti-actions-wrapper">
                    <button id="btn-delete-selected" type="button" onclick="eliminaSelezionati()" class="${Dt.btnDanger} btn-fade-action">
                        <i class="fas fa-trash"></i><span class="btn-elimina-label"> Elimina (<span id="count-selected">0</span>)</span>
                    </button>
                    <button id="btn-mode-select" type="button" onclick="toggleSelezioneMultipla()" class="${Dt.btn}">
                        <i class="fas fa-tasks"></i><span class="btn-sel-txt"> Seleziona</span>
                    </button>
                    <button type="button" class="btn-nuovo-fisso btn-sezione-new ${Dt.btn}" onclick="apriModalNuovaSezione()" title="Nuova sezione">
                        <i class="fas fa-folder-plus"></i>
                    </button>
                    <button type="button" class="btn-nuovo-fisso ${Dt.btnSuccess}" onclick="apriModalNuovo()">
            ...
            <div id="lista-materiali-grid">`,d=window.innerWidth<=768;if(J.forEach((u,f)=>{let p=c[u]||[],m=r(u);l+=`
                ...
                <div class="materiale-card ${Dt.card}" data-idx="${w}" data-search="${(v+" "+E+" "+C).toLowerCase().replace(/"/g,"")}">
                ...
                </div>`}),l+="</div>",e!==null&&e!==window._latestNavRequest||dt!=="catalogo")return;N.set("MATERIALE_DA_ORDINARE",l).catch(()=>{}),pt["MATERIALE DA ORDINARE"]=l,Ct["MATERIALE DA ORDINARE"]=Date.now(),Z("_html_MATERIALE DA ORDINARE",l),a.innerHTML=l,z(a),window.aggiornaListaFiltrabili?.()}catch(s){if(s.name==="AbortError")return;console.error("Errore caricamento materiali:",s),a&&(a.innerHTML="<div class='centered-error-bold'>Errore nel caricamento del catalogo.</div>",z(a))}}finally{Bo=!1}}}function ir(t,e){let o=document.getElementById(t);o&&(o.value=Math.max(1,(parseInt(o.value)||1)+e))}function nr(t,e,o){let i=document.getElementById(o),n=parseInt(i.value)||1,a=document.querySelector(`[data-prod="${t}"]`),s=a?a.querySelector("img"):null,r=s?s.src:null;At.push({prodotto:t,quantita:n,fornitore:e,foto:r}),Be();let c=event.target.closest("button"),l=c.innerHTML;c.innerHTML='<i class="fas fa-check"></i>',c.style.background="linear-gradient(135deg,#059669,#10b981)",c.style.boxShadow="0 2px 8px rgba(16,185,129,0.45)",setTimeout(()=>{c.innerHTML=l,c.style.background="",c.style.boxShadow="",i.value=1},1400)}function jo(){let t=document.getElementById("modal-carrello"),e=document.getElementById("lista-articoli-carrello"),o=document.getElementById("btn-invia-alessio");At.length===0?(e.innerHTML="<p class='empty-cart-msg'>Il tuo carrello \xE8 vuoto.</p>",o&&(o.style.display="none")):(e.innerHTML=At.map((i,n)=>`
            ...
            </div>`).join(""),o&&(o.style.display="block")),t.style.display="flex",requestAnimationFrame(()=>requestAnimationFrame(()=>t.classList.add("cart-open")))}function ar(t){At.splice(t,1),Be(),jo()}function gn(){let t=document.getElementById("modal-carrello");t.classList.remove("cart-open"),setTimeout(()=>{t.style.display="none"},300)}function sr(){jo()}function Be(){let t=At.length,e=document.getElementById("badge-carrello-count"),o=document.getElementById("cart-qty-val");e&&(e.innerText=t,e.style.display=t>0?"flex":"none"),o&&(o.innerText=t)}async function rr(){if(At.length===0){alert("Il carrello \xE8 vuoto!");return}if(!confirm(`Vuoi inviare la lista di ${At.length} articoli all'ufficio acquisti?`))return;M.pauseFor(6e3);let e=document.getElementById("btn-invia-alessio");e&&(e.disabled=!0,e.innerText="Invio in corso...");let o=String(Date.now());try{let i=await Promise.all(At.map(async r=>({...r,foto:await Ks(r.foto,80)}))),n={azione:"inviaOrdineAcquisti",operatore:b?.nome||"Utente",id_gruppo:o,articoli:i},s=await(await fetch(x,{method:"POST",body:JSON.stringify(n)})).json();if(s.status==="success")At=[],Be(),gn(),g("\u2705 Ordine inviato ad Alessio!"),delete pt._acq_ordini,delete Ct._acq_ordini,U("_html__acq_ordini"),dt="ordini",window._acquistTabAttivo="ordini",setTimeout(()=>window.cambiaPagina?.("MATERIALE DA ORDINARE",null),800);else throw new Error(s.message)}catch(i){g("Errore invio ordine: "+i.message,"error")}finally{e&&(e.disabled=!1,e.innerText="Invia ad Alessio")}}function cr(t){let e=`[data-prod="${t.replace(/"/g,'\\"')}"]`,o=document.querySelector(e);if(!o)return;if(o.querySelector("img")){vn(o.querySelector("img").src);return}let i=document.createElement("input");i.type="file",i.accept="image/*",i.onchange=n=>{let a=n.target.files[0];if(!a)return;let s=new FileReader;s.onload=r=>{let c=r.target.result,l=o.getAttribute("data-fornitore")||"";o.innerHTML=`
                <img src="${y(c)}"
                     class="modal-img"
            ...
            <i class="fas fa-camera mat-img-icon"></i>
            <span class="mat-img-hint">Scatta foto</span>
            ${o?`<span class="mat-badge-fornitore">${y(o)}</span>`:""}`,e.style.border="";let i=e.closest(".materiale-card");if(i){let n=i.querySelector(".btn-menu-elimina-foto");n&&(n.style.display="none")}}}function vn(t){let e=document.createElement("div");e.style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.9);z-index:200000;display:flex;justify-content:center;align-items:center;cursor:zoom-out;";let o=document.createElement("img");o.src=t,o.className="overlay-img",e.appendChild(o),e.onclick=()=>document.body.removeChild(e),document.body.appendChild(e)}function dr(t,e){t.preventDefault(),t.stopPropagation(),document.querySelectorAll(".menu-popup-opzioni").forEach(i=>{i.id!==`menu-opzioni-${e}`&&i.classList.remove("open")});let o=document.getElementById(`menu-opzioni-${e}`);o&&o.classList.toggle("open")}function pr(){document.getElementById("titolo-modal-articolo").innerText="Nuovo Articolo",document.getElementById("edit-id-riga").value="",document.getElementById("edit-nome").value="",document.getElementById("edit-codice").value="",document.getElementById("edit-fornitore").value="";let t=document.getElementById("modal-gestione-articolo");t.style.display="flex",t.offsetHeight,t.classList.add("active"),bn(),setTimeout(()=>{document.getElementById("edit-nome")?.focus()},180)}function ur(t,e,o,i){let n=document.getElementById("modal-gestione-articolo");document.getElementById("titolo-modal-articolo").innerText=t?"Modifica Articolo":"Nuovo Articolo",document.getElementById("edit-id-riga").value=t||"",document.getElementById("edit-nome").value=e||"",document.getElementById("edit-codice").value=i&&i!=="undefined"?i:"",document.getElementById("edit-fornitore").value=o||"",n.style.display="flex",n.offsetHeight,n.classList.add("active"),bn()}function hn(){let t=document.getElementById("modal-gestione-articolo");t.classList.remove("active"),yn(),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300)}function bn(){window.visualViewport&&(yn(),Bt=function(){let t=document.getElementById("modal-gestione-articolo");if(!t||t.style.display==="none")return;let e=window.visualViewport,o=Math.max(0,window.innerHeight-e.offsetTop-e.height),i=t.querySelector(".modal-articoli-box");i&&(i.style.marginBottom=o>0?o+"px":""),t.style.top=e.offsetTop+"px",t.style.height=e.height+"px"},window.visualViewport.addEventListener("resize",Bt),window.visualViewport.addEventListener("scroll",Bt))}function yn(){if(!window.visualViewport||!Bt)return;window.visualViewport.removeEventListener("resize",Bt),window.visualViewport.removeEventListener("scroll",Bt),Bt=null;let t=document.getElementById("modal-gestione-articolo");if(!t)return;t.style.top="",t.style.height="";let e=t.querySelector(".modal-articoli-box");e&&(e.style.marginBottom="")}async function mr(){let t=document.getElementById("btn-salva-articolo"),e=document.getElementById("edit-nome").value.trim();if(!e){g("Inserisci il nome del prodotto!","warning");return}t.textContent="Salvataggio...",t.disabled=!0,M.pauseFor(8e3);try{let o=await ct({azione:"gestisciMateriale",id_riga:document.getElementById("edit-id-riga").value,nome:e,codice:document.getElementById("edit-codice").value,fornitore:document.getElementById("edit-fornitore").value},1e4,{noDedupe:!0});if(o&&o.status==="auth_error"){window._gestisciAuthError_?.(o.message);return}if(o&&o.status==="forbidden"){g(o.message||"Non hai i permessi per questa operazione.","error");return}o&&o.status==="success"?(hn(),bt(!0)):g("Errore durante il salvataggio.","error")}catch(o){g(o?.name==="TimeoutError"?"Timeout: il server non risponde. Riprova.":"Errore di rete. Riprova.","error")}finally{t.textContent="Salva",t.disabled=!1}}async function fr(t,e,o,i){rt("Duplica Articolo",`Duplicare l'articolo: "${e}"?`,async()=>{M.pauseFor(6e3);let n=document.querySelector(`[data-id="${t}"]`).closest(".materiale-card"),a=Date.now(),s=`qty-item-temp-${a}`,r=document.createElement("div");r.innerHTML=`
            <div class="materiale-card ${Dt.card}">
                <div class="mat-card-img img-preview-container"
    ...
    </div>`}function Or(t){let e=document.getElementById("modal-of-dettaglio");e&&e.remove();let o=t.dataset,i=document.createElement("div");i.id="modal-of-dettaglio",i.className="modal-overlay active",i.onclick=n=>{n.target===i&&On()},i.innerHTML=`<div class="modal-content of-modal-content">
...
Non comparir\xE0 pi\xF9 nella lista principale.`))return;let e=document.querySelector(`.of-ordine-wrapper[data-nordine="${t}"]`);e&&(e.style.opacity="0.4",e.style.pointerEvents="none");try{let o=await ct({azione:"archiviaOrdineFornitori",n_ordine:t},8e3,{retries:1});if(!o||o.status!=="ok"){g("Errore archiviazione","error"),e&&(e.style.opacity="",e.style.pointerEvents="");return}e&&e.remove();let i=document.querySelector(".acquisti-subtitle");if(i){let n=document.querySelectorAll(".of-ordine-wrapper"),a=document.querySelectorAll(".of-ordine-missing").length,s=a>0?` \xB7 <span style="color:#d97706;font-weight:600">\u26A0 ${a} da revisionare</span>`:"";i.innerHTML=`${n.length} ordini${s}`}if(!document.querySelector(".of-ordine-missing")){let n=document.querySelector(".of-review-banner");n&&n.remove()}Go()}catch{g("Errore connessione","error"),e&&(e.style.opacity="",e.style.pointerEvents="")}}function kr(t,e){return t?t.length>e?t.substring(0,e)+"\u2026":t:"-"}function Lr(t){return!t&&t!==0?"-":Number(t).toLocaleString("it-IT",{minimumFractionDigits:2,maximumFractionDigits:2})+" \u20AC"}async function Pr(t,e,o=!1){let i=document.getElementById("contenitore-dati");if(i){if(!o){let n=Xt[it],a=te[it]||0;if(n&&Date.now()-a<3e5){i.innerHTML=n,z(i),window.aggiornaListaFiltrabili?.();return}try{let r=await N.get(it);if(r&&r.dati){i.innerHTML=r.dati,Xt[it]=r.dati,te[it]=r.timestamp,z(i),window.aggiornaListaFiltrabili?.(),r.isStale&&zr(e);return}}catch{}let s=ht(Ue,3e5);if(s){i.innerHTML=s,Xt[it]=s,te[it]=Date.now(),z(i),window.aggiornaListaFiltrabili?.();return}i.innerHTML=`<div class="centered-msg" id="_of-loader">
    ...
    `).join(""),n.dataset.idRiga=t||"",n.dataset.nOrdine=o,n.dataset.cliente=i||"";let s=document.getElementById("modal-ordine-row");s&&(s.style.display="none"),document.getElementById("messaggio-aiuto").value="",Wo("DOMANDA")}function zn(t={}){let e=document.getElementById("modalAiuto");if(e.style.display==="flex")return;e._openedAt=Date.now(),e.style.display="flex",e.offsetHeight,e.classList.add("active"),document.getElementById("modal-titolo").innerText="Nuova Richiesta";let o=window.listaOperatori||[];document.getElementById("wrapper-operatori").innerHTML=o.map(n=>`
        <label class="op-label">
    ...
    `).join(""),e.dataset.idRiga="",e.dataset.nOrdine=t.ordine||"",e.dataset.cliente=t.cliente||"",document.getElementById("messaggio-aiuto").value="",Wo("DOMANDA");let i=document.getElementById("modal-ordine-row");if(i){i.style.display="block";let n=document.getElementById("modal-ordine-input");n&&(n.value=t.ordine||"",t.ordine&&(e.dataset.nOrdine=t.ordine,e.dataset.cliente=t.cliente||""),Ur(n))}He.length===0&&fetch(x,{method:"POST",body:JSON.stringify({pagina:"PROGRAMMA PRODUZIONE DEL MESE"})}).then(n=>n.json()).then(n=>{let a=new Set;He=n.filter(s=>String(s.archiviato||"").toUpperCase()!=="TRUE").map(s=>({ordine:s.ordine||"",cliente:s.cliente||"",riferimento:s.riferimento||""})).filter(s=>!s.ordine||a.has(s.ordine)?!1:(a.add(s.ordine),!0))}).catch(()=>{})}function Ur(t){t.oninput=function(){let e=this.value.trim().toLowerCase(),o=document.getElementById("ordine-autocomplete");if(!o)return;if(!e){o.style.display="none",o.innerHTML="";return}let i=He.filter(n=>n.ordine.toLowerCase().includes(e)||n.cliente.toLowerCase().includes(e)).slice(0,8);if(i.length===0){o.style.display="none",o.innerHTML="";return}o.innerHTML=i.map(n=>`
        ...
        `).join(""),o.style.display="block"},t.onblur=function(){setTimeout(()=>{let e=document.getElementById("ordine-autocomplete");e&&(e.style.display="none")},200)}}function Fr(t,e){let o=document.getElementById("modal-ordine-input");o&&(o.value=t);let i=document.getElementById("ordine-autocomplete");i&&(i.style.display="none",i.innerHTML="");let n=document.getElementById("modalAiuto");n&&(n.dataset.nOrdine=t,n.dataset.cliente=e||"")}function Wo(t){let e=t.toUpperCase();document.getElementById("modalAiuto").dataset.tipoAzione=e,document.getElementById("btn-tipo-assegna").classList.toggle("active",e==="ASSEGNAZIONE"),document.getElementById("btn-tipo-domanda").classList.toggle("active",e==="DOMANDA")}function Zo(){let t=document.getElementById("modalAiuto");t.style.display="",t.classList.remove("active")}async function Hr(){let t=document.getElementById("modalAiuto");if(t){M.pauseFor(6e3);try{let e=t.dataset.idRiga,o=document.getElementById("modal-ordine-row"),i=document.getElementById("modal-ordine-input"),n=o&&o.style.display!=="none"&&i&&i.value.trim()?i.value.trim():t.dataset.nOrdine,a=document.getElementById("messaggio-aiuto").value,s=t.dataset.tipoAzione,r=document.querySelectorAll('input[name="destinatario"]:checked');if(r.length===0){alert("Per favore, seleziona almeno un operatore.");return}let c=Array.from(r).map(p=>p.getAttribute("data-nome")).join(", "),l=Array.from(r).map(p=>p.getAttribute("data-nome"));document.getElementById("messaggio-aiuto").value="",Zo(),g(s==="ASSEGNAZIONE"?"\u2705 Assegnazione inviata":"\u2705 Richiesta inviata"),Ie();let d=`${x}?azione=assegnaOperatori&ordine=${encodeURIComponent(n)}&operatori=${encodeURIComponent(c)}&id_riga=${e}&mittente=${encodeURIComponent(b.nome.toUpperCase().trim())}&registra=0`,u=(t.dataset.cliente||"").trim(),f={azione:"supporto_multiplo",n_ordine:n,cliente:u,tipo:s,messaggio:a||(s==="ASSEGNAZIONE"?"Nuova assegnazione":"Nuova domanda"),mittente:b.nome.toUpperCase().trim(),destinatari:l};Promise.all([s==="ASSEGNAZIONE"?fetch(d).catch(()=>g("Errore assegnazione operatore.","error")):Promise.resolve(),fetch(x,{method:"POST",body:JSON.stringify(f)}).catch(()=>g("Errore invio richiesta.","error"))]).then(()=>{window.paginaAttuale==="STORICO_RICHIESTE"?Ot().catch(()=>{}):(ne().then(p=>Ge(p.attive)).catch(()=>{}),window.caricaDati?.(window.paginaAttuale).catch(()=>{}))})}catch{g("Errore invio richiesta.","error")}}}function Mn(t){let e=document.getElementById("box-risposta-"+t),o=document.getElementById("box-conferma-"+t);if(e)if(o&&(o.style.display="none",o.style.opacity="0"),e.style.display==="none"||e.style.display===""){e.style.display="block",setTimeout(()=>{e.style.opacity="1",e.style.transform="translateY(0)"},10);let i=document.getElementById("input-risposta-"+t);i&&(i.focus(),setTimeout(()=>{e.scrollIntoView({behavior:"smooth",block:"center"})},400))}else e.style.opacity="0",e.style.transform="translateY(-10px)",setTimeout(()=>{e.style.display="none"},300)}function jr(t){let e=document.getElementById("box-conferma-"+t),o=document.getElementById("box-risposta-"+t);e&&(o&&(o.style.display="none",o.style.opacity="0"),e.style.display==="none"||e.style.display===""?(e.style.display="block",setTimeout(()=>{e.style.opacity="1",e.style.transform="translateY(0)"},10)):(e.style.opacity="0",e.style.transform="translateY(-10px)",setTimeout(()=>{e.style.display="none"},300)))}async function Gr(t,e,o,i){try{let n=document.getElementById("input-risposta-"+t),a=n.value.trim();if(!a)return;M.pauseFor(6e3),n.value="",Mn(t),g("\u2705 Risposta inviata"),Ie();let s={azione:"supporto_multiplo",n_ordine:e,cliente:(i||"").trim(),tipo:"RISPOSTA",messaggio:a,mittente:b.nome.toUpperCase().trim(),destinatari:String(o).split(",").map(r=>r.trim().toUpperCase()).filter(Boolean)};fetch(x,{method:"POST",body:JSON.stringify(s)}).then(()=>{window.paginaAttuale==="STORICO_RICHIESTE"&&Ot().catch(()=>{})}).catch(()=>g("Errore invio risposta.","error"))}catch{g("Errore invio risposta.","error")}}function Nn(t,e){try{localStorage.setItem("_rg_"+t,e.open?"1":"0")}catch{}}function Ve(t){let e=String(t??"").trim();if(!e)return 0;let o=e.replace(/\./g,"").replace(",","."),i=Number(o);return Number.isFinite(i)?i:0}function _t(t){return Number.isFinite(t)?Math.abs(t-Math.round(t))<1e-4?String(Math.round(t)):t.toLocaleString("it-IT",{minimumFractionDigits:0,maximumFractionDigits:2}):"0"}function Qo(t){let e=String(t||"").trim().toUpperCase();return["IMBALLATO","SPEDITO","CONSEGNATO","SPEDITO/CONSEGNATO","SPEDITI/CONSEGNATI","ANNULLATO","ANNULLATI"].includes(e)}function Vr(t){document.querySelectorAll(".fabprod-modal-overlay").forEach(e=>e.remove()),window.cambiaPagina?.("PROGRAMMA PRODUZIONE DEL MESE",null),setTimeout(()=>{["universal-search","mobile-search"].forEach(e=>{let o=document.getElementById(e);o&&(o.value=t,o.dispatchEvent(new Event("input")))}),typeof window.filtraUniversale=="function"&&window.filtraUniversale()},420)}function Jr(t,e){document.getElementById("fabprod-modal-ordine")?.remove();let o=e?` \xB7 ${e}`:"",i=t.replace(/\\/g,"\\\\").replace(/'/g,"\\'"),n=document.createElement("div");n.id="fabprod-modal-ordine",n.className="fabprod-modal-overlay",n.innerHTML=`
        ...
        <div class="fabprod-modal-box fabprod-modal-box--art">
            ...
            <div class="fabprod-modal-art-qty">${_t(o.qty)} pz totali richiesti</div>
            <div class="fabprod-modal-art-orders">${i||'<span style="color:#94a3b8;font-size:0.8rem">Nessun ordine</span>'}</div>
        </div>`,n.addEventListener("click",a=>{a.target===n&&n.remove()}),document.body.appendChild(n)}function Wr(t){window.innerWidth<=768&&qn(t)}function tt(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Dn(){if(Array.isArray(ie))return ie.map(o=>{let i=Number(o.qtyTotale??o.qty??0)||0,n=Array.isArray(o.ordini)?o.ordini:[];return{codice:String(o.codice||"").trim(),prodotto:String(o.prodotto||"").trim(),ordini:n,qtyTotale:i,formulaQty:String(o.formulaQty||_t(i))}});let t=new Map;for(let o of je||[]){if(!o||!o.ordine||String(o.archiviato||"").toUpperCase()==="TRUE"||Qo(o.stato))continue;let i=String(o.ordine||"").trim();if(!i||!Q.has(i))continue;let n=Ve(o.qty),a=Ve(o.qty_evasa),s=Math.max(n-a,0);if(s<=0)continue;let r=String(o.prodotto||"").trim();if(!r)continue;let c=String(o.codice||"").trim(),l=String(o.descrizione||o.dettaglio||o.riferimento||o.rif_articolo||o.note||"").trim(),d=`${c.toUpperCase()}|${r.toUpperCase()}`;t.has(d)||t.set(d,{codice:c,prodotto:r,descrizione:l,qtyTotale:0,ordiniMap:new Map});let u=t.get(d);u.qtyTotale+=s,!u.descrizione&&l&&(u.descrizione=l);let f=String(o.cliente||"").trim();u.ordiniMap.has(i)||u.ordiniMap.set(i,{ordine:i,cliente:f,qty:0}),u.ordiniMap.get(i).qty+=s}let e=[...t.values()].map(o=>{let i=[...o.ordiniMap.values()].sort((s,r)=>s.ordine.localeCompare(r.ordine,"it",{numeric:!0,sensitivity:"base"})),n=i.map(s=>_t(s.qty)),a=i.length>1?`${n.join(" + ")} = ${_t(o.qtyTotale)}`:_t(o.qtyTotale);return{codice:o.codice,prodotto:o.prodotto,ordini:i,qtyTotale:o.qtyTotale,formulaQty:a}});return e.sort((o,i)=>{let n=o.prodotto.localeCompare(i.prodotto,"it",{sensitivity:"base"});return n!==0?n:(o.codice||"").localeCompare(i.codice||"","it",{sensitivity:"base"})}),e}function We(){ie=null,ee=""}function Zr(t){let e=(t||[]).reduce((o,i)=>o+(Number(i.qtyTotale||0)||0),0);return{titolo:"Fabbisogno Produzione",generatedAt:Date.now(),generatedBy:String(b?.nome||"Sistema"),righe:Array.isArray(t)?t.length:0,totaleQty:e}}function Qr(t){let e=t&&t.distinta;if(!e||!Array.isArray(e.sezioni))return;let o=[];for(let i of e.sezioni)for(let n of i.righe||[]){let a=Number(n.totale||0),s=Number(n.disponibile||0);s<a&&o.push({nome:String(n.nome||"").trim(),delta:Math.max(0,a-s)})}kn={ts:Date.now(),kitNome:String(t.kitNome||"").trim(),documento:String(t.documento||"").trim(),righe:Number(e.totaleRighe||0),deficits:o},window._kitFabbisognoSummary=kn,window.paginaAttuale==="STORICO_RICHIESTE"&&Ft()}function Kr(t){let e=new Date,o=[...Q].sort((s,r)=>s.localeCompare(r,"it",{numeric:!0,sensitivity:"base"})),i=o.length<=6?o.join(", "):`${o.slice(0,6).join(", ")} +${o.length-6}`,n=t.reduce((s,r)=>s+(Number(r.qtyTotale)||0),0),a=t.map(s=>`
        <tr>
            <td>${tt(s.codice||"-")}</td>
            <td>${tt(s.prodotto)}</td>
            <td>${s.ordini.map(r=>`ORD. ${tt(r.ordine)}${r.cliente?` \xB7 ${tt(r.cliente)}`:""}`).join("<br>")}</td>
            <td class="qty">${tt(s.formulaQty)}</td>
        </tr>
    ...
    <div class="stage">
        <div class="page">
            ...
            <div class="meta">
                ...
                <div class="meta-card"><div class="meta-k">Totale quantit\xE0</div><div class="meta-v">${tt(_t(n))} pz</div></div>
            </div>
            ...
            <div class="footer">Generato da PROD - ${tt(String(b?.nome||"Sistema"))}</div>
        </div>
    </div>
    <script>
...
' + document.documentElement.outerHTML;
    ...
    <\/script>
...
</html>`}function Yr(){let t=Dn();if(!t.length){g("Nessuna riga utile da stampare per il fabbisogno corrente.","warning");return}let e=window.open("","_blank");if(!e){g("Popup bloccato: abilita l'anteprima di stampa.","warning");return}e.document.open(),e.document.write(Kr(t)),e.document.close(),e.focus()}async function Xr(){let t=Dn();if(!t.length){g("Nessuna riga utile da salvare nel fabbisogno.","warning");return}let e=[...Q].sort((i,n)=>i.localeCompare(n,"it",{numeric:!0,sensitivity:"base"})),o={id:ee||void 0,ordini:e,rows:t,meta:Zr(t)};try{let i=await xt({azione:"saveFabbisogno",payload:o});if(i?.status!=="ok")throw new Error(i?.message||"saveFabbisogno failed");ee=String(i.id||ee||""),g("Fabbisogno salvato su Sheets.","success")}catch(i){console.error("[richieste] saveFabbisogno error:",i),g("Errore salvataggio fabbisogno.","error")}}function Fe(t){return t.length?t.map(function(e){let o=Array.isArray(e.ordini)?e.ordini:[],i=o.length?o.slice(0,4).join(", ")+(o.length>4?" ...":""):"Ordini non specificati",n=e.updatedAt?new Date(e.updatedAt).toLocaleString("it-IT"):"-",a=Number(e.rowsCount||0)||0,r=String(e.status||"ACTIVE").toUpperCase()==="ARCHIVED"?"Archiviato":"Attivo",c=tt(e.id||"");return`
            <div class="fabprod-sel-item fabprod-sel-item--checked" style="display:block;cursor:default">
                <div class="fabprod-sel-item-info" style="gap:4px">
                    ...
                    <span class="fabprod-sel-cli">Stato: ${tt(r)} \xB7 Aggiornato: ${tt(n)} \xB7 Righe: ${a}</span>
        ...
        </div>`,t.addEventListener("click",function(o){o.target===t&&Ko()}),document.body.appendChild(t),requestAnimationFrame(function(){t.classList.add("fabprod-sel-modal-overlay--in")});let e=document.getElementById("fabprod-archivio-body");try{let o=await xt({azione:"getFabbisogni",includeArchived:!1}),i=Array.isArray(o?.items)?o.items:[];if(i.length){oe=i,e&&(e.innerHTML=Fe(oe));return}let n=await xt({azione:"getFabbisogni",includeArchived:!0}),a=Array.isArray(n?.items)?n.items:[];if(oe=a,!e)return;if(!a.length){e.innerHTML=Fe([]);return}e.innerHTML=`
            <div class="empty-msg" style="margin:0 0 12px">Nessun snapshot attivo: mostro anche gli archiviati.</div>
            ${Fe(a)}
        `}catch(o){console.error("[richieste] getFabbisogni error:",o),e&&(e.innerHTML='<div class="empty-msg" style="margin:16px 0">Errore caricamento fabbisogni.</div>'),g("Errore caricamento fabbisogni.","error")}}function Ko(){document.getElementById("fabprod-archivio-modal")?.remove()}async function ec(t){let e=String(t||"").trim();if(e)try{let o=await xt({azione:"getFabbisognoById",id:e});if(o?.status!=="ok"||!o?.item)throw new Error(o?.message||"Snapshot non trovato");let i=o.item;ee=String(i.id||e),ie=Array.isArray(i.rows)?i.rows.map(function(n){return{codice:String(n?.codice||"").trim(),prodotto:String(n?.prodotto||"").trim(),qty:Number(n?.qtyTotale??n?.qty??0)||0,formulaQty:String(n?.formulaQty||""),ordini:Array.isArray(n?.ordini)?n.ordini:[]}}):[],Q=new Set(Array.isArray(i.ordini)?i.ordini.map(function(n){return String(n||"").trim()}).filter(Boolean):[]),Ko(),Ft(),Xo(),g("Snapshot fabbisogno caricato.","success")}catch(o){console.error("[richieste] getFabbisognoById error:",o),g("Errore apertura snapshot fabbisogno.","error")}}async function oc(t){let e=String(t||"").trim();if(!(!e||!await rt("Archiviare questo fabbisogno condiviso?",{title:"Archivia snapshot",confirmText:"Archivia",cancelText:"Annulla",confirmType:"danger"})))try{let i=await xt({azione:"archiveFabbisogno",id:e});if(i?.status!=="ok")throw new Error(i?.message||"Archiviazione non riuscita");oe=oe.filter(function(a){return String(a.id||"")!==e});let n=document.getElementById("fabprod-archivio-body");n&&(n.innerHTML=Fe(oe)),ee===e&&We(),Ft(),g("Snapshot archiviato.","success")}catch(i){console.error("[richieste] archiveFabbisogno error:",i),g("Errore archiviazione snapshot.","error")}}function Ft(){let t=document.getElementById("fabprod-list"),e=document.getElementById("fabprod-sel-badge"),o=document.getElementById("fabprod-cnt-badge");if(!t)return;let i=Array.isArray(ie)?ie:Un(je.filter(r=>!r||!r.ordine?!1:Q.has(String(r.ordine).trim())));i.length?(window._fabprodCurrentRows=i,t.innerHTML=i.map((r,c)=>{let l=r.ordini.map(u=>{let f=u.ordine.replace(/\\/g,"\\\\").replace(/'/g,"\\'"),p=(u.cliente||"").replace(/\\/g,"\\\\").replace(/'/g,"\\'");return`<span class="fabprod-order-pill fabprod-order-pill--click" onclick="event.stopPropagation();_fabprodApriModalOrdine('${f}','${p}')">ORD. ${u.ordine}${u.cliente?`<span class="fabprod-pill-cliente"> \xB7 ${u.cliente}</span>`:""}</span>`}).join(""),d=r.ordini.length>1?`<div class="fabprod-qty-breakdown">${r.ordini.map(u=>`${_t(u.qty)}\xA0(${u.ordine})`).join(" + ")}</div>`:"";return`<div class="fabprod-card" onclick="_fabprodCardClick(${c})">
                <div class="fabprod-top">
                    <div class="fabprod-name">${r.codice?`<span class="fabprod-code">${r.codice}</span>`:""}${r.prodotto}</div>
                    <span class="fabprod-qty">${_t(r.qty)} pz</span>
                </div>
                ${d}
                <div class="fabprod-orders">${l}</div>
            </div>`}).join("")):t.innerHTML=Q.size===0?'<div class="fabprod-empty-sel"><i class="fas fa-hand-pointer fabprod-empty-sel-icon"></i><div class="fabprod-empty-sel-text">Seleziona gli ordini per vedere il fabbisogno</div></div>':'<div class="empty-msg" style="margin:16px 0 8px">Nessun articolo attivo per gli ordini selezionati.</div>';let n=Q.size;e&&(e.textContent=n,e.style.display=n>0?"":"none"),o&&(o.textContent=i.length,o.style.display=i.length>0?"":"none");let a=document.getElementById("fabprod-print-btn");a&&(a.disabled=i.length===0);let s=document.getElementById("fabprod-save-btn");s&&(s.disabled=i.length===0)}function ic(){document.getElementById("fabprod-sel-modal")?.remove();let t=n=>{let a=(n||"").trim().toLowerCase(),s=a?Ut.filter(r=>r.ordine.toLowerCase().includes(a)||(r.cliente||"").toLowerCase().includes(a)):Ut;return!s.length&&!Ut.length?'<div class="empty-msg" style="margin:16px 0">Nessun ordine attivo disponibile.</div>':s.length?s.map(r=>{let c=Q.has(r.ordine)?" checked":"",l=r.ordine.replace(/"/g,"&quot;");return`<label class="fabprod-sel-item${c?" fabprod-sel-item--checked":""}">
        ...
        <div class="fabprod-sel-modal-box">
        ...
        </div>`,e.addEventListener("change",n=>{if(!n.target.classList.contains("fabprod-sel-chk"))return;n.target.closest(".fabprod-sel-item")?.classList.toggle("fabprod-sel-item--checked",n.target.checked);let a=e.querySelectorAll(".fabprod-sel-chk:checked").length,s=e.querySelector("#fabprod-sel-apply");s&&(s.textContent=a>0?`Applica (${a})`:"Applica")}),e.addEventListener("input",n=>{if(n.target.id!=="fabprod-sel-search")return;e.querySelectorAll(".fabprod-sel-chk").forEach(c=>{c.checked?Q.add(c.value):Q.delete(c.value)});let a=e.querySelector("#fabprod-sel-modal-body");a&&(a.innerHTML=t(n.target.value));let s=Q.size,r=e.querySelector("#fabprod-sel-apply");r&&(r.textContent=s>0?`Applica (${s})`:"Applica")});let o=Q.size,i=e.querySelector("#fabprod-sel-apply");i&&o>0&&(i.textContent=`Applica (${o})`),e.addEventListener("click",n=>{n.target===e&&Yo()}),document.body.appendChild(e),requestAnimationFrame(()=>{e.classList.add("fabprod-sel-modal-overlay--in"),e.querySelector("#fabprod-sel-search")?.focus()})}function nc(){let t=document.getElementById("fabprod-sel-modal");t&&(Q=new Set([...t.querySelectorAll(".fabprod-sel-chk:checked")].map(e=>e.value)),We(),Yo(),Ft())}function Yo(){document.getElementById("fabprod-sel-modal")?.remove()}function Xo(){let t=document.getElementById("rg-fabbisogno-produzione");t&&(t.open||(t.open=!0,Nn("fabbisogno_produzione",t)),t.scrollIntoView({behavior:"smooth",block:"start"}))}function Bn(t){let e=[...document.querySelectorAll(".scad-card[data-ordine]")];if(!e.length)return[];let o=new Set((Ut||[]).map(n=>String(n.ordine||"").trim())),i=[];return e.forEach(n=>{let a=n.querySelector(".scad-fab-chk");if(t&&!a?.checked)return;let s=String(n.dataset.ordine||"").trim();!s||!o.has(s)||i.push(s)}),[...new Set(i)].sort((n,a)=>n.localeCompare(a,"it",{numeric:!0,sensitivity:"base"}))}function ac(){let t=Bn(!1);if(!t.length){g("Nessuna scadenza utile trovata per creare il fabbisogno.","warning");return}Q=new Set(t),We(),Ft(),Xo(),g(`Fabbisogno impostato su ${t.length} ordini da scadenze.`,"success")}function sc(){let t=Bn(!0);if(!t.length){g("Flagga almeno una scadenza per creare il fabbisogno.","warning");return}Q=new Set(t),We(),Ft(),Xo(),g(`Fabbisogno impostato su ${t.length} ordini selezionati.`,"success")}function Un(t){let e=new Map;return(t||[]).forEach(o=>{if(!o||String(o.archiviato||"").toUpperCase()==="TRUE"||Qo(o.stato))return;let i=String(o.prodotto||"").trim();if(!i)return;let n=Ve(o.qty),a=Ve(o.qty_evasa),s=Math.max(n-a,0);if(s<=0)return;let r=i.toLocaleUpperCase("it-IT");e.has(r)||e.set(r,{prodotto:i,codice:String(o.codice||"").trim(),qty:0,ordini:new Map});let c=e.get(r);if(!c.codice&&o.codice&&(c.codice=String(o.codice).trim()),c.qty+=s,o.ordine){let l=String(o.ordine).trim(),d=String(o.cliente||"").trim();c.ordini.has(l)||c.ordini.set(l,{cliente:d,qty:0}),c.ordini.get(l).qty+=s}}),Array.from(e.values()).map(o=>({prodotto:o.prodotto,codice:o.codice,qty:o.qty,ordini:Array.from(o.ordini.entries()).map(([i,{cliente:n,qty:a}])=>({ordine:i,cliente:n,qty:a})).sort((i,n)=>i.ordine.localeCompare(n.ordine,"it"))})).sort((o,i)=>(o.codice||"").localeCompare(i.codice||"","it",{sensitivity:"base"}))}async function rc(){let t=window._attiviProd;if(Array.isArray(t)&&t.length)return t;let e=null;if(A.dashBundle)e=A.dashBundle,A.dashBundle=null,A.dashPromise=null;else if(A.dashPromise)e=await A.dashPromise,A.dashBundle=null,A.dashPromise=null;else{let o=await fetch(x,{method:"POST",body:JSON.stringify({azione:"getAllDashboard",includeArchivio:!1})});if(!o.ok)throw new Error(`HTTP ${o.status}`);e=await o.json()}return e&&e.produzione||[]}async function ne(t=null){async function e(){if(A.rqBundle){let s=A.rqBundle;return A.rqBundle=null,A.rqPromise=null,s}if(A.rqPromise){let s=await A.rqPromise;return A.rqBundle=null,A.rqPromise=null,s}let a=await fetch(x,{method:"POST",body:JSON.stringify({azione:"getAllRichieste"}),...t?{signal:t}:{}});if(!a.ok)throw new Error(`HTTP ${a.status}`);return a.json()}let[o,i]=await Promise.all([e(),rc().catch(a=>(console.warn("Fabbisogno Produzione non disponibile:",a),[]))]);if(!o)throw new Error("bundle vuoto");let n=Un(i);return{attive:o.attive||[],archivio:o.archivio||[],fabbisogno:n,fabbisognoRaw:i}}async function Ot(t=null,e=null){let o=document.getElementById("contenitore-dati");if(!o)return;o.innerHTML="<div class='centered-msg' id='_ric-loader'>Caricamento messaggi in corso...</div>";let i=setTimeout(()=>{let n=document.getElementById("_ric-loader");n&&(n.innerHTML=`\u26A0\uFE0F Connessione lenta o server non raggiungibile.<br>
            <button onclick="cambiaPagina('STORICO_RICHIESTE', null)"
                ...
                &#x21bb; Riprova
            </button>`)},12e3);try{let n=await ne(e);if(clearTimeout(i),Ge(n.attive),t!==null&&t!==window._latestNavRequest)return;window.aggiornaBadgeNotifiche?.(n.attive),Ae(n)}catch(n){if(clearTimeout(i),n.name==="AbortError")return;console.error("Errore caricamento richieste:",n),o.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('STORICO_RICHIESTE',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(o)}}function Ae(t){if(window.paginaAttuale!=="STORICO_RICHIESTE")return;let e=document.getElementById("contenitore-dati");if(!e)return;let o=t.attive||[],i=t.archivio||[],n=t.fabbisogno||[];je=t.fabbisognoRaw||[],Q=new Set;{let P=new Set;Ut=[];for(let I of je){if(!I||!I.ordine||String(I.archiviato||"").toUpperCase()==="TRUE"||Qo(I.stato))continue;let T=String(I.ordine).trim();!T||P.has(T)||(P.add(T),Ut.push({ordine:T,cliente:String(I.cliente||"").trim()}))}Ut.sort((I,T)=>I.ordine.localeCompare(T.ordine,"it"))}let a=(b?.nome||"").toUpperCase().trim(),s=P=>{let I={};return P.forEach(T=>{I[T.ORDINE]||(I[T.ORDINE]=[]),I[T.ORDINE].push(T)}),I},r=s(o),c=s(i),l=(()=>{if(Nr())return()=>!0;let P=yt(b?.nome||"").toUpperCase();return I=>I.some(T=>{if(yt(T.DA||"").toUpperCase()===P)return!0;var B=String(T.A||"");return B.split(",").some(function(Y){return yt(Y.trim()).toUpperCase()===P})})})(),d=P=>{let I={};return Object.keys(P).forEach(T=>{l(P[T])&&(I[T]=P[T])}),I},u=d(r),f=d(c),p={},m={},h={};Object.keys(u).forEach(P=>{let T=u[P].filter(B=>(B.TIPO||"").toUpperCase()!=="SCADENZA");T.length>0&&((T[0].TIPO||"MSG").toUpperCase()==="ASSEGNAZIONE"?p[P]=T:m[P]=T)}),Object.keys(r).forEach(P=>{let I=r[P].filter(T=>(T.TIPO||"").toUpperCase()==="SCADENZA");I.length>0&&(h[P+"_scad"]=I)});let w=localStorage.getItem("_rg_assegnazioni")!=="0",v=localStorage.getItem("_rg_richieste")!=="0",E=localStorage.getItem("_rg_scadenze")!=="0",C=localStorage.getItem("_rg_fabbisogno_produzione")!=="0",_=Object.keys(p).length,$=Object.keys(m).length,k=Object.values(h).reduce((P,I)=>P+I.length,0),R=n.length,O=(P,I)=>{let T="";return Object.keys(P).reverse().forEach(B=>{T+=Vo(P[B],I,!1)}),T||'<div class="empty-msg" style="margin:16px 0 8px">Nessun elemento.</div>'},q=()=>{let P=Object.values(h).flat();return P.sort((I,T)=>{let B=Jo(I),Y=Jo(T);return!B&&!Y?0:B?Y?B-Y:-1:1}),P.map(I=>jn(I,a)).join("")||'<div class="empty-msg" style="margin:16px 0 8px">Nessuna scadenza.</div>'},G=P=>!P||!P.length?Q.size===0?`<div class="fabprod-empty-sel">
                    ...
                    </div>`:'<div class="empty-msg" style="margin:16px 0 8px">Nessun articolo attivo per gli ordini selezionati.</div>':(window._fabprodCurrentRows=P,P.map((I,T)=>{let B=I.ordini.map(st=>{let L=st.ordine.replace(/\\/g,"\\\\").replace(/'/g,"\\'"),F=(st.cliente||"").replace(/\\/g,"\\\\").replace(/'/g,"\\'");return`<span class="fabprod-order-pill fabprod-order-pill--click" onclick="event.stopPropagation();_fabprodApriModalOrdine('${L}','${F}')">ORD. ${st.ordine}${st.cliente?`<span class="fabprod-pill-cliente"> \xB7 ${st.cliente}</span>`:""}</span>`}).join(""),Y=I.ordini.length>1?`<div class="fabprod-qty-breakdown">${I.ordini.map(st=>`${_t(st.qty)}\xA0(${st.ordine})`).join(" + ")}</div>`:"";return`
                <div class="fabprod-card" onclick="_fabprodCardClick(${T})">
                    <div class="fabprod-top">
                        ...
                        <span class="fabprod-qty">${_t(I.qty)} pz</span>
                    </div>
                    ${Y}
                    <div class="fabprod-orders">${B}</div>
                </div>`}).join("")),mt="";Object.keys(f).length===0?mt='<div class="empty-msg" style="margin:20px 0">Nessuna richiesta archiviata.</div>':Object.keys(f).reverse().forEach(P=>{mt+=Vo(f[P],a,!0)});let at=`
        ...
        <div class="req-card${o?" archiviata":""}${r?" sollecitata":""}" data-id-riga="${String(i.id_riga||"")}" data-ordine="${String(a||"")}" data-cliente="${(s||"").toLowerCase().replace(/"/g,"")}" data-riferimento="${(i.RIFERIMENTO||"").toLowerCase().replace(/"/g,"")}">
            ...
            <div class="rc-foot">
                <span class="rc-date">${Je(i["DATA ORA"])}</span>
            ...
            <div id="rc-body-${i.id_riga}" class="rc-body">
                ${t.map(h=>{let w=String(h.DA).toUpperCase().trim()===e,v=String(h.MESSAGGIO||"").includes("|")?h.MESSAGGIO.split("|")[1]:h.MESSAGGIO,E=h["DATA ORA"]?Je(h["DATA ORA"]):"";return`
    ...
    <div class="scad-card ${d}" data-id-riga="${String(t.id_riga||"")}" data-ordine="${tt(a)}" data-cliente="${s.toLowerCase().replace(/"/g,"")}">
        ...
        <div class="rc-foot">
            <span class="rc-lbl">Da</span>
            <span class="rc-val">${c}</span>
            <span class="rc-date" style="margin-left:auto">${Je(l)}</span>
        </div>
        <div class="rc-actions">
    ...
    </div>`}function Gn(){window.chiudiModal=Zo,window.confermaInvioSupporto=Hr,window.setTipoAzione=Wo,window.chiudiModalSollecito=Pn,window.confermaInvioSollecito=Dr,window.apriNuovaRichiesta=zn,window.apriModalAiuto=Br,window.apriModalSollecito=qr,window.toggleAreaRisposta=Mn,window.toggleBoxArchivia=jr,window.inviaRisposta=Gr,window._selezionaOrdine=Fr,window.aggiornaBadgeRichieste=Ge,window.aggiornaBadgeSidebar=Ge,window.caricaRichieste=Ot,window._fetchDatiRichieste=ne,window._renderDatiRichieste=Ae,window._saveReqGroup=Nn,window._fabprodCardClick=Wr,window._fabprodApriModalOrdine=Jr,window._fabprodApriModalArticolo=qn,window._fabprodVaiOrdine=Vr,window._aggiornaPannelloFabbisogno=Ft,window._apriModalFabbisognoSel=ic,window._applicaSelFabbisogno=nc,window._chiudiModalFabbisognoSel=Yo,window._fabprodDaScadenzeTutte=ac,window._fabprodDaScadenzeFlaggate=sc,window._fabprodStampaFabbisognoSel=Yr,window._fabprodSalvaSnapshotCondiviso=Xr,window._fabprodApriArchivioSnapshot=tc,window._fabprodChiudiArchivioSnapshot=Ko,window._fabprodApriSnapshotById=ec,window._fabprodArchiviaSnapshotById=oc,window._notificaFabbisognoNuovo=Qr,window.aggiornaRichiesta=Fn,window._sollecitaConferma=pc,window._archiviaConferma=uc,window.sollecitaRichiesta=Hn,window.cambiaVistaUtente=cc,window._toggleRcBody=mc,window.formattaData=Je,window.generaCardRichiesta=Vo,window.generaCardScadenza=jn,window._getScadDate=Jo}function Vn(){let t=document.getElementById("btn-nuova-richiesta");t&&t.addEventListener("click",function(o){o.stopPropagation(),zn()});let e=document.getElementById("modalAiuto");e&&e.addEventListener("click",function(o){o.target===this&&(Date.now()-(this._openedAt||0)<800||Zo())}),document.addEventListener("click",function(o){o.target.closest(".req-card")||document.querySelectorAll(".box-risposta, .box-conferma").forEach(function(i){i.style.display!=="none"&&i.style.display!==""&&(i.style.opacity="0",i.style.transform="translateY(-10px)",setTimeout(function(){i.style.display="none"},300))})})}var He,Q,je,Ut,ie,ee,oe,kn,yt,Jn=W(()=>{ft();zt();vt();_e();gt();Kt();Qt();It();He=[],Q=new Set,je=[],Ut=[],ie=null,ee="",oe=[],kn=null,yt=t=>window._normNome?window._normNome(t):t&&String(t).trim()});async function Wn(){return ct({azione:"getManuali"},1e4,{retries:2})}async function Zn(t){return ct({azione:"salvaManualeNuovo",...t},18e4,{noDedupe:!0})}async function Qn(t){return ct({azione:"aggiornaManuale",...t},18e4,{noDedupe:!0})}async function Kn(t){return ct({azione:"getStoricoManuale",id:t},1e4,{retries:2})}var Yn=W(()=>{"use strict";_e()});function Xe(t){if(!t)return"-";let e=new Date(t);return Number.isNaN(e.getTime())?String(t):e.toLocaleString("it-IT",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}function et(t){let e=String(t||"").trim();return e&&(/^data:image\/(png|jpe?g|webp|gif);base64,[a-z0-9+/=\s]+$/i.test(e)||/^https?:\/\/[^\s]+$/i.test(e))?e:""}function to(t){return t&&t.sections&&t.sections._v===2?t.sections:null}function Ye(t,e,o){if(t){let i=o?`<button type="button" onclick="${o}" title="Rimuovi foto" style="position:absolute;top:-7px;right:-7px;background:#ef4444;border:none;color:#fff;width:22px;height:22px;border-radius:50%;font-size:1.1rem;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 1px 4px rgba(0,0,0,.3)">&times;</button>`:"";return`<div class="foto-wrapper" style="position:relative;display:inline-block;max-width:100%;margin-bottom:6px"><img src="${t}" alt="${e}" style="max-width:100%;max-height:180px;border-radius:10px;border:1px solid #e2e8f0;display:block">${i}</div>`}return'<div class="text-xs text-slate-400" style="margin-bottom:6px">Nessuna foto</div>'}function Ce(t){return`
    <input type="file" class="manuale-file-input" accept="image/*" onchange="${t}">
    <label class="manuale-file-label" onclick="this.previousElementSibling.click()"><i class="fas fa-upload"></i> Carica foto</label>`}function hc(t){let e=to(t),o=e?Array.isArray(e.procedimenti)?e.procedimenti.length:0:Array.isArray(t.steps)?t.steps.length:0,i=et(t.copertina),n=i?`<img src="${i}" alt="copertina" class="w-full h-40 object-cover rounded-t-xl">`:'<div class="w-full h-40 bg-slate-100 rounded-t-xl flex items-center justify-center text-slate-400 text-3xl"><i class="fas fa-book-open"></i></div>';return`
    <article class="manuale-card materiale-card ${window.TW?.card||""} !p-0 overflow-hidden" data-codice="${y((t.titolo||"")+" "+(t.categoria||""))}">
        ...
        <div class="p-4">
            ...
            <div class="mt-3 text-xs text-slate-600 space-y-1">
                ...
                <p><b>Aggiornato:</b> ${y(Xe(t.updatedAt))}</p>
    ...
    </article>`}function ta(){let t=document.getElementById("contenitore-dati");if(!t)return;let e=se.map(hc).join(""),o=b?.nome?.toUpperCase().trim()==="ALESSIO";t.innerHTML=`
    ...
    <div class="scheda-row scheda-row-custom" data-row-idx="${o}" style="display:grid;grid-template-columns:1fr 1fr 36px;gap:6px;align-items:center">
        ...
        <input type="text" class="input-field-modern" data-field="valore" placeholder="Valore" value="${y(e||"")}">
        <button type="button" class="${window.TW?.btnDanger||""}" onclick="rimuoviSchedaRow(${o})" title="Rimuovi"><i class="fas fa-trash"></i></button>
    </div>`}function oi(t,e){let o=et(t&&t.foto||"");return`
    <div class="occorrente-item border border-slate-200 rounded-xl p-3 bg-white" data-item-idx="${e}"${o?` data-foto="${y(o)}"`:""}>
        ...
        ${Ye(o,`occ-${e}`,"eliminaFotoOccorrente(this)")}
        ${Ce(`cambiaFotoOccorrente(this, ${e})`)}
    </div>`}function ii(t,e){let o=et(t&&t.foto||""),i=et(t&&t.foto2||"");return`
    <div class="proc-step border border-slate-200 rounded-xl p-3 bg-white" data-step-idx="${e}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:6px">
            <div class="proc-foto-slot" data-slot="1"${o?` data-foto="${y(o)}"`:""}>
                ...
                ${Ye(o,`proc-${e}-1`,"eliminaFotoProcedimento(this)")}
                ${Ce(`cambiaFotoProcedimento(this,${e},1)`)}
            </div>
            <div class="proc-foto-slot" data-slot="2"${i?` data-foto="${y(i)}"`:""}>
                ...
                ${Ye(i,`proc-${e}-2`,"eliminaFotoProcedimento(this)")}
                ${Ce(`cambiaFotoProcedimento(this,${e},2)`)}
            </div>
        </div>
    ...
    </div>`}function yc(t){let e=et(t||"");return`
    <div id="manuali-disegno-wrap"${e?` data-foto="${y(e)}"`:""} class="border border-slate-200 rounded-xl p-3 bg-white">
        ${Ye(e,"disegno-tecnico","eliminaFotoDisegno(this)")}
        ${Ce("cambiaFotoDisegno(this)")}
    </div>`}function re(t){return`<button type="button" class="manuali-modal-close" onclick="${t}" aria-label="Chiudi" title="Chiudi"><i class="fas fa-times"></i></button>`}function ni(t,e){let o=document.getElementById("manuali-modal-host");if(!o)return;let i=to(e),n=i?i.schedaTecnica||[]:[],a=i?i.occorrente||[]:[],s=i?i.procedimenti||[]:Array.isArray(e?.steps)?e.steps.map(function(v){return{descrizione:v.descrizione||v.titolo||"",foto:v.foto||""}}):[],r=i&&i.disegnoTecnico?.foto||"",c={},l=[];n.forEach(function(v){Xn.includes(v.voce)?c[v.voce]=v.valore:l.push(v)});let d=0,u=Xn.map(function(v){return bc(v,c[v]||"",d++)}).join("")+l.map(function(v){return ea(v.voce,v.valore,d++)}).join(""),f=a.length>0?a.map(function(v,E){return oi(v,E)}).join(""):oi(null,0),p=s.length>0?s.map(function(v,E){return ii(v,E)}).join(""):ii(null,0),m=et(e?.copertina||""),h=m?`<img id="manuali-copertina-preview" src="${m}" alt="copertina" style="max-width:100%;max-height:200px;border-radius:10px;border:1px solid #e2e8f0;display:block;margin-bottom:6px">`:'<div id="manuali-copertina-preview" class="text-xs text-slate-400" style="margin-bottom:6px">Nessuna copertina</div>',w=(v,E)=>`<h3 style="font-weight:700;font-size:.8rem;text-transform:uppercase;letter-spacing:.05em;color:#64748b;margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid #e2e8f0"><i class="${v}"></i> &nbsp;${E}</h3>`;o.innerHTML=`
    ...
    </div>`,Ke=t==="edit"&&e?.id||null}function wc(){let t=[];document.querySelectorAll("#manuali-scheda-edit .scheda-row").forEach(function(a){let s=String(a.getAttribute("data-voce")||a.querySelector('[data-field="voce"]')?.value||"").trim(),r=String(a.querySelector('[data-field="valore"]')?.value||"").trim();(s||r)&&t.push({voce:s,valore:r})});let e=[];document.querySelectorAll("#manuali-occorrente-edit .occorrente-item").forEach(function(a){let s=String(a.querySelector('[data-field="lettera"]')?.value||"").trim(),r=String(a.querySelector('[data-field="nome"]')?.value||"").trim(),c=String(a.querySelector('[data-field="codice"]')?.value||"").trim(),l=String(a.getAttribute("data-foto")||"").trim();(s||r||c||l)&&e.push({lettera:s,nome:r,codice:c,foto:l})});let o=[];document.querySelectorAll("#manuali-proc-edit .proc-step").forEach(function(a){let s=String(a.querySelector('[data-field="descrizione"]')?.value||"").trim(),r=a.querySelector('.proc-foto-slot[data-slot="1"]'),c=a.querySelector('.proc-foto-slot[data-slot="2"]'),l=String(r?.getAttribute("data-foto")||a.getAttribute("data-foto")||"").trim(),d=String(c?.getAttribute("data-foto")||"").trim();(s||l||d)&&o.push({descrizione:s,foto:l,foto2:d})});let i=document.getElementById("manuali-disegno-wrap"),n={foto:String(i?.getAttribute("data-foto")||"").trim()};return{_v:2,schedaTecnica:t,occorrente:e,procedimenti:o,disegnoTecnico:n}}async function oa(t){return new Promise(function(e,o){let i=new FileReader;i.onload=function(){e(String(i.result||""))},i.onerror=o,i.readAsDataURL(t)})}async function ia(t,e=1200){return new Promise(function(o){let i=new Image;i.onload=function(){let n=Math.min(e/i.width,e/i.height,1),a=document.createElement("canvas");a.width=Math.max(1,Math.round(i.width*n)),a.height=Math.max(1,Math.round(i.height*n));let s=a.getContext("2d");if(!s)return o(t);s.drawImage(i,0,0,a.width,a.height),o(a.toDataURL("image/jpeg",.8))},i.onerror=function(){o(t)},i.src=t})}async function Sc(t,e=800){return new Promise(function(o){let i=new Image;i.onload=function(){let n=Math.min(i.width,i.height),a=Math.round((i.width-n)/2),s=Math.round((i.height-n)/2),r=Math.min(n,e),c=document.createElement("canvas");c.width=r,c.height=r;let l=c.getContext("2d");if(!l)return o(t);l.drawImage(i,a,s,n,n,0,0,r,r),o(c.toDataURL("image/jpeg",.8))},i.onerror=function(){o(t)},i.src=t})}async function _c(t){try{let e=t?.files&&t.files[0];if(!e)return;let o=await oa(e),i=await ia(o,800);if(!i||i.length>ai){g("Immagine di copertina troppo grande, riduci la risoluzione.","warning");return}let n=document.getElementById("manuali-copertina-wrap");if(!n)return;n.setAttribute("data-copertina",i);let a=n.querySelector("img");if(a)a.src=i;else{let s=n.querySelector("#manuali-copertina-preview");s&&s.remove();let r=document.createElement("img");r.id="manuali-copertina-preview",r.src=i,r.alt="copertina",r.style.cssText="max-width:100%;max-height:200px;border-radius:10px;border:1px solid #e2e8f0;display:block;margin-bottom:6px",n.insertBefore(r,n.firstChild)}}catch{g("Errore nel caricamento immagine di copertina.","error")}}async function si(t,e){try{let o=await oa(t),i=await ia(o,800);if(!i||i.length>ai){g("Immagine troppo grande, riduci la risoluzione.","warning");return}e(i)}catch{g("Errore nel caricamento immagine.","error")}}function ri(t,e,o){t.setAttribute("data-foto",e);let i=t.querySelector(".foto-wrapper");if(i){i.querySelector("img").src=e;return}let n=t.querySelector("img");if(n){n.src=e;return}let a=t.querySelector("div.text-xs");a&&a.remove();let s=document.createElement("div");s.className="foto-wrapper",s.style.cssText="position:relative;display:inline-block;max-width:100%;margin-bottom:6px";let r=document.createElement("img");r.src=e,r.style.cssText="max-width:100%;max-height:180px;border-radius:10px;border:1px solid #e2e8f0;display:block",s.appendChild(r),o&&s.insertAdjacentHTML("beforeend",`<button type="button" onclick="${o}" title="Rimuovi foto" style="position:absolute;top:-7px;right:-7px;background:#ef4444;border:none;color:#fff;width:22px;height:22px;border-radius:50%;font-size:1.1rem;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 1px 4px rgba(0,0,0,.3)">&times;</button>`);let c=t.querySelector(".manuale-file-input")||t.querySelector('input[type="file"]');t.insertBefore(s,c)}function ci(t){t.removeAttribute("data-foto");let e=t.querySelector(".foto-wrapper");if(e){let o=document.createElement("div");o.className="text-xs text-slate-400",o.style.marginBottom="6px",o.textContent="Nessuna foto",e.replaceWith(o)}}async function Ec(t,e){let o=t?.files&&t.files[0];o&&await si(o,function(i){let n=document.querySelector(`.occorrente-item[data-item-idx="${e}"]`);n&&ri(n,i,"eliminaFotoOccorrente(this)")})}async function xc(t,e,o){let i=t?.files&&t.files[0];i&&await si(i,function(n){let a=document.querySelector(`.proc-step[data-step-idx="${e}"]`);if(!a)return;let s=a.querySelector(`.proc-foto-slot[data-slot="${o||1}"]`)||a;ri(s,n,"eliminaFotoProcedimento(this)")})}async function Ic(t){let e=t?.files&&t.files[0];e&&await si(e,function(o){let i=document.getElementById("manuali-disegno-wrap");i&&ri(i,o,"eliminaFotoDisegno(this)")})}function Ac(t){ce("Rimuovere la foto da questo elemento?",function(){let e=t.closest(".occorrente-item");e&&ci(e)})}function Cc(t){ce("Rimuovere la foto da questo step?",function(){let e=t.closest(".proc-foto-slot")||t.closest(".proc-step");e&&ci(e)})}function Oc(t){ce("Rimuovere la foto del disegno tecnico?",function(){let e=document.getElementById("manuali-disegno-wrap");e&&ci(e)})}function $c(t){if(!t){ni("new",null);return}let e=jt[t];if(!e){g("Manuale non trovato.","warning");return}ni("edit",e)}function na(){let t=document.getElementById("manuali-modal");t&&t.parentElement&&(t.parentElement.innerHTML=""),Ke=null}function ce(t,e){let o=document.getElementById("manuali-confirm-overlay");o&&o.remove();let i=document.createElement("div");i.id="manuali-confirm-overlay",i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:11000;display:flex;align-items:center;justify-content:center",i.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:28px 32px;max-width:380px;width:90%;box-shadow:0 20px 40px rgba(0,0,0,0.18);text-align:center">
        ...
        <div style="display:flex;gap:10px;justify-content:center">
            ...
            <button id="manuali-confirm-si" class="btn-modal-send" style="min-width:100px;background:#ef4444">Elimina</button>
    ...
    </div>`,document.body.appendChild(i),document.getElementById("manuali-confirm-no").onclick=function(){i.remove()},document.getElementById("manuali-confirm-si").onclick=function(){i.remove(),e()},i.addEventListener("click",function(n){n.target===i&&i.remove()})}function aa(t,e,o,i,n){let a=document.querySelector(t);a&&a.querySelectorAll(e).forEach(function(s,r){s.setAttribute(o,String(r));let c=s.querySelector("button");if(c&&c.setAttribute("onclick",`${i}(${r})`),n){let l=s.querySelector('input[type="file"]');l&&l.setAttribute("onchange",`${n}(this, ${r})`)}})}function Tc(){let t=document.getElementById("manuali-scheda-edit");if(!t)return;let e=t.querySelectorAll(".scheda-row").length;if(e>=gc){g("Numero massimo voci raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",ea("","",e))}function Rc(t){ce("Eliminare questa voce dalla scheda tecnica?",function(){let e=document.querySelector(`.scheda-row[data-row-idx="${t}"]`);e&&e.remove(),aa("#manuali-scheda-edit",".scheda-row","data-row-idx","rimuoviSchedaRow",null)})}function kc(){let t=document.getElementById("manuali-occorrente-edit");if(!t)return;let e=t.querySelectorAll(".occorrente-item").length;if(e>=vc){g("Numero massimo elementi raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",oi({lettera:"ABCDEFGHIJKLMNOPQRSTUVWXYZ"[e]||"",nome:"",codice:"",foto:""},e))}function Lc(t){ce("Eliminare questo elemento dal materiale occorrente?",function(){let e=document.querySelector(`.occorrente-item[data-item-idx="${t}"]`);e&&e.remove(),aa("#manuali-occorrente-edit",".occorrente-item","data-item-idx","rimuoviOccorrenteItem","cambiaFotoOccorrente")})}function Pc(){let t=document.getElementById("manuali-proc-edit");if(!t)return;let e=t.querySelectorAll(".proc-step").length;if(e>=fc){g("Numero massimo step raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",ii(null,e))}function zc(t){ce("Eliminare questo step del procedimento?",function(){let e=document.querySelector(`.proc-step[data-step-idx="${t}"]`);e&&e.remove();let o=document.getElementById("manuali-proc-edit");o&&o.querySelectorAll(".proc-step").forEach(function(i,n){i.setAttribute("data-step-idx",String(n));let a=i.querySelector("h4");a&&(a.textContent="Step "+(n+1));let s=i.querySelector("button");s&&s.setAttribute("onclick",`rimuoviProcStep(${n})`);let r=i.querySelector('input[type="file"]');r&&r.setAttribute("onchange",`cambiaFotoProcedimento(this, ${n})`)})})}async function Mc(){let t=String(document.getElementById("manuali-titolo")?.value||"").trim(),e=String(document.getElementById("manuali-categoria")?.value||"").trim(),o=String(document.getElementById("manuali-copertina-wrap")?.getAttribute("data-copertina")||"").trim(),i=wc();if(!t){g("Inserisci un titolo manuale.","warning");return}try{g("Salvataggio manuale in corso...","info");let n;if(Ke?n=await Qn({id:Ke,titolo:t,categoria:e,copertina:o,sections:i}):n=await Zn({titolo:t,categoria:e,copertina:o,sections:i}),!n||n.status!=="ok")throw new Error(n&&(n.message||n.msg)||"Errore salvataggio manuale");await N.invalidate(ti),delete window.cacheContenuti?.MANUALI_PRODOTTI,delete window.cacheFetchTime?.MANUALI_PRODOTTI,na(),await $e(null,null,!1),g("Manuale salvato correttamente.","success")}catch(n){let a=n&&n.name==="TimeoutError"?"Il salvataggio sta impiegando troppo tempo. Riprova con meno immagini.":n&&n.name==="AbortError"?"Richiesta interrotta. Controlla la connessione e riprova.":n&&n.message||"Errore durante il salvataggio.";g(a,"error")}}function Nc(t){let e=to(t),o=e?e.schedaTecnica||[]:[],i=e?e.occorrente||[]:[],n=e?e.procedimenti||[]:Array.isArray(t.steps)?t.steps.map(function(a){return{descrizione:String(a.descrizione||a.titolo||"").trim(),foto:String(a.foto||"").trim(),foto2:""}}):[];return{titolo:String(t.titolo||"(Senza titolo)"),categoria:String(t.categoria||"Generale"),versione:Number(t.version||1),aggiornato:Xe(t.updatedAt),copertina:et(t.copertina),schedaTecnica:Array.isArray(o)?o:[],occorrente:Array.isArray(i)?i:[],procedimenti:Array.isArray(n)?n:[],disegnoTecnico:et(e?.disegnoTecnico?.foto||"")}}function ae(t){return`<div class="man-print-placeholder">${y(t||"Sezione non disponibile")}</div>`}function qc(t){let e=Nc(t),o=e.copertina?`<img class="man-cover-image" src="${e.copertina}" alt="copertina manuale">`:ae("Copertina non disponibile"),i=e.schedaTecnica.map(function(r){return`<tr>
                <td>${y(r?.voce||"")}</td>
                <td>${y(r?.valore||"")}</td>
            </tr>`}).join(""),n=e.occorrente.map(function(r){let c=et(r?.foto||""),l=c?`<img src="${c}" alt="${y(r?.nome||"componente")}" class="man-occ-image">`:'<div class="man-occ-image man-occ-image--empty">Immagine non disponibile</div>';return`<article class="man-occ-card">
                ...
                ${l}
            </article>`}).join(""),a=e.procedimenti.map(function(r,c){let l=et(r?.foto||""),d=et(r?.foto2||""),u=[l,d].filter(Boolean),f=u.length?`<div class="man-proc-photos">${u.map(function(p){return`<img src="${p}" alt="step-${c+1}">`}).join("")}</div>`:ae("Immagini step non disponibili");return`<article class="man-proc-step">
                <div class="man-proc-num">${c+1}</div>
                <div class="man-proc-body">
                    ${f}
                    <p>${y(r?.descrizione||"Descrizione non disponibile")}</p>
                </div>
            </article>`}).join(""),s=e.disegnoTecnico?`<img class="man-dt-image" src="${e.disegnoTecnico}" alt="disegno tecnico">`:ae("Disegno tecnico non disponibile");return`<!doctype html>
<html lang="it">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<body>
    ...
    <div class="man-print-stage">
        ...
        <section class="man-print-page man-break">
            ...
            ${i?`<table class="man-print-table"><thead><tr><th>Caratteristica</th><th>Valore</th></tr></thead><tbody>${i}</tbody></table>`:ae("Scheda tecnica non disponibile")}
        ...
        <section class="man-print-page man-break">
            ...
            ${n?`<div class="man-occ-grid">${n}</div>`:ae("Materiale occorrente non disponibile")}
        ...
        <section class="man-print-page man-break">
            ...
            ${a||ae("Procedimento non disponibile")}
            <div class="man-page-footer">Sezione step operativi</div>
        </section>
    </div>
</body>
</html>`}function Dc(t){let e=jt[t];if(!e){g("Manuale non trovato.","error");return}let o=window.open("","_blank");if(!o){g("Popup bloccato dal browser. Consenti le finestre popup per stampare.","warning");return}o.document.open(),o.document.write(qc(e)),o.document.close()}function Bc(t){let e=jt[t];if(!e)return;let o=document.getElementById("manuali-modal-host");if(!o)return;let i=et(e.copertina),n=i?`<img src="${i}" alt="copertina" style="max-width:100%;max-height:260px;border-radius:10px;border:1px solid #e2e8f0;margin-bottom:16px">`:"",a=(c,l)=>`<h3 style="font-weight:700;font-size:.8rem;text-transform:uppercase;letter-spacing:.05em;color:#64748b;margin:18px 0 8px;padding-bottom:6px;border-bottom:2px solid #e2e8f0"><i class="${c}"></i> &nbsp;${l}</h3>`,s=to(e),r="";if(s){let c=(s.schedaTecnica||[]).map(function(f){return`<tr>
            ...
            <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden">
                ...
                <tbody>${c}</tbody>
            </table>`),Qe=[];let l=(s.occorrente||[]).map(function(f){let p=et(f.foto||""),m=-1;return p&&(Qe.push({lettera:f.lettera||"",nome:f.nome||"",foto:p}),m=Qe.length-1),`<div ${p?`onclick="_apriLightboxOcc_(${m})"`:""} style="${`padding:10px;border:1px solid #e2e8f0;border-radius:10px;background:#fff${p?";cursor:pointer":""}`}">
            ...
            </div>`}).join("");l&&(r+=a("fas fa-boxes-stacked","Materiale Occorrente")+`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px">${l}</div>`);let d=(s.procedimenti||[]).map(function(f,p){let m=et(f.foto||""),h=et(f.foto2||""),w=[m,h].filter(Boolean),v=w.length?`<div style="display:grid;grid-template-columns:repeat(${w.length},1fr);gap:6px;margin-bottom:6px">${w.map(E=>`<img src="${E}" style="width:100%;border-radius:10px;border:1px solid #e2e8f0">`).join("")}</div>`:"";return`<details class="border border-slate-200 rounded-xl p-3 bg-white" ${p===0?"open":""}>
            ...
            </details>`}).join("");d&&(r+=a("fas fa-list-check","Procedimento")+`<div class="grid gap-2">${d}</div>`);let u=et(s.disegnoTecnico?.foto||"");u&&(r+=a("fas fa-drafting-compass","Disegno Tecnico")+`<img src="${u}" alt="disegno-tecnico" style="max-width:100%;border-radius:10px;border:1px solid #e2e8f0">`)}else r=(e.steps||[]).map(function(c,l){let d=et(c.foto),u=d?`<img src="${d}" alt="step-${l+1}" style="max-width:100%;border-radius:10px;border:1px solid #e2e8f0">`:'<div class="text-xs text-slate-400">Nessuna immagine</div>';return`
    ...
    <div id="manuali-modal" class="modal-overlay active" style="display:flex;z-index:4500">
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap">
            <div>
                ...
                <p class="text-xs text-slate-500 mb-3">${y(e.categoria||"Generale")} \xB7 v${Number(e.version||1)} \xB7 aggiornato ${y(Xe(e.updatedAt))}</p>
            </div>
    ...
    <div id="manuali-storico-modal" class="modal-overlay active" style="display:flex;z-index:4501">
            <div class="modal-content manuali-modal-box" style="max-width:980px;max-height:90vh;overflow:auto;">
                ${re("chiudiStoricoManuale()")}
        <h2>Storico versioni</h2>
        <div class="centered-msg">Caricamento storico...</div>
      </div>
    </div>`;try{let o=await Kn(t);if(!o||o.status!=="ok")throw new Error("Storico non disponibile");let n=(Array.isArray(o.storico)?o.storico:[]).map(function(a,s){let r=a.snapshot||{},c=Array.isArray(r.steps)?r.steps.length:0;return`
            <details class="border border-slate-200 rounded-xl p-3 bg-white" ${s===0?"open":""}>
                <summary class="cursor-pointer font-semibold text-slate-800">
                    v${Number(a.version||0)} \xB7 ${y(a.changeType||"UPDATE")} \xB7 ${y(Xe(a.changedAt))}
                </summary>
    ...
    <div id="manuali-modal" class="modal-overlay active" style="display:flex;z-index:4500">
    ...
    </div>`}async function rl(){let t=String(document.getElementById("pptx-titolo")?.value||"").trim();if(!t){g("Inserisci un titolo per il manuale.","warning");return}let e=document.getElementById("pptx-categoria"),o=String(e?.value||"").trim(),i=document.getElementById("pptx-btn-crea");i&&(i.disabled=!0,i.innerHTML='<i class="fas fa-spinner fa-spin"></i> Elaborazione immagini\u2026');let n=ei;if(!n||typeof n!="object"||Array.isArray(n)){g("Nessun risultato da importare.","error"),i&&(i.disabled=!1,i.innerHTML=`<i class="fas fa-arrow-right"></i> Apri nell'editor`);return}async function a(d){if(!d)return"";try{let u=await Sc(d,800);return u&&u.length<=ai?u:""}catch{return""}}let s=await a(n.copertina),r=[];for(let d of n.procedimenti||[]){let u=await a(d.imageBase64),f=await a(d.imageBase642);r.push({descrizione:d.descrizione||"",foto:u,foto2:f})}let c=[];for(let d of n.occorrente||[]){let u=await a(d.foto);c.push({lettera:d.lettera,nome:d.nome||"",codice:d.codice||"",foto:u})}let l={titolo:t,categoria:o,copertina:s,sections:{_v:2,schedaTecnica:n.schedaTecnica||[],occorrente:c,procedimenti:r,disegnoTecnico:n.disegnoTecnico||{foto:""}}};ei=[],ni("new",l)}function ca(){window.apriManuale=Bc,window.stampaManuale=Dc,window._apriLightboxOcc_=Hc,window.apriFormManuale=$c,window.chiudiFormManuale=na,window.aggiungiSchedaRow=Tc,window.rimuoviSchedaRow=Rc,window.aggiungiOccorrenteItem=kc,window.rimuoviOccorrenteItem=Lc,window.eliminaFotoOccorrente=Ac,window.eliminaFotoProcedimento=Cc,window.eliminaFotoDisegno=Oc,window.cambiaFotoOccorrente=Ec,window.aggiungiProcStep=Pc,window.rimuoviProcStep=zc,window.cambiaFotoProcedimento=xc,window.cambiaFotoDisegno=Ic,window.cambiaCopertina=_c,window.salvaManualeCorrente=Mc,window.apriStoricoManuale=Uc,window.chiudiStoricoManuale=Fc,window.importaPptx=nl,window._confermImportPptx=rl}var ti,fc,gc,vc,ai,Xn,se,jt,Ke,Qe,ei,Gt,sa,li,Jc,Wc,la=W(()=>{"use strict";zt();gt();Yn();vt();ti="MANUALI_PRODOTTI",fc=20,gc=30,vc=20,ai=4e6,Xn=["Dimensione della sfera","Finiture","Ottiche","Grado di Protezione IP","Tipologia di installazione","Potenza assorbita","Alimentazione","Dimmerazione","Temperatura colore","Indice di resa cromatica","Tolleranza cromatica","Flusso luminoso","Efficienza luminosa","Mantenimento del flusso luminoso","Temperatura di esercizio"],se=[],jt={},Ke=null,Qe=[],ei=[];Gt="http://schemas.openxmlformats.org/drawingml/2006/main",sa="http://schemas.openxmlformats.org/officeDocument/2006/relationships",li="http://schemas.openxmlformats.org/presentationml/2006/main";Jc=/\.(jpe?g|png|gif|webp)$/i,Wc={jpg:"image/jpeg",jpeg:"image/jpeg",png:"image/png",gif:"image/gif",webp:"image/webp"}});function eo(t,e,o,i={}){let n=t.getBoundingClientRect(),a=t.cloneNode(!0);a.removeAttribute("id");let s=i.opacity??.88,r=i.scale??"1.04",c=i.rotate??"-1deg",l=i.borderRadius??"10px",d=i.shadow??"0 10px 30px rgba(0,0,0,0.35)",u=i.background?`background:${i.background};`:"",f=i.border?`border:${i.border};`:"",p=i.transition?`transition:${i.transition};`:"";return a.style.cssText=cl+`width:${n.width}px;height:${n.height}px;left:${n.left}px;top:${n.top}px;opacity:${s};border-radius:${l};box-shadow:${d};transform:scale(${r}) rotate(${c});`+u+f+p,document.body.appendChild(a),{ghost:a,offX:e-n.left,offY:o-n.top}}function oo(t,e,o,i,n){t.style.left=e-i+"px",t.style.top=o-n+"px"}function io(t){t&&t.remove()}function da(t,e,o,i){t&&(t.style.visibility="hidden");let n=document.elementFromPoint(e,o);return t&&(t.style.visibility=""),n?n.closest(i):null}var cl,no=W(()=>{cl="position:fixed;pointer-events:none;user-select:none;-webkit-user-select:none;z-index:99999;transition:none;"});function pa(){try{let t=localStorage.getItem("qrPostazioni");K=t?JSON.parse(t):[...De]}catch{K=[...De]}ma()}function ua(){try{localStorage.setItem("qrPostazioni",JSON.stringify(K))}catch{}ma()}function ma(){ui={},K.forEach(t=>{ui[t.codice.toUpperCase()]=t})}function ll(){let t=/iPad|iPhone|iPod/.test(navigator.userAgent)||navigator.platform==="MacIntel"&&navigator.maxTouchPoints>1,e=window.navigator.standalone===!0||window.matchMedia("(display-mode: standalone)").matches;return t&&e}async function fa(){if(pi)return;pi=!0,setTimeout(()=>{pi=!1},800);let t=document.getElementById("modal-qr-scanner"),e=document.getElementById("qr-error-msg");if(!t)return;e&&(e.style.display="none");let o=document.getElementById("qr-manual-input");if(o&&(o.value=""),ll()){let i=document.createElement("input");i.type="file",i.accept="image/*",i.capture="environment",i.style.display="none",document.body.appendChild(i),i.onchange=()=>{let n=i.files&&i.files[0];if(document.body.removeChild(i),!n)return;let a=new FileReader;a.onload=s=>{let r=new Image;r.onload=()=>{if(typeof jsQR>"u"){alert("\u26A0\uFE0F Libreria scanner non caricata. Usa il campo manuale.");return}let c=document.createElement("canvas");c.width=r.width,c.height=r.height;let l=c.getContext("2d");l.drawImage(r,0,0);let d=l.getImageData(0,0,r.width,r.height),u=jsQR(d.data,d.width,d.height,{inversionAttempts:"attemptBoth"});if(u&&u.data){try{navigator.vibrate&&navigator.vibrate(80)}catch{}ro(u.data.trim())}else t.style.display="flex",t.offsetHeight,t.classList.add("active"),e&&(e.textContent="\u26A0\uFE0F QR non riconosciuto nell'immagine. Riprova o usa il campo manuale.",e.style.display="block")},r.src=s.target.result},a.readAsDataURL(n)},i.oncancel=()=>document.body.removeChild(i),i.click();return}if(t.style.display="flex",t.offsetHeight,t.classList.add("active"),typeof jsQR>"u"){e&&(e.textContent="\u26A0\uFE0F Libreria scanner non caricata. Usa il campo manuale.",e.style.display="block");return}if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){e&&(e.textContent="\u26A0\uFE0F Fotocamera non supportata da questo browser. Usa il campo manuale.",e.style.display="block");return}try{if(!Mt||!Mt.active){Mt=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:"environment"}}});try{localStorage.setItem("qrCameraGranted","1")}catch{}}let i=document.getElementById("qr-video");i.srcObject=Mt,await i.play(),dl()}catch(i){let n="\u26A0\uFE0F Impossibile avviare la fotocamera.";i.name==="NotAllowedError"&&(n="\u26A0\uFE0F Permesso fotocamera negato. Abilitalo dalle impostazioni del browser, poi riprova."),i.name==="NotFoundError"&&(n="\u26A0\uFE0F Nessuna fotocamera trovata sul dispositivo."),i.name==="NotReadableError"&&(n="\u26A0\uFE0F Fotocamera occupata da un'altra applicazione."),e&&(e.textContent=n,e.style.display="block")}}function dl(){let t=document.getElementById("qr-video"),e=document.getElementById("qr-canvas");if(!t||!e)return;let o=e.getContext("2d");function i(){if(Mt){if(t.readyState===t.HAVE_ENOUGH_DATA){e.width=t.videoWidth,e.height=t.videoHeight,o.drawImage(t,0,0,e.width,e.height);let n=o.getImageData(0,0,e.width,e.height),a=jsQR(n.data,n.width,n.height,{inversionAttempts:"dontInvert"});if(a&&a.data){try{navigator.vibrate&&navigator.vibrate(80)}catch{}so(),ro(a.data.trim());return}}Te=requestAnimationFrame(i)}}Te=requestAnimationFrame(i)}function so(){Te&&(cancelAnimationFrame(Te),Te=null),Mt&&(Mt.getTracks().forEach(o=>o.stop()),Mt=null);let t=document.getElementById("qr-video");t&&(t.pause(),t.srcObject=null);let e=document.getElementById("modal-qr-scanner");e&&(e.classList.remove("active"),setTimeout(()=>{e.classList.contains("active")||(e.style.display="none")},300))}function ro(t){if(!t)return;let e=ui[t.toUpperCase()];if(!e){g("\u26A0\uFE0F QR non riconosciuto come postazione: "+t,"error");return}ao={codice:t.toUpperCase(),...e},pl()}function pl(){let t=ao;if(!t)return;ke=null,ut=null,document.getElementById("qr-badge-nome").textContent=t.icona+"  "+t.nome,document.getElementById("qr-azione-domanda").textContent=t.domanda;let e=document.getElementById("qr-search-input");e&&(e.value="",setTimeout(()=>e.focus(),350));let o=document.getElementById("qr-search-dropdown");o&&(o.style.display="none",o.innerHTML=""),document.getElementById("qr-articoli-wrap").style.display="none",document.getElementById("qr-stato-wrap").style.display="none",document.getElementById("btn-qr-conferma").disabled=!0,(window._ordiniAutocompleteCache||[]).length===0&&window.fetchJson("PROGRAMMA PRODUZIONE DEL MESE").then(a=>{let s=new Set;window._ordiniAutocompleteCache=a.filter(r=>String(r.archiviato||"").toUpperCase()!=="TRUE").map(r=>({ordine:r.ordine||"",cliente:r.cliente||"",riferimento:r.riferimento||""})).filter(r=>!r.ordine||s.has(r.ordine)?!1:(s.add(r.ordine),!0))}).catch(()=>{});let n=document.getElementById("modal-qr-azione");n.style.display="flex",n.offsetHeight,n.classList.add("active")}function mi(){let t=document.getElementById("modal-qr-azione");t&&(t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300),ao=null,ke=null,ut=null)}function ga(t){let e=document.getElementById("qr-search-dropdown");if(!e)return;let o=(t||"").trim().toLowerCase();if(document.getElementById("qr-articoli-wrap").style.display="none",document.getElementById("qr-stato-wrap").style.display="none",document.getElementById("btn-qr-conferma").disabled=!0,ke=null,ut=null,!o){e.style.display="none",e.innerHTML="";return}let n=(window._ordiniAutocompleteCache||[]).filter(a=>a.ordine.toLowerCase().includes(o)||a.cliente.toLowerCase().includes(o)||(a.riferimento||"").toLowerCase().includes(o)).slice(0,8);if(n.length===0){e.style.display="none",e.innerHTML="";return}e.innerHTML=n.map(a=>`
        ...
        <label class="qr-articolo-row" for="qr-art-${d.id_riga}">
            ...
            <div class="qr-art-info">
                ...
                <span class="qr-art-stato-badge" style="border-color:${f.colore};color:${f.colore}">${(d.stato||"IN ATTESA").toUpperCase()}</span>
        ...
        </label>`}).join(""),document.querySelectorAll(".qr-art-chk").forEach(d=>d.addEventListener("change",Re)),ul(),document.getElementById("qr-stato-wrap").style.display="block",Re()}function ul(){let t=ao,e=document.getElementById("qr-stato-pills");if(!e)return;let o=t?t.statoDefault.toUpperCase():"",i=window.listaStati||[],n=i.length>0?i:[{nome:"IN ATTESA",colore:"#94a3b8"},{nome:"PREPARARE PER LAVORAZIONE",colore:"#64748b"},{nome:"IN LAVORAZIONE",colore:"#f59e0b"},{nome:"IN PRODUZIONE",colore:"#242424"},{nome:"IMBALLATO",colore:"#22c55e"}];ut=null,e.innerHTML=n.map(a=>{let s=a.nome.toUpperCase()===o;return s&&(ut=a.nome),`<button type="button"
                    ...
                    ${a.nome}
                </button>`}).join("")}function ha(t,e){ut=e;let o=window.listaStati||[];document.querySelectorAll(".qr-stato-pill").forEach(a=>{let s=o.find(c=>c.nome===a.dataset.stato),r=s?s.colore:"#94a3b8";a.classList.remove("qr-stato-pill-sel"),a.style.background="",a.style.color=r,a.style.borderColor=r});let i=o.find(a=>a.nome===t.dataset.stato),n=i?i.colore:"#94a3b8";t.classList.add("qr-stato-pill-sel"),t.style.background=n,t.style.color="#fff",t.style.borderColor=n,Re()}function ba(){document.querySelectorAll(".qr-art-chk").forEach(t=>t.checked=!0),Re()}function ya(){document.querySelectorAll(".qr-art-chk").forEach(t=>t.checked=!1),Re()}function Re(){let t=document.getElementById("btn-qr-conferma");if(!t)return;let e=document.querySelectorAll(".qr-art-chk:checked").length;t.disabled=!(e>0&&ut)}async function wa(){if(!ut||!ke)return;let t=Array.from(document.querySelectorAll(".qr-art-chk:checked"));if(t.length===0){g("Seleziona almeno un articolo.","error");return}let e=document.getElementById("btn-qr-conferma");e&&(e.disabled=!0,e.innerHTML='<i class="fas fa-spinner fa-spin"></i> Salvataggio...');let o=t.map(r=>r.dataset.idRiga),i={},n=window._attiviProd||[];o.forEach(r=>{let c=n.find(l=>String(l.id_riga)===String(r));i[r]=c?c.stato:null}),o.forEach(r=>{let c=n.find(l=>String(l.id_riga)===String(r));c&&(c.stato=ut),window._syncKanbanFromStato&&window._syncKanbanFromStato(r,ut)});let a=0,s=[];for(let r of o)await window.aggiornaDato(null,r,"stato",ut,!0)||(a++,s.push(r));a===0?(g(`\u2705 ${o.length} articolo/i \u2192 ${ut}`),window.cacheContenuti&&delete window.cacheContenuti["PROGRAMMA PRODUZIONE DEL MESE"],U("_html_PROGRAMMA PRODUZIONE DEL MESE"),N.invalidate("PROGRAMMA_PRODUZIONE").catch(()=>{})):(s.forEach(r=>{let c=i[r],l=n.find(d=>String(d.id_riga)===String(r));c&&l&&(l.stato=c),c&&window._syncKanbanFromStato&&window._syncKanbanFromStato(r,c)}),g(`\u26A0\uFE0F ${a} errori su ${o.length} articoli \u2014 riprova`,"error"),console.error("[QR Postazione] Rollback",{falliti:s,statiPrec:i})),mi()}function Sa(){Ea(null)}function _a(t){Ea(t)}function Ea(t){let e=t==null,o=e?{icona:"\u{1F4CD}",nome:"",codice:"",domanda:"",statoDefault:""}:K[t];document.getElementById("qr-edit-titolo").innerHTML=`<i class="fas fa-map-marker-alt" style="margin-right:8px"></i>${e?"Nuova Postazione":"Modifica Postazione"}`,document.getElementById("qr-edit-icona").value=o.icona||"",document.getElementById("qr-edit-nome").value=o.nome||"",document.getElementById("qr-edit-codice").value=o.codice||"",document.getElementById("qr-edit-domanda").value=o.domanda||"",document.getElementById("qr-edit-idx").value=e?"":t;let i=document.getElementById("qr-edit-stato"),n=window.listaStati||[],a=n.length>0?n:De.map(c=>({nome:c.statoDefault,colore:"#94a3b8"})),s=[...new Map(a.map(c=>[c.nome,c])).values()];i.innerHTML=s.map(c=>`<option value="${c.nome}" ${c.nome===(o.statoDefault||"")?"selected":""}>${c.nome}</option>`).join("");let r=document.getElementById("modal-qr-edit");r.style.display="flex",r.offsetHeight,r.classList.add("active"),requestAnimationFrame(()=>co())}function fi(){let t=document.getElementById("modal-qr-edit");t&&(t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300))}function gi(){let e="PROD:"+(document.getElementById("qr-edit-nome")?.value||"").trim().toUpperCase().replace(/[ÀÁÂÃÄÅ]/g,"A").replace(/[ÈÉÊË]/g,"E").replace(/[ÌÍÎÏ]/g,"I").replace(/[ÒÓÔÕÖ]/g,"O").replace(/[ÙÚÛÜ]/g,"U").replace(/[^A-Z0-9]/g,""),o=document.getElementById("qr-edit-codice");o&&(o.value=e)}function xa(){gi(),co()}async function co(){let t=(document.getElementById("qr-edit-codice")?.value||"").trim(),e=(document.getElementById("qr-edit-nome")?.value||"").trim(),o=document.getElementById("qr-preview-canvas"),i=document.getElementById("qr-preview-nome"),n=document.getElementById("qr-preview-codice");if(i&&(i.textContent=e||"\u2014"),n&&(n.textContent=t||"\u2014"),!(!o||!t))try{typeof QRCode<"u"&&typeof QRCode.toDataURL=="function"?o.src=await QRCode.toDataURL(t,{width:160,margin:2,color:{dark:"#111827",light:"#ffffff"}}):o.src=`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(t)}`}catch{o.src=`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(t)}`}}function Ia(){let t=(document.getElementById("qr-edit-icona")?.value||"").trim()||"\u{1F4CD}",e=(document.getElementById("qr-edit-nome")?.value||"").trim(),o=(document.getElementById("qr-edit-codice")?.value||"").trim().toUpperCase(),i=(document.getElementById("qr-edit-domanda")?.value||"").trim(),n=document.getElementById("qr-edit-stato")?.value||"",a=document.getElementById("qr-edit-idx")?.value;if(!e){g("Inserisci un nome per la postazione.","error");return}if(!o){g("Il codice QR non pu\xF2 essere vuoto.","error");return}let s={icona:t,nome:e,codice:o,domanda:i,statoDefault:n},r=a!==""&&a!==null&&a!==void 0?parseInt(a):null;r!==null&&!isNaN(r)?K[r]=s:K.push(s),ua(),fi(),g("\u2705 Postazione salvata."),window.caricaInterfacciaImpostazioni(),setTimeout(()=>Ca(),120)}function Aa(t){let e=K[t];e&&rt("Elimina Postazione",`Vuoi eliminare la postazione "${e.nome}"? Il QR code stampato associato non funzioner\xE0 pi\xF9.`,()=>{K.splice(t,1),ua(),g("Postazione eliminata."),window.caricaInterfacciaImpostazioni(),setTimeout(()=>Ca(),120)},"Elimina")}function Ca(){let t=document.getElementById("section-qr-postazioni");if(!t)return;t.style.display="block";let e=t.previousElementSibling;if(e){e.classList.add("settings-row-active");let o=e.querySelector(".settings-row-arrow");o&&(o.style.transform="rotate(180deg)")}}async function vi(){for(let t=0;t<K.length;t++){let e=document.getElementById(`qr-list-canvas-${t}`);if(!e)continue;let o=K[t].codice||"";if(o)try{typeof QRCode<"u"&&typeof QRCode.toDataURL=="function"?e.src=await QRCode.toDataURL(o,{width:56,margin:1,color:{dark:"#0f172a",light:"#f8fafc"}}):e.src=`https://api.qrserver.com/v1/create-qr-code/?size=56x56&data=${encodeURIComponent(o)}`}catch{e.src=`https://api.qrserver.com/v1/create-qr-code/?size=56x56&data=${encodeURIComponent(o)}`}}}function Oa(){let t=(document.getElementById("qr-edit-codice")?.value||"").trim(),e=(document.getElementById("qr-edit-nome")?.value||"").trim(),o=(document.getElementById("qr-edit-icona")?.value||"").trim()||"\u{1F4CD}",i=(document.getElementById("qr-edit-domanda")?.value||"").trim();if(!t){g("Inserisci nome e codice prima di stampare.","error");return}hi([{codice:t,nome:e,icona:o,domanda:i}])}function $a(t){let e=K[t];e&&hi([e])}function Ta(){if(K.length===0){g("Nessuna postazione da stampare.","error");return}hi(K)}async function hi(t){let o=`<!DOCTYPE html>
<html lang="it">
<head>
<meta charset="UTF-8">
...
</html>`,i=new Blob([o],{type:"text/html; charset=utf-8"}),n=URL.createObjectURL(i);window.open(n,"_blank","width=900,height=700")?setTimeout(()=>URL.revokeObjectURL(n),3e4):(URL.revokeObjectURL(n),g("\u26A0\uFE0F Abilita i popup per la stampa.","error"))}var K,ui,Mt,Te,ao,ut,ke,pi,Ra=W(()=>{ft();zt();gt();It();K=[],ui={};Mt=null,Te=null,ao=null,ut=null,ke=null;pi=!1});function ka(t){return window.hashSHA256(t)}function La(){return b?.nome?"avatarColorRecenti_"+b.nome.toUpperCase().trim():null}function Pa(){return b?.nome?"avatarColorHidden_"+b.nome.toUpperCase().trim():null}function $t(){let t=La();if(!t)return[];try{return JSON.parse(localStorage.getItem(t)||"[]")}catch{return[]}}function le(t){let e=La();if(e)try{localStorage.setItem(e,JSON.stringify(t.slice(0,7)))}catch{}}function za(){let t=Pa();if(!t)return[];try{return JSON.parse(localStorage.getItem(t)||"[]")}catch{return[]}}function Ma(t){let e=Pa();if(e)try{localStorage.setItem(e,JSON.stringify(t))}catch{}}function bi(){let t=document.getElementById("avatar-predefined-swatches");if(!t)return;let e=za();t.innerHTML="",(window._PREDEFINED_AVATAR_COLORS||[]).forEach(o=>{if(e.includes(o))return;let i=document.createElement("button");i.className="avatar-color-swatch",i.style.background=o,i.dataset.color=o,i.title="Clicca per applicare o eliminare",i.onclick=n=>{n.stopPropagation(),gl(o,n)},t.appendChild(i)})}function Vt(){let t=["avatar-custom-swatches","avatar-custom-swatches-mob"],e=$t();t.forEach(o=>{let i=document.getElementById(o);if(!i)return;let n=o.endsWith("-mob");i.innerHTML="",e.forEach((a,s)=>{let r=document.createElement("button");r.className="avatar-color-swatch avatar-color-custom-swatch",r.style.background=a,r.dataset.color=a,r.title="Clicca per modificare o eliminare",r.onclick=n?c=>{c.stopPropagation(),Sl(s,c)}:c=>{c.stopPropagation(),fl(s,c)},i.appendChild(r)})})}function yi(t,e){let o=document.getElementById("avatar-color-editor"),i=document.getElementById("avatar-color-edit-input"),n=document.getElementById("avatar-editor-delete");!o||!i||(i.value=t||"#ff0000",n&&(n.style.display=e?"":"none"),o.style.display="flex")}function wi(){let t=document.getElementById("avatar-color-editor");t&&(t.style.display="none"),j=null}function ml(t){t&&t.stopPropagation(),j=null,yi("#ff0000",!1)}function fl(t,e){e&&e.stopPropagation();let o=$t();j={type:"custom",idx:t},yi(o[t]||"#ff0000",!0)}function gl(t,e){e&&e.stopPropagation(),j={type:"predefined",color:t},yi(t,!0)}function vl(t){t&&t.stopPropagation();let e=document.getElementById("avatar-color-edit-input");if(!e)return;let o=e.value;if(j===null){let i=$t();i.unshift(o),le(i),Vt()}else if(j.type==="custom"){let i=$t();i[j.idx]=o,le(i),Vt()}wi(),_i(o)}function hl(t){t&&t.stopPropagation(),wi()}function bl(t){if(t&&t.stopPropagation(),!!j){if(j.type==="custom"){let e=$t();e.splice(j.idx,1),le(e),Vt()}else if(j.type==="predefined"){let e=za();e.includes(j.color)||e.push(j.color),Ma(e),bi()}wi()}}function yl(t){t&&t.stopPropagation(),Ma([]),bi()}function Na(t,e){let o=document.getElementById("avatar-color-editor-mob"),i=document.getElementById("avatar-color-edit-input-mob"),n=document.getElementById("avatar-editor-delete-mob");!o||!i||(i.value=t||"#ff0000",n&&(n.style.display=e?"":"none"),o.style.display="flex")}function Si(){let t=document.getElementById("avatar-color-editor-mob");t&&(t.style.display="none"),j=null}function wl(t){t&&t.stopPropagation(),j=null,Na("#ff0000",!1)}function Sl(t,e){e&&e.stopPropagation();let o=$t();j={type:"custom",idx:t},Na(o[t]||"#ff0000",!0)}function _l(t){t&&t.stopPropagation();let e=document.getElementById("avatar-color-edit-input-mob");if(!e)return;let o=e.value;if(j===null){let i=$t();i.unshift(o),le(i),Vt()}else if(j.type==="custom"){let i=$t();i[j.idx]=o,le(i),Vt()}Si(),_i(o)}function El(t){t&&t.stopPropagation(),Si()}function xl(t){if(t&&t.stopPropagation(),!!j){if(j.type==="custom"){let e=$t();e.splice(j.idx,1),le(e),Vt()}Si()}}function qa(t){document.documentElement.style.setProperty("--avatar-user-color",t);let e=document.getElementById("user-avatar-btn"),o=document.getElementById("account-ddrop-avatar"),i=document.getElementById("user-avatar-btn-mobile"),n=document.getElementById("account-ddrop-avatar-mob");e&&(e.style.setProperty("background",t,"important"),e.style.setProperty("box-shadow",`0 2px 8px ${t}66`,"important")),i&&(i.style.setProperty("background",t,"important"),i.style.setProperty("box-shadow",`0 2px 8px ${t}66`,"important")),o&&o.style.setProperty("background",t,"important"),n&&n.style.setProperty("background",t,"important"),document.querySelectorAll(".avatar-color-swatch").forEach(a=>{a.classList.toggle("active",a.dataset.color===t)})}function _i(t){if(!b||!b.nome)return;let e=b.nome.toUpperCase().trim();try{localStorage.setItem("avatarColor_"+e,t)}catch{}window._avatarColorsCache&&(window._avatarColorsCache[e]=t),b.nome&&fetch(x,{method:"POST",body:JSON.stringify({azione:"setAvatarColor",username:b.nome,color:t})}).catch(()=>{}),qa(t)}function Il(t){t&&t.stopPropagation();let e=document.getElementById("account-dropdown");e&&e.classList.toggle("open")}function Al(){let t=document.getElementById("account-dropdown");t&&t.classList.remove("open")}function Cl(){U("_impostazioni_cache"),window.paginaAttuale&&(window.cacheContenuti&&delete window.cacheContenuti[window.paginaAttuale],U("_html_"+window.paginaAttuale)),window.location.reload()}function Ol(t){t&&t.stopPropagation();let e=document.getElementById("account-dropdown-mobile");e&&e.classList.toggle("open")}function $l(){let t=document.getElementById("account-dropdown-mobile");t&&t.classList.remove("open")}async function Tl(){try{let o=(await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"pushInfo"})})).json().catch(()=>({}))).subscriptions||[];if(!o.length){g("Nessun dispositivo registrato in PUSH_SUBSCRIPTIONS","error");return}let i={};o.forEach(s=>{i[s.user]||(i[s.user]=0),i[s.user]++});let n=Object.entries(i).sort((s,r)=>s[0].localeCompare(r[0])).map(([s,r])=>`<tr><td style="padding:6px 10px;font-weight:600">${s}</td><td style="padding:6px 10px;text-align:center">${r} dispositivo${r>1?"i":""}</td></tr>`).join(""),a=`<div style="position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:9999;display:flex;align-items:center;justify-content:center" onclick="this.remove()">
...
`)[0]||"",s=a.split(";").length>=a.split(",").length?";":",";i&&(i.style.display="block",i.innerHTML='<div style="display:flex;align-items:center;gap:8px;color:#64748b;font-size:0.88rem"><i class="fas fa-spinner fa-spin"></i> Import Lista di Carico in corso\u2026</div>');try{let c=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"importaListaDiCarico",csvText:n,separatore:s})})).json().catch(()=>({}));if(i)if(c.status==="ok"){let l=`<strong>\u2705 Import completato</strong><br>Nuovi: <strong>${c.nuove||0}</strong> \xB7 Aggiornati: <strong>${c.aggiornate||0}</strong> \xB7 Invariati: <strong>${c.invariate||0}</strong>`;c.corrette>0&&(l+=`<br><span style="color:#b45309"><i class="fas fa-exclamation-triangle"></i> ${c.corrette} righe corrette automaticamente (campi con separatore nel testo)</span>`),c.missingCount>0&&(l+=`<br><span style="color:#d97706">\u26A0 <strong>${c.missingCount}</strong> ordini non presenti nel CSV \u2014 vai nel tab Fornitori per archiviarli.</span>`),i.innerHTML=`<div style="background:#dcfce7;border:1px solid #86efac;border-radius:10px;padding:12px 16px;font-size:0.88rem;color:#166534">${l}</div>`,typeof window.invalidateOFCache=="function"&&window.invalidateOFCache()}else i.innerHTML=po(c.msg||c.message||"Errore sconosciuto")}catch(r){i&&(i.innerHTML=po(r.message))}t.value=""}async function Le(){let t="_impostazioni_cache",e="_impostazioni_stati_forn_cache",i=ht(t,1/0);if(i){try{let n=typeof i=="string"?JSON.parse(i):i;if(n.stati&&n.stati.length){window.listaStati=n.stati,window.listaOperatori=n.operatori||[],window._distintaHeaderAzienda=String(n.distintaHeaderAzienda||""),Da(n.overviewStati);let a=ht(e,1/0);if(a)try{let s=typeof a=="string"?JSON.parse(a):a;Array.isArray(s)&&s.length&&(window.listaStatiFornitori=s)}catch{}ht(t,3e5)||uo().catch(s=>console.warn("[impostazioni] bg refresh:",s));return}}catch(n){console.warn("[impostazioni] cache JSON corrotta, ricarico dal server:",n)}U(t)}await uo()}async function uo(){let t="_impostazioni_cache",e="_impostazioni_stati_forn_cache";try{let i=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"getImpostazioni",noCache:!0})})).json();window.listaStati=i.stati&&i.stati.length?i.stati:window._defaultListaStati_(),window.listaOperatori=i.operatori||[],window._distintaHeaderAzienda=String(i.distintaHeaderAzienda||""),Da(i.overviewStati),Z(t,JSON.stringify({stati:window.listaStati,operatori:window.listaOperatori,overviewStati:i.overviewStati,distintaHeaderAzienda:window._distintaHeaderAzienda}))}catch(o){console.warn("[Boot] _fetchImpostazioniDaServer:",o),g("Impostazioni non aggiornate \u2014 uso dati locali.","warning")}try{let i=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"getStatiFornitoriConfig"})})).json();i.stati&&i.stati.length&&(window.listaStatiFornitori=i.stati,Z(e,JSON.stringify(i.stati)))}catch{}}function Da(t){t&&(Array.isArray(t.art)&&t.art.length&&(window._ovStatiArt=t.art.map(e=>e.toUpperCase().trim())),Array.isArray(t.ord)&&t.ord.length&&(window._ovStatiOrd=t.ord.map(e=>e.toUpperCase().trim())))}function Dl(t,e){let o=document.getElementById(t);if(!o)return;let i=e.querySelector(".settings-row-arrow"),n=o.style.display==="block";o.style.display=n?"none":"block",i&&(i.style.transform=n?"":"rotate(180deg)"),e.classList.toggle("settings-row-active",!n),!n&&(t==="section-utenti"||t==="section-team-utenti")&&Pe(),!n&&t==="section-qr-postazioni"&&requestAnimationFrame(()=>vi())}async function Pe(){let t=document.getElementById("lista-utenti-config");if(t){t.innerHTML='<div class="centered-msg small">Caricamento...</div>';try{let o=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"getUtenti"})})).json();if(!o.length){t.innerHTML='<p class="centered-msg small">Nessun utente creato. Clicca "+ Aggiungi Utente".</p>';return}t.innerHTML=o.map(i=>{let n=i.id_riga,a=(i.username||"").trim(),s=(i.email||"").trim(),r=(i.ruolo||"OPERATORE").trim().toUpperCase(),c=Number(i.max_utenti)||1;return`
    ...
    <div class="item-card ${i.card}${l}" data-id-riga="${t.id_riga}" data-codice="${s.toLowerCase().replace(/"/g,"")}">
        ...
        ${(()=>{let p=fo(t.data_ordine),m=fo(t.data_consegna);if(!p&&!m)return"";let h=p?Ai(p):"\u2014",w=m?`<span style="${Ha(m)}">${Ai(m)}</span>`:"\u2014";return`<div class="card-date-row"><span class="label-sm ${i.label}" style="margin-bottom:1px">Date</span><span class="card-date-vals"><span class="card-date-item"><i class="fas fa-file-signature" style="color:#94a3b8;font-size:.7rem;margin-right:3px"></i><span class="card-date-lbl">Ord.</span> ${h}</span><span class="card-date-sep">\xB7</span><span class="card-date-item"><i class="fas fa-truck" style="color:#94a3b8;font-size:.7rem;margin-right:3px"></i><span class="card-date-lbl">Cons.</span> ${w}</span></span></div>`})()}
        <div class="qty-cell">
    ...
    <div class="item-card archivio-layout ${o.card}">
    ...
    </div>`}var Ci=W(()=>{mo();gt();vt()});function Nt(){let t=pe(),e=(S.attiviProd||[]).filter(n=>t.includes((n.stato||"").toUpperCase().trim())).length,o=document.querySelector("#ov-accordion .ov-summary-meta");o&&(o.textContent=`${e} art. in lavorazione`);let i=document.getElementById("ov-content");if(i){if(i.querySelector(".ov-lazy-placeholder")){let n=document.getElementById("ov-accordion");if(!n||!n.open)return;i.innerHTML=ue(S.attiviProd),requestAnimationFrame(Tt);return}i.innerHTML=ue(S.attiviProd),requestAnimationFrame(Tt)}}function ja(t){if(!t.parentElement.open){let o=document.getElementById("ov-content");o&&o.querySelector(".ov-lazy-placeholder")&&(o.innerHTML=ue(S.attiviProd),requestAnimationFrame(Tt))}}function Ga(t){let e=document.getElementById(t);if(!e)return;let o=e.querySelector(".sezione-archiviata"),i=S.datiArchLazy||S.ultimiDatiProduzione&&S.ultimiDatiProduzione.archivio;if(o&&i&&(S.datiArchLazy||!o.children.length)){let a=Wt(i,!0)||"<div class='empty-msg'>L'archivio \xE8 vuoto.</div>";o.innerHTML=a,window.aggiornaListaFiltrabili?.(),H.ARCHIVIO_ORDINI||(H.ARCHIVIO_ORDINI=a,X.ARCHIVIO_ORDINI=Date.now(),Z("_html_ARCHIVIO_ORDINI",a)),S.datiArchLazy=null}e.open=!0,requestAnimationFrame(()=>{e.querySelector("summary")?.scrollIntoView({behavior:"smooth",block:"start"})})}function Oi(t){let e=(t||[]).filter(p=>!wt(p.stato)),o=["Riccardo","Fabio T.","Niccol\xF2","Alessio"],i=new Map;o.forEach(p=>i.set(p,[]));let n=new Map;o.forEach(p=>n.set(p,new Set));function a(p){let m=window._normNome(p);return o.find(h=>h===m||h.toUpperCase()===String(p).trim().toUpperCase())}e.forEach(p=>{if(!p.assegna||p.assegna===""||p.assegna==="undefined")return;let m=String(p.ordine||"").trim();m&&p.assegna.split(",").forEach(h=>{let w=h.trim();if(!w)return;let v=a(w);v&&!n.get(v).has(m)&&(n.get(v).add(m),i.get(v).push(p))})});let s={};(window.listaStati||[]).forEach(p=>{s[p.nome.toUpperCase()]=p.colore});function r(p){let m=String(p.cliente||"").trim().toUpperCase();if(!m||m==="DA DEFINIRE"){let w=String(p.riferimento||"").trim();return w||""}let h=p.cliente.trim().split(/\s+/).slice(0,2).join(" ");return h.length>14?h.substring(0,13)+"\u2026":h}let c=o.map(p=>{let m=i.get(p)||[],h=window._getOpColor(p),w=m.length===0?'<div class="ov-op-item ov-op-item-free"><span class="ov-op-item-cod" style="color:#475569">Libero</span></div>':m.map(v=>{let E=(v.stato||"IN ATTESA").toUpperCase().trim(),C=s[E]||"#94a3b8",_=String(v.ordine||"").trim(),$=r(v),k=_.length>10?_.substring(0,9)+"\u2026":_;return`<div class="ov-op-item">
        ...
        `,H[i]=o.innerHTML,X[i]=Date.now(),Z("_html_"+i,o.innerHTML),N.set("PROGRAMMA_PRODUZIONE",t).catch(()=>{}),z(o),s&&h.size&&o.querySelectorAll(".ordine-wrapper").forEach(v=>{if(h.has(v.dataset.ordine)){let E=v.querySelector(".riga-ordine"),C=v.querySelector(".dettagli-container");E&&C&&(E.classList.add("open"),C.style.display="block")}}),window.aggiornaListaFiltrabili(),requestAnimationFrame(Tt),requestAnimationFrame(()=>{S.attiviProd&&S.attiviProd.forEach(v=>{if(parseFloat(v.qty_evasa)>0){let E=document.getElementById("qty-evasa-block-"+v.id_riga),C=E&&E.closest(".qty-cell")?.querySelector(".btn-qty-evasa-toggle");E&&(E.style.display="inline-flex"),C&&C.classList.add("active")}}),he(),Ni()&&be()}),_o(),S.ordiniAutocompleteCache=n.filter(v=>String(v.archiviato||"").toUpperCase()!=="TRUE").map(v=>({ordine:v.ordine||"",cliente:v.cliente||"",riferimento:v.riferimento||""}));let w=new Set;S.ordiniAutocompleteCache=S.ordiniAutocompleteCache.filter(v=>w.has(v.ordine)?!1:(w.add(v.ordine),!0)),window._ordiniAutocompleteCache=S.ordiniAutocompleteCache}async function So(t,e=!1,o=null,i=null){let n=document.getElementById("contenitore-dati");!e&&n&&(n.innerHTML="<div class='inline-msg' id='_prod-loader'>Caricamento Dashboard...</div>",z(n));let a=e?null:setTimeout(()=>{let r=document.getElementById("_prod-loader");r&&(r.innerHTML="<i class='fas fa-spinner fa-spin'></i> Connessione lenta, sto ancora caricando...")},3500),s=e?null:setTimeout(()=>{let r=document.getElementById("_prod-loader");r&&(r.innerHTML=`\u26A0\uFE0F Server occupato o rete instabile.<br>
    ...
    <div class="pf-wrap">
      ...
      <div class="iop-box" onclick="event.stopPropagation()">
        ...
        <p class="iop-hint">Date e qty evasa vengono aggiornate al caricamento del CSV Yello.</p>
      </div>`,a.addEventListener("click",()=>a.remove()),document.body.appendChild(a)}function Ka(){window.toggleAccordion=fd,window.toggleStatoDropdown=sd,window.selezionaStato=ud,window.selezionaStatoOrdine=md,window.toggleOpDropdown=ad,window.selezionaOpAssegna=cd,window.selezionaOpAssegnaOrdine=ld,window.autoAssegnami=dd,window.autoAssegnamiOrdine=pd,window.rimuoviOperatore=nd,window.gestisciArchiviazione=vd,window.gestisciRipristino=yd,window.toggleQtyEvasa=hd,window.aggiornaRimanente=Ja,window.salvaQtyEvasa=bd,window.chiudiTuttiMenuAzioni=Li,window.toggleMenuAzioni=rd,window.aggiornaDato=Pi,window.toggleFiltroArticoli=Ed,window._aggiornaVisibilitaFiltroArticoli=Mi,window._renderProdFilterBar=he,window._applicaFiltriProd=be,window._pfToggleStato=Id,window._pfSetSort=Ad,window._pfToggleRimanente=Cd,window._pfReset=Od,window._pfTogglePanel=xd,window.apriInfoOrdine=Td,window._pfHasActiveFilters=Ni,window._getAttiviProd=()=>S.attiviProd,window._ovLoadIfNeeded=ja,window._apriArchivio=Ga,window._scrollToOrdineList=$i,window._initKanbanDnd=Tt,window._startPollingProduzione=_o,window._stopPollingProduzione=ve,window._pollProdStep=zi,window._repaintOpColors=Et,window.caricaDati=So,window.caricaArchivio=ge,window._syncKanbanFromStato=Za,window._setAssegnaLocalByOrdine=ki,window._refreshOverview=Nt,window._invalidateProduzioneCache=lt,window.csvReviewResolve=$d}function Ya(){document.addEventListener("click",function(t){t.target.closest(".ord-azioni-menu")||Li()}),document.addEventListener("click",function(t){t.target.closest(".op-dropdown")||document.querySelectorAll(".op-dropdown.open").forEach(e=>{e.classList.remove("open");let o=e.closest(".item-card");o&&o.classList.remove("op-aperto");let i=e.closest(".riga-ordine");i&&i.classList.remove("op-aperto-ord")})},!0),document.addEventListener("click",function(t){!t.target.closest(".stato-dropdown")&&!t.target.closest(".stato-dropdown-ord")&&document.querySelectorAll(".stato-dropdown.open, .stato-dropdown-ord.open").forEach(e=>{e.classList.remove("open");let o=e.closest(".item-card");o&&o.classList.remove("stato-aperto");let i=e.closest(".riga-ordine");i&&i.classList.remove("stato-aperto-ord")})},!0),document.addEventListener("click",function(t){if(!t.target.closest(".pf-wrap")){let e=document.getElementById("pf-panel"),o=document.getElementById("pf-caret");e&&e.style.display!=="none"&&(e.style.display="none",o&&(o.style.transform=""))}}),document.addEventListener("visibilitychange",function(){document.visibilityState==="visible"&&window.paginaAttuale==="PROGRAMMA PRODUZIONE DEL MESE"&&zi()})}var Ti,ho,Qa,D,Xa=W(()=>{ft();zt();gt();vt();Kt();It();Qt();no();mo();Ci();Va();Ti=!1,ho=null;Qa="_prod_filtri_v1",D=(()=>{try{return JSON.parse(localStorage.getItem(Qa))||{}}catch{return{}}})();D.stati||(D.stati=[]);D.sortBy||(D.sortBy="default");D.soloRimanente||(D.soloRimanente=!1)});async function qi(t){let e=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(t));return Array.from(new Uint8Array(e)).map(o=>o.toString(16).padStart(2,"0")).join("")}function ts(){return Date.now()<Io}function Rd(){xo++,xo>=5&&(Io=Date.now()+3e4,xo=0)}function kd(){xo=0,Io=0}async function es(){let t=document.getElementById("login-error");if(t.innerText="",t.style.color="",document.getElementById("login-view-admin")?.style.display!=="none"){let s=(document.getElementById("login-codice")?.value||"").trim();t.innerText="Usa il pulsante Entra per accedere come admin.";return}if(ts()){let s=Math.ceil((Io-Date.now())/1e3);t.innerText="Troppi tentativi. Riprova tra "+s+" secondi.";return}let o=(document.getElementById("login-email")?.value||"").trim().toLowerCase(),i=(document.getElementById("login-username")?.value||"").trim(),n=document.getElementById("login-password")?.value||"";if(!o||!i||!n){t.innerText="Compila tutti i campi: email, nome utente e password.";return}let a=document.getElementById("btn-login");a.disabled=!0,a.innerHTML='<i class="fas fa-spinner fa-spin"></i> Verifica...';try{let s=await qi(n),c=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"verificaLogin",email:o,username:i,hash:s})})).json();c.status==="success"?(kd(),Lt({nome:c.nome,ruolo:c.ruolo,email:c.email,vistaSimulata:c.nome,sessionToken:c.sessionToken||"",sessionExpiresAt:c.sessionExpiresAt||""}),window.salvaEApriDashboard()):(Rd(),ts()?t.innerText="Troppi tentativi. Riprova tra 30 secondi.":t.innerText=c.message||"Credenziali non valide.")}catch{t.innerText="Errore di connessione. Riprova."}a.disabled=!1,a.innerHTML='Entra nel Sistema <i class="fas fa-arrow-right"></i>'}async function Ld(){let t=document.getElementById("login-error");t&&(t.innerText=""),t&&(t.style.color="");let e=(document.getElementById("login-email")?.value||"").trim().toLowerCase(),o=(document.getElementById("login-username")?.value||"").trim(),i=document.getElementById("login-password")?.value||"";if(!e||!o||!i){t&&(t.innerText="Per creare l'account compila email, nome utente e password.");return}let n=document.getElementById("btn-login"),a=document.getElementById("btn-signup"),s=n?n.innerHTML:"",r=a?a.innerHTML:"";n&&(n.disabled=!0),a&&(a.disabled=!0,a.innerHTML='<i class="fas fa-spinner fa-spin"></i> Creazione...');try{let c=await qi(i),d=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"creaUtentePubblico",email:e,username:o,hash:c})})).json();d.status==="success"?(t&&(t.style.color="#22c55e"),t&&(t.innerText="Account creato. Accesso in corso..."),await es()):(t&&(t.style.color=""),t&&(t.innerText=d.message||"Impossibile creare l'account."))}catch{t&&(t.style.color=""),t&&(t.innerText="Errore di connessione. Riprova.")}finally{n&&(n.disabled=!1,n.innerHTML=s||'Entra nel Sistema <i class="fas fa-arrow-right"></i>'),a&&(a.disabled=!1,a.innerHTML=r||'<i class="fas fa-user-plus"></i> Nuovo utente? Crea account')}}function os(){window.hashSHA256=qi,window._verificaAccessoUtente=es,window._creaAccountUtente=Ld}var xo,Io,is=W(()=>{ft();vt();xo=0,Io=0});function Bi(t){if(!t)return 0;if(t instanceof Date)return t.getTime();let e=String(t).trim();if(!e)return 0;let o=e.match(/^(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?\s+(\d{1,2}):(\d{2})(?::(\d{2}))?$/);if(o){let n=parseInt(o[1],10),a=parseInt(o[2],10)-1,s=parseInt(o[3]||String(new Date().getFullYear()),10);s<100&&(s+=2e3);let r=parseInt(o[4],10),c=parseInt(o[5],10),l=parseInt(o[6]||"0",10);return new Date(s,a,n,r,c,l).getTime()}let i=new Date(e);return Number.isFinite(i.getTime())?i.getTime():0}function Ui(t){let e=Date.now();return(t||[]).filter(function(o){let i=Bi(o&&o._ts);return i?e-i<=Pd:!0})}function zd(t){t&&(t.stopPropagation(),t.preventDefault());let e=document.getElementById("modal-notifiche");if(e){e.classList.add("is-open"),Ud();try{localStorage.setItem("_notifLastRead",String(Date.now()))}catch{}try{localStorage.setItem("_notifBadgeCount","0")}catch{}Fi(0),b&&b.nome&&fetch(x,{method:"POST",body:JSON.stringify({azione:"segnaLetteNotifiche",username:b.nome.toUpperCase()})}).catch(function(){})}}function as(){let t=document.getElementById("modal-notifiche");t&&t.classList.remove("is-open")}async function Md(t,e,o,i){let n=i?i.closest(".notif-azioni-accesso"):null;n&&(n.querySelectorAll("button").forEach(function(a){a.disabled=!0}),n.innerHTML='<span class="notif-risposta-wait">\u23F3 Invio in corso\u2026</span>');try{let s=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"rispondiAccessoFuoriOrario",id:t,ok:o,json:1})})).json();if(s.status==="ok"){let r=o==="SI"?"\u2705 Accesso consentito":"\u{1F6AB} Accesso negato";Nd(t,r),n&&(n.innerHTML='<span class="notif-risposta-ok">'+Zt(r)+"</span>")}else n&&(n.innerHTML='<span class="notif-risposta-err">\u26A0\uFE0F '+Zt(s.msg||"Errore")+"</span>")}catch{n&&(n.innerHTML='<span class="notif-risposta-err">\u26A0\uFE0F Errore di rete</span>')}}function ss(){try{return JSON.parse(localStorage.getItem("_accRispIdx_")||"{}")}catch{return{}}}function Nd(t,e){try{let o=ss();o[t]=e,localStorage.setItem("_accRispIdx_",JSON.stringify(o))}catch{}}async function qd(t,e,o,i){let n=decodeURIComponent(e||""),a=decodeURIComponent(o||""),s=String(t||"").trim(),r=i?i.closest(".notifica-item"):null;r&&(r.classList.add("notif-removing"),await new Promise(function(l){setTimeout(l,190)})),Dd(s,n,a),r&&r.remove();let c=document.getElementById("notifiche-list");c&&!c.querySelector(".notifica-item")&&(c.innerHTML=Di([]));try{let l=b&&b.nome?b.nome.toUpperCase():"";if(!l)return;let u=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"eliminaNotifica",username:l,rid:s,titolo:n,corpo:a})})).json().catch(()=>({}));u.status!=="ok"&&u.status!=="not_found"&&console.warn("[notifiche] eliminaNotifica non ok:",u)}catch(l){console.warn("[notifiche] eliminaNotifica errore rete:",l)}}function Dd(t,e,o){try{let i=JSON.parse(localStorage.getItem("_notificheArr")||"[]"),n=!1,a=i.filter(function(s){if(n)return!0;let r=t&&String(s.rid||"")===String(t),c=String(s.titolo||"")===String(e||"")&&String(s.corpo||"")===String(o||"");return r||c?(n=!0,!1):!0});localStorage.setItem("_notificheArr",JSON.stringify(a))}catch{}}function Fi(t){let e=document.getElementById("badge-notifiche-desktop"),o=document.getElementById("badge-notifiche-mobile"),i=document.getElementById("badge-notifiche-mobile-menu");e&&(e.textContent=t>0?t:"",e.style.display=t>0?"flex":"none"),o&&(o.textContent=t>0?t:"",o.style.display=t>0?"flex":"none"),i&&(i.textContent=t>0?t:"",i.style.display=t>0?"flex":"none")}function Bd(t){return t?/stato/i.test(t)?"fa-rotate":/richiesta|comunic/i.test(t)?"fa-comment-dots":/assegnaz/i.test(t)?"fa-user-check":"fa-bell":"fa-bell"}function Di(t){return t.length?t.map(function(e,o){let i=Bd(e.titolo||""),n=Zt(e.titolo||"Notifica"),a=Zt(e.rid||""),s=encodeURIComponent(e.titolo||""),r=encodeURIComponent(e.corpo||""),c=Zt(e._ts||""),l=c?`<span class="notifica-ts">${c}</span>`:"",d="";try{let u=JSON.parse(e.corpo||"");if(u&&u.tipo==="accesso_richiesta"){let f=encodeURIComponent(u.id||""),p=encodeURIComponent(u.nome||""),m=ss();m[u.id]?d=`<div class="notifica-corpo"><span class="notif-risposta-ok">${Zt(m[u.id])}</span></div>`:d=`<div class="notifica-corpo">Vuole entrare fuori orario.</div>
                  ...
                  </div>`}}catch{}return d||(d=`<div class="notifica-corpo">${Zt(e.corpo||"")}</div>`),`<div class="notifica-item" onclick="apriDettaglioNotifica(${o})" role="button" tabindex="0">
        ...
        </div>`}).join(""):'<div class="notif-empty"><i class="far fa-bell-slash"></i><p>Nessuna notifica recente</p></div>'}function Ud(){let t=document.getElementById("notifiche-list");if(!t)return;let e=Ui(JSON.parse(localStorage.getItem("_notificheArr")||"[]"));try{localStorage.setItem("_notificheArr",JSON.stringify(e))}catch{}t.innerHTML=Di(e),b&&b.nome&&fetch(x,{method:"POST",body:JSON.stringify({azione:"getStoricoNotifiche",username:b.nome.toUpperCase(),days:ns})}).then(function(o){return o.json()}).then(function(o){o&&o.status==="ok"&&o.all&&o.all.length&&(Ao(o.all),t.innerHTML=Di(JSON.parse(localStorage.getItem("_notificheArr")||"[]")))}).catch(function(o){console.warn("[notifiche] renderNotificheList fetch fallito:",o)})}function Ao(t){try{let e=new Date().toLocaleString("it-IT",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}),o=t.map(function(l){return Object.assign({},l,{_ts:l._ts||e})}),i=JSON.parse(localStorage.getItem("_notificheArr")||"[]"),n={};o.forEach(function(l){n[l.titolo+"||"+l.corpo]=l}),i.forEach(function(l){let d=l.titolo+"||"+l.corpo;n[d]||(n[d]=l)});let a=Ui(Object.values(n)).slice(0,200);localStorage.setItem("_notificheArr",JSON.stringify(a));let s=parseInt(localStorage.getItem("_notifLastRead")||"0"),c=o.filter(function(l){let d=Bi(l._ts);return!d||d>s}).length;try{localStorage.setItem("_notifBadgeCount",String(c))}catch{}Fi(c)}catch{}}function Fd(){if(!b||!b.nome)return!1;let t=String(b.nome).toUpperCase().trim();return t==="ALESSIO"||t==="0000"||b.ruolo==="MASTER"}function Hd(){let t=new Date,e=t.getHours()*60+t.getMinutes();return e>=540&&e<1170}function jd(t){if(!(!t||t<=0)&&!(!b||!b.nome)&&!Fd()&&Hd()){try{let e=new Date().toLocaleDateString("it-IT"),o="_notifMorningToast_"+String(b.nome).toUpperCase().trim()+"_"+e;if(localStorage.getItem(o)==="1")return;localStorage.setItem(o,"1")}catch{}g("\u{1F514} Hai "+t+" notific"+(t===1?"a":"he")+" da leggere")}}function Hi(){try{let t=Ui(JSON.parse(localStorage.getItem("_notificheArr")||"[]")),e=parseInt(localStorage.getItem("_notifLastRead")||"0"),o=t.filter(function(i){let n=Bi(i._ts);return!n||n>e});o.length>0&&Fi(o.length)}catch{}!b||!b.nome||fetch(x,{method:"POST",body:JSON.stringify({azione:"getNotifiche",username:b.nome.toUpperCase(),markRead:0})}).then(function(t){return t.json()}).then(function(t){if(t&&t.status==="ok"&&t.all&&t.all.length&&(Ao(t.all),jd(t.all.length),t.titolo&&"serviceWorker"in navigator&&navigator.serviceWorker.controller))try{navigator.serviceWorker.controller.postMessage({type:"CACHE_NOTIF",titolo:t.titolo,corpo:t.corpo||""})}catch{}}).catch(function(t){console.warn("[notifiche] _initBadgeNotifiche fetch fallito:",t)})}function Gd(t){var e=String(t&&t.titolo||""),o=String(t&&t.corpo||""),i=(e+" "+o).replace(/\s+/g," ").trim();if(!i)return"";var n=i.match(/\bORD(?:INE)?\.?\s*[:#-]?\s*([A-Z0-9/-]{2,})/i);if(n&&n[1])return String(n[1]).trim();var a=i.match(/\b([A-Z]{2,}[A-Z0-9]*-[A-Z0-9-]{2,})\b/i);return a&&a[1]?String(a[1]).trim():""}async function Vd(t,e){try{var o=String(e||"").trim();if(!o){var i=JSON.parse(localStorage.getItem("_notificheArr")||"[]"),n=i[Number(t)];if(!n)return;o=Gd(n)}if(as(),!o){g("Nessun riferimento ordine/codice trovato in questa notifica");return}typeof window.cambiaPagina=="function"&&await window.cambiaPagina("PROGRAMMA PRODUZIONE DEL MESE",null),setTimeout(function(){["universal-search","mobile-search"].forEach(function(a){var s=document.getElementById(a);s&&(s.value=o,s.dispatchEvent(new Event("input")))}),typeof window.filtraUniversale=="function"&&window.filtraUniversale()},280)}catch(a){console.warn("[notifiche] apriDettaglioNotifica errore:",a)}}function rs(){window.apriPopupNotifiche=zd,window.chiudiPopupNotifiche=as,window.eliminaNotificaApp=qd,window.rispondiAccessoApp=Md,window.apriDettaglioNotifica=Vd}var ns,Pd,Zt,cs=W(()=>{ft();vt();gt();ns=7,Pd=ns*24*60*60*1e3;Zt=y});function Oo(){let t=document.getElementById("user-name-display"),e=document.getElementById("user-avatar-icon"),o=document.getElementById("account-ddrop-avatar"),i=document.getElementById("account-ddrop-name"),n=document.getElementById("account-ddrop-role"),a=document.getElementById("user-avatar-icon-mobile"),s=document.getElementById("account-ddrop-avatar-mob"),r=document.getElementById("account-ddrop-name-mob"),c=document.getElementById("account-ddrop-role-mob");if(b&&b.nome){let l=b.nome.charAt(0).toUpperCase(),d=b.nome.toUpperCase();t&&(t.innerText=d),e&&(e.innerText=l),o&&(o.innerText=l),i&&(i.innerText=d),n&&(n.innerText=(b.ruolo||"Utente").toUpperCase()),a&&(a.innerText=l),s&&(s.innerText=l),r&&(r.innerText=d),c&&(c.innerText=(b.ruolo||"Utente").toUpperCase())}Wd()}function ps(t){try{let e=String(t||"").toUpperCase().trim();return Co[e]?Co[e]:localStorage.getItem("avatarColor_"+e)||"#374151"}catch{return"#374151"}}function Gi(t){if(!t)return t;let e=String(t).trim().toUpperCase();return ls[e]?ls[e]:String(t).trim().toLowerCase().replace(/(?:^|\s|\.)\S/g,o=>o.toUpperCase())}function Wd(){if(!b||!b.nome)return;let t=ps(b.nome);window._renderCustomSwatches&&window._renderCustomSwatches(),window._applyAvatarColorUI&&window._applyAvatarColorUI(t)}async function Vi(){try{let t=await fetch(x,{method:"POST",body:JSON.stringify({azione:"getAvatarColors"})});if(!t.ok)return;let e=await t.json();if(typeof e!="object"||Array.isArray(e))return;if(Object.entries(e).forEach(([o,i])=>{if(!i)return;let n=o.toUpperCase().trim();Co[n]=i;try{localStorage.setItem("avatarColor_"+n,i)}catch{}}),b?.nome){let o=e[b.nome.toUpperCase().trim()];o&&window._applyAvatarColorUI&&window._applyAvatarColorUI(o)}typeof window._repaintOpColors=="function"&&window._repaintOpColors()}catch(t){console.warn("_caricaColoriAvatarDaServer:",t)}}function us(){return!!(b&&b.nome)}function Zd(){return b?String(b.ruolo||"").toUpperCase()==="COMMERCIALE":!1}function Qd(){let t=new Date,e=t.getHours()*60+t.getMinutes();return e>=510&&e<1170}function Ji(t){return sessionStorage.getItem("_accesso_extra_")==="1"||us()||Qd()?(ds(),!0):(t!==!1&&$o(),!1)}function $o(){if(document.getElementById("_lock-screen_"))return;let t=b&&b.nome?b.nome:"",e=document.createElement("div");e.id="_lock-screen_",e.style.cssText=["position:fixed","top:0","left:0","width:100%","height:100%","background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%)","z-index:99999","display:flex","flex-direction:column","align-items:center","justify-content:center","gap:16px","color:#e2e8f0","font-family:inherit"].join(";");let o=t?`<div style="margin-top:8px;font-size:0.82rem;color:#64748b">
               Accesso come: <strong style="color:#94a3b8">${Gi?Gi(t):t}</strong>
        ...
        <div id="_lock-stato_" style="font-size:0.82rem;color:#64748b;min-height:1.2em;text-align:center;max-width:260px"></div>`,document.body.appendChild(e)}function ds(){let t=document.getElementById("_lock-screen_");t&&t.remove(),Yd()}function Yd(){ji&&(clearInterval(ji),ji=null),Kd=null}function ms(){window._normNome=Gi,window._PREDEFINED_AVATAR_COLORS=Jd,window._avatarColorsCache=Co,window._getOpColor=ps,window._isUtenteEsente=us,window._isCommerciale=Zd}var Co,ls,Jd,Kd,ji,fs=W(()=>{ft();vt();Co={};ls={ALESSIO:"Alessio",RICCARDO:"Riccardo",FABIO:"Fabio T.","FABIO T":"Fabio T.","FABIO T.":"Fabio T.",NICCOLO:"Niccol\xF2","NICCOLO'":"Niccol\xF2","NICCOL\xD2'":"Niccol\xF2",RAYMOND:"Raymond",SIMONE:"Simone",GIACOMO:"Giacomo"};Jd=["#8fe45e","#6366f1","#f59e0b","#ec4899","#06b6d4","#f87171","#a78bfa","#34d399"];Kd=null,ji=null;setInterval(function(){b&&b.nome&&Ji(!0)},60*1e3)});var gp=Ps(()=>{ft();zt();vt();It();Qt();cn();Kt();gt();En();Rn();Jn();la();Fa();Xa();is();cs();fs();try{let t=document.getElementById("critical-init");t&&t.remove()}catch{}"serviceWorker"in navigator&&navigator.serviceWorker.addEventListener("controllerchange",()=>{window.location.reload()});"serviceWorker"in navigator&&navigator.serviceWorker.addEventListener("message",function(t){if(t.data&&t.data.type==="NUOVE_NOTIFICHE"){Ao(t.data.notifiche||[]);var e=t.data.notifiche||[];e.length>0&&e[0].titolo&&"caches"in window&&caches.open("prod-last-notif").then(function(i){i.put("last",new Response(JSON.stringify({titolo:e[0].titolo,corpo:e[0].corpo||""})))}).catch(function(){});return}if(t.data&&t.data.type==="OPEN_CSV_MODAL"){typeof window.cambiaPagina=="function"&&window.cambiaPagina("IMPOSTAZIONI",null).catch(function(){});return}if(t.data&&t.data.type==="OPEN_NOTIFICATION_TARGET")try{var o=String(t.data.target||"").trim();o&&typeof window.apriDettaglioNotifica=="function"&&window.apriDettaglioNotifica(-1,o)}catch{}});var gs=!1,ye=null,we=null,To=0,Me=0,vs=0,Wi=!1,Xd=0,hs=0;function tp(){let t=b?.sessionExpiresAt;if(!t)return 0;let e=Number(t);if(Number.isFinite(e)&&e>0)return e;let o=new Date(t).getTime();return Number.isFinite(o)?o:0}function bs(){let t=tp();if(!t)return;let e=t-Date.now();if(e<=0||e>1440*60*1e3||Date.now()-hs<180*1e3)return;hs=Date.now();let o=Math.max(1,Math.floor(e/36e5));g("Sessione in scadenza tra circa "+o+" ore. Rientra per rinnovarla.","warning")}function Qi(t){Xd=Number(t)||Date.now()}function Po(){return en()}function ep(){on()}function xs(){if(gs||typeof window.fetch!="function")return;let t=window.fetch.bind(window);function e(o){return ep(),o.clone().text().then(function(i){try{let n=JSON.parse(i);n&&n.status==="auth_error"&&As(n.message||n.msg||"Sessione scaduta."),n&&n.status==="fuori_orario"&&typeof $o=="function"&&$o()}catch{}}).catch(function(){}),o}window.fetch=function(o,i){try{let n=Po(),a=typeof o=="string"?o:o&&o.url?o.url:"";if(!a||a.indexOf(x)!==0)return t(o,i);let s=String(i&&i.method||"GET").toUpperCase();if(s==="GET")return t(o,i).then(e);if(s==="POST"&&i&&typeof i.body=="string")try{let r=JSON.parse(i.body||"{}");if(n&&!r.sessionToken){r.sessionToken=n;let c=Object.assign({},i,{body:JSON.stringify(r)});return t(o,c).then(e)}}catch{}return t(o,i).then(e)}catch{}return t(o,i)},gs=!0}xs();async function zo(){let t=Po();if(!t)return!1;let e=b||null;if(!e)try{let o=localStorage.getItem("sessioneUtente")||sessionStorage.getItem("sessioneUtente");e=o?JSON.parse(o):null}catch{}try{let i=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"refreshSession",sessionToken:t,username:e&&e.nome?String(e.nome):"",email:e&&e.email?String(e.email):""})})).json();if(i&&i.status==="success"&&i.sessionToken){To=0,Me=0,b||Lt({}),b.sessionToken=i.sessionToken,b.sessionExpiresAt=i.sessionExpiresAt||"",b.expiresAt=Date.now()+2592e6,!b.nome&&i.nome&&(b.nome=i.nome),!b.email&&i.email&&(b.email=i.email),!b.ruolo&&i.ruolo&&(b.ruolo=i.ruolo);try{localStorage.setItem("sessioneUtente",JSON.stringify(b))}catch{}try{sessionStorage.setItem("sessioneUtente",JSON.stringify(b))}catch{}return!0}if(i&&i.status==="auth_error")return To++,To>=3&&(To=0,Ne()),!1}catch{}return!1}function Is(){ye&&clearInterval(ye),we&&clearInterval(we),ye=setInterval(zo,300*1e3),we=setInterval(bs,60*1e3),bs()}window.addEventListener("storage",function(t){if(!(t.key!=="sessioneUtente"||!t.newValue))try{let e=JSON.parse(t.newValue);if(!e||!e.sessionToken)return;b||Lt({}),b.sessionToken=String(e.sessionToken),e.sessionExpiresAt&&(b.sessionExpiresAt=e.sessionExpiresAt)}catch{}});document.addEventListener("visibilitychange",function(){document.hidden||zo()});var ys=!1;async function As(t){if(ys)return;var e=document.getElementById("login-overlay");if(e&&e.style.display!=="none")return;let o=Date.now();if(o-vs>3e4&&(Me=0),vs=o,Me++,Me===1&&!Wi){Wi=!0;try{if(await zo()){Me=0;return}}finally{Wi=!1}}ys=!0,g(t||"Sessione scaduta. Effettua nuovamente il login.","error"),setTimeout(function(){Ne()},2e3)}try{let t=localStorage.getItem("sessioneUtente")||sessionStorage.getItem("sessioneUtente");if(t){let e=JSON.parse(t);if(e&&e.sessionToken){let o=document.getElementById("login-overlay");o&&(o.style.display="none"),document.documentElement.classList.add("has-session")}}}catch{}var Ki=null,Yi=!1;window.filtroRicercaArticoli=Yi;var op=[];function ip(){return[{nome:"PREPARARE",colore:"#94a3b8"},{nome:"PREPARARE PER LAVORAZIONE",colore:"#64748b"},{nome:"MANDA IN LAVORAZIONE",colore:"#475569"},{nome:"IN LAVORAZIONE",colore:"#f59e0b"},{nome:"TORNATO DALLA LAVORAZIONE",colore:"#7c3aed"},{nome:"IN PRODUZIONE",colore:"#242424"},{nome:"IMBALLATO",colore:"#22c55e"},{nome:"SPEDITO/CONSEGNATO",colore:"#06b6d4"}]}var np=[],ws=!1,ap=0,Zi=0,Ro=null,Ss=0,Rt=null,kt=null,Cs="BAHqp3uv56mQSAeTv_66-f4GYkzaESwuJNOP5DJCVMi197n-EKl9TW9XPrKeIIDpzBz0HTM42AcUCXWmOP5BSYI";async function Os(){if(!(!("serviceWorker"in navigator)||!("PushManager"in window)))try{let t=await navigator.serviceWorker.register("sw.js",{scope:"./"});await navigator.serviceWorker.ready,"caches"in window&&await(await caches.open("prod-auth")).put("username",new Response(b.nome.toUpperCase()));let e=await t.pushManager.getSubscription(),o=Notification.permission;if(!e&&o==="granted")try{e=await t.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:window._vapidB64ToUint8_?window._vapidB64ToUint8_(Cs):null})}catch(a){console.warn("[Push] Auto-subscribe failed:",a);try{localStorage.setItem("_pushStato","errore-subscribe")}catch{}return}if(!e){try{localStorage.setItem("_pushStato","no-permesso")}catch{}return}let i=e.toJSON(),n=await $s({endpoint:i.endpoint,p256dh:i.keys?.p256dh,auth:i.keys?.auth});if(n&&(n.status==="saved"||n.status==="updated"))try{localStorage.setItem("_pushStato","ok")}catch{}else if(n&&n.status==="errore-verifica"){try{localStorage.setItem("_pushStato","errore-verifica")}catch{}g('\xE2\u0161\xA0\xEF\xB8\x8F Subscription creata ma NON confermata sul server. Riprova "Ri-registra subscription".',"error")}else try{localStorage.setItem("_pushStato","errore-salvataggio")}catch{}window._aggiornaUINotifiche&&window._aggiornaUINotifiche()}catch(t){console.warn("[Push] initPush:",t);try{localStorage.setItem("_pushStato","errore:"+t.message)}catch{}}}async function $s(t){try{let o=await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"salvaSottoscrizione",username:b.nome.toUpperCase(),endpoint:t.endpoint,p256dh:t.p256dh||"",auth:t.auth||""})})).json().catch(()=>({}));if(o&&(o.status==="saved"||o.status==="updated"))try{(await(await fetch(x,{method:"POST",body:JSON.stringify({azione:"verificaIscrizione",username:b.nome.toUpperCase(),endpoint:t.endpoint})})).json().catch(()=>({}))).found||(console.warn("[Push] verificaIscrizione: endpoint NON trovato nel foglio dopo il salvataggio!"),o.status="errore-verifica")}catch(i){console.warn("[Push] verificaIscrizione error:",i)}return o}catch(e){console.warn("[Push] _salvaSubVAPID_ error:",e)}}var Lo=null,_s=null,sp={card:"bg-white/90 border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow",cardGrid:"grid gap-3",label:"text-[10px] uppercase tracking-wide text-slate-500 font-semibold",value:"text-slate-900 font-semibold",btn:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-slate-200 bg-white hover:bg-slate-50 active:scale-[0.99] transition",btnPrimary:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 active:scale-[0.99] transition",btnSuccess:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 active:scale-[0.99] transition",btnWarning:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-orange-200 bg-orange-50 text-orange-700 hover:bg-orange-100 active:scale-[0.99] transition",btnDanger:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-red-200 bg-red-50 text-red-600 hover:bg-red-100 active:scale-[0.99] transition",btnPrimaryLg:"inline-flex items-center gap-2 rounded-xl px-10 py-3.5 text-sm font-bold bg-slate-900 text-white hover:bg-slate-700 active:scale-[0.98] transition shadow-sm",pill:"inline-flex items-center justify-center rounded-full px-2 py-1 text-[10px] font-bold bg-slate-100 text-slate-600"};function Mo(){Lo=document.querySelectorAll(".ordine-wrapper, .chat-card, .materiale-card, .manuale-card")}function Ts(){if(typeof x>"u")return;U("_html__acq_ordini"),A.dashPromise=fetch(x,{method:"POST",body:JSON.stringify({azione:"getAllDashboard",includeArchivio:!1})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null}),A.rqPromise=fetch(x,{method:"POST",body:JSON.stringify({azione:"getAllRichieste"})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null}),A.matPromise=fetch(x,{method:"POST",body:JSON.stringify({pagina:"MATERIALE DA ORDINARE"})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null});let e=b?.nome?.toUpperCase().trim()==="ALESSIO"?"":b?.nome||"";A.ordiniPromise=fetch(x,{method:"POST",body:JSON.stringify({azione:"getOrdiniAcquisti",operatore:e})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null}),A.dashPromise.then(function(o){o&&(A.dashBundle=o)}),A.rqPromise.then(function(o){o&&(A.rqBundle=o)}),A.matPromise.then(function(o){o&&(A.matBundle=o)}),A.ordiniPromise.then(function(o){o&&(A.ordiniBundle=o)})}function rp(){let t=null;try{t=localStorage.getItem("ultimaPaginaProduzione")}catch{}(!t||t==="undefined"||t==="null")&&(t="PROGRAMMA PRODUZIONE DEL MESE"),delete H[t],X[t]=0,U("_html_"+t)}function Rs(){xs(),Is(),Rt&&Rt.registerGlobals(),kt&&kt.registerGlobals(),Vn(),Ua(),Ya(),window.cambiaPagina=ko,window.aggiornaListaFiltrabili=Mo,ln({onRemoteChange:function(i){g("\u{1F504} "+i+" ha aggiornato i dati"),Qi(Date.now()),an();try{N.clear()}catch{}switch(Object.keys(H).forEach(n=>{delete H[n]}),Object.keys(X).forEach(n=>{X[n]=0}),Ki){case"PROGRAMMA PRODUZIONE DEL MESE":Se("PROGRAMMA_PRODUZIONE",yo,wo,!0).catch(n=>console.warn("[RevisionPoller] refresh failed:",n));break;case"STORICO_RICHIESTE":Ot();break;case"MATERIALE DA ORDINARE":Yt(null);break;case"MANUALI_PRODOTTI":$e(null,null,!0);break;case"ARCHIVIO_ORDINI":typeof ge=="function"&&ge();break}},onUsersOnline:function(i){pp(i)},getUtenteAttuale:function(){return b},getPaginaCorrente:function(){return Ki}}),M.start(),Qi(Date.now());let e=null;try{e=localStorage.getItem("ultimaPaginaProduzione")}catch{}(!e||e==="undefined"||e==="null")&&(e="PROGRAMMA PRODUZIONE DEL MESE");let o=document.querySelector(`.menu-item[data-page="${e}"]`);ko(e,o).catch(i=>{i&&i.name!=="AbortError"&&console.warn("[init] cambiaPagina:",i)});try{if(new URLSearchParams(window.location.search).get("action")==="openCsvModal"){let a=window.location.pathname+window.location.hash;window.history.replaceState(null,"",a),setTimeout(function(){ko("IMPOSTAZIONI",null).then(function(){setTimeout(function(){typeof window._apriCsvPendingModal_=="function"&&window._apriCsvPendingModal_()},300)}).catch(function(){})},400)}}catch{}}window.onload=async function(){if(ws)return;ws=!0;let t=document.getElementById("login-overlay"),e=null;try{e=localStorage.getItem("sessioneUtente")||sessionStorage.getItem("sessioneUtente")}catch{}if(e){if(Lt(JSON.parse(e)),b.expiresAt&&Date.now()>b.expiresAt){Lt(null);try{localStorage.removeItem("sessioneUtente"),sessionStorage.removeItem("sessioneUtente")}catch{}document.documentElement.classList.remove("has-session"),t&&(t.style.display="flex",t.style.opacity="1");let o=document.getElementById("login-error");o&&(o.innerText="Sessione scaduta. Effettua nuovamente il login.",o.style.color="#ef4444");return}if(b.ruolo!=="MASTER"&&!b.sessionToken){Lt(null);try{localStorage.removeItem("sessioneUtente"),sessionStorage.removeItem("sessioneUtente")}catch{}document.documentElement.classList.remove("has-session"),t&&(t.style.display="flex",t.style.opacity="1");let o=document.getElementById("login-error");o&&(o.innerText="Sessione non pi\xC3\xB9 valida. Effettua di nuovo il login.",o.style.color="#ef4444");return}Ts(),Oo(),Os(),Hi(),(window.requestIdleCallback||function(o){setTimeout(o,3e3)})(function(){Vi()}),t&&(t.style.display="none")}else document.documentElement.classList.remove("has-session"),t&&(t.style.display="flex",t.style.opacity="1");if(e&&typeof Le=="function"&&await Le().catch(o=>console.warn("[Boot] caricaDatiIniziali:",o)),e){if(b.ruolo!=="MASTER"&&!b.nome){document.documentElement.classList.remove("has-session");try{localStorage.removeItem("sessioneUtente"),sessionStorage.removeItem("sessioneUtente")}catch{}t&&(t.style.display="flex",t.style.opacity="1");return}Rs()}};function cp(){let t=document.getElementById("main-sidebar");if(!t)return;let e=t.classList.toggle("collapsed");document.body.classList.toggle("sidebar-collapsed",e);try{localStorage.setItem("sidebarCollapsed",e?"1":"0")}catch{}}function lp(){try{let t=localStorage.getItem("sidebarCollapsed"),e=document.getElementById("main-sidebar");t==="1"&&(e&&e.classList.add("collapsed"),document.body.classList.add("sidebar-collapsed"))}catch{}}document.addEventListener("DOMContentLoaded",lp);async function dp(){if(!Ji(!0))return;b&&(b.expiresAt=Date.now()+2592e6);try{localStorage.setItem("sessioneUtente",JSON.stringify(b))}catch{}try{sessionStorage.setItem("sessioneUtente",JSON.stringify(b))}catch{}Is(),zo();let t=document.getElementById("login-overlay");t.style.transition="opacity 0.4s ease",t.style.opacity="0",Ts(),U("_impostazioni_cache"),await Promise.all([Le().catch(e=>console.warn("caricaDatiIniziali post-login:",e)),new Promise(e=>setTimeout(e,400))]),t.style.display="none",document.documentElement.classList.add("has-session"),typeof Oo=="function"&&Oo(),Os(),Hi(),(window.requestIdleCallback||function(e){setTimeout(e,3e3)})(function(){Vi()}),rp(),Rs()}function Ne(){if(!Ne._running){Ne._running=!0,M.stop(),ve();try{let t=Po();t&&fetch(x,{method:"POST",body:JSON.stringify({azione:"logout",sessionToken:t})}).catch(function(){}),ye&&(clearInterval(ye),ye=null),we&&(clearInterval(we),we=null);try{N.clear()}catch{}let e={};for(let o=0;o<localStorage.length;o++){let i=localStorage.key(o);i&&(rn.some(n=>i.startsWith(n))||sn.includes(i))&&(e[i]=localStorage.getItem(i))}localStorage.clear(),sessionStorage.clear(),Object.entries(e).forEach(([o,i])=>{try{localStorage.setItem(o,i)}catch{}}),window.location.href=window.location.origin+window.location.pathname+"?logout="+Date.now()}catch(t){console.error("Errore durante il logout:",t),window.location.reload()}}}function pp(t){var e=b&&b.nome?b.nome.toUpperCase():"",o=t.filter(function(c){return c.nome.toUpperCase()!==e}),i=document.getElementById("user-avatar-btn"),n=document.getElementById("user-avatar-btn-mobile");if(o.length===0){var a=document.getElementById("online-indicator");a&&a.remove();var s=document.getElementById("online-indicator-mob");s&&s.remove();return}var r="Online ora: "+o.map(function(c){return c.nome+(c.pagina?" ("+c.pagina+")":"")}).join(", ");[{parent:i,id:"online-indicator"},{parent:n,id:"online-indicator-mob"}].forEach(function(c){if(c.parent){var l=document.getElementById(c.id);l||(l=document.createElement("span"),l.id=c.id,c.parent.appendChild(l)),l.title=r}})}document.addEventListener("visibilitychange",function(){M._timer&&(document.hidden?M._schedule(M.INTERVAL_BG_MS):(M._check(),M._schedule(M.INTERVAL_FOCUS_MS)))});window.addEventListener("online",function(){M._timer&&M._check()});async function ko(t,e){let o=Date.now();if(o-Ss<300)return;if(Ss=o,Ro)try{Ro.abort()}catch{}Ro=new AbortController;let i=Ro.signal,n=++ap;Zi=n,window._latestNavRequest=n,Lo=null,t!=="PROGRAMMA PRODUZIONE DEL MESE"&&ve();let a=document.getElementById("universal-search");a&&(a.value="");let s=document.getElementById("desk-search-input");s&&(s.value=""),(!t||t==="undefined"||t==="null")&&(t="PROGRAMMA PRODUZIONE DEL MESE"),localStorage.setItem("ultimaPaginaProduzione",t),Ki=t,window.paginaAttuale=t,Mi(t),document.body.classList.toggle("page-pip",t==="PIPISTRELLI"),t!=="PIPISTRELLI"&&Rt&&Rt.resetPipFetch(),t!=="KIT_PRODOTTI"&&kt&&kt.resetKitFetch();let r=document.getElementById("acq-tab-bar");r&&(r.style.display=t==="MATERIALE DA ORDINARE"?"flex":"none"),document.querySelectorAll(".menu-item").forEach(m=>m.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(m=>m.classList.remove("active")),e||(e=document.querySelector(`.menu-item[data-page="${t}"]`)),e&&e.classList.add("active");let c=document.querySelector(`.tab-item[data-page="${t}"]`);c&&c.classList.add("active");let l={IMPOSTAZIONI:"Impostazioni Sistema",STORICO_RICHIESTE:"La mia Casella",ARCHIVIO_ORDINI:"Archivio Ordini","MATERIALE DA ORDINARE":"Gestione Acquisti",MANUALI_PRODOTTI:"Manuali Prodotti","PROGRAMMA PRODUZIONE DEL MESE":"Dashboard Produzione",PIPISTRELLI:"\xF0\u0178\xA6\u2021 Pipistrelli",KIT_PRODOTTI:"\xF0\u0178\xA7\xB0 Kit Prodotti"},d=document.getElementById("titolo-pagina");d&&(d.innerText=l[t]||t);let u=document.getElementById("page-title-desktop");u&&(u.innerText=l[t]||t);let f=document.getElementById("floating-cart-btn");if(f){let m=t==="MATERIALE DA ORDINARE";f.style.display=m?"flex":"none",!m&&typeof chiudiModalCarrello=="function"&&chiudiModalCarrello()}let p=document.getElementById("contenitore-dati");if(!H[t]){let m="_html_"+t,h=ht(m,3e5);if(h){H[t]=h;try{let w=localStorage.getItem(m),v=w?JSON.parse(w):null;X[t]=v&&v.ts?v.ts:Date.now()-3e5-1e3}catch{X[t]=Date.now()-3e5-1e3}}}if(H[t]?p.innerHTML="":p.innerHTML=`<div class="nav-skeleton">
        ...
        </div>`,["modalAiuto","modal-conferma","modal-gestione-articolo","modal-carrello"].forEach(m=>{let h=document.getElementById(m);if(h){if(m==="modal-carrello"){h.classList.remove("cart-open");return}if(m==="modal-gestione-articolo"){h.classList.remove("active"),setTimeout(()=>{h.classList.contains("active")||(h.style.display="none")},300);return}h.classList.remove("active"),setTimeout(()=>{h.classList.contains("active")||(h.style.display="none")},300)}}),t==="STORICO_RICHIESTE"){let m=document.getElementById("badge-richieste-count");m&&(m.style.display="none",m.classList.remove("badge-sollecito-attivo"));let h=document.getElementById("badge-mobile-notif");h&&(h.style.display="none");let w=document.getElementById("badge-bottom-richieste");w&&(w.style.display="none",w.classList.remove("badge-sollecito-attivo"))}if(H[t]){p.innerHTML=H[t],Qi(X[t]||Date.now()),z(p),Mo(),requestAnimationFrame(Tt),t==="PROGRAMMA PRODUZIONE DEL MESE"&&(_o(),requestAnimationFrame(()=>{typeof window._renderProdFilterBar=="function"&&window._renderProdFilterBar(),typeof window._applicaFiltriProd=="function"&&window._pfHasActiveFilters&&window._pfHasActiveFilters()&&window._applicaFiltriProd();let w=window._getAttiviProd?window._getAttiviProd():null;w&&w.forEach(v=>{if(parseFloat(v.qty_evasa)>0){let E=document.getElementById("qty-evasa-block-"+v.id_riga),C=E&&E.closest(".qty-cell")?.querySelector(".btn-qty-evasa-toggle");E&&(E.style.display="inline-flex"),C&&C.classList.add("active")}})}));let m=Date.now(),h=X[t]||0;m-h>3e5&&(t==="PROGRAMMA PRODUZIONE DEL MESE"?So(t,!0,n,i):t==="MATERIALE DA ORDINARE"?Yt(null,n,i,!0):t==="STORICO_RICHIESTE"?Se("STORICO_RICHIESTE",()=>ne(i),Ae,!0).catch(()=>{}):t==="MANUALI_PRODOTTI"?$e(n,i,!0).catch(()=>{}):t==="ARCHIVIO_ORDINI"&&ge());return}switch(t){case"IMPOSTAZIONI":Jt();break;case"STORICO_RICHIESTE":{let m=document.getElementById("contenitore-dati");m&&(m.innerHTML="<div class='centered-msg' id='_ric-loader'>Caricamento messaggi in corso...</div>"),Se("STORICO_RICHIESTE",ne,Ae).catch(async h=>{if(h&&h.name==="AbortError")return;let w=null;try{w=await N.get("STORICO_RICHIESTE")}catch{}if(w){let v=new Date(w.timestamp),E=String(v.getHours()).padStart(2,"0"),C=String(v.getMinutes()).padStart(2,"0");g("Connessione assente \xE2\u20AC\u201D mostro dati salvati alle "+E+":"+C,"warning")}else{let v=document.getElementById("contenitore-dati");v&&(v.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('STORICO_RICHIESTE',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(v))}});break}case"ARCHIVIO_ORDINI":ge();break;case"MATERIALE DA ORDINARE":Yt(e?"catalogo":null,n,i);break;case"MANUALI_PRODOTTI":$e(n,i,!1);break;case"ORDINI_ACQUISTI":Yt("ordini",n,i);return;case"PIPISTRELLI":try{Rt||(Rt=await import("./chunk-pipistrelli-YHWPXY32.js"),Rt.registerGlobals()),Rt.caricaPipistrelli()}catch(m){if(m&&m.name==="AbortError")return;console.warn("[PIPISTRELLI] Errore caricamento modulo:",m);let h=document.getElementById("contenitore-dati");h&&(h.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('PIPISTRELLI',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(h)),Rt=null}break;case"KIT_PRODOTTI":try{if(kt||(kt=await import("./chunk-kit-prodotti-NW57ALDA.js"),kt.registerGlobals()),n!==Zi||window.paginaAttuale!=="KIT_PRODOTTI")return;kt.caricaKitProdotti()}catch(m){if(m&&m.name==="AbortError"||n!==Zi||window.paginaAttuale!=="KIT_PRODOTTI")return;console.warn("[KIT_PRODOTTI] Errore caricamento modulo:",m);let h=document.getElementById("contenitore-dati");h&&(h.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('KIT_PRODOTTI',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(h)),kt=null}break;default:{let m=document.getElementById("contenitore-dati");m&&(m.innerHTML="<div class='inline-msg' id='_prod-loader'>Caricamento Dashboard...</div>",z(m)),Se("PROGRAMMA_PRODUZIONE",yo,wo).catch(async h=>{if(h&&h.name==="AbortError")return;let w=null;try{w=await N.get("PROGRAMMA_PRODUZIONE")}catch{}if(w){let v=new Date(w.timestamp),E=String(v.getHours()).padStart(2,"0"),C=String(v.getMinutes()).padStart(2,"0");g("Connessione assente \xE2\u20AC\u201D mostro dati salvati alle "+E+":"+C,"warning")}else{let v=document.getElementById("contenitore-dati");v&&(v.innerHTML=`<div class='inline-error'>Errore nel caricamento dati.
                                <button onclick="cambiaPagina('PROGRAMMA PRODUZIONE DEL MESE', null)"
                                    style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">
                                    &#x21bb; Riprova</button></div>`,z(v))}})}}}function up(t){return t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function mp(t,e){return!e||t.trimStart().startsWith(e)?!0:t.split(/[\s(,;]+/).some(o=>o.toLowerCase().startsWith(e))}function fp(){clearTimeout(_s),_s=setTimeout(function(){let t=document.getElementById("universal-search"),e=document.getElementById("mobile-search"),o=t&&t.value!==""?t.value:e&&e.value!==""?e.value:t?t.value:"",i=String(o||"").trim().toLowerCase();Lo||Mo(),typeof window.filtroRicercaArticoli<"u"&&(Yi=!!window.filtroRicercaArticoli);let n=!!Yi,a=i?new RegExp(up(i),"i"):null;(Lo||[]).forEach(function(r){if(!i){r.classList.remove("hidden-search");return}let c=(r.textContent||"").toLowerCase(),l=(r.getAttribute("data-codice")||"").toLowerCase(),d=!1;if(n)d=l?l.indexOf(i)!==-1:c.indexOf(i)!==-1;else{let u=(r.getAttribute("data-cliente")||"").toLowerCase(),f=(r.getAttribute("data-ordine")||r.getAttribute("data-codice")||"").toLowerCase(),p=(r.getAttribute("data-riferimento")||"").toLowerCase(),m=(r.getAttribute("data-codici")||"").toLowerCase(),h=c+" "+u+" "+f+" "+p+" "+m,w=mp(h,i),v=a?a.test(h):!1;d=w||i.length>=2&&v}r.classList.toggle("hidden-search",!d)});let s=document.getElementById("sezione-archivio");s&&(s.style.display=i===""?"block":"none")},120)}os();rs();ms();tn();_n();Tn();Gn();ca();Ba();Ka();window.cambiaPagina=ko;window.aggiornaListaFiltrabili=Mo;window.filtraUniversale=fp;window.toggleSidebar=cp;window.logout=Ne;window.salvaEApriDashboard=dp;window.cacheContenuti=H;window.TW=sp;window.listaStati=np;window.listaOperatori=op;window._VAPID_PUBLIC_KEY=Cs;window._salvaSubVAPID_=$s;window._gestisciAuthError_=As;window._getSessionToken_=Po;window._defaultListaStati_=ip});export default gp();
