import{a as ge,b as D,c as Tt}from"./chunk-chunk-VLVTJAKH.js";import{a as V,b as ws,c as I,d as ke,e as lt,f as y,g,h as z,i as Ui,j as et,k as Fi,l as dt,m as h,n as Ct,o as Hi,p as ji,q as ut,r as $t,s as Ss,t as yt,u as ot,v as Gi,w as ve}from"./chunk-chunk-NM2LGQZU.js";function pt(t,e){try{let o=localStorage.getItem(t);if(!o)return null;let i=JSON.parse(o);return Date.now()-i.ts<e?i.data:null}catch{return null}}function J(t,e){try{let o=typeof e=="string"?e:JSON.stringify(e);if(o.length>25e5)return;localStorage.setItem(t,JSON.stringify({ts:Date.now(),data:o}))}catch{}}function U(t){try{localStorage.removeItem(t)}catch{}}function Vi(){try{let t=[];for(let e=0;e<localStorage.length;e++){let o=localStorage.key(e);o&&o.startsWith("_html_")&&t.push(o)}t.forEach(e=>localStorage.removeItem(e))}catch{}}var wt=V(()=>{});var F,Q,A,Vt=V(()=>{"use strict";F={},Q={},A={dashBundle:null,dashPromise:null,rqBundle:null,rqPromise:null,matBundle:null,matPromise:null,ordiniBundle:null,ordiniPromise:null}});var _s,Es,xs,Is,As,Os,Cs,$s,Ts,Ji,Wi,Zi=V(()=>{"use strict";_s="_pushStato",Es="notifPrefs",xs="mlPipQty",Is="mlPipCaricato",As="mlPipPronti",Os="mlPipMovimenti",Cs="avatarColor_",$s="avatarColorRecenti_",Ts="avatarColorHidden_",Ji=[Es,_s,xs,Is,Os,As],Wi=[Cs,$s,Ts]});function Qi(t){Co=t.onRemoteChange,$o=t.onUsersOnline,he=t.getUtenteAttuale,Pt=t.getPaginaCorrente}var Co,$o,he,Pt,Ki,M,Jt=V(()=>{ve();lt();Ss();Co=null,$o=null,he=null,Pt=null;Ki={INTERVAL_MS:2e4,INTERVAL_FOCUS_MS:3e4,INTERVAL_BG_MS:3e4,PING_INTERVAL_MS:6e4,MAX_BACKOFF_MS:6e4,_timer:null,_pingTimer:null,_lastRevision:null,_lastCheck:0,_paused:!1,_errorStreak:0,_offPageTick:0,lastRevisionValue:null,lastOnlineList:[],lastCheckTs:0,start:function(){this.stop(),this._lastRevision=null,this._paused=!1,this._errorStreak=0,this._offPageTick=0,this._schedule(document.hidden?this.INTERVAL_BG_MS:this.INTERVAL_FOCUS_MS),this._schedulePing(5e3)},stop:function(){this._timer&&(clearTimeout(this._timer),this._timer=null),this._pingTimer&&(clearTimeout(this._pingTimer),this._pingTimer=null),this._lastRevision=null,this._paused=!1;var t=document.getElementById("online-indicator");t&&t.remove();var e=document.getElementById("online-indicator-mob");e&&e.remove()},pauseFor:function(t){t||(t=5e3),this._paused=!0;var e=this;setTimeout(function(){e._paused=!1},t)},_schedule:function(t){this._timer&&clearTimeout(this._timer);var e=this;this._timer=setTimeout(function(){e._tick()},t)},_tick:function(){var t=this;this._check().finally(function(){var e=Pt?String(Pt()||"").toUpperCase().trim():"",o=e==="PROGRAMMA PRODUZIONE DEL MESE",i=document.hidden?t.INTERVAL_BG_MS:document.hasFocus&&document.hasFocus()?t.INTERVAL_FOCUS_MS:t.INTERVAL_MS;o||(i=Math.max(i,t.INTERVAL_BG_MS));var n=i;if(t._errorStreak>0){var a=Math.min(4,1+t._errorStreak*.5);n=Math.min(t.MAX_BACKOFF_MS,Math.round(i*a))}t._schedule(n)})},_check:async function(){if(!this._paused){var t=Date.now(),e=Pt?String(Pt()||"").toUpperCase().trim():"",o=e==="PROGRAMMA PRODUZIONE DEL MESE";if(!(!o&&(this._offPageTick=(this._offPageTick+1)%3,this._offPageTick!==0)))try{var i=await Gi();if(!i||i.status!=="ok"){this._errorStreak=Math.min(this._errorStreak+1,10),$t("poller_check",{action:e||"UNKNOWN_PAGE",status:"invalid_payload",durationMs:Date.now()-t},{sampleRate:.5});return}var n=Number(i.revision);if(this._errorStreak=0,this._lastCheck=Date.now(),this.lastRevisionValue=n,this.lastCheckTs=Date.now(),this._lastRevision===null){this._lastRevision=n,$t("poller_check",{action:e||"UNKNOWN_PAGE",status:"baseline",durationMs:Date.now()-t},{sampleRate:.3});return}if(n===this._lastRevision){$t("poller_check",{action:e||"UNKNOWN_PAGE",status:"unchanged",durationMs:Date.now()-t},{sampleRate:.2});return}var a=he?he():null,r=a&&a.nome?a.nome.toUpperCase():"",s=i.utente?String(i.utente).toUpperCase():"";if(this._lastRevision=n,s===r)return;var c=i.utente||"Qualcuno";$t("poller_check",{action:e||"UNKNOWN_PAGE",status:"remote_change",durationMs:Date.now()-t,detail:String(c||"")},{sampleRate:1}),Co&&Co(c)}catch(l){this._errorStreak=Math.min(this._errorStreak+1,10),$t("poller_check",{action:e||"UNKNOWN_PAGE",status:"error",durationMs:Date.now()-t,error:l&&l.message?l.message:String(l||"")},{sampleRate:.7}),l&&l.name!=="AbortError"&&console.warn("[RevisionPoller]",l)}}},_schedulePing:function(t){this._pingTimer&&clearTimeout(this._pingTimer);var e=this;this._pingTimer=setTimeout(function(){e._pingServer().finally(function(){e._schedulePing(e.PING_INTERVAL_MS)})},t)},_pingServer:async function(){var t=he?he():null;if(!(!t||!t.nome))try{var e=await yt({azione:"ping",pagina:(Pt?Pt():"")||""});e&&e.status==="ok"&&Array.isArray(e.online)&&($o&&$o(e.online),Ki.lastOnlineList=e.online,$t("poller_ping",{action:"ping",status:"ok",detail:String(e.online.length)},{sampleRate:.3}))}catch(o){$t("poller_ping",{action:"ping",status:"error",error:o&&o.message?o.message:String(o||"")},{sampleRate:.7})}}},M=Ki});function Xi(){return"ORDINI_ACQUISTI_"+(h?.nome?.toUpperCase()||"_")}function Ro(){let t=document.getElementById("contenitore-dati");t&&(st._acq_ordini=t.innerHTML,_t._acq_ordini=Date.now(),D.set(Xi(),t.innerHTML).catch(()=>{}))}function zs(){A.ordiniBundle=null,A.ordiniPromise=null}function Yi(t,e,o){t.classList.toggle("is-ordinato",o),e.classList.toggle("checked",o),e.title=o?"Segna In Attesa":"Segna Ordinato";let i=e.querySelector("i");i&&(i.className="fas "+(o?"fa-check-circle":"fa-circle"));let n=t.querySelector(".oi-stato-badge");n&&(n.className="oi-stato-badge "+(o?"badge-ordinato-sm":"badge-attesa-sm"),n.innerHTML=o?'<i class="fas fa-circle-check"></i> ORDINATO':"IN ATTESA");let a=t.querySelector(".oi-stato-dot");a&&(a.className="oi-stato-dot "+(o?"dot-ordinato":"dot-attesa"));let r=t.closest(".ordine-group");if(r){let l=r.querySelectorAll(".ordine-item").length,d=r.querySelectorAll(".ordine-item.is-ordinato").length,p=r.querySelector(".og-progress");if(p&&(p.textContent=d+"/"+l),d===l){r.classList.add("all-done");let f=r.querySelector(".og-left");f&&!f.querySelector(".og-done-badge")&&f.insertAdjacentHTML("beforeend",'<span class="og-done-badge"><i class="fas fa-check-circle"></i> Completato</span>')}else{r.classList.remove("all-done");let f=r.querySelector(".og-done-badge");f&&f.remove()}}let s=document.querySelector(".acquisti-subtitle");if(s){let c=document.querySelectorAll(".ordine-item:not(.is-ordinato)").length;s.textContent=c>0?`${c} articoli in attesa`:"Tutto ordinato \u2705"}}function tn(t,e){return fetch(I,{method:"POST",body:JSON.stringify({pagina:t}),...e?{signal:e}:{}}).then(o=>{if(!o.ok)throw new Error(`HTTP ${o.status}`);return o.json()})}function Lo(){let t=document.getElementById("acq-tab-catalogo"),e=document.getElementById("acq-tab-ordini"),o=document.getElementById("acq-tab-fornitori");t&&t.classList.toggle("active",at==="catalogo"),e&&e.classList.toggle("active",at==="ordini"),o&&o.classList.toggle("active",at==="fornitori")}function Ms(t){if(t===at)return;at=t,window._acquistTabAttivo=t,Lo();let e=document.getElementById("contenitore-dati");if(e){if(t==="fornitori"){typeof window.caricaOrdiniFornitori=="function"&&window.caricaOrdiniFornitori(null,null,!1);return}if(t==="ordini"){let o=st._acq_ordini,i=_t._acq_ordini||0;if(o&&Date.now()-i<3e5){e.innerHTML=o,z(e),window.aggiornaListaFiltrabili?.();return}en(null,null)}else{let o=st["MATERIALE DA ORDINARE"],i=_t["MATERIALE DA ORDINARE"]||0;if(o&&Date.now()-i<3e5){e.innerHTML=o,z(e),window.aggiornaListaFiltrabili?.();return}let n=pt("_html_MATERIALE DA ORDINARE",3e5);if(n){st["MATERIALE DA ORDINARE"]=n,_t["MATERIALE DA ORDINARE"]=Date.now(),e.innerHTML=n,z(e),window.aggiornaListaFiltrabili?.();return}mt(!1,null,null)}}}async function en(t=null,e=null){if(be)return;be=!0;let o=document.getElementById("contenitore-dati");if(!o){be=!1;return}let i=h?.nome?.toUpperCase().trim()==="ALESSIO",n=i?"":h?.nome||"",a=Xi(),r=!1;try{let s=await D.get(a);if(at!=="ordini"){be=!1;return}s&&s.dati&&(o.innerHTML=s.dati,st._acq_ordini=s.dati,_t._acq_ordini=Date.now(),z(o),window.aggiornaListaFiltrabili?.(),r=!0)}catch{}if(r||(o.innerHTML="<div class='centered-msg'><i class='fas fa-spinner fa-spin'></i> Caricamento ordini...</div>"),!(r&&t!==null))try{let s;if(A.ordiniBundle?(s=A.ordiniBundle,A.ordiniBundle=null,A.ordiniPromise=null):A.ordiniPromise?(s=await A.ordiniPromise,A.ordiniBundle=null,A.ordiniPromise=null):s=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"getOrdiniAcquisti",operatore:n}),...e?{signal:e}:{}})).json(),e?.aborted)return;if(!Array.isArray(s)||s.length===0){o.innerHTML=`<div class='empty-msg'>${i?"Nessun ordine ricevuto.":"Non hai ancora inviato ordini."}</div>`,z(o);return}let c={};s.forEach(f=>{let u=f.id_gruppo||f.data+"_"+f.operatore;c[u]||(c[u]={data:f.data,operatore:f.operatore,items:[]}),c[u].items.push(f)});let l=Object.keys(c).reverse(),d=s.filter(f=>f.stato!=="ORDINATO").length,p=`<div class="ordini-acq-page">
            ...
            <details class="ordine-group ${w?"all-done":""}" ${w?"":"open"}>
                <summary class="ordine-group-summary">
                    <span class="og-left">
                        ...
                        <span class="og-data">${Ds(u.data)}</span>
                ...
                <div class="ordine-items">
                    ${u.items.map(v=>qs(v,i)).join("")}
                </div>
            </details>`}),p+="</div></div>",t!==null&&t!==window._latestNavRequest||at!=="ordini")return;D.set(a,p).catch(()=>{}),st._acq_ordini=p,_t._acq_ordini=Date.now(),o.innerHTML=p,z(o),window.aggiornaListaFiltrabili?.()}catch(s){if(s.name==="AbortError")return;o.innerHTML="<div class='centered-error-bold'>Errore nel caricamento ordini.</div>"}finally{be=!1}}function Ns(t,e){return t?new Promise(o=>{let i=new Image;i.onload=()=>{let n=Math.min(e/i.width,e/i.height,1),a=Math.round(i.width*n),r=Math.round(i.height*n),s=document.createElement("canvas");s.width=a,s.height=r,s.getContext("2d").drawImage(i,0,0,a,r),o(s.toDataURL("image/jpeg",.72))},i.onerror=()=>o(null),i.src=t}):Promise.resolve(null)}function Ds(t){try{let e=new Date(t);if(isNaN(e))return t;let o=i=>String(i).padStart(2,"0");return`${o(e.getDate())}/${o(e.getMonth()+1)}/${e.getFullYear()} ${o(e.getHours())}:${o(e.getMinutes())}`}catch{return t}}function qs(t,e){let o=t.stato==="ORDINATO";return`
    <div class="ordine-item ${o?"is-ordinato":""}" id="oi-${t.id_riga}" data-search="${String(t.articolo).toLowerCase()} ${String(t.fornitore).toLowerCase()}">
        ...
        <span class="oi-stato-badge ${o?"badge-ordinato-sm":"badge-attesa-sm"}">${o?'<i class="fas fa-circle-check"></i> ORDINATO':"IN ATTESA"}</span>
            ...
            <div id="lista-materiali-grid">`,d=window.innerWidth<=768;if(j.forEach((p,f)=>{let u=c[p]||[],m=s(p);l+=`
                ...
                <div class="materiale-card ${zt.card}" data-idx="${w}" data-search="${(v+" "+x+" "+O).toLowerCase().replace(/"/g,"")}">
                    ...
                    <div class="mat-card-opts">
                        <input type="checkbox" class="select-materiale mat-sel-chk" data-id="${T}" onclick="aggiornaConteggioSelezionati()">
                        ...
                        <div id="menu-opzioni-${w}" class="menu-popup-opzioni">
                            <button type="button" class="menu-item-opt" onclick="apriModalModifica('${T}', \`${v}\`, \`${x}\`, \`${O}\`)"><i class="fas fa-edit"></i> Modifica</button>
                            <button type="button" class="menu-item-opt" onclick="duplicaArticolo('${T}', \`${v}\`, \`${x}\`, \`${O}\`)"><i class="fas fa-copy"></i> Duplica</button>
                            <button type="button" class="menu-item-opt" onclick="apriModalSpostaSezione('${T}')"><i class="fas fa-folder-open"></i> Sezione</button>
            ...
            </div>`).join(""),o&&(o.style.display="block")),t.style.display="flex",requestAnimationFrame(()=>requestAnimationFrame(()=>t.classList.add("cart-open")))}function Gs(t){St.splice(t,1),Le(),Po()}function on(){let t=document.getElementById("modal-carrello");t.classList.remove("cart-open"),setTimeout(()=>{t.style.display="none"},300)}function Vs(){Po()}function Le(){let t=St.length,e=document.getElementById("badge-carrello-count"),o=document.getElementById("cart-qty-val");e&&(e.innerText=t,e.style.display=t>0?"flex":"none"),o&&(o.innerText=t)}async function Js(){if(St.length===0){alert("Il carrello \xE8 vuoto!");return}if(!confirm(`Vuoi inviare la lista di ${St.length} articoli all'ufficio acquisti?`))return;M.pauseFor(6e3);let e=document.getElementById("btn-invia-alessio");e&&(e.disabled=!0,e.innerText="Invio in corso...");let o=String(Date.now());try{let i=await Promise.all(St.map(async s=>({...s,foto:await Ns(s.foto,80)}))),n={azione:"inviaOrdineAcquisti",operatore:h?.nome||"Utente",id_gruppo:o,articoli:i},r=await(await fetch(I,{method:"POST",body:JSON.stringify(n)})).json();if(r.status==="success")St=[],Le(),on(),g("\u2705 Ordine inviato ad Alessio!"),delete st._acq_ordini,delete _t._acq_ordini,U("_html__acq_ordini"),at="ordini",window._acquistTabAttivo="ordini",setTimeout(()=>window.cambiaPagina?.("MATERIALE DA ORDINARE",null),800);else throw new Error(r.message)}catch(i){g("Errore invio ordine: "+i.message,"error")}finally{e&&(e.disabled=!1,e.innerText="Invia ad Alessio")}}function Ws(t){let e=`[data-prod="${t.replace(/"/g,'\\"')}"]`,o=document.querySelector(e);if(!o)return;if(o.querySelector("img")){nn(o.querySelector("img").src);return}let i=document.createElement("input");i.type="file",i.accept="image/*",i.onchange=n=>{let a=n.target.files[0];if(!a)return;let r=new FileReader;r.onload=s=>{let c=s.target.result,l=o.getAttribute("data-fornitore")||"";o.innerHTML=`
                <img src="${y(c)}"
                     class="modal-img"
                     onclick="event.stopPropagation(); apriImmagineIntera(this.src)">
                ${l?`<span class="mat-badge-fornitore">${y(l)}</span>`:""}`,o.style.border="";let d=o.closest(".materiale-card");if(d){let p=d.querySelector(".btn-menu-elimina-foto");p&&(p.style.display="")}},r.readAsDataURL(a)},i.click()}function Zs(t){if(confirm("Vuoi rimuovere l'immagine da questo prodotto?")){let e=document.querySelector(`[data-prod="${t}"]`);if(!e)return;let o=e.getAttribute("data-fornitore")||"";e.innerHTML=`
            <i class="fas fa-camera mat-img-icon"></i>
            <span class="mat-img-hint">Scatta foto</span>
            ${o?`<span class="mat-badge-fornitore">${y(o)}</span>`:""}`,e.style.border="";let i=e.closest(".materiale-card");if(i){let n=i.querySelector(".btn-menu-elimina-foto");n&&(n.style.display="none")}}}function nn(t){let e=document.createElement("div");e.style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.9);z-index:200000;display:flex;justify-content:center;align-items:center;cursor:zoom-out;";let o=document.createElement("img");o.src=t,o.className="overlay-img",e.appendChild(o),e.onclick=()=>document.body.removeChild(e),document.body.appendChild(e)}function Qs(t,e){t.preventDefault(),t.stopPropagation(),document.querySelectorAll(".menu-popup-opzioni").forEach(i=>{i.id!==`menu-opzioni-${e}`&&i.classList.remove("open")});let o=document.getElementById(`menu-opzioni-${e}`);o&&o.classList.toggle("open")}function Ks(){document.getElementById("titolo-modal-articolo").innerText="Nuovo Articolo",document.getElementById("edit-id-riga").value="",document.getElementById("edit-nome").value="",document.getElementById("edit-codice").value="",document.getElementById("edit-fornitore").value="";let t=document.getElementById("modal-gestione-articolo");t.style.display="flex",t.offsetHeight,t.classList.add("active"),sn(),setTimeout(()=>{document.getElementById("edit-nome")?.focus()},180)}function Ys(t,e,o,i){let n=document.getElementById("modal-gestione-articolo");document.getElementById("titolo-modal-articolo").innerText=t?"Modifica Articolo":"Nuovo Articolo",document.getElementById("edit-id-riga").value=t||"",document.getElementById("edit-nome").value=e||"",document.getElementById("edit-codice").value=i&&i!=="undefined"?i:"",document.getElementById("edit-fornitore").value=o||"",n.style.display="flex",n.offsetHeight,n.classList.add("active"),sn()}function an(){let t=document.getElementById("modal-gestione-articolo");t.classList.remove("active"),rn(),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300)}function sn(){window.visualViewport&&(rn(),Mt=function(){let t=document.getElementById("modal-gestione-articolo");if(!t||t.style.display==="none")return;let e=window.visualViewport,o=Math.max(0,window.innerHeight-e.offsetTop-e.height),i=t.querySelector(".modal-articoli-box");i&&(i.style.marginBottom=o>0?o+"px":""),t.style.top=e.offsetTop+"px",t.style.height=e.height+"px"},window.visualViewport.addEventListener("resize",Mt),window.visualViewport.addEventListener("scroll",Mt))}function rn(){if(!window.visualViewport||!Mt)return;window.visualViewport.removeEventListener("resize",Mt),window.visualViewport.removeEventListener("scroll",Mt),Mt=null;let t=document.getElementById("modal-gestione-articolo");if(!t)return;t.style.top="",t.style.height="";let e=t.querySelector(".modal-articoli-box");e&&(e.style.marginBottom="")}async function Xs(){let t=document.getElementById("btn-salva-articolo"),e=document.getElementById("edit-nome").value.trim();if(!e){g("Inserisci il nome del prodotto!","warning");return}t.textContent="Salvataggio...",t.disabled=!0,M.pauseFor(8e3);try{let o=await ot({azione:"gestisciMateriale",id_riga:document.getElementById("edit-id-riga").value,nome:e,codice:document.getElementById("edit-codice").value,fornitore:document.getElementById("edit-fornitore").value},1e4,{noDedupe:!0});if(o&&o.status==="auth_error"){window._gestisciAuthError_?.(o.message);return}if(o&&o.status==="forbidden"){g(o.message||"Non hai i permessi per questa operazione.","error");return}o&&o.status==="success"?(an(),mt(!0)):g("Errore durante il salvataggio.","error")}catch(o){g(o?.name==="TimeoutError"?"Timeout: il server non risponde. Riprova.":"Errore di rete. Riprova.","error")}finally{t.textContent="Salva",t.disabled=!1}}async function tr(t,e,o,i){et("Duplica Articolo",`Duplicare l'articolo: "${e}"?`,async()=>{M.pauseFor(6e3);let n=document.querySelector(`[data-id="${t}"]`).closest(".materiale-card"),a=Date.now(),r=`qty-item-temp-${a}`,s=document.createElement("div");s.innerHTML=`
            ...
            </div>`;let c=s.firstElementChild;c.style.opacity="0",c.style.transform="translateY(-10px)",n.after(c),requestAnimationFrame(()=>{c.style.transition="opacity 0.3s, transform 0.3s",c.style.opacity="1",c.style.transform="translateY(0)"});try{let l=await ot({azione:"duplicaMateriale",id_riga:t,nome:e,codice:i,fornitore:o},1e4,{noDedupe:!0});if(l&&l.status==="auth_error"){window._gestisciAuthError_?.(l.message);return}if(l&&l.status==="forbidden"){c.remove(),g(l.message||"Non hai i permessi.","error");return}l&&l.status==="success"?mt(!0):(c.style.border="1px solid red",g("Errore di sincronizzazione.","error"))}catch(l){c.style.border="1px solid red",g(l?.name==="TimeoutError"?"Timeout: il server non risponde.":"Errore di rete.","error")}},"Duplica")}function er(t){let e=document.getElementById(t);if(!e)return;let o=e.style.display!=="none";e.style.display=o?"none":"";let n=e.closest(".sezione-materiali-wrapper")?.querySelector(".sezione-arrow");n&&(n.style.transform=o?"rotate(-90deg)":"")}function or(t){document.querySelectorAll(".menu-popup-opzioni.open").forEach(i=>i.classList.remove("open"));let e=document.getElementById("sposta-sezione-select");e.innerHTML=j.map(i=>`<option value="${y(i)}">${y(i)}</option>`).join(""),document.getElementById("sposta-id-riga").value=t;let o=document.getElementById("modal-sposta-sezione");o.style.display="flex",o.offsetHeight,o.classList.add("active")}function cn(){let t=document.getElementById("modal-sposta-sezione");t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300)}async function ir(){let t=document.getElementById("sposta-id-riga").value,e=document.getElementById("sposta-sezione-select").value;cn(),M.pauseFor(6e3);try{await fetch(I,{method:"POST",body:JSON.stringify({azione:"spostaSezione",id_riga:t,sezione:e})}),delete st["MATERIALE DA ORDINARE"],U("_html_MATERIALE DA ORDINARE"),mt(!1)}catch{g("Errore durante lo spostamento.","error")}}function nr(t){document.getElementById("rinomina-sezione-nome").value=t,document.getElementById("rinomina-sezione-vecchio").value=t;let e=document.getElementById("modal-rinomina-sezione");e.style.display="flex",e.offsetHeight,e.classList.add("active"),setTimeout(()=>{let o=document.getElementById("rinomina-sezione-nome");o&&(o.focus(),o.select())},100)}function ko(){let t=document.getElementById("modal-rinomina-sezione");t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300)}async function ar(){let t=document.getElementById("rinomina-sezione-nome").value.trim(),e=document.getElementById("rinomina-sezione-vecchio").value;if(!t||t===e){ko();return}if(j.includes(t)){g("Esiste gi\xE0 una sezione con questo nome.","error");return}ko(),M.pauseFor(6e3),j=j.map(o=>o===e?t:o),localStorage.setItem("sezioniMateriali",JSON.stringify(j));try{await fetch(I,{method:"POST",body:JSON.stringify({azione:"rinominaSezione",vecchioNome:e,nuovoNome:t})}),delete st["MATERIALE DA ORDINARE"],U("_html_MATERIALE DA ORDINARE"),mt(!1),g(`Sezione rinominata in "${t}"`,"success")}catch{g("Errore durante il salvataggio.","error")}}function sr(){document.getElementById("nuova-sezione-nome").value="";let t=document.getElementById("modal-nuova-sezione");t.style.display="flex",t.offsetHeight,t.classList.add("active"),setTimeout(()=>document.getElementById("nuova-sezione-nome")?.focus(),100)}function ln(){let t=document.getElementById("modal-nuova-sezione");t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300)}function rr(){let t=document.getElementById("nuova-sezione-nome").value.trim();t&&(j.includes(t)||(j=[...j,t],localStorage.setItem("sezioniMateriali",JSON.stringify(j)),Fs()),ln(),delete st["MATERIALE DA ORDINARE"],U("_html_MATERIALE DA ORDINARE"),mt(!1))}function cr(){let t=document.getElementById("lista-materiali-grid"),e=document.getElementById("btn-delete-selected"),o=document.getElementById("btn-mode-select");if(!t)return;let i=t.classList.toggle("grid-sel-mode");t.querySelectorAll(".mat-sel-chk").forEach(a=>{a.checked=!1}),e&&e.classList.remove("visible"),o&&(o.innerHTML=i?'<i class="fas fa-times"></i> <span class="btn-txt">Annulla</span>':'<i class="fas fa-tasks"></i> <span class="btn-txt">Seleziona</span>');let n=document.getElementById("count-selected");n&&(n.innerText="0")}function lr(){let t=document.querySelectorAll(".mat-sel-chk:checked").length,e=document.getElementById("btn-delete-selected");document.getElementById("count-selected").innerText=t,t>0?e.classList.add("visible"):e.classList.remove("visible")}async function dr(t){et("Elimina Articolo","Eliminare definitivamente questo articolo dal catalogo?",async()=>{M.pauseFor(6e3);let e=document.querySelector(`[data-id="${t}"]`).closest(".materiale-card");e.style.transition="all 0.3s ease",e.style.transform="scale(0.8)",e.style.opacity="0",setTimeout(()=>e.style.display="none",300);try{let o=await ot({azione:"eliminaMateriale",id_riga:t},1e4,{noDedupe:!0});if(o&&o.status==="auth_error"){window._gestisciAuthError_?.(o.message);return}if(o&&o.status==="forbidden"){e.style.display="flex",e.style.opacity="1",e.style.transform="",g(o.message||"Non hai i permessi.","error");return}if(o&&o.status!=="success")throw new Error;mt(!0)}catch(o){e.style.display="flex",e.style.opacity="1",e.style.transform="",g(o?.name==="TimeoutError"?"Timeout: il server non risponde.":"Errore durante l'eliminazione.","error")}},"Elimina")}async function ur(){let t=document.querySelectorAll(".mat-sel-chk:checked"),e=Array.from(t).map(o=>o.getAttribute("data-id")).filter(o=>o&&o!=="temp"&&o!=="null");if(e.length===0){alert("Nessun articolo valido selezionato. Attendi il salvataggio dei nuovi duplicati prima di eliminarli.");return}if(confirm(`Sei sicuro di voler eliminare ${e.length} articoli?`))try{t.forEach(i=>{let n=i.closest(".materiale-card");n&&(n.style.opacity="0.3",n.style.pointerEvents="none")});let o=await ot({azione:"eliminaMateriale",id_riga:e},1e4,{noDedupe:!0});if(o&&o.status==="auth_error"){window._gestisciAuthError_?.(o.message);return}if(o&&o.status==="forbidden"){g(o.message||"Non hai i permessi.","error"),mt(!0);return}if(o&&o.status==="success"){g("Articoli eliminati con successo");let i=document.getElementById("btn-delete-selected");i&&i.classList.remove("visible"),mt(!1)}else throw new Error(o&&o.message)}catch(o){alert("Errore durante l'eliminazione multipla: "+o.message),mt(!0)}}async function Wt(t=null,e=null,o=null,i=!1){if(t!==null&&(at=t,window._acquistTabAttivo=t),Lo(),at==="fornitori"){await window.caricaOrdiniFornitori?.(e,o,i);return}at==="ordini"?await en(e,o):await mt(i,e,o)}function dn(){window.caricaAcquisti=Wt,window._switchAcquistiTab=Ms,window._aggiornaTabAcquisti=Lo,window._toggleOrdinato=Bs,window.aggiornaBadgeCarrello=Le,window.apriModalCarrello=Vs,window.chiudiModalCarrello=on,window.rimuoviDalCarrello=Gs,window.toggleMostraCarrello=Po,window.inviaOrdineAcquisti=Js,window.cambiaQty=Hs,window.aggiungiAlCarrello=js,window.scattaFoto=Ws,window.resetFoto=Zs,window.apriImmagineIntera=nn,window.toggleMenuOpzioni=Qs,window.apriModalNuovo=Ks,window.apriModalModifica=Ys,window.chiudiModalArticolo=an,window.salvaArticolo=Xs,window.duplicaArticolo=tr,window.toggleSezione=er,window.apriModalSpostaSezione=or,window.chiudiModalSpostaSezione=cn,window.confermaSpostaSezione=ir,window.apriModalRinominaSezione=nr,window.chiudiModalRinominaSezione=ko,window.confermaRinominaSezione=ar,window.apriModalNuovaSezione=sr,window.chiudiModalNuovaSezione=ln,window.confermaNuovaSezione=rr,window.toggleSelezioneMultipla=cr,window.aggiornaConteggioSelezionati=lr,window.eliminaArticolo=dr,window.eliminaSelezionati=ur,window.fetchJson=tn}var St,at,j,Mt,st,_t,be,To,zt,un=V(()=>{lt();ut();dt();Jt();ve();wt();Vt();Tt();St=[],at="catalogo";window._acquistTabAttivo="catalogo";j=JSON.parse(localStorage.getItem("sezioniMateriali")||'["Strumenti","Bombolette","Rifiuti"]'),Mt=null,st={},_t={},be=!1,To=!1;zt={card:"bg-white/90 border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow",cardGrid:"grid gap-3",label:"text-[10px] uppercase tracking-wide text-slate-500 font-semibold",value:"text-slate-900 font-semibold",btn:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-slate-200 bg-white hover:bg-slate-50 active:scale-[0.99] transition",btnPrimary:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 active:scale-[0.99] transition",btnSuccess:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 active:scale-[0.99] transition",btnWarning:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-orange-200 bg-orange-50 text-orange-700 hover:bg-orange-100 active:scale-[0.99] transition",btnDanger:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-red-200 bg-red-50 text-red-600 hover:bg-red-100 active:scale-[0.99] transition",btnPrimaryLg:"inline-flex items-center gap-2 rounded-xl px-10 py-3.5 text-sm font-bold bg-slate-900 text-white hover:bg-slate-700 active:scale-[0.98] transition shadow-sm",pill:"inline-flex items-center justify-center rounded-full px-2 py-1 text-[10px] font-bold bg-slate-100 text-slate-600"};document.addEventListener("click",()=>{document.querySelectorAll(".menu-popup-opzioni.open").forEach(t=>t.classList.remove("open"))})});function mn(){return window.listaStatiFornitori&&window.listaStatiFornitori.length?window.listaStatiFornitori:pn}function pr(t){let o=mn().find(i=>(i.stato||i.nome||"").toUpperCase()===(t||"").toUpperCase());return o&&o.colore||"#94a3b8"}async function fn(t){let e=await ot({azione:"getListaDiCarico"},1e4,{signal:t,retries:1});if(!e||e.status!=="ok")throw new Error(e?.msg||"Errore caricamento");return e.righe||[]}function gn(t){if(!t||t.length===0)return`<div class="centered-msg" style="padding:40px 20px;text-align:center;color:#64748b">
        ...
        <div>
        ...
        </div>`),a.forEach(u=>{let m=e[u],b=m[0].fornitore||"-",w=m[0].data_consegna||"-",v=m.reduce((_,$)=>_+$.quantita,0),x=m.reduce((_,$)=>_+$.qta_evasa,0),O=v>0?Math.round(x/v*100):0,E=O===100?"#22c55e":O>0?"#f59e0b":"#e2e8f0",T=u.length>14?u.substring(0,14)+"\u2026":u,k=m.some(_=>_.review_missing),R=y(u),C=m[0].stato||pn[0].stato,q=pr(C),G=mn().map(_=>{let $=_.stato||_.nome||"",P=_.colore||"#94a3b8",N=$.toUpperCase()===C.toUpperCase(),B=N?" is-selected":"",nt=N?'<i class="fas fa-check stato-check-icon"></i>':"";return`<button type="button" class="stato-option${B}" onclick="event.stopPropagation(); _selezionaStatoOF(this,'${R}','${y($)}','${P}')"><span class="stato-opt-dot" style="background:${P}"></span><span>${y($)}</span>${nt}</button>`}).join(""),ct=k?" of-ordine-missing":"",tt=k?'<span class="of-badge-missing"><i class="fas fa-exclamation-triangle"></i> Da revisionare</span>':"",L=`<button class="of-btn-archivia${k?"":" of-btn-archivia-quieta"}" onclick="event.stopPropagation(); _archiviaOrdineOF('${y(u)}')" title="Archivia ordine"><i class="fas fa-archive"></i>${k?" Archivia":""}</button>`;f+=`<div class="ordine-wrapper of-ordine-wrapper${ct}" data-nordine="${R}">
            <div class="riga-ordine of-riga-ordine" onclick="toggleAccordion(this)">
                ...
                <div class="order-info">
                    <div class="badge-count"><span class="badge-ord-num">${y(T)}</span><span class="badge-sep">\xB7</span>${m.length} ART.</div>
        ...
        </div>`}),f}function mr(t){let e=y(t.codice||"-"),o=y(br(t.prodotto,60)),i=y(t.prodotto||"-"),n=y(t.fornitore||"-"),a=y(t.n_ordine||"-"),r=y(t.data_consegna||"-"),s=t.quantita||0,c=t.qta_evasa||0,l=t.qta_da_consegnare||0,d=yr(t.importo),p=h?.nome?.toUpperCase().trim()==="ALESSIO",f=s>0?Math.round(c/s*100):0,u=f===100?"#22c55e":f>0?"#f59e0b":"#94a3b8",m=`data-codice="${e}" data-prodotto="${i}" data-fornitore="${n}" data-ordine="${a}" data-data="${r}" data-qty="${s}" data-evasa="${c}" data-daconsegnare="${l}"${p?` data-importo="${d}"`:""}`;return`<div class="item-card of-item-card${p?"":" of-item-card-no-importo"}" onclick="_apriDettaglioOF(this)" ${m}>
    ...
    </div>`}function fr(t){let e=document.getElementById("modal-of-dettaglio");e&&e.remove();let o=t.dataset,i=document.createElement("div");i.id="modal-of-dettaglio",i.className="modal-overlay active",i.onclick=n=>{n.target===i&&vn()},i.innerHTML=`<div class="modal-content of-modal-content">
    ...
    </div>`,document.body.appendChild(i),requestAnimationFrame(()=>i.classList.add("active"))}function vn(){let t=document.getElementById("modal-of-dettaglio");t&&(t.classList.remove("active"),setTimeout(()=>t.remove(),200))}function gr(t){let e=t.closest(".stato-dropdown"),o=e.classList.contains("open");if(document.querySelectorAll(".of-ordine-wrapper .stato-dropdown.open").forEach(i=>{i.classList.remove("open");let n=i.closest(".riga-ordine");n&&n.classList.remove("stato-aperto-ord")}),!o){e.classList.add("open");let i=e.closest(".riga-ordine");i&&i.classList.add("stato-aperto-ord")}}function vr(t,e,o,i){let n=t.closest(".stato-dropdown"),a=n.querySelector(".stato-dot"),r=n.querySelector(".stato-label-txt");a&&(a.style.background=i),r&&(r.textContent=o),n.querySelectorAll(".stato-option").forEach(l=>{l.classList.remove("is-selected");let d=l.querySelector(".stato-check-icon");d&&d.remove()}),t.classList.add("is-selected");let s=document.createElement("i");s.className="fas fa-check stato-check-icon",t.appendChild(s),n.classList.remove("open");let c=n.closest(".riga-ordine");c&&c.classList.remove("stato-aperto-ord"),hn(e,o)}async function hn(t,e){try{let o=await ot({azione:"setStatoOrdineFornitori",n_ordine:t,stato:e},8e3,{retries:1});if(!o||o.status!=="ok"){g("Errore salvataggio stato","error");return}zo()}catch{g("Errore connessione","error")}}async function hr(t){if(!confirm(`Archiviare l'ordine ${t}?
Non comparir\xE0 pi\xF9 nella lista principale.`))return;let e=document.querySelector(`.of-ordine-wrapper[data-nordine="${t}"]`);e&&(e.style.opacity="0.4",e.style.pointerEvents="none");try{let o=await ot({azione:"archiviaOrdineFornitori",n_ordine:t},8e3,{retries:1});if(!o||o.status!=="ok"){g("Errore archiviazione","error"),e&&(e.style.opacity="",e.style.pointerEvents="");return}e&&e.remove();let i=document.querySelector(".acquisti-subtitle");if(i){let n=document.querySelectorAll(".of-ordine-wrapper"),a=document.querySelectorAll(".of-ordine-missing").length,r=a>0?` \xB7 <span style="color:#d97706;font-weight:600">\u26A0 ${a} da revisionare</span>`:"";i.innerHTML=`${n.length} ordini${r}`}if(!document.querySelector(".of-ordine-missing")){let n=document.querySelector(".of-review-banner");n&&n.remove()}zo()}catch{g("Errore connessione","error"),e&&(e.style.opacity="",e.style.pointerEvents="")}}function br(t,e){return t?t.length>e?t.substring(0,e)+"\u2026":t:"-"}function yr(t){return!t&&t!==0?"-":Number(t).toLocaleString("it-IT",{minimumFractionDigits:2,maximumFractionDigits:2})+" \u20AC"}async function wr(t,e,o=!1){let i=document.getElementById("contenitore-dati");if(i){if(!o){let n=Zt[X],a=Qt[X]||0;if(n&&Date.now()-a<3e5){i.innerHTML=n,z(i),window.aggiornaListaFiltrabili?.();return}try{let s=await D.get(X);if(s&&s.dati){i.innerHTML=s.dati,Zt[X]=s.dati,Qt[X]=s.timestamp,z(i),window.aggiornaListaFiltrabili?.(),s.isStale&&Sr(e);return}}catch{}let r=pt(Pe,3e5);if(r){i.innerHTML=r,Zt[X]=r,Qt[X]=Date.now(),z(i),window.aggiornaListaFiltrabili?.();return}i.innerHTML=`<div class="centered-msg" id="_of-loader">
            <i class="fas fa-spinner fa-spin"></i> Caricamento ordini fornitori\u2026
        </div>`}try{let n=await fn(e);if(window._acquistTabAttivo!=="fornitori")return;let a=gn(n);i.innerHTML=a,z(i),window.aggiornaListaFiltrabili?.(),Zt[X]=a,Qt[X]=Date.now(),J(Pe,a),D.set(X,a).catch(()=>{})}catch(n){if(n&&n.name==="AbortError")return;if(!o){let a=document.getElementById("contenitore-dati");a&&a.querySelector("#_of-loader")&&(a.innerHTML=`<div class="centered-error-bold">Errore nel caricamento.
                    <button onclick="_switchAcquistiTab('fornitori')"
                        ...
                        Riprova</button></div>`,z(a))}}}}async function Sr(t){try{let e=await fn(t);if(window._acquistTabAttivo!=="fornitori")return;let o=gn(e),i=document.getElementById("contenitore-dati");i&&(i.innerHTML=o,window.aggiornaListaFiltrabili?.()),Zt[X]=o,Qt[X]=Date.now(),J(Pe,o),D.set(X,o).catch(()=>{})}catch{}}function zo(){delete Zt[X],delete Qt[X],D.invalidate(X).catch(()=>{});try{localStorage.removeItem(Pe)}catch{}}function bn(){window._apriDettaglioOF=fr,window._chiudiDettaglioOF=vn,window._setStatoOF=hn,window._selezionaStatoOF=vr,window._toggleStatoDropdownOF=gr,window._archiviaOrdineOF=hr,window.caricaOrdiniFornitori=wr,window.invalidateOFCache=zo,window._ofDropdownListenerAdded||(document.addEventListener("click",function(t){t.target.closest(".of-ordine-wrapper .stato-dropdown")||document.querySelectorAll(".of-ordine-wrapper .stato-dropdown.open").forEach(e=>{e.classList.remove("open");let o=e.closest(".riga-ordine");o&&o.classList.remove("stato-aperto-ord")})},!0),window._ofDropdownListenerAdded=!0)}var Zt,Qt,X,Pe,pn,yn=V(()=>{lt();ve();dt();wt();Tt();ut();Zt={},Qt={},X="ORDINI_FORNITORI",Pe="_html_ORDINI_FORNITORI",pn=[{stato:"IN ATTESA",colore:"#94a3b8"},{stato:"ORDINATO",colore:"#fbbf24"},{stato:"PARZ. EVASO",colore:"#f97316"},{stato:"EVASO",colore:"#22c55e"},{stato:"ANNULLATO",colore:"#ef4444"}]});function ye(){window.cacheContenuti&&delete window.cacheContenuti.STORICO_RICHIESTE,window.cacheFetchTime&&delete window.cacheFetchTime.STORICO_RICHIESTE,U("_html_STORICO_RICHIESTE"),A.rqBundle=null,A.rqPromise=null,D.invalidate("STORICO_RICHIESTE").catch(()=>{})}function Sn(){let t=document.getElementById("contenitore-dati");t&&(window.cacheContenuti&&(window.cacheContenuti.STORICO_RICHIESTE=t.innerHTML),window.cacheFetchTime&&(window.cacheFetchTime.STORICO_RICHIESTE=Date.now()),J("_html_STORICO_RICHIESTE",t.innerHTML))}function _r(t){let e=[`.req-card[data-id-riga="${CSS.escape(String(t))}"]`,`.scad-card[data-id-riga="${CSS.escape(String(t))}"]`,`#box-conferma-${CSS.escape(String(t))}`,`#box-risposta-${CSS.escape(String(t))}`,`#rc-body-${CSS.escape(String(t))}`];for(let o of e){let i=document.querySelector(o),n=i?.classList?.contains("req-card")||i?.classList?.contains("scad-card")?i:i?.closest(".req-card, .scad-card");if(n){let a=n.closest(".req-group");if(a){let r=a.querySelector(".rg-count");if(r){let s=parseInt(r.textContent,10)||0;s<=1?r.remove():r.textContent=s-1}}return n.remove(),!0}}return!1}function Er(){if(!h||!h.nome)return!1;let t=h.nome.toUpperCase();return t==="ALESSIO"||t==="0000"||h.ruolo==="MASTER"}function De(t){let e=document.getElementById("badge-richieste-count"),o=document.getElementById("nome-utente-sidebar"),i=document.getElementById("img-avatar-sidebar");if(!e)return;let n=(h.vistaSimulata||"MASTER").toUpperCase().trim();if(o&&(o.innerText=n),i&&(i.src=`https://ui-avatars.com/api/?name=${n}&background=2563eb&color=fff`),window.paginaAttuale==="STORICO_RICHIESTE"){e.style.display="none",e.classList.remove("badge-sollecito-attivo");return}let a=t.filter(d=>{let p=String(d.RISOLTO).toLowerCase()!=="true";if(n==="MASTER")return p;var f=String(d.A||"").split(",").some(function(u){return u.trim().toUpperCase()===n});return f&&p}),r=a.length,s=a.filter(d=>String(d.SOLLECITO).toLowerCase()==="true").length;r>0?(e.innerText=r,e.style.display="inline-block",s>0?e.classList.add("badge-sollecito-attivo"):e.classList.remove("badge-sollecito-attivo")):(e.style.display="none",e.classList.remove("badge-sollecito-attivo"));let c=document.getElementById("badge-mobile-notif");c&&(r>0&&window.paginaAttuale!=="STORICO_RICHIESTE"?(c.innerText=r,c.style.display="inline-block",c.style.background=s>0?"#f97316":"#ef4444"):c.style.display="none");let l=document.getElementById("badge-bottom-richieste");l&&(r>0&&window.paginaAttuale!=="STORICO_RICHIESTE"?(l.innerText=r,l.style.display="inline-block",s>0?l.classList.add("badge-sollecito-attivo"):l.classList.remove("badge-sollecito-attivo")):(l.style.display="none",l.classList.remove("badge-sollecito-attivo")))}function xr(t,e,o,i){let n=document.getElementById("modalSollecito");n.style.display="flex",n.offsetHeight,n.classList.add("active"),document.getElementById("sollecito-titolo").textContent=i&&i!=="Intero Ordine"?`Sollecita \u2013 ${i}`:`Sollecita \u2013 Ord. ${e}`,document.getElementById("sollecito-id-riga").value=t||"",document.getElementById("sollecito-nord").value=e,document.getElementById("sollecito-cliente").value=o||"",document.getElementById("sollecito-rif").value=i||"",document.getElementById("sollecito-data").value="",document.getElementById("sollecito-note").value=""}function _n(){let t=document.getElementById("modalSollecito");t.style.display="",t.classList.remove("active")}async function Ir(){let t=document.getElementById("sollecito-nord").value,e=document.getElementById("sollecito-id-riga").value,o=document.getElementById("sollecito-cliente").value,i=document.getElementById("sollecito-rif").value,n=document.getElementById("sollecito-data").value,a=document.getElementById("sollecito-note").value.trim();if(!n){g("Seleziona una data di scadenza.","error");return}_n(),M.pauseFor(6e3),ye();let r={azione:"supporto_multiplo",n_ordine:t,cliente:o,prodotto:i&&i!=="Intero Ordine"?i:"",tipo:"SCADENZA",messaggio:`SCAD:${n}|${a||"\u2013"}`,mittente:h.nome.toUpperCase().trim(),destinatari:["ALESSIO"]};try{await fetch(I,{method:"POST",body:JSON.stringify(r)}),g("\u2705 Scadenza aggiunta"),window.paginaAttuale==="STORICO_RICHIESTE"&&Et()}catch{g("\u2705 Scadenza aggiunta")}}function Ar(t,e,o,i){let n=document.getElementById("modalAiuto");if(n.style.display==="flex")return;n._openedAt=Date.now(),n.style.display="flex",n.offsetHeight,n.classList.add("active"),document.getElementById("modal-titolo").innerText=t?`Messaggio Art. ${e}`:`Messaggio Ordine ${o}`;let a=window.listaOperatori||[];document.getElementById("wrapper-operatori").innerHTML=a.map(s=>`
    ...
    `).join(""),n.dataset.idRiga=t||"",n.dataset.nOrdine=o,n.dataset.cliente=i||"";let r=document.getElementById("modal-ordine-row");r&&(r.style.display="none"),document.getElementById("messaggio-aiuto").value="",Do("DOMANDA")}function En(t={}){let e=document.getElementById("modalAiuto");if(e.style.display==="flex")return;e._openedAt=Date.now(),e.style.display="flex",e.offsetHeight,e.classList.add("active"),document.getElementById("modal-titolo").innerText="Nuova Richiesta";let o=window.listaOperatori||[];document.getElementById("wrapper-operatori").innerHTML=o.map(n=>`
        <label class="op-label">
...
<body>
    ...
    <div class="stage">
        <div class="page">
            ...
            <div class="meta">
                <div class="meta-card"><div class="meta-k">Data emissione</div><div class="meta-v">${K(e.toLocaleString("it-IT"))}</div></div>
                <div class="meta-card"><div class="meta-k">Ordini selezionati</div><div class="meta-v">${K(String(o.length))}</div></div>
                <div class="meta-card"><div class="meta-k">Totale quantit\xE0</div><div class="meta-v">${K(ht(n))} pz</div></div>
            </div>
            <div class="orders"><strong>Fabbisogno per ordini:</strong> ${K(i)}</div>
            <table>
                <thead>
                    <tr>
            ...
            <div class="footer">Generato da PROD - ${K(String(h?.nome||"Sistema"))}</div>
        </div>
    </div>
    <script>
...
' + document.documentElement.outerHTML;
    ...
    <\/script>
...
</html>`}function Dr(){let t=On();if(!t.length){g("Nessuna riga utile da stampare per il fabbisogno corrente.","warning");return}let e=window.open("","_blank");if(!e){g("Popup bloccato: abilita l'anteprima di stampa.","warning");return}e.document.open(),e.document.write(Nr(t)),e.document.close(),e.focus()}async function qr(){let t=On();if(!t.length){g("Nessuna riga utile da salvare nel fabbisogno.","warning");return}let e=[...W].sort((i,n)=>i.localeCompare(n,"it",{numeric:!0,sensitivity:"base"})),o={id:Kt||void 0,ordini:e,rows:t,meta:zr(t)};try{let i=await yt({azione:"saveFabbisogno",payload:o});if(i?.status!=="ok")throw new Error(i?.message||"saveFabbisogno failed");Kt=String(i.id||Kt||""),g("Fabbisogno salvato su Sheets.","success")}catch(i){console.error("[richieste] saveFabbisogno error:",i),g("Errore salvataggio fabbisogno.","error")}}function ze(t){return t.length?t.map(function(e){let o=Array.isArray(e.ordini)?e.ordini:[],i=o.length?o.slice(0,4).join(", ")+(o.length>4?" ...":""):"Ordini non specificati",n=e.updatedAt?new Date(e.updatedAt).toLocaleString("it-IT"):"-",a=Number(e.rowsCount||0)||0,s=String(e.status||"ACTIVE").toUpperCase()==="ARCHIVED"?"Archiviato":"Attivo",c=K(e.id||"");return`
            <div class="fabprod-sel-item fabprod-sel-item--checked" style="display:block;cursor:default">
                <div class="fabprod-sel-item-info" style="gap:4px">
                    ...
                    <span class="fabprod-sel-cli">Stato: ${K(s)} \xB7 Aggiornato: ${K(n)} \xB7 Righe: ${a}</span>
                    <span class="fabprod-sel-cli">Ordini: ${K(i)}</span>
                    <span class="fabprod-sel-cli">Autore: ${K(e.updatedBy||e.createdBy||"-")}</span>
                </div>
            ...
            </div>`}).join(""):'<div class="empty-msg" style="margin:16px 0">Nessun fabbisogno disponibile.</div>'}async function Br(){document.getElementById("fabprod-archivio-modal")?.remove();let t=document.createElement("div");t.id="fabprod-archivio-modal",t.className="fabprod-sel-modal-overlay",t.innerHTML=`
        <div class="fabprod-sel-modal-box">
            ...
            ${ze(a)}
        `}catch(o){console.error("[richieste] getFabbisogni error:",o),e&&(e.innerHTML='<div class="empty-msg" style="margin:16px 0">Errore caricamento fabbisogni.</div>'),g("Errore caricamento fabbisogni.","error")}}function Uo(){document.getElementById("fabprod-archivio-modal")?.remove()}async function Ur(t){let e=String(t||"").trim();if(e)try{let o=await yt({azione:"getFabbisognoById",id:e});if(o?.status!=="ok"||!o?.item)throw new Error(o?.message||"Snapshot non trovato");let i=o.item;Kt=String(i.id||e),Xt=Array.isArray(i.rows)?i.rows.map(function(n){return{codice:String(n?.codice||"").trim(),prodotto:String(n?.prodotto||"").trim(),qty:Number(n?.qtyTotale??n?.qty??0)||0,formulaQty:String(n?.formulaQty||""),ordini:Array.isArray(n?.ordini)?n.ordini:[]}}):[],W=new Set(Array.isArray(i.ordini)?i.ordini.map(function(n){return String(n||"").trim()}).filter(Boolean):[]),Uo(),Dt(),Ho(),g("Snapshot fabbisogno caricato.","success")}catch(o){console.error("[richieste] getFabbisognoById error:",o),g("Errore apertura snapshot fabbisogno.","error")}}async function Fr(t){let e=String(t||"").trim();if(!(!e||!await et("Archiviare questo fabbisogno condiviso?",{title:"Archivia snapshot",confirmText:"Archivia",cancelText:"Annulla",confirmType:"danger"})))try{let i=await yt({azione:"archiveFabbisogno",id:e});if(i?.status!=="ok")throw new Error(i?.message||"Archiviazione non riuscita");Yt=Yt.filter(function(a){return String(a.id||"")!==e});let n=document.getElementById("fabprod-archivio-body");n&&(n.innerHTML=ze(Yt)),Kt===e&&Ue(),Dt(),g("Snapshot archiviato.","success")}catch(i){console.error("[richieste] archiveFabbisogno error:",i),g("Errore archiviazione snapshot.","error")}}function Dt(){let t=document.getElementById("fabprod-list"),e=document.getElementById("fabprod-sel-badge"),o=document.getElementById("fabprod-cnt-badge");if(!t)return;let i=Array.isArray(Xt)?Xt:$n(Ne.filter(s=>!s||!s.ordine?!1:W.has(String(s.ordine).trim())));i.length?(window._fabprodCurrentRows=i,t.innerHTML=i.map((s,c)=>{let l=s.ordini.map(p=>{let f=p.ordine.replace(/\\/g,"\\\\").replace(/'/g,"\\'"),u=(p.cliente||"").replace(/\\/g,"\\\\").replace(/'/g,"\\'");return`<span class="fabprod-order-pill fabprod-order-pill--click" onclick="event.stopPropagation();_fabprodApriModalOrdine('${f}','${u}')">ORD. ${p.ordine}${p.cliente?`<span class="fabprod-pill-cliente"> \xB7 ${p.cliente}</span>`:""}</span>`}).join(""),d=s.ordini.length>1?`<div class="fabprod-qty-breakdown">${s.ordini.map(p=>`${ht(p.qty)}\xA0(${p.ordine})`).join(" + ")}</div>`:"";return`<div class="fabprod-card" onclick="_fabprodCardClick(${c})">
            ...
            </div>`}).join("")):t.innerHTML=W.size===0?'<div class="fabprod-empty-sel"><i class="fas fa-hand-pointer fabprod-empty-sel-icon"></i><div class="fabprod-empty-sel-text">Seleziona gli ordini per vedere il fabbisogno</div></div>':'<div class="empty-msg" style="margin:16px 0 8px">Nessun articolo attivo per gli ordini selezionati.</div>';let n=W.size;e&&(e.textContent=n,e.style.display=n>0?"":"none"),o&&(o.textContent=i.length,o.style.display=i.length>0?"":"none");let a=document.getElementById("fabprod-print-btn");a&&(a.disabled=i.length===0);let r=document.getElementById("fabprod-save-btn");r&&(r.disabled=i.length===0)}function Hr(){document.getElementById("fabprod-sel-modal")?.remove();let t=n=>{let a=(n||"").trim().toLowerCase(),r=a?Nt.filter(s=>s.ordine.toLowerCase().includes(a)||(s.cliente||"").toLowerCase().includes(a)):Nt;return!r.length&&!Nt.length?'<div class="empty-msg" style="margin:16px 0">Nessun ordine attivo disponibile.</div>':r.length?r.map(s=>{let c=W.has(s.ordine)?" checked":"",l=s.ordine.replace(/"/g,"&quot;");return`<label class="fabprod-sel-item${c?" fabprod-sel-item--checked":""}">
        ...
        <div class="fabprod-sel-modal-box">
            ...
            <button onclick="cambiaPagina('STORICO_RICHIESTE', null)"
                ...
                &#x21bb; Riprova
            </button>`)},12e3);try{let n=await te(e);if(clearTimeout(i),De(n.attive),t!==null&&t!==window._latestNavRequest)return;window.aggiornaBadgeNotifiche?.(n.attive),we(n)}catch(n){if(clearTimeout(i),n.name==="AbortError")return;console.error("Errore caricamento richieste:",n),o.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('STORICO_RICHIESTE',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(o)}}function we(t){if(window.paginaAttuale!=="STORICO_RICHIESTE")return;let e=document.getElementById("contenitore-dati");if(!e)return;let o=t.attive||[],i=t.archivio||[],n=t.fabbisogno||[];Ne=t.fabbisognoRaw||[],W=new Set;{let L=new Set;Nt=[];for(let _ of Ne){if(!_||!_.ordine||String(_.archiviato||"").toUpperCase()==="TRUE"||Bo(_.stato))continue;let $=String(_.ordine).trim();!$||L.has($)||(L.add($),Nt.push({ordine:$,cliente:String(_.cliente||"").trim()}))}Nt.sort((_,$)=>_.ordine.localeCompare($.ordine,"it"))}let a=(h?.nome||"").toUpperCase().trim(),r=L=>{let _={};return L.forEach($=>{_[$.ORDINE]||(_[$.ORDINE]=[]),_[$.ORDINE].push($)}),_},s=r(o),c=r(i),l=(()=>{if(Er())return()=>!0;let L=ft(h?.nome||"").toUpperCase();return _=>_.some($=>{if(ft($.DA||"").toUpperCase()===L)return!0;var P=String($.A||"");return P.split(",").some(function(N){return ft(N.trim()).toUpperCase()===L})})})(),d=L=>{let _={};return Object.keys(L).forEach($=>{l(L[$])&&(_[$]=L[$])}),_},p=d(s),f=d(c),u={},m={},b={};Object.keys(p).forEach(L=>{let $=p[L].filter(P=>(P.TIPO||"").toUpperCase()!=="SCADENZA");$.length>0&&(($[0].TIPO||"MSG").toUpperCase()==="ASSEGNAZIONE"?u[L]=$:m[L]=$)}),Object.keys(s).forEach(L=>{let _=s[L].filter($=>($.TIPO||"").toUpperCase()==="SCADENZA");_.length>0&&(b[L+"_scad"]=_)});let w=localStorage.getItem("_rg_assegnazioni")!=="0",v=localStorage.getItem("_rg_richieste")!=="0",x=localStorage.getItem("_rg_scadenze")!=="0",O=localStorage.getItem("_rg_fabbisogno_produzione")!=="0",E=Object.keys(u).length,T=Object.keys(m).length,k=Object.values(b).reduce((L,_)=>L+_.length,0),R=n.length,C=(L,_)=>{let $="";return Object.keys(L).reverse().forEach(P=>{$+=Mo(L[P],_,!1)}),$||'<div class="empty-msg" style="margin:16px 0 8px">Nessun elemento.</div>'},q=()=>{let L=Object.values(b).flat();return L.sort((_,$)=>{let P=No(_),N=No($);return!P&&!N?0:P?N?P-N:-1:1}),L.map(_=>kn(_,a)).join("")||'<div class="empty-msg" style="margin:16px 0 8px">Nessuna scadenza.</div>'},G=L=>!L||!L.length?W.size===0?`<div class="fabprod-empty-sel">
                    ...
                    </div>`:'<div class="empty-msg" style="margin:16px 0 8px">Nessun articolo attivo per gli ordini selezionati.</div>':(window._fabprodCurrentRows=L,L.map((_,$)=>{let P=_.ordini.map(B=>{let nt=B.ordine.replace(/\\/g,"\\\\").replace(/'/g,"\\'"),Lt=(B.cliente||"").replace(/\\/g,"\\\\").replace(/'/g,"\\'");return`<span class="fabprod-order-pill fabprod-order-pill--click" onclick="event.stopPropagation();_fabprodApriModalOrdine('${nt}','${Lt}')">ORD. ${B.ordine}${B.cliente?`<span class="fabprod-pill-cliente"> \xB7 ${B.cliente}</span>`:""}</span>`}).join(""),N=_.ordini.length>1?`<div class="fabprod-qty-breakdown">${_.ordini.map(B=>`${ht(B.qty)}\xA0(${B.ordine})`).join(" + ")}</div>`:"";return`
                <div class="fabprod-card" onclick="_fabprodCardClick(${$})">
                    <div class="fabprod-top">
                    ...
                    ${N}
                    <div class="fabprod-orders">${P}</div>
                </div>`}).join("")),ct="";Object.keys(f).length===0?ct='<div class="empty-msg" style="margin:20px 0">Nessuna richiesta archiviata.</div>':Object.keys(f).reverse().forEach(L=>{ct+=Mo(f[L],a,!0)});let tt=`
                ...
                <details id="rg-richieste" class="req-group" ${v?"open":""}
                    ...
                    <summary class="req-group-summary">
                        <span class="rg-left">
                            ...
                            ${T>0?`<span class="rg-count rg-count-dom">${T}</span>`:""}
                        </span>
                        <i class="fas fa-chevron-down rg-chevron"></i>
                    </summary>
        ...
        </div>`}function Xr(t,e){let o=document.getElementById("rc-body-"+t);if(!o)return;let i=o.classList.toggle("open");e&&e.classList.toggle("open",i)}function No(t){let e=String(t.MESSAGGIO||"").split("|");if(e.length>=2){let o=e[1]||"";if(o.startsWith("SCAD:")){let i=new Date(o.slice(5));if(!isNaN(i))return i}}return null}function kn(t,e){let o=String(t.MESSAGGIO||"").split("|"),i=null,n="\u2013";if(o.length>=2){let f=o[1]||"";f.startsWith("SCAD:")&&(i=new Date(f.slice(5)),isNaN(i)&&(i=null)),n=o.slice(2).join("|").trim()||"\u2013"}let a=t.ORDINE||"\u2013",r=t.CLIENTE||"",s=t.PRODOTTO&&t.PRODOTTO!==""?t.PRODOTTO:"",c=ft(t.DA||""),l=t["DATA ORA"]||"",d="scad-ok",p="\u2013";if(i){let f=Math.ceil((i-new Date)/864e5);p=i.toLocaleDateString("it-IT",{day:"2-digit",month:"short",year:"numeric"}),f<0?d="scad-scaduta":f<=3?d="scad-urgente":f<=7?d="scad-vicina":d="scad-ok"}return`
    <div class="scad-card ${d}" data-id-riga="${String(t.id_riga||"")}" data-ordine="${K(a)}" data-cliente="${r.toLowerCase().replace(/"/g,"")}">
        <div class="scad-top">
        ...
        <div class="rc-actions">
            <label class="scad-fab-pick" onclick="event.stopPropagation()" title="Seleziona questa scadenza per il fabbisogno">
                <input type="checkbox" class="scad-fab-chk" value="${K(a)}">
    ...
    </div>`}function Ln(){window.chiudiModal=qo,window.confermaInvioSupporto=$r,window.setTipoAzione=Do,window.chiudiModalSollecito=_n,window.confermaInvioSollecito=Ir,window.apriNuovaRichiesta=En,window.apriModalAiuto=Ar,window.apriModalSollecito=xr,window.toggleAreaRisposta=xn,window.toggleBoxArchivia=Tr,window.inviaRisposta=Rr,window._selezionaOrdine=Cr,window.aggiornaBadgeRichieste=De,window.aggiornaBadgeSidebar=De,window.caricaRichieste=Et,window._fetchDatiRichieste=te,window._renderDatiRichieste=we,window._saveReqGroup=In,window._fabprodCardClick=Pr,window._fabprodApriModalOrdine=Lr,window._fabprodApriModalArticolo=An,window._fabprodVaiOrdine=kr,window._aggiornaPannelloFabbisogno=Dt,window._apriModalFabbisognoSel=Hr,window._applicaSelFabbisogno=jr,window._chiudiModalFabbisognoSel=Fo,window._fabprodDaScadenzeTutte=Gr,window._fabprodDaScadenzeFlaggate=Vr,window._fabprodStampaFabbisognoSel=Dr,window._fabprodSalvaSnapshotCondiviso=qr,window._fabprodApriArchivioSnapshot=Br,window._fabprodChiudiArchivioSnapshot=Uo,window._fabprodApriSnapshotById=Ur,window._fabprodArchiviaSnapshotById=Fr,window._notificaFabbisognoNuovo=Mr,window.aggiornaRichiesta=Tn,window._sollecitaConferma=Kr,window._archiviaConferma=Yr,window.sollecitaRichiesta=Rn,window.cambiaVistaUtente=Wr,window._toggleRcBody=Xr,window.formattaData=Be,window.generaCardRichiesta=Mo,window.generaCardScadenza=kn,window._getScadDate=No}function Pn(){let t=document.getElementById("btn-nuova-richiesta");t&&t.addEventListener("click",function(o){o.stopPropagation(),En()});let e=document.getElementById("modalAiuto");e&&e.addEventListener("click",function(o){o.target===this&&(Date.now()-(this._openedAt||0)<800||qo())}),document.addEventListener("click",function(o){o.target.closest(".req-card")||document.querySelectorAll(".box-risposta, .box-conferma").forEach(function(i){i.style.display!=="none"&&i.style.display!==""&&(i.style.opacity="0",i.style.transform="translateY(-10px)",setTimeout(function(){i.style.display="none"},300))})})}var Me,W,Ne,Nt,Xt,Kt,Yt,wn,ft,zn=V(()=>{lt();Tt();ut();ve();dt();Jt();Vt();wt();Me=[],W=new Set,Ne=[],Nt=[],Xt=null,Kt="",Yt=[],wn=null,ft=t=>window._normNome?window._normNome(t):t&&String(t).trim()});async function Mn(){return ot({azione:"getManuali"},1e4,{retries:2})}async function Nn(t){return ot({azione:"salvaManualeNuovo",...t},18e4,{noDedupe:!0})}async function Dn(t){return ot({azione:"aggiornaManuale",...t},18e4,{noDedupe:!0})}async function qn(t){return ot({azione:"getStoricoManuale",id:t},1e4,{retries:2})}var Bn=V(()=>{"use strict";ve()});function Ve(t){if(!t)return"-";let e=new Date(t);return Number.isNaN(e.getTime())?String(t):e.toLocaleString("it-IT",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}function Y(t){let e=String(t||"").trim();return e&&(/^data:image\/(png|jpe?g|webp|gif);base64,[a-z0-9+/=\s]+$/i.test(e)||/^https?:\/\/[^\s]+$/i.test(e))?e:""}function Je(t){return t&&t.sections&&t.sections._v===2?t.sections:null}function Ge(t,e,o){if(t){let i=o?`<button type="button" onclick="${o}" title="Rimuovi foto" style="position:absolute;top:-7px;right:-7px;background:#ef4444;border:none;color:#fff;width:22px;height:22px;border-radius:50%;font-size:1.1rem;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 1px 4px rgba(0,0,0,.3)">&times;</button>`:"";return`<div class="foto-wrapper" style="position:relative;display:inline-block;max-width:100%;margin-bottom:6px"><img src="${t}" alt="${e}" style="max-width:100%;max-height:180px;border-radius:10px;border:1px solid #e2e8f0;display:block">${i}</div>`}return'<div class="text-xs text-slate-400" style="margin-bottom:6px">Nessuna foto</div>'}function Se(t){return`
    <input type="file" class="manuale-file-input" accept="image/*" onchange="${t}">
    <label class="manuale-file-label" onclick="this.previousElementSibling.click()"><i class="fas fa-upload"></i> Carica foto</label>`}function ic(t){let e=Je(t),o=e?Array.isArray(e.procedimenti)?e.procedimenti.length:0:Array.isArray(t.steps)?t.steps.length:0,i=Y(t.copertina),n=i?`<img src="${i}" alt="copertina" class="w-full h-40 object-cover rounded-t-xl">`:'<div class="w-full h-40 bg-slate-100 rounded-t-xl flex items-center justify-center text-slate-400 text-3xl"><i class="fas fa-book-open"></i></div>';return`
    ...
    </article>`}function Fn(){let t=document.getElementById("contenitore-dati");if(!t)return;let e=oe.map(ic).join(""),o=h?.nome?.toUpperCase().trim()==="ALESSIO";t.innerHTML=`
    ...
    <div id="manuali-storico-host"></div>`,z(t),window.aggiornaListaFiltrabili?.()}function nc(t,e,o){return`
    ...
    <div class="scheda-row scheda-row-custom" data-row-idx="${o}" style="display:grid;grid-template-columns:1fr 1fr 36px;gap:6px;align-items:center">
        ...
        <input type="text" class="input-field-modern" data-field="valore" placeholder="Valore" value="${y(e||"")}">
        <button type="button" class="${window.TW?.btnDanger||""}" onclick="rimuoviSchedaRow(${o})" title="Rimuovi"><i class="fas fa-trash"></i></button>
    </div>`}function Vo(t,e){let o=Y(t&&t.foto||"");return`
    ...
    </div>`}function Jo(t,e){let o=Y(t&&t.foto||""),i=Y(t&&t.foto2||"");return`
    ...
    </div>`}function ac(t){let e=Y(t||"");return`
    ...
    </div>`}function ie(t){return`<button type="button" class="manuali-modal-close" onclick="${t}" aria-label="Chiudi" title="Chiudi"><i class="fas fa-times"></i></button>`}function Wo(t,e){let o=document.getElementById("manuali-modal-host");if(!o)return;let i=Je(e),n=i?i.schedaTecnica||[]:[],a=i?i.occorrente||[]:[],r=i?i.procedimenti||[]:Array.isArray(e?.steps)?e.steps.map(function(v){return{descrizione:v.descrizione||v.titolo||"",foto:v.foto||""}}):[],s=i&&i.disegnoTecnico?.foto||"",c={},l=[];n.forEach(function(v){Un.includes(v.voce)?c[v.voce]=v.valore:l.push(v)});let d=0,p=Un.map(function(v){return nc(v,c[v]||"",d++)}).join("")+l.map(function(v){return Hn(v.voce,v.valore,d++)}).join(""),f=a.length>0?a.map(function(v,x){return Vo(v,x)}).join(""):Vo(null,0),u=r.length>0?r.map(function(v,x){return Jo(v,x)}).join(""):Jo(null,0),m=Y(e?.copertina||""),b=m?`<img id="manuali-copertina-preview" src="${m}" alt="copertina" style="max-width:100%;max-height:200px;border-radius:10px;border:1px solid #e2e8f0;display:block;margin-bottom:6px">`:'<div id="manuali-copertina-preview" class="text-xs text-slate-400" style="margin-bottom:6px">Nessuna copertina</div>',w=(v,x)=>`<h3 style="font-weight:700;font-size:.8rem;text-transform:uppercase;letter-spacing:.05em;color:#64748b;margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid #e2e8f0"><i class="${v}"></i> &nbsp;${x}</h3>`;o.innerHTML=`
    ...
    </div>`,je=t==="edit"&&e?.id||null}function sc(){let t=[];document.querySelectorAll("#manuali-scheda-edit .scheda-row").forEach(function(a){let r=String(a.getAttribute("data-voce")||a.querySelector('[data-field="voce"]')?.value||"").trim(),s=String(a.querySelector('[data-field="valore"]')?.value||"").trim();(r||s)&&t.push({voce:r,valore:s})});let e=[];document.querySelectorAll("#manuali-occorrente-edit .occorrente-item").forEach(function(a){let r=String(a.querySelector('[data-field="lettera"]')?.value||"").trim(),s=String(a.querySelector('[data-field="nome"]')?.value||"").trim(),c=String(a.querySelector('[data-field="codice"]')?.value||"").trim(),l=String(a.getAttribute("data-foto")||"").trim();(r||s||c||l)&&e.push({lettera:r,nome:s,codice:c,foto:l})});let o=[];document.querySelectorAll("#manuali-proc-edit .proc-step").forEach(function(a){let r=String(a.querySelector('[data-field="descrizione"]')?.value||"").trim(),s=a.querySelector('.proc-foto-slot[data-slot="1"]'),c=a.querySelector('.proc-foto-slot[data-slot="2"]'),l=String(s?.getAttribute("data-foto")||a.getAttribute("data-foto")||"").trim(),d=String(c?.getAttribute("data-foto")||"").trim();(r||l||d)&&o.push({descrizione:r,foto:l,foto2:d})});let i=document.getElementById("manuali-disegno-wrap"),n={foto:String(i?.getAttribute("data-foto")||"").trim()};return{_v:2,schedaTecnica:t,occorrente:e,procedimenti:o,disegnoTecnico:n}}async function jn(t){return new Promise(function(e,o){let i=new FileReader;i.onload=function(){e(String(i.result||""))},i.onerror=o,i.readAsDataURL(t)})}async function Gn(t,e=1200){return new Promise(function(o){let i=new Image;i.onload=function(){let n=Math.min(e/i.width,e/i.height,1),a=document.createElement("canvas");a.width=Math.max(1,Math.round(i.width*n)),a.height=Math.max(1,Math.round(i.height*n));let r=a.getContext("2d");if(!r)return o(t);r.drawImage(i,0,0,a.width,a.height),o(a.toDataURL("image/jpeg",.8))},i.onerror=function(){o(t)},i.src=t})}async function rc(t,e=800){return new Promise(function(o){let i=new Image;i.onload=function(){let n=Math.min(i.width,i.height),a=Math.round((i.width-n)/2),r=Math.round((i.height-n)/2),s=Math.min(n,e),c=document.createElement("canvas");c.width=s,c.height=s;let l=c.getContext("2d");if(!l)return o(t);l.drawImage(i,a,r,n,n,0,0,s,s),o(c.toDataURL("image/jpeg",.8))},i.onerror=function(){o(t)},i.src=t})}async function cc(t){try{let e=t?.files&&t.files[0];if(!e)return;let o=await jn(e),i=await Gn(o,800);if(!i||i.length>Zo){g("Immagine di copertina troppo grande, riduci la risoluzione.","warning");return}let n=document.getElementById("manuali-copertina-wrap");if(!n)return;n.setAttribute("data-copertina",i);let a=n.querySelector("img");if(a)a.src=i;else{let r=n.querySelector("#manuali-copertina-preview");r&&r.remove();let s=document.createElement("img");s.id="manuali-copertina-preview",s.src=i,s.alt="copertina",s.style.cssText="max-width:100%;max-height:200px;border-radius:10px;border:1px solid #e2e8f0;display:block;margin-bottom:6px",n.insertBefore(s,n.firstChild)}}catch{g("Errore nel caricamento immagine di copertina.","error")}}async function Qo(t,e){try{let o=await jn(t),i=await Gn(o,800);if(!i||i.length>Zo){g("Immagine troppo grande, riduci la risoluzione.","warning");return}e(i)}catch{g("Errore nel caricamento immagine.","error")}}function Ko(t,e,o){t.setAttribute("data-foto",e);let i=t.querySelector(".foto-wrapper");if(i){i.querySelector("img").src=e;return}let n=t.querySelector("img");if(n){n.src=e;return}let a=t.querySelector("div.text-xs");a&&a.remove();let r=document.createElement("div");r.className="foto-wrapper",r.style.cssText="position:relative;display:inline-block;max-width:100%;margin-bottom:6px";let s=document.createElement("img");s.src=e,s.style.cssText="max-width:100%;max-height:180px;border-radius:10px;border:1px solid #e2e8f0;display:block",r.appendChild(s),o&&r.insertAdjacentHTML("beforeend",`<button type="button" onclick="${o}" title="Rimuovi foto" style="position:absolute;top:-7px;right:-7px;background:#ef4444;border:none;color:#fff;width:22px;height:22px;border-radius:50%;font-size:1.1rem;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 1px 4px rgba(0,0,0,.3)">&times;</button>`);let c=t.querySelector(".manuale-file-input")||t.querySelector('input[type="file"]');t.insertBefore(r,c)}function Yo(t){t.removeAttribute("data-foto");let e=t.querySelector(".foto-wrapper");if(e){let o=document.createElement("div");o.className="text-xs text-slate-400",o.style.marginBottom="6px",o.textContent="Nessuna foto",e.replaceWith(o)}}async function lc(t,e){let o=t?.files&&t.files[0];o&&await Qo(o,function(i){let n=document.querySelector(`.occorrente-item[data-item-idx="${e}"]`);n&&Ko(n,i,"eliminaFotoOccorrente(this)")})}async function dc(t,e,o){let i=t?.files&&t.files[0];i&&await Qo(i,function(n){let a=document.querySelector(`.proc-step[data-step-idx="${e}"]`);if(!a)return;let r=a.querySelector(`.proc-foto-slot[data-slot="${o||1}"]`)||a;Ko(r,n,"eliminaFotoProcedimento(this)")})}async function uc(t){let e=t?.files&&t.files[0];e&&await Qo(e,function(o){let i=document.getElementById("manuali-disegno-wrap");i&&Ko(i,o,"eliminaFotoDisegno(this)")})}function pc(t){ne("Rimuovere la foto da questo elemento?",function(){let e=t.closest(".occorrente-item");e&&Yo(e)})}function mc(t){ne("Rimuovere la foto da questo step?",function(){let e=t.closest(".proc-foto-slot")||t.closest(".proc-step");e&&Yo(e)})}function fc(t){ne("Rimuovere la foto del disegno tecnico?",function(){let e=document.getElementById("manuali-disegno-wrap");e&&Yo(e)})}function gc(t){if(!t){Wo("new",null);return}let e=Bt[t];if(!e){g("Manuale non trovato.","warning");return}Wo("edit",e)}function Vn(){let t=document.getElementById("manuali-modal");t&&t.parentElement&&(t.parentElement.innerHTML=""),je=null}function ne(t,e){let o=document.getElementById("manuali-confirm-overlay");o&&o.remove();let i=document.createElement("div");i.id="manuali-confirm-overlay",i.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:11000;display:flex;align-items:center;justify-content:center",i.innerHTML=`
    <div style="background:#fff;border-radius:16px;padding:28px 32px;max-width:380px;width:90%;box-shadow:0 20px 40px rgba(0,0,0,0.18);text-align:center">
        ...
        <div style="display:flex;gap:10px;justify-content:center">
            ...
            <button id="manuali-confirm-si" class="btn-modal-send" style="min-width:100px;background:#ef4444">Elimina</button>
    ...
    </div>`,document.body.appendChild(i),document.getElementById("manuali-confirm-no").onclick=function(){i.remove()},document.getElementById("manuali-confirm-si").onclick=function(){i.remove(),e()},i.addEventListener("click",function(n){n.target===i&&i.remove()})}function Jn(t,e,o,i,n){let a=document.querySelector(t);a&&a.querySelectorAll(e).forEach(function(r,s){r.setAttribute(o,String(s));let c=r.querySelector("button");if(c&&c.setAttribute("onclick",`${i}(${s})`),n){let l=r.querySelector('input[type="file"]');l&&l.setAttribute("onchange",`${n}(this, ${s})`)}})}function vc(){let t=document.getElementById("manuali-scheda-edit");if(!t)return;let e=t.querySelectorAll(".scheda-row").length;if(e>=ec){g("Numero massimo voci raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",Hn("","",e))}function hc(t){ne("Eliminare questa voce dalla scheda tecnica?",function(){let e=document.querySelector(`.scheda-row[data-row-idx="${t}"]`);e&&e.remove(),Jn("#manuali-scheda-edit",".scheda-row","data-row-idx","rimuoviSchedaRow",null)})}function bc(){let t=document.getElementById("manuali-occorrente-edit");if(!t)return;let e=t.querySelectorAll(".occorrente-item").length;if(e>=oc){g("Numero massimo elementi raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",Vo({lettera:"ABCDEFGHIJKLMNOPQRSTUVWXYZ"[e]||"",nome:"",codice:"",foto:""},e))}function yc(t){ne("Eliminare questo elemento dal materiale occorrente?",function(){let e=document.querySelector(`.occorrente-item[data-item-idx="${t}"]`);e&&e.remove(),Jn("#manuali-occorrente-edit",".occorrente-item","data-item-idx","rimuoviOccorrenteItem","cambiaFotoOccorrente")})}function wc(){let t=document.getElementById("manuali-proc-edit");if(!t)return;let e=t.querySelectorAll(".proc-step").length;if(e>=tc){g("Numero massimo step raggiunto.","warning");return}t.insertAdjacentHTML("beforeend",Jo(null,e))}function Sc(t){ne("Eliminare questo step del procedimento?",function(){let e=document.querySelector(`.proc-step[data-step-idx="${t}"]`);e&&e.remove();let o=document.getElementById("manuali-proc-edit");o&&o.querySelectorAll(".proc-step").forEach(function(i,n){i.setAttribute("data-step-idx",String(n));let a=i.querySelector("h4");a&&(a.textContent="Step "+(n+1));let r=i.querySelector("button");r&&r.setAttribute("onclick",`rimuoviProcStep(${n})`);let s=i.querySelector('input[type="file"]');s&&s.setAttribute("onchange",`cambiaFotoProcedimento(this, ${n})`)})})}async function _c(){let t=String(document.getElementById("manuali-titolo")?.value||"").trim(),e=String(document.getElementById("manuali-categoria")?.value||"").trim(),o=String(document.getElementById("manuali-copertina-wrap")?.getAttribute("data-copertina")||"").trim(),i=sc();if(!t){g("Inserisci un titolo manuale.","warning");return}try{g("Salvataggio manuale in corso...","info");let n;if(je?n=await Dn({id:je,titolo:t,categoria:e,copertina:o,sections:i}):n=await Nn({titolo:t,categoria:e,copertina:o,sections:i}),!n||n.status!=="ok")throw new Error(n&&(n.message||n.msg)||"Errore salvataggio manuale");await D.invalidate(jo),delete window.cacheContenuti?.MANUALI_PRODOTTI,delete window.cacheFetchTime?.MANUALI_PRODOTTI,Vn(),await Ee(null,null,!1),g("Manuale salvato correttamente.","success")}catch(n){let a=n&&n.name==="TimeoutError"?"Il salvataggio sta impiegando troppo tempo. Riprova con meno immagini.":n&&n.name==="AbortError"?"Richiesta interrotta. Controlla la connessione e riprova.":n&&n.message||"Errore durante il salvataggio.";g(a,"error")}}function Ec(t){let e=Je(t),o=e?e.schedaTecnica||[]:[],i=e?e.occorrente||[]:[],n=e?e.procedimenti||[]:Array.isArray(t.steps)?t.steps.map(function(a){return{descrizione:String(a.descrizione||a.titolo||"").trim(),foto:String(a.foto||"").trim(),foto2:""}}):[];return{titolo:String(t.titolo||"(Senza titolo)"),categoria:String(t.categoria||"Generale"),versione:Number(t.version||1),aggiornato:Ve(t.updatedAt),copertina:Y(t.copertina),schedaTecnica:Array.isArray(o)?o:[],occorrente:Array.isArray(i)?i:[],procedimenti:Array.isArray(n)?n:[],disegnoTecnico:Y(e?.disegnoTecnico?.foto||"")}}function ee(t){return`<div class="man-print-placeholder">${y(t||"Sezione non disponibile")}</div>`}function xc(t){let e=Ec(t),o=e.copertina?`<img class="man-cover-image" src="${e.copertina}" alt="copertina manuale">`:ee("Copertina non disponibile"),i=e.schedaTecnica.map(function(s){return`<tr>
                <td>${y(s?.voce||"")}</td>
                <td>${y(s?.valore||"")}</td>
            </tr>`}).join(""),n=e.occorrente.map(function(s){let c=Y(s?.foto||""),l=c?`<img src="${c}" alt="${y(s?.nome||"componente")}" class="man-occ-image">`:'<div class="man-occ-image man-occ-image--empty">Immagine non disponibile</div>';return`<article class="man-occ-card">
                ...
                ${l}
            </article>`}).join(""),a=e.procedimenti.map(function(s,c){let l=Y(s?.foto||""),d=Y(s?.foto2||""),p=[l,d].filter(Boolean),f=p.length?`<div class="man-proc-photos">${p.map(function(u){return`<img src="${u}" alt="step-${c+1}">`}).join("")}</div>`:ee("Immagini step non disponibili");return`<article class="man-proc-step">
...
<body>
    ...
    <div class="man-print-stage">
        ...
        <section class="man-print-page man-break">
...
</html>`}function Ic(t){let e=Bt[t];if(!e){g("Manuale non trovato.","error");return}let o=window.open("","_blank");if(!o){g("Popup bloccato dal browser. Consenti le finestre popup per stampare.","warning");return}o.document.open(),o.document.write(xc(e)),o.document.close()}function Ac(t){let e=Bt[t];if(!e)return;let o=document.getElementById("manuali-modal-host");if(!o)return;let i=Y(e.copertina),n=i?`<img src="${i}" alt="copertina" style="max-width:100%;max-height:260px;border-radius:10px;border:1px solid #e2e8f0;margin-bottom:16px">`:"",a=(c,l)=>`<h3 style="font-weight:700;font-size:.8rem;text-transform:uppercase;letter-spacing:.05em;color:#64748b;margin:18px 0 8px;padding-bottom:6px;border-bottom:2px solid #e2e8f0"><i class="${c}"></i> &nbsp;${l}</h3>`,r=Je(e),s="";if(r){let c=(r.schedaTecnica||[]).map(function(f){return`<tr>
            ...
            <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden">
                ...
                <tbody>${c}</tbody>
            </table>`),He=[];let l=(r.occorrente||[]).map(function(f){let u=Y(f.foto||""),m=-1;return u&&(He.push({lettera:f.lettera||"",nome:f.nome||"",foto:u}),m=He.length-1),`<div ${u?`onclick="_apriLightboxOcc_(${m})"`:""} style="${`padding:10px;border:1px solid #e2e8f0;border-radius:10px;background:#fff${u?";cursor:pointer":""}`}">
            ...
            </div>`}).join("");l&&(s+=a("fas fa-boxes-stacked","Materiale Occorrente")+`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px">${l}</div>`);let d=(r.procedimenti||[]).map(function(f,u){let m=Y(f.foto||""),b=Y(f.foto2||""),w=[m,b].filter(Boolean),v=w.length?`<div style="display:grid;grid-template-columns:repeat(${w.length},1fr);gap:6px;margin-bottom:6px">${w.map(x=>`<img src="${x}" style="width:100%;border-radius:10px;border:1px solid #e2e8f0">`).join("")}</div>`:"";return`<details class="border border-slate-200 rounded-xl p-3 bg-white" ${u===0?"open":""}>
            ...
            </details>`}).join("");d&&(s+=a("fas fa-list-check","Procedimento")+`<div class="grid gap-2">${d}</div>`);let p=Y(r.disegnoTecnico?.foto||"");p&&(s+=a("fas fa-drafting-compass","Disegno Tecnico")+`<img src="${p}" alt="disegno-tecnico" style="max-width:100%;border-radius:10px;border:1px solid #e2e8f0">`)}else s=(e.steps||[]).map(function(c,l){let d=Y(c.foto),p=d?`<img src="${d}" alt="step-${l+1}" style="max-width:100%;border-radius:10px;border:1px solid #e2e8f0">`:'<div class="text-xs text-slate-400">Nessuna immagine</div>';return`
    ...
    </div>`}async function Oc(t){let e=document.getElementById("manuali-storico-host");if(e){e.innerHTML=`
    <div id="manuali-storico-modal" class="modal-overlay active" style="display:flex;z-index:4501">
    ...
    </div>`;try{let o=await qn(t);if(!o||o.status!=="ok")throw new Error("Storico non disponibile");let n=(Array.isArray(o.storico)?o.storico:[]).map(function(a,r){let s=a.snapshot||{},c=Array.isArray(s.steps)?s.steps.length:0;return`
        ...
        </div>`}}}function Cc(){let t=document.getElementById("manuali-storico-host");t&&(t.innerHTML="")}async function Ee(t=null,e=null,o=!1){let i=document.getElementById("contenitore-dati");if(i){o||(i.innerHTML="<div class='centered-msg'><i class='fas fa-spinner fa-spin'></i> Caricamento manuali...</div>");try{let n=null;if(!o)try{n=await D.get(jo)}catch{}if(n&&Array.isArray(n.manuali)&&n.manuali.length&&(oe=n.manuali,Bt={},oe.forEach(function(r){Bt[r.id]=r}),Fn(),!o))return;let a=await Mn();if(e?.aborted)return;if(!a||a.status!=="ok")throw new Error(a&&(a.message||a.msg)||"Errore caricamento manuali");oe=Array.isArray(a.manuali)?a.manuali:[],Bt={},oe.forEach(function(r){Bt[r.id]=r}),await D.set(jo,{manuali:oe}),Fn(),window.cacheContenuti&&(window.cacheContenuti.MANUALI_PRODOTTI=i.innerHTML),window.cacheFetchTime&&(window.cacheFetchTime.MANUALI_PRODOTTI=Date.now())}catch(n){if(console.error("[manuali] Errore caricaManuali:",n&&n.message?n.message:String(n),n),o)return;i.innerHTML="<div class='centered-error-bold'>Errore nel caricamento manuali.<br><small style='font-size:.75rem;color:#666'>"+y(n&&n.message||"sconosciuto")+"</small></div>"}}}function $c(t){let e=He;if(!e.length)return;let o=t;function i(){let p=document.getElementById("_occ_lightbox");p&&p.remove(),document.removeEventListener("keydown",r)}function n(p){o=p;let f=e[p];document.getElementById("_occ_lb_img").src=f.foto,document.getElementById("_occ_lb_badge").textContent=f.lettera,document.getElementById("_occ_lb_nome").textContent=f.nome,document.getElementById("_occ_lb_counter").textContent=e.length>1?`${p+1} / ${e.length}`:""}function a(p){n((o+p+e.length)%e.length)}function r(p){p.key==="ArrowLeft"?a(-1):p.key==="ArrowRight"?a(1):p.key==="Escape"&&i()}document.getElementById("_occ_lightbox")?.remove(),window._occLbKeyHandler&&document.removeEventListener("keydown",window._occLbKeyHandler);let s=e.length>1,c="background:rgba(255,255,255,.15);border:none;color:#fff;width:48px;height:48px;border-radius:50%;font-size:1.8rem;cursor:pointer;flex-shrink:0;line-height:1;display:flex;align-items:center;justify-content:center;",l=document.createElement("div");l.id="_occ_lightbox",l.style.cssText="position:fixed;z-index:99999;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.92);display:flex;align-items:center;justify-content:center",l.innerHTML=`
      ...
      </div>`,document.body.appendChild(l),document.getElementById("_occ_lb_close").addEventListener("click",i),document.getElementById("_occ_lb_prev").addEventListener("click",function(){a(-1)}),document.getElementById("_occ_lb_next").addEventListener("click",function(){a(1)}),l.addEventListener("click",function(p){p.target===l&&i()});let d=0;l.addEventListener("touchstart",function(p){d=p.changedTouches[0].clientX},{passive:!0}),l.addEventListener("touchend",function(p){let f=p.changedTouches[0].clientX-d;Math.abs(f)>50&&a(f<0?1:-1)},{passive:!0}),document.addEventListener("keydown",r),window._occLbKeyHandler=r,n(t)}function qt(t,e){return t[e]|t[e+1]<<8}function Fe(t,e){return(t[e]|t[e+1]<<8|t[e+2]<<16|t[e+3]<<24)>>>0}async function Tc(t){let e=new DecompressionStream("deflate-raw"),o=e.writable.getWriter(),i=e.readable.getReader();o.write(t),o.close();let n=[],a=0;for(;;){let{done:c,value:l}=await i.read();if(c)break;n.push(l),a+=l.length}let r=new Uint8Array(a),s=0;for(let c of n)r.set(c,s),s+=c.length;return r}async function Rc(t){let e=-1,o=Math.max(0,t.length-65558);for(let c=t.length-22;c>=o;c--)if(t[c]===80&&t[c+1]===75&&t[c+2]===5&&t[c+3]===6){e=c;break}if(e<0)throw new Error("File ZIP non valido (EOCD non trovato)");let i=qt(t,e+10),n=Fe(t,e+16),a=Object.create(null),r=new TextDecoder("utf-8",{fatal:!1}),s=n;for(let c=0;c<i&&Fe(t,s)===33639248;c++){let l=qt(t,s+10),d=Fe(t,s+20),p=qt(t,s+28),f=qt(t,s+30),u=qt(t,s+32),m=Fe(t,s+42),b=r.decode(t.slice(s+46,s+46+p));if(s+=46+p+f+u,b.endsWith("/")||b.endsWith("\\"))continue;let w=qt(t,m+26),v=qt(t,m+28),x=m+30+w+v,O=t.slice(x,x+d);l===0?a[b]=O:l===8&&(a[b]=await Tc(O))}return a}function _e(t){let e=new TextDecoder("utf-8",{fatal:!1}).decode(t);return new DOMParser().parseFromString(e,"text/xml")}function ti(t){return[...t.getElementsByTagName("Relationship")].map(e=>({id:e.getAttribute("Id")||"",type:e.getAttribute("Type")||"",target:e.getAttribute("Target")||""}))}function kc(t){let e=t["ppt/presentation.xml"],o=t["ppt/_rels/presentation.xml.rels"];if(!e||!o)return null;let i={};ti(_e(o)).forEach(c=>{i[c.id]=c.target});let n=_e(e),a=[...n.getElementsByTagNameNS(Xo,"sldId")];a.length||(a=[...n.getElementsByTagName("p:sldId")]);let r=[],s=new Set;return a.forEach(c=>{let l=c.getAttributeNS(Wn,"id")||c.getAttribute("r:id")||"",d=i[l];if(!d)return;let p="ppt/"+d.replace(/^\.\.\//,"");s.has(p)||(s.add(p),r.push(p))}),r.length?r:null}function zc(t){if(!t)return{};let e={};return ti(_e(t)).forEach(o=>{o.type.includes("image")&&(e[o.id]=o.target)}),e}function Mc(t,e){let o=t.split("/").pop();if(!Lc.test(o))return null;let i=e["ppt/media/"+o];if(!i)return null;let n=Pc[o.split(".").pop().toLowerCase()];if(!n)return null;let a="";for(let r=0;r<i.length;r+=8192)a+=String.fromCharCode(...i.subarray(r,Math.min(r+8192,i.length)));return`data:${n};base64,${btoa(a)}`}function Zn(t){let e=t.getElementsByTagNameNS(Ut,"off")[0]||t.getElementsByTagName("a:off")[0];return e?{x:parseInt(e.getAttribute("x")||"0",10),y:parseInt(e.getAttribute("y")||"0",10)}:{x:0,y:0}}function Nc(t,e,o,i){let n=[...t.getElementsByTagNameNS(Xo,"pic")];n.length||(n=[...t.getElementsByTagName("p:pic")]);let a=[];for(let r of n){let s=r.getElementsByTagNameNS(Ut,"blip")[0]||r.getElementsByTagName("a:blip")[0];if(!s)continue;let c=s.getAttributeNS(Wn,"embed")||s.getAttribute("r:embed")||"";if(!c||!e[c])continue;let l=e[c],d=l.split("/").pop();if(i&&i.has(d))continue;let p=Mc(l,o);if(!p)continue;let f=Zn(r);a.push({dataUrl:p,x:f.x,y:f.y})}return a.sort((r,s)=>r.y!==s.y?r.y-s.y:r.x-s.x)}function Dc(t){let e=[...t.getElementsByTagNameNS(Ut,"tbl")];if(e.length||(e=[...t.getElementsByTagName("a:tbl")]),!e.length)return null;let o=[...e[0].getElementsByTagNameNS(Ut,"tr")];o.length||(o=[...e[0].getElementsByTagName("a:tr")]);let i=[];for(let n of o){let a=[...n.getElementsByTagNameNS(Ut,"tc")];if(a.length||(a=[...n.getElementsByTagName("a:tc")]),a.length<2)continue;let r=l=>{let d=[...l.getElementsByTagNameNS(Ut,"t")];return d.length||(d=[...l.getElementsByTagName("a:t")]),d.map(p=>(p.textContent||"").trim()).filter(Boolean).join(" ")},s=r(a[0]),c=r(a[1]);s&&i.push({voce:s,valore:c||""})}return i.length>=2?i:null}function qc(t){let e=[...t.getElementsByTagNameNS(Xo,"sp")];e.length||(e=[...t.getElementsByTagName("p:sp")]);let o=[];for(let i of e){let n=[...i.getElementsByTagNameNS(Ut,"t")];n.length||(n=[...i.getElementsByTagName("a:t")]);let a=n.map(s=>(s.textContent||"").trim()).filter(Boolean).join(" ").trim();if(!a)continue;let r=Zn(i);o.push({text:a,x:r.x,y:r.y})}return o.sort((i,n)=>i.y!==n.y?i.y-n.y:i.x-n.x)}function Bc(t,e,o,i,n){let a=e.map(l=>l.text).join(" ");if(/disegno\s+tecnico/i.test(a)&&o.length>=1)return"disegno";if(t&&t.length>=2)return"scheda";if(/materiale\s+occorrente/i.test(a))return"occorrente";let r=l=>/^[A-Z]-?\d/.test(l),s=l=>(/^[A-Z][\s\-–\.\:]/.test(l)||/^[A-Z]$/.test(l))&&!r(l),c=e.filter(l=>s(l.text.trim()));return!n&&i<4&&o.length===1&&!t&&a.length<300&&c.length===0?"title":i<3&&o.length===0&&!t&&c.length===0?"intro":c.length>=2||c.length===1&&/^[A-Z]$/.test(c[0].text.trim())&&o.length>=1?"occorrente":"procedimento"}function Uc(t,e,o){let i=/^([A-Z])[\s\-–\.\:]*(.*)/s,n=t.filter(a=>/^[A-Z][\s\-–\.\:]|^[A-Z]$/.test(a.text.trim()));for(let a of n){let r=a.text.trim().match(i);if(!r)continue;let s=r[1],l=(r[2]||"").trim().split(/\s{2,}|\s+[-–]\s+/),d=(l[0]||"").trim().slice(0,80),p=(l[1]||"").trim().slice(0,40),f=o.find(u=>u.lettera===s);f?(!f.nome&&d&&(f.nome=d),!f.codice&&p&&(f.codice=p)):o.push({lettera:s,nome:d,codice:p,foto:""})}if(e.length)if(e.length===1&&n.length===1){let a=n[0].text.trim()[0],r=o.find(s=>s.lettera===a);r&&!r.foto&&(r.foto=e[0].dataUrl)}else{let a=[...n].sort((r,s)=>r.y!==s.y?r.y-s.y:r.x-s.x);e.forEach((r,s)=>{if(s<a.length){let c=a[s].text.trim()[0],l=o.find(d=>d.lettera===c);l&&!l.foto&&(l.foto=r.dataUrl)}})}}function Fc(t,e){let i=[...t.filter(c=>{let l=c.text.trim();return!(/^REVISIONE\s/i.test(l)||/^\d+$/.test(l)||/^[<>]$/.test(l)||l===l.toUpperCase()&&l.length<60&&/^[A-Z\s\d\-:;./]+$/.test(l))}).map(c=>({kind:"text",y:c.y,x:c.x||0,v:c.text})),...e.map(c=>({kind:"img",y:c.y,x:c.x||0,v:c.dataUrl}))].sort((c,l)=>c.y!==l.y?c.y-l.y:c.x-l.x),n=[],a=[],r=[];function s(){let c=a.join(" ").trim(),l=r[0]||null,d=r[1]||null;(c||l)&&n.push({descrizione:c,imageBase64:l,imageBase642:d}),a=[],r=[]}for(let c of i)c.kind==="text"?(r.length>0&&s(),a.push(c.v)):r.push(c.v);if(s(),!n.length){let c=t.map(p=>p.text).join(" ").trim(),l=e.length?e[0].dataUrl:null,d=e.length>1?e[1].dataUrl:null;(c||l)&&n.push({descrizione:c,imageBase64:l,imageBase642:d})}return n}async function Hc(t){let e=new Uint8Array(t),o=await Rc(e);if(!o["ppt/presentation.xml"])throw new Error("File non valido: manca ppt/presentation.xml");let i=kc(o);(!i||!i.length)&&(i=Object.keys(o).filter(m=>/^ppt\/slides\/slide\d+\.xml$/.test(m)).sort((m,b)=>{let w=parseInt(m.match(/(\d+)\.xml$/)?.[1]||"0",10),v=parseInt(b.match(/(\d+)\.xml$/)?.[1]||"0",10);return w-v}));let n={};for(let m of i){let b=m.split("/").pop(),w=o[`ppt/slides/_rels/${b}.rels`];if(!w)continue;let v=new Set;for(let x of ti(_e(w))){if(!x.type.includes("image"))continue;let O=x.target.split("/").pop();v.has(O)||(v.add(O),n[O]=(n[O]||0)+1)}}let a=new Set,r=Math.max(2,Math.ceil(i.length*.4));for(let[m,b]of Object.entries(n))b>=r&&a.add(m);let s="",c="",l="",d=[],p=[],f=[],u={foto:""};for(let m=0;m<i.length;m++){let b=o[i[m]];if(!b)continue;let w=_e(b),v=i[m].split("/").pop(),x=zc(o[`ppt/slides/_rels/${v}.rels`]),O=Dc(w),E=qc(w),T=Nc(w,x,o,a),k=Bc(O,E,T,m,!!s);if(k!=="intro")if(k==="title"){if(!l&&T.length&&(l=T[0].dataUrl),!s&&E.length){let R=T.length?T[0].y:1/0,C=E.filter(q=>q.y<=R&&q.text.length>3).filter(q=>!/^[A-Z0-9\-]+$/.test(q.text)).filter(q=>!/^REVISIONE\s/i.test(q.text));C.length>=2?(c=C[0].text.slice(0,80).trim(),s=C[C.length-1].text.slice(0,120).trim()):C.length===1?s=C[0].text.slice(0,120).trim():s=E[0].text.slice(0,120).trim()}}else if(k==="scheda")O.forEach(R=>{d.find(C=>C.voce===R.voce)||d.push(R)});else if(k==="occorrente")Uc(E,T,p);else if(k==="disegno")!u.foto&&T.length&&(u={foto:T[0].dataUrl});else{let R=Fc(E,T);for(let C of R)(C.descrizione||C.imageBase64)&&f.push(C)}}return{titolo:s,categoria:c,copertina:l,schedaTecnica:d,occorrente:p,procedimenti:f,disegnoTecnico:u}}function jc(){if(h?.nome?.toUpperCase().trim()!=="ALESSIO")return;let t=document.getElementById("_pptx-file-inp");t||(t=document.createElement("input"),t.type="file",t.id="_pptx-file-inp",t.accept=".pptx",t.style.display="none",t.addEventListener("change",function(){Gc(t)}),document.body.appendChild(t)),t.value="",t.click()}async function Gc(t){let e=t?.files?.[0];if(e){g("Analisi PPTX in corso...","info");try{let o=await e.arrayBuffer(),i=await Hc(o);if(!i.titolo&&!i.procedimenti.length&&!i.schedaTecnica.length&&!i.occorrente.length){g("Nessun contenuto riconosciuto nel file.","warning");return}i.titolo||(i.titolo=e.name.replace(/\.pptx$/i,"").replace(/[-_]/g," ")),Go=i,Vc(i)}catch(o){console.error("[PPTX]",o),g("Errore nel parsing PPTX: "+(o?.message||"file non valido"),"error")}}}function Vc(t){let e=document.getElementById("manuali-modal-host");if(!e)return;let o=t.copertina?`<img src="${t.copertina}" style="width:80px;height:60px;object-fit:cover;border-radius:8px;border:1px solid #e2e8f0;flex-shrink:0">`:'<div style="width:80px;height:60px;background:#f1f5f9;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#cbd5e1;font-size:10px;flex-shrink:0">nessuna</div>',i=t.schedaTecnica.length?t.schedaTecnica.slice(0,6).map(c=>`<tr><td style="padding:3px 8px;color:#475569;font-size:11px;border-bottom:1px solid #f1f5f9">${y(c.voce)}</td><td style="padding:3px 8px;font-size:11px;color:#1e293b;border-bottom:1px solid #f1f5f9">${y(c.valore)}</td></tr>`).join("")+(t.schedaTecnica.length>6?`<tr><td colspan="2" style="padding:3px 8px;color:#94a3b8;font-size:10px">+ altre ${t.schedaTecnica.length-6} voci\u2026</td></tr>`:""):'<tr><td colspan="2" style="padding:6px 8px;color:#94a3b8;font-size:11px">Nessuna voce riconosciuta</td></tr>',n=t.occorrente.length?t.occorrente.slice(0,8).map(c=>{let l=c.foto?`<img src="${c.foto}" style="width:36px;height:36px;object-fit:cover;border-radius:5px;border:1px solid #e2e8f0;flex-shrink:0">`:'<div style="width:36px;height:36px;background:#f1f5f9;border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:9px;color:#94a3b8;flex-shrink:0">\u2013</div>';return`<div style="display:flex;align-items:center;gap:8px;padding:4px 0;border-bottom:1px solid #f8fafc">
    ...
    <div id="manuali-modal" class="modal-overlay active" style="display:flex;z-index:4500">
        ...
        </label>`}).join(""),document.querySelectorAll(".qr-art-chk").forEach(d=>d.addEventListener("change",Ie)),Yc(),document.getElementById("qr-stato-wrap").style.display="block",Ie()}function Yc(){let t=Ye,e=document.getElementById("qr-stato-pills");if(!e)return;let o=t?t.statoDefault.toUpperCase():"",i=window.listaStati||[],n=i.length>0?i:[{nome:"IN ATTESA",colore:"#94a3b8"},{nome:"PREPARARE PER LAVORAZIONE",colore:"#64748b"},{nome:"IN LAVORAZIONE",colore:"#f59e0b"},{nome:"IN PRODUZIONE",colore:"#242424"},{nome:"IMBALLATO",colore:"#22c55e"}];rt=null,e.innerHTML=n.map(a=>{let r=a.nome.toUpperCase()===o;return r&&(rt=a.nome),`<button type="button"
...
</html>`,i=new Blob([o],{type:"text/html; charset=utf-8"}),n=URL.createObjectURL(i);window.open(n,"_blank","width=900,height=700")?setTimeout(()=>URL.revokeObjectURL(n),3e4):(URL.revokeObjectURL(n),g("\u26A0\uFE0F Abilita i popup per la stampa.","error"))}var Z,oi,Rt,xe,Ye,rt,Ae,ei,ya=V(()=>{lt();Tt();dt();wt();Z=[],oi={};Rt=null,xe=null,Ye=null,rt=null,Ae=null;ei=!1});function wa(t){return window.hashSHA256(t)}function Sa(){return h?.nome?"avatarColorRecenti_"+h.nome.toUpperCase().trim():null}function _a(){return h?.nome?"avatarColorHidden_"+h.nome.toUpperCase().trim():null}function xt(){let t=Sa();if(!t)return[];try{return JSON.parse(localStorage.getItem(t)||"[]")}catch{return[]}}function ae(t){let e=Sa();if(e)try{localStorage.setItem(e,JSON.stringify(t.slice(0,7)))}catch{}}function Ea(){let t=_a();if(!t)return[];try{return JSON.parse(localStorage.getItem(t)||"[]")}catch{return[]}}function xa(t){let e=_a();if(e)try{localStorage.setItem(e,JSON.stringify(t))}catch{}}function ci(){let t=document.getElementById("avatar-predefined-swatches");if(!t)return;let e=Ea();t.innerHTML="",(window._PREDEFINED_AVATAR_COLORS||[]).forEach(o=>{if(e.includes(o))return;let i=document.createElement("button");i.className="avatar-color-swatch",i.style.background=o,i.dataset.color=o,i.title="Clicca per applicare o eliminare",i.onclick=n=>{n.stopPropagation(),el(o,n)},t.appendChild(i)})}function Ft(){let t=["avatar-custom-swatches","avatar-custom-swatches-mob"],e=xt();t.forEach(o=>{let i=document.getElementById(o);if(!i)return;let n=o.endsWith("-mob");i.innerHTML="",e.forEach((a,r)=>{let s=document.createElement("button");s.className="avatar-color-swatch avatar-color-custom-swatch",s.style.background=a,s.dataset.color=a,s.title="Clicca per modificare o eliminare",s.onclick=n?c=>{c.stopPropagation(),rl(r,c)}:c=>{c.stopPropagation(),tl(r,c)},i.appendChild(s)})})}function li(t,e){let o=document.getElementById("avatar-color-editor"),i=document.getElementById("avatar-color-edit-input"),n=document.getElementById("avatar-editor-delete");!o||!i||(i.value=t||"#ff0000",n&&(n.style.display=e?"":"none"),o.style.display="flex")}function di(){let t=document.getElementById("avatar-color-editor");t&&(t.style.display="none"),H=null}function Xc(t){t&&t.stopPropagation(),H=null,li("#ff0000",!1)}function tl(t,e){e&&e.stopPropagation();let o=xt();H={type:"custom",idx:t},li(o[t]||"#ff0000",!0)}function el(t,e){e&&e.stopPropagation(),H={type:"predefined",color:t},li(t,!0)}function ol(t){t&&t.stopPropagation();let e=document.getElementById("avatar-color-edit-input");if(!e)return;let o=e.value;if(H===null){let i=xt();i.unshift(o),ae(i),Ft()}else if(H.type==="custom"){let i=xt();i[H.idx]=o,ae(i),Ft()}di(),pi(o)}function il(t){t&&t.stopPropagation(),di()}function nl(t){if(t&&t.stopPropagation(),!!H){if(H.type==="custom"){let e=xt();e.splice(H.idx,1),ae(e),Ft()}else if(H.type==="predefined"){let e=Ea();e.includes(H.color)||e.push(H.color),xa(e),ci()}di()}}function al(t){t&&t.stopPropagation(),xa([]),ci()}function Ia(t,e){let o=document.getElementById("avatar-color-editor-mob"),i=document.getElementById("avatar-color-edit-input-mob"),n=document.getElementById("avatar-editor-delete-mob");!o||!i||(i.value=t||"#ff0000",n&&(n.style.display=e?"":"none"),o.style.display="flex")}function ui(){let t=document.getElementById("avatar-color-editor-mob");t&&(t.style.display="none"),H=null}function sl(t){t&&t.stopPropagation(),H=null,Ia("#ff0000",!1)}function rl(t,e){e&&e.stopPropagation();let o=xt();H={type:"custom",idx:t},Ia(o[t]||"#ff0000",!0)}function cl(t){t&&t.stopPropagation();let e=document.getElementById("avatar-color-edit-input-mob");if(!e)return;let o=e.value;if(H===null){let i=xt();i.unshift(o),ae(i),Ft()}else if(H.type==="custom"){let i=xt();i[H.idx]=o,ae(i),Ft()}ui(),pi(o)}function ll(t){t&&t.stopPropagation(),ui()}function dl(t){if(t&&t.stopPropagation(),!!H){if(H.type==="custom"){let e=xt();e.splice(H.idx,1),ae(e),Ft()}ui()}}function Aa(t){document.documentElement.style.setProperty("--avatar-user-color",t);let e=document.getElementById("user-avatar-btn"),o=document.getElementById("account-ddrop-avatar"),i=document.getElementById("user-avatar-btn-mobile"),n=document.getElementById("account-ddrop-avatar-mob");e&&(e.style.setProperty("background",t,"important"),e.style.setProperty("box-shadow",`0 2px 8px ${t}66`,"important")),i&&(i.style.setProperty("background",t,"important"),i.style.setProperty("box-shadow",`0 2px 8px ${t}66`,"important")),o&&o.style.setProperty("background",t,"important"),n&&n.style.setProperty("background",t,"important"),document.querySelectorAll(".avatar-color-swatch").forEach(a=>{a.classList.toggle("active",a.dataset.color===t)})}function pi(t){if(!h||!h.nome)return;let e=h.nome.toUpperCase().trim();try{localStorage.setItem("avatarColor_"+e,t)}catch{}window._avatarColorsCache&&(window._avatarColorsCache[e]=t),h.nome&&fetch(I,{method:"POST",body:JSON.stringify({azione:"setAvatarColor",username:h.nome,color:t})}).catch(()=>{}),Aa(t)}function ul(t){t&&t.stopPropagation();let e=document.getElementById("account-dropdown");e&&e.classList.toggle("open")}function pl(){let t=document.getElementById("account-dropdown");t&&t.classList.remove("open")}function ml(){U("_impostazioni_cache"),window.paginaAttuale&&(window.cacheContenuti&&delete window.cacheContenuti[window.paginaAttuale],U("_html_"+window.paginaAttuale)),window.location.reload()}function fl(t){t&&t.stopPropagation();let e=document.getElementById("account-dropdown-mobile");e&&e.classList.toggle("open")}function gl(){let t=document.getElementById("account-dropdown-mobile");t&&t.classList.remove("open")}async function vl(){try{let o=(await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"pushInfo"})})).json().catch(()=>({}))).subscriptions||[];if(!o.length){g("Nessun dispositivo registrato in PUSH_SUBSCRIPTIONS","error");return}let i={};o.forEach(r=>{i[r.user]||(i[r.user]=0),i[r.user]++});let n=Object.entries(i).sort((r,s)=>r[0].localeCompare(s[0])).map(([r,s])=>`<tr><td style="padding:6px 10px;font-weight:600">${r}</td><td style="padding:6px 10px;text-align:center">${s} dispositivo${s>1?"i":""}</td></tr>`).join(""),a=`<div style="position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:9999;display:flex;align-items:center;justify-content:center" onclick="this.remove()">
...
`)[0]||"",r=a.split(";").length>=a.split(",").length?";":",";i&&(i.style.display="block",i.innerHTML='<div style="display:flex;align-items:center;gap:8px;color:#64748b;font-size:0.88rem"><i class="fas fa-spinner fa-spin"></i> Import Lista di Carico in corso\u2026</div>');try{let c=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"importaListaDiCarico",csvText:n,separatore:r})})).json().catch(()=>({}));if(i)if(c.status==="ok"){let l=`<strong>\u2705 Import completato</strong><br>Nuovi: <strong>${c.nuove||0}</strong> \xB7 Aggiornati: <strong>${c.aggiornate||0}</strong> \xB7 Invariati: <strong>${c.invariate||0}</strong>`;c.corrette>0&&(l+=`<br><span style="color:#b45309"><i class="fas fa-exclamation-triangle"></i> ${c.corrette} righe corrette automaticamente (campi con separatore nel testo)</span>`),c.missingCount>0&&(l+=`<br><span style="color:#d97706">\u26A0 <strong>${c.missingCount}</strong> ordini non presenti nel CSV \u2014 vai nel tab Fornitori per archiviarli.</span>`),i.innerHTML=`<div style="background:#dcfce7;border:1px solid #86efac;border-radius:10px;padding:12px 16px;font-size:0.88rem;color:#166534">${l}</div>`,typeof window.invalidateOFCache=="function"&&window.invalidateOFCache()}else i.innerHTML=io(c.msg||c.message||"Errore sconosciuto")}catch(s){i&&(i.innerHTML=io(s.message))}t.value=""}async function Oe(){let t="_impostazioni_cache",e="_impostazioni_stati_forn_cache",i=pt(t,1/0);if(i){try{let n=typeof i=="string"?JSON.parse(i):i;if(n.stati&&n.stati.length){window.listaStati=n.stati,window.listaOperatori=n.operatori||[],window._distintaHeaderAzienda=String(n.distintaHeaderAzienda||""),Oa(n.overviewStati);let a=pt(e,1/0);if(a)try{let r=typeof a=="string"?JSON.parse(a):a;Array.isArray(r)&&r.length&&(window.listaStatiFornitori=r)}catch{}pt(t,3e5)||no().catch(r=>console.warn("[impostazioni] bg refresh:",r));return}}catch(n){console.warn("[impostazioni] cache JSON corrotta, ricarico dal server:",n)}U(t)}await no()}async function no(){let t="_impostazioni_cache",e="_impostazioni_stati_forn_cache";try{let i=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"getImpostazioni",noCache:!0})})).json();window.listaStati=i.stati&&i.stati.length?i.stati:window._defaultListaStati_(),window.listaOperatori=i.operatori||[],window._distintaHeaderAzienda=String(i.distintaHeaderAzienda||""),Oa(i.overviewStati),J(t,JSON.stringify({stati:window.listaStati,operatori:window.listaOperatori,overviewStati:i.overviewStati,distintaHeaderAzienda:window._distintaHeaderAzienda}))}catch(o){console.warn("[Boot] _fetchImpostazioniDaServer:",o),g("Impostazioni non aggiornate \u2014 uso dati locali.","warning")}try{let i=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"getStatiFornitoriConfig"})})).json();i.stati&&i.stati.length&&(window.listaStatiFornitori=i.stati,J(e,JSON.stringify(i.stati)))}catch{}}function Oa(t){t&&(Array.isArray(t.art)&&t.art.length&&(window._ovStatiArt=t.art.map(e=>e.toUpperCase().trim())),Array.isArray(t.ord)&&t.ord.length&&(window._ovStatiOrd=t.ord.map(e=>e.toUpperCase().trim())))}function Il(t,e){let o=document.getElementById(t);if(!o)return;let i=e.querySelector(".settings-row-arrow"),n=o.style.display==="block";o.style.display=n?"none":"block",i&&(i.style.transform=n?"":"rotate(180deg)"),e.classList.toggle("settings-row-active",!n),!n&&(t==="section-utenti"||t==="section-team-utenti")&&Ce(),!n&&t==="section-qr-postazioni"&&requestAnimationFrame(()=>si())}async function Ce(){let t=document.getElementById("lista-utenti-config");if(t){t.innerHTML='<div class="centered-msg small">Caricamento...</div>';try{let o=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"getUtenti"})})).json();if(!o.length){t.innerHTML='<p class="centered-msg small">Nessun utente creato. Clicca "+ Aggiungi Utente".</p>';return}t.innerHTML=o.map(i=>{let n=i.id_riga,a=(i.username||"").trim(),r=(i.email||"").trim(),s=(i.ruolo||"OPERATORE").trim().toUpperCase(),c=Number(i.max_utenti)||1;return`
    ...
    `,z(t),requestAnimationFrame(()=>si()),h&&Bl("lista-stati-config",i=>{let n=[...i.querySelectorAll("[data-idx]")],a=n.map(r=>(window.listaStati||[])[+r.dataset.idx]);window.listaStati&&(window.listaStati.length=0,a.forEach((r,s)=>{window.listaStati.push(r),n[s].dataset.idx=s})),se()})}async function Ll(){let t=document.getElementById("diag-revision"),e=document.getElementById("diag-lastcheck"),o=document.getElementById("diag-online"),i=document.getElementById("diag-cache");if(!t)return;if(t.textContent=M.lastRevisionValue!==null?String(M.lastRevisionValue):"\u2014",M.lastCheckTs){let a=new Date(M.lastCheckTs);e.textContent=a.toLocaleTimeString("it-IT")}else e.textContent="\u2014";let n=M.lastOnlineList;n&&n.length>0?o.textContent=n.map(a=>a.nome+(a.pagina?" ("+a.pagina+")":"")).join(", "):o.textContent="Nessuno";try{let a=await D.listEntries();a.length?i.innerHTML=a.map(r=>{let s=Math.round((Date.now()-r.timestamp)/1e3),c=Date.now()-r.timestamp>D.TTL;return`<span style="display:block;font-family:monospace;font-size:0.78rem;color:${c?"#ef4444":"#16a34a"}">${r.chiave} <em style="color:#94a3b8">(${s}s fa${c?" \xB7 stale":""})</em></span>`}).join(""):i.textContent="Vuota"}catch{i.textContent="Errore lettura cache"}}async function Pl(){try{let e=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"forceRevisionBump",sessionToken:window._getSessionToken_()})})).json();e&&e.status==="ok"?g("\u2714 Revision bumped a "+e.nuovaRevision+" \u2014 tutti i client aggiorneranno entro 15s"):e&&e.status==="auth_error"?window._gestisciAuthError_&&window._gestisciAuthError_(e.message):g("\u26A0\uFE0F "+(e&&e.message?e.message:"Errore"),"error")}catch{g("\u26A0\uFE0F Errore di rete","error")}}async function zl(){if(confirm("Svuota la cache IndexedDB e ricarica la pagina?")){try{await D.clear()}catch{}location.reload()}}function Ml(t){confirm("Sei sicuro di voler eliminare questo stato?")&&((window.listaStati||[]).splice(t,1),se(),Ht())}function Nl(){(window.listaStati||[]).push({nome:"NUOVO",colore:"#94a3b8"}),se(),Ht()}function Dl(t){confirm("Sei sicuro di voler eliminare questo stato?")&&((window.listaStatiFornitori||[]).splice(t,1),se(),Ht())}function ql(){window.listaStatiFornitori||(window.listaStatiFornitori=[]),window.listaStatiFornitori.push({stato:"NUOVO",colore:"#94a3b8"}),se(),Ht()}function se(){window.modifichePendenti=!0;let t=document.getElementById("btn-salva-globale");t&&(t.style.background="#ef4444",t.innerHTML="<i class='fas fa-exclamation-triangle'></i> Salva Modifiche Ora!")}function Bl(t,e){let o=document.getElementById(t);if(!o)return;let i=null;o.addEventListener("dragstart",function(c){if(!(o.querySelector(".dnd-handle, .drag-handle")&&!c.target.closest(".dnd-handle, .drag-handle"))){if(i=c.target.closest('[draggable="true"]'),!i||!o.contains(i)){i=null;return}i.classList.add("dnd-dragging"),c.dataTransfer.effectAllowed="move",c.dataTransfer.setData("text/plain","")}}),o.addEventListener("dragover",function(c){if(c.preventDefault(),!i)return;let l=c.target.closest('[draggable="true"]');if(!l||l===i||!o.contains(l))return;let d=l.getBoundingClientRect();c.clientY<d.top+d.height/2?o.insertBefore(i,l):o.insertBefore(i,l.nextSibling)}),o.addEventListener("dragend",function(c){i&&(i.classList.remove("dnd-dragging"),e&&e(o)),i=null}),o.addEventListener("drop",function(c){c.preventDefault(),c.stopPropagation()});let n=null,a=null,r=0,s=0;o.addEventListener("touchstart",function(c){let l=!!o.querySelector(".dnd-handle, .drag-handle"),d=c.target.closest('[draggable="true"]');if(!d||!o.contains(d)||l&&!c.target.closest(".dnd-handle, .drag-handle"))return;n=d;let p=c.touches[0],f=We(d,p.clientX,p.clientY,{opacity:.85,borderRadius:"14px",shadow:"0 10px 30px rgba(0,0,0,0.25)"});a=f.ghost,r=f.offX,s=f.offY,d.style.opacity="0.25",d.style.transform="scale(0.97)"},{passive:!0}),o.addEventListener("touchmove",function(c){if(!n||!a)return;c.preventDefault();let l=c.touches[0];Ze(a,l.clientX,l.clientY,r,s);let d=Yn(a,l.clientX,l.clientY,'[draggable="true"]');if(d&&d!==n&&o.contains(d)){let p=d.getBoundingClientRect();o.insertBefore(n,l.clientY<p.top+p.height/2?d:d.nextSibling)}},{passive:!1}),o.addEventListener("touchend",function(){n&&(n.style.opacity="",n.style.transform="",Qe(a),a=null,e&&e(o),n=null)})}async function Ul(){try{let e=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"salva_impostazioni_globali",sessionToken:window._getSessionToken_(),stati:window.listaStati||[],operatori:[],distintaHeaderAzienda:String(window._distintaHeaderAzienda||"").trim()})})).json().catch(()=>({}));if(e.status==="success"){if(window.listaStatiFornitori&&window.listaStatiFornitori.length)try{await fetch(I,{method:"POST",body:JSON.stringify({azione:"saveStatiFornitoriConfig",stati:window.listaStatiFornitori})})}catch{}g("Impostazioni salvate correttamente!"),window.modifichePendenti=!1;let o=document.getElementById("btn-salva-globale");o&&(o.style.background="",o.innerHTML="<i class='fas fa-save'></i> Salva Impostazioni"),U("_impostazioni_cache"),U("_impostazioni_stati_forn_cache"),window.cacheContenuti&&Object.keys(window.cacheContenuti).forEach(i=>delete window.cacheContenuti[i]),Object.keys(localStorage).filter(i=>i.startsWith("_html_")).forEach(i=>localStorage.removeItem(i)),await no()}else g("Errore: "+(e.message||"risposta inattesa dal server"),"error")}catch{g("Errore nel salvataggio.","error")}}function Ca(){window._avatarStartAdd=Xc,window._avatarConfirmEdit=ol,window._avatarCancelEdit=il,window._avatarDeleteEdit=nl,window._avatarRipristinaPredefiniti=al,window._renderPredefinedSwatches=ci,window._renderCustomSwatches=Ft,window._applyAvatarColorUI=Aa,window._setAvatarColor=pi,window._avatarStartAddMob=sl,window._avatarConfirmEditMob=cl,window._avatarCancelEditMob=ll,window._avatarDeleteEditMob=dl,window.toggleAccountMenu=ul,window.chiudiAccountMenu=pl,window._aggiornaPagina=ml,window.toggleAccountMenuMobile=fl,window.chiudiAccountMenuMobile=gl,window._vapidB64ToUint8_=fi,window._mostraDiagnosticaPush=vl,window._forzaRiregistraPush=hl,window._testPushNotifica=bl,window._togglePushPermission=Sl,window._aggiornaUINotifiche=mi,window._onNotifPrefChange=wl,window._getNotifPrefs=oo,window.importaCSVDaFile=_l,window.importaListaDiCaricoDaFile=xl,window.caricaInterfacciaImpostazioni=Ht,window.caricaDatiIniziali=Oe,window._fetchImpostazioniDaServer=no,window.toggleSettingsSection=Il,window.caricaListaUtenti=Ce,window.salvaModificheUtente=Al,window.apriFormNuovoUtente=Ol,window.salvaUtenteNuovo=Cl,window.eliminaUtente=$l,window._caricaSessionStats_=gi,window._revocaSessioniUtenteDaUI=Rl,window._revocaTutteSessioniDaUI=kl,window._aggiornaDiagnosticaSync=Ll,window._forceRevisionBump=Pl,window._svuotaCacheLocale=zl,window.azioneEliminaStato=Ml,window.azioneAggiungiStato=Nl,window.azioneEliminaStatoFornitori=Dl,window.azioneAggiungiStatoFornitori=ql,window.segnaModifica=se,window.salvaTutteImpostazioni=Ul,window.apriScannerQR=oa,window._chiudiScannerQR=Xe,window._processaQR=to,window._chiudiModaleQRAzione=ii,window._qrFiltroOrdini=ia,window._qrSelezionaOrdine=na,window._qrScegliStato=aa,window._qrSelezionaTutti=sa,window._qrDeselezionaTutti=ra,window._confermaSpostaPostazione=ca,window._qrApriModalNuova=la,window._qrApriModalModifica=da,window._qrChiudiModalEdit=ni,window._qrAggiornaCodice=ai,window._qrRicalcolaCodice=pa,window._qrSalvaPostazione=ma,window._qrEliminaPostazione=fa,window._qrStampaSingola=va,window._qrStampaSingolaIdx=ha,window._qrStampaTutte=ba,window._qrAggiornaPrevQR=eo}function $a(){Xn(),document.addEventListener("click",function(t){let e=document.getElementById("account-dropdown"),o=document.getElementById("user-avatar-btn");e&&e.classList.contains("open")&&!e.contains(t.target)&&t.target!==o&&!o.contains(t.target)&&e.classList.remove("open")}),document.addEventListener("click",function(t){let e=document.getElementById("account-dropdown-mobile"),o=document.getElementById("user-avatar-btn-mobile");!e||!o||e.classList.contains("open")&&!e.contains(t.target)&&t.target!==o&&!o.contains(t.target)&&e.classList.remove("open")}),document.addEventListener("visibilitychange",function(){document.visibilityState==="hidden"&&Xe()})}var H,Ta=V(()=>{lt();Tt();ut();dt();Jt();wt();Ke();ya();H=null});function re(){return[...S.ovStatiArt,...S.ovStatiOrd]}function gt(t){let e=String(t||"").toUpperCase().trim();return e==="IMBALLATO"||e==="SPEDITO/CONSEGNATO"||e==="SPEDITO"||e==="CONSEGNATO"}var S,ao=V(()=>{S={ultimiDatiProduzione:null,pollProdTimer:null,POLL_PROD_MS:1e4,lastKanbanDragTs:0,mutationInFlight:0,mutationLastDone:0,prodCacheInvalidateTimer:null,attiviProd:[],ordiniAutocompleteCache:[],ovStatiArt:["PREPARARE","MANDA IN LAVORAZIONE","IN LAVORAZIONE","TORNATO DALLA LAVORAZIONE"],ovStatiOrd:["IN PRODUZIONE","IMBALLATO"],datiArchLazy:null}});function jt(t,e){if(!t||t.length===0)return"";let o=window.TW,i={};t.forEach(r=>{if(!e&&String(r.archiviato).toUpperCase()==="TRUE")return;let s=r.ordine||"N.D.";i[s]||(i[s]=[]),i[s].push(r)});let n="";return Object.keys(i).sort((r,s)=>{let c=i[r].some(m=>String(m.last_modified_by||"").startsWith("CSV_REVIEW")),l=i[s].some(m=>String(m.last_modified_by||"").startsWith("CSV_REVIEW"));if(c&&!l)return-1;if(!c&&l)return 1;let d=(i[r][0].cliente||"").trim().toUpperCase(),p=!d||d==="DA DEFINIRE"?(i[r][0].riferimento||r).toUpperCase():d,f=(i[s][0].cliente||"").trim().toUpperCase(),u=!f||f==="DA DEFINIRE"?(i[s][0].riferimento||s).toUpperCase():f;return p<u?-1:p>u?1:r<s?-1:r>s?1:0}).forEach(r=>{let s=i[r],c=s[0].cliente,l=s[0].riferimento||"",d=l?`<span class="riferimento-label">(${y(l)})</span>`:"",p=e?"archivio-wrapper":"",u=s.some(_=>String(_.last_modified_by||"").startsWith("CSV_REVIEW"))?" csv-review-order":"",m=e?"archivio-header":"",b=e?"#475569":"inherit",w;if(r.includes("/")){let _=r.indexOf("/"),$=r.substring(0,_),P=r.substring(_+1),N=P.length>3?P.substring(0,3)+".":P;w=`${$}/${N}`}else w=r.length>14?r.substring(0,14)+"\u2026":r;let v="";if(!e)if(window._isUtenteEsente()){let _=[...new Set(s.flatMap(N=>!gt(N.stato)&&N.assegna&&N.assegna!==""&&N.assegna!=="undefined"?N.assegna.split(",").map(B=>window._normNome(B.trim())).filter(Boolean):[]))],$=_.length?_.map(window._normNome).join(", "):"Libero",P=window.listaOperatori.map(N=>{let B=_.some(ys=>ys.toUpperCase()===window._normNome(N.nome).toUpperCase()),nt=window._getOpColor(N.nome.trim()),Lt=N.nome.trim().replace(/'/g,"\\'"),bs=r.replace(/'/g,"\\'");return`<button type="button" class="op-option${B?" is-selected":""}" onclick="selezionaOpAssegnaOrdine(this,'${bs}','${Lt}')"><span class="op-opt-dot" style="background:${nt}"></span><span>${window._normNome(N.nome)}</span>${B?'<i class="fas fa-check op-check-icon"></i>':""}</button>`}).join("");v=`<div class="op-dropdown op-dropdown-ord" data-nord="${r}" data-assegna-ord="${_.join(",").replace(/"/g,"&quot;")}"><button type="button" class="op-trigger op-trigger-ord" onclick="event.stopPropagation(); toggleOpDropdown(this)"><i class="fas fa-user-tag op-icon"></i><span class="op-trigger-label">${$}</span><i class="fas fa-chevron-down op-chevron"></i></button><div class="op-popup">${P}</div></div>`}else{let _=(h?.nome||"").toUpperCase().trim();s.some(P=>!gt(P.stato)&&P.assegna&&P.assegna.split(",").some(N=>N.trim().toUpperCase()===_))||(v=`<button class="btn-assegnami btn-assegnami-ord" onclick="event.stopPropagation(); autoAssegnamiOrdine('${r.replace(/'/g,"\\'")}')" title="Assegnami a tutto l'ordine"><i class="fas fa-user-plus"></i></button>`)}let x=r.replace(/'/g,"\\'"),O=(c||"").replace(/'/g,"\\'"),E=s[0].id_riga,T="";if(!e){let _=s.map(B=>String(B.stato||"IN ATTESA").toUpperCase().trim()).filter((B,nt,Lt)=>Lt.indexOf(B)===nt),$=_.length===1?_[0]:`${_.length} Stati`,P=window.listaStati.find(B=>B.nome===_[0])||{colore:"#e2e8f0"},N=window.listaStati.map(B=>`<button type="button" class="stato-option" onclick="event.stopPropagation(); selezionaStatoOrdine(this,'${r.replace(/'/g,"\\'")}','${B.nome}','${B.colore}')"><span class="stato-opt-dot" style="background:${B.colore}"></span><span>${B.nome}</span></button>`).join("");T=`<div class="stato-dropdown stato-dropdown-ord" data-nord="${r}"><button type="button" class="stato-trigger" onclick="event.stopPropagation(); toggleStatoDropdown(this)" title="Cambia stato tutte righe"><span class="stato-dot" style="background:${P.colore}"></span><span class="stato-label-txt">${$}</span><i class="fas fa-chevron-down stato-chevron"></i></button><div class="stato-popup">${N}</div></div>`}let k=`apriModalAiuto('${E}', 'INTERO ORDINE', '${x}', '${O}')`,R=`gestisciArchiviazione('${x}')`,C=`apriModalSollecito('','${x}','${O}','Intero Ordine')`,q=`gestisciRipristino('${x}', 'ORDINE')`,G="";e?G=`<button class="ord-menu-item" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${q}"><i class="fa-solid fa-rotate-left"></i> Ripristina</button>`:(G+=`<button class="ord-menu-item" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${k}"><i class="fa-regular fa-envelope"></i> Chiedi</button>`,G+=`<button class="ord-menu-item ord-menu-item--danger" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${R}"><i class="fa-solid fa-box-archive"></i> Archivia</button>`,(window._isCommerciale()||window._isUtenteEsente())&&(G+=`<button class="ord-menu-item ord-menu-item--warn" onclick="event.stopPropagation();chiudiTuttiMenuAzioni();${C}"><i class="fa-solid fa-calendar-alt"></i> Scadenza</button>`));let ct=e?"":`${v}${T}`,tt=`<div class="ord-azioni-menu" onclick="event.stopPropagation()">
    ...
    <div class="item-card archivio-layout ${o.card}">
        ...
        </details>`,i.size&&o.querySelectorAll(".ordine-wrapper").forEach(d=>{if(i.has(d.dataset.ordine)){let p=d.querySelector(".riga-ordine"),f=d.querySelector(".dettagli-container");p&&f&&(p.classList.add("open"),f.style.display="block")}}),F["PROGRAMMA PRODUZIONE DEL MESE"]=o.innerHTML,Q["PROGRAMMA PRODUZIONE DEL MESE"]=Date.now(),window.aggiornaListaFiltrabili(),requestAnimationFrame(It)}function Ma(t,e){let o=document.getElementById("ov-kanban-grid");if(!o)return;let i=(e||"").toUpperCase().trim(),n=o.querySelector(`.ov-kanban-item[data-id-riga="${t}"]`);if(n||o.querySelectorAll(".ov-kanban-item").forEach(s=>{(s.dataset.idRighe||"").split(",").map(c=>c.trim()).includes(String(t))&&(n=s)}),!n||(n.dataset.statoCorrente||"").toUpperCase().trim()===i)return;let a=o.querySelector(`.ov-stato-body[data-stato-drop="${i}"]`);if(!a)return;a.querySelectorAll(".ov-empty-lbl").forEach(s=>s.remove()),n.dataset.statoCorrente=i,n.style.transition="opacity 0.18s, transform 0.18s",n.style.opacity="0",n.style.transform="scale(0.92)",a.appendChild(n);let r=a.closest(".ov-stato-card");r&&(r.open=!0),so(o),ro(o),requestAnimationFrame(()=>{n.style.opacity="1",n.style.transform="",setTimeout(()=>{n.style.transition=""},200)})}function ld(){window.filtroRicercaArticoli=!window.filtroRicercaArticoli,document.querySelectorAll(".btn-filtro-articoli").forEach(n=>{n.classList.toggle("active",window.filtroRicercaArticoli)});let t=window.filtroRicercaArticoli?"Cerca codice articolo...":"Cerca in tutte le pagine...",e=window.filtroRicercaArticoli?"Cerca articolo":"Cerca",o=document.getElementById("universal-search"),i=document.getElementById("mobile-search");o&&(o.placeholder=t),i&&(i.placeholder=e),document.querySelectorAll(".item-card.hidden-search").forEach(n=>n.classList.remove("hidden-search")),window.filtraUniversale()}function Ii(t){let e=t==="PROGRAMMA PRODUZIONE DEL MESE";if(document.querySelectorAll(".btn-filtro-articoli").forEach(o=>{o.style.display=e?"flex":"none"}),!e&&window.filtroRicercaArticoli){window.filtroRicercaArticoli=!1,document.querySelectorAll(".btn-filtro-articoli").forEach(n=>n.classList.remove("active"));let o=document.getElementById("universal-search"),i=document.getElementById("mobile-search");o&&(o.placeholder="Cerca in tutte le pagine..."),i&&(i.placeholder="Cerca"),document.querySelectorAll(".item-card.hidden-search").forEach(n=>n.classList.remove("hidden-search"))}}async function dd(t,e){e&&(e.disabled=!0,e.innerHTML='<i class="fas fa-spinner fa-spin"></i>');try{let i=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"csvReviewResolve",id_riga:t,mittente:h&&h.nome?h.nome.toUpperCase():""})})).json();if(i&&i.status==="auth_error"){window._gestisciAuthError_(i.message);return}if(i&&i.status==="ok"){let n=e?e.closest(".item-card"):null;if(n){n.classList.remove("csv-review-blink","csv-review-missing","csv-review-finish");let a=n.querySelector(".csv-review-banner");a&&a.remove()}if(S.attiviProd){let a=S.attiviProd.find(r=>String(r.id_riga)===String(t));a&&(a.last_modified_by=h&&h.nome?h.nome.toUpperCase():"")}if(n){let a=n.closest(".ordine-wrapper");a&&!a.querySelector(".csv-review-blink")&&a.classList.remove("csv-review-order")}it(),g("\u2713 Riga risolta","success")}else g("\u26A0\uFE0F Errore risoluzione \u2014 riprova","error"),e&&(e.disabled=!1,e.innerHTML='<i class="fas fa-check"></i> Risolvi')}catch(o){console.error("[csvReviewResolve]",o),g("\u26A0\uFE0F Errore rete \u2014 riprova","error"),e&&(e.disabled=!1,e.innerHTML='<i class="fas fa-check"></i> Risolvi')}}function Na(){window.toggleAccordion=td,window.toggleStatoDropdown=Vl,window.selezionaStato=Yl,window.selezionaStatoOrdine=Xl,window.toggleOpDropdown=Gl,window.selezionaOpAssegna=Wl,window.selezionaOpAssegnaOrdine=Zl,window.autoAssegnami=Ql,window.autoAssegnamiOrdine=Kl,window.rimuoviOperatore=jl,window.gestisciArchiviazione=od,window.gestisciRipristino=ad,window.toggleQtyEvasa=id,window.aggiornaRimanente=Pa,window.salvaQtyEvasa=nd,window.chiudiTuttiMenuAzioni=_i,window.toggleMenuAzioni=Jl,window.aggiornaDato=Ei,window.toggleFiltroArticoli=ld,window._aggiornaVisibilitaFiltroArticoli=Ii,window._ovLoadIfNeeded=Ra,window._apriArchivio=ka,window._scrollToOrdineList=bi,window._initKanbanDnd=It,window._startPollingProduzione=fo,window._stopPollingProduzione=pe,window._pollProdStep=xi,window._repaintOpColors=bt,window.caricaDati=mo,window.caricaArchivio=ue,window._syncKanbanFromStato=Ma,window._setAssegnaLocalByOrdine=Si,window._refreshOverview=kt,window._invalidateProduzioneCache=it,window.csvReviewResolve=dd}function Da(){document.addEventListener("click",function(t){t.target.closest(".ord-azioni-menu")||_i()}),document.addEventListener("click",function(t){t.target.closest(".op-dropdown")||document.querySelectorAll(".op-dropdown.open").forEach(e=>{e.classList.remove("open");let o=e.closest(".item-card");o&&o.classList.remove("op-aperto");let i=e.closest(".riga-ordine");i&&i.classList.remove("op-aperto-ord")})},!0),document.addEventListener("click",function(t){!t.target.closest(".stato-dropdown")&&!t.target.closest(".stato-dropdown-ord")&&document.querySelectorAll(".stato-dropdown.open, .stato-dropdown-ord.open").forEach(e=>{e.classList.remove("open");let o=e.closest(".item-card");o&&o.classList.remove("stato-aperto");let i=e.closest(".riga-ordine");i&&i.classList.remove("stato-aperto-ord")})},!0),document.addEventListener("visibilitychange",function(){document.visibilityState==="visible"&&window.paginaAttuale==="PROGRAMMA PRODUZIONE DEL MESE"&&xi()})}var yi,co,qa=V(()=>{lt();Tt();dt();ut();Jt();wt();Vt();Ke();ao();vi();La();yi=!1,co=null});async function Ai(t){let e=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(t));return Array.from(new Uint8Array(e)).map(o=>o.toString(16).padStart(2,"0")).join("")}function Ba(){return Date.now()<vo}function ud(){go++,go>=5&&(vo=Date.now()+3e4,go=0)}function pd(){go=0,vo=0}async function Ua(){let t=document.getElementById("login-error");if(t.innerText="",t.style.color="",document.getElementById("login-view-admin")?.style.display!=="none"){let r=(document.getElementById("login-codice")?.value||"").trim();t.innerText="Usa il pulsante Entra per accedere come admin.";return}if(Ba()){let r=Math.ceil((vo-Date.now())/1e3);t.innerText="Troppi tentativi. Riprova tra "+r+" secondi.";return}let o=(document.getElementById("login-email")?.value||"").trim().toLowerCase(),i=(document.getElementById("login-username")?.value||"").trim(),n=document.getElementById("login-password")?.value||"";if(!o||!i||!n){t.innerText="Compila tutti i campi: email, nome utente e password.";return}let a=document.getElementById("btn-login");a.disabled=!0,a.innerHTML='<i class="fas fa-spinner fa-spin"></i> Verifica...';try{let r=await Ai(n),c=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"verificaLogin",email:o,username:i,hash:r})})).json();c.status==="success"?(pd(),Ct({nome:c.nome,ruolo:c.ruolo,email:c.email,vistaSimulata:c.nome,sessionToken:c.sessionToken||"",sessionExpiresAt:c.sessionExpiresAt||""}),window.salvaEApriDashboard()):(ud(),Ba()?t.innerText="Troppi tentativi. Riprova tra 30 secondi.":t.innerText=c.message||"Credenziali non valide.")}catch{t.innerText="Errore di connessione. Riprova."}a.disabled=!1,a.innerHTML='Entra nel Sistema <i class="fas fa-arrow-right"></i>'}async function md(){let t=document.getElementById("login-error");t&&(t.innerText=""),t&&(t.style.color="");let e=(document.getElementById("login-email")?.value||"").trim().toLowerCase(),o=(document.getElementById("login-username")?.value||"").trim(),i=document.getElementById("login-password")?.value||"";if(!e||!o||!i){t&&(t.innerText="Per creare l'account compila email, nome utente e password.");return}let n=document.getElementById("btn-login"),a=document.getElementById("btn-signup"),r=n?n.innerHTML:"",s=a?a.innerHTML:"";n&&(n.disabled=!0),a&&(a.disabled=!0,a.innerHTML='<i class="fas fa-spinner fa-spin"></i> Creazione...');try{let c=await Ai(i),d=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"creaUtentePubblico",email:e,username:o,hash:c})})).json();d.status==="success"?(t&&(t.style.color="#22c55e"),t&&(t.innerText="Account creato. Accesso in corso..."),await Ua()):(t&&(t.style.color=""),t&&(t.innerText=d.message||"Impossibile creare l'account."))}catch{t&&(t.style.color=""),t&&(t.innerText="Errore di connessione. Riprova.")}finally{n&&(n.disabled=!1,n.innerHTML=r||'Entra nel Sistema <i class="fas fa-arrow-right"></i>'),a&&(a.disabled=!1,a.innerHTML=s||'<i class="fas fa-user-plus"></i> Nuovo utente? Crea account')}}function Fa(){window.hashSHA256=Ai,window._verificaAccessoUtente=Ua,window._creaAccountUtente=md}var go,vo,Ha=V(()=>{lt();ut();go=0,vo=0});function Ci(t){if(!t)return 0;if(t instanceof Date)return t.getTime();let e=String(t).trim();if(!e)return 0;let o=e.match(/^(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?\s+(\d{1,2}):(\d{2})(?::(\d{2}))?$/);if(o){let n=parseInt(o[1],10),a=parseInt(o[2],10)-1,r=parseInt(o[3]||String(new Date().getFullYear()),10);r<100&&(r+=2e3);let s=parseInt(o[4],10),c=parseInt(o[5],10),l=parseInt(o[6]||"0",10);return new Date(r,a,n,s,c,l).getTime()}let i=new Date(e);return Number.isFinite(i.getTime())?i.getTime():0}function $i(t){let e=Date.now();return(t||[]).filter(function(o){let i=Ci(o&&o._ts);return i?e-i<=fd:!0})}function gd(t){t&&(t.stopPropagation(),t.preventDefault());let e=document.getElementById("modal-notifiche");if(e){e.classList.add("is-open"),Sd();try{localStorage.setItem("_notifLastRead",String(Date.now()))}catch{}try{localStorage.setItem("_notifBadgeCount","0")}catch{}Ti(0),h&&h.nome&&fetch(I,{method:"POST",body:JSON.stringify({azione:"segnaLetteNotifiche",username:h.nome.toUpperCase()})}).catch(function(){})}}function Ga(){let t=document.getElementById("modal-notifiche");t&&t.classList.remove("is-open")}async function vd(t,e,o,i){let n=i?i.closest(".notif-azioni-accesso"):null;n&&(n.querySelectorAll("button").forEach(function(a){a.disabled=!0}),n.innerHTML='<span class="notif-risposta-wait">\u23F3 Invio in corso\u2026</span>');try{let r=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"rispondiAccessoFuoriOrario",id:t,ok:o,json:1})})).json();if(r.status==="ok"){let s=o==="SI"?"\u2705 Accesso consentito":"\u{1F6AB} Accesso negato";hd(t,s),n&&(n.innerHTML='<span class="notif-risposta-ok">'+Gt(s)+"</span>")}else n&&(n.innerHTML='<span class="notif-risposta-err">\u26A0\uFE0F '+Gt(r.msg||"Errore")+"</span>")}catch{n&&(n.innerHTML='<span class="notif-risposta-err">\u26A0\uFE0F Errore di rete</span>')}}function Va(){try{return JSON.parse(localStorage.getItem("_accRispIdx_")||"{}")}catch{return{}}}function hd(t,e){try{let o=Va();o[t]=e,localStorage.setItem("_accRispIdx_",JSON.stringify(o))}catch{}}async function bd(t,e,o,i){let n=decodeURIComponent(e||""),a=decodeURIComponent(o||""),r=String(t||"").trim(),s=i?i.closest(".notifica-item"):null;s&&(s.classList.add("notif-removing"),await new Promise(function(l){setTimeout(l,190)})),yd(r,n,a),s&&s.remove();let c=document.getElementById("notifiche-list");c&&!c.querySelector(".notifica-item")&&(c.innerHTML=Oi([]));try{let l=h&&h.nome?h.nome.toUpperCase():"";if(!l)return;let p=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"eliminaNotifica",username:l,rid:r,titolo:n,corpo:a})})).json().catch(()=>({}));p.status!=="ok"&&p.status!=="not_found"&&console.warn("[notifiche] eliminaNotifica non ok:",p)}catch(l){console.warn("[notifiche] eliminaNotifica errore rete:",l)}}function yd(t,e,o){try{let i=JSON.parse(localStorage.getItem("_notificheArr")||"[]"),n=!1,a=i.filter(function(r){if(n)return!0;let s=t&&String(r.rid||"")===String(t),c=String(r.titolo||"")===String(e||"")&&String(r.corpo||"")===String(o||"");return s||c?(n=!0,!1):!0});localStorage.setItem("_notificheArr",JSON.stringify(a))}catch{}}function Ti(t){let e=document.getElementById("badge-notifiche-desktop"),o=document.getElementById("badge-notifiche-mobile"),i=document.getElementById("badge-notifiche-mobile-menu");e&&(e.textContent=t>0?t:"",e.style.display=t>0?"flex":"none"),o&&(o.textContent=t>0?t:"",o.style.display=t>0?"flex":"none"),i&&(i.textContent=t>0?t:"",i.style.display=t>0?"flex":"none")}function wd(t){return t?/stato/i.test(t)?"fa-rotate":/richiesta|comunic/i.test(t)?"fa-comment-dots":/assegnaz/i.test(t)?"fa-user-check":"fa-bell":"fa-bell"}function Oi(t){return t.length?t.map(function(e,o){let i=wd(e.titolo||""),n=Gt(e.titolo||"Notifica"),a=Gt(e.rid||""),r=encodeURIComponent(e.titolo||""),s=encodeURIComponent(e.corpo||""),c=Gt(e._ts||""),l=c?`<span class="notifica-ts">${c}</span>`:"",d="";try{let p=JSON.parse(e.corpo||"");if(p&&p.tipo==="accesso_richiesta"){let f=encodeURIComponent(p.id||""),u=encodeURIComponent(p.nome||""),m=Va();m[p.id]?d=`<div class="notifica-corpo"><span class="notif-risposta-ok">${Gt(m[p.id])}</span></div>`:d=`<div class="notifica-corpo">Vuole entrare fuori orario.</div>
        ...
        </div>`}).join(""):'<div class="notif-empty"><i class="far fa-bell-slash"></i><p>Nessuna notifica recente</p></div>'}function Sd(){let t=document.getElementById("notifiche-list");if(!t)return;let e=$i(JSON.parse(localStorage.getItem("_notificheArr")||"[]"));try{localStorage.setItem("_notificheArr",JSON.stringify(e))}catch{}t.innerHTML=Oi(e),h&&h.nome&&fetch(I,{method:"POST",body:JSON.stringify({azione:"getStoricoNotifiche",username:h.nome.toUpperCase(),days:ja})}).then(function(o){return o.json()}).then(function(o){o&&o.status==="ok"&&o.all&&o.all.length&&(ho(o.all),t.innerHTML=Oi(JSON.parse(localStorage.getItem("_notificheArr")||"[]")))}).catch(function(o){console.warn("[notifiche] renderNotificheList fetch fallito:",o)})}function ho(t){try{let e=new Date().toLocaleString("it-IT",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}),o=t.map(function(l){return Object.assign({},l,{_ts:l._ts||e})}),i=JSON.parse(localStorage.getItem("_notificheArr")||"[]"),n={};o.forEach(function(l){n[l.titolo+"||"+l.corpo]=l}),i.forEach(function(l){let d=l.titolo+"||"+l.corpo;n[d]||(n[d]=l)});let a=$i(Object.values(n)).slice(0,200);localStorage.setItem("_notificheArr",JSON.stringify(a));let r=parseInt(localStorage.getItem("_notifLastRead")||"0"),c=o.filter(function(l){let d=Ci(l._ts);return!d||d>r}).length;try{localStorage.setItem("_notifBadgeCount",String(c))}catch{}Ti(c)}catch{}}function _d(){if(!h||!h.nome)return!1;let t=String(h.nome).toUpperCase().trim();return t==="ALESSIO"||t==="0000"||h.ruolo==="MASTER"}function Ed(){let t=new Date,e=t.getHours()*60+t.getMinutes();return e>=540&&e<1170}function xd(t){if(!(!t||t<=0)&&!(!h||!h.nome)&&!_d()&&Ed()){try{let e=new Date().toLocaleDateString("it-IT"),o="_notifMorningToast_"+String(h.nome).toUpperCase().trim()+"_"+e;if(localStorage.getItem(o)==="1")return;localStorage.setItem(o,"1")}catch{}g("\u{1F514} Hai "+t+" notific"+(t===1?"a":"he")+" da leggere")}}function Ri(){try{let t=$i(JSON.parse(localStorage.getItem("_notificheArr")||"[]")),e=parseInt(localStorage.getItem("_notifLastRead")||"0"),o=t.filter(function(i){let n=Ci(i._ts);return!n||n>e});o.length>0&&Ti(o.length)}catch{}!h||!h.nome||fetch(I,{method:"POST",body:JSON.stringify({azione:"getNotifiche",username:h.nome.toUpperCase(),markRead:0})}).then(function(t){return t.json()}).then(function(t){if(t&&t.status==="ok"&&t.all&&t.all.length&&(ho(t.all),xd(t.all.length),t.titolo&&"serviceWorker"in navigator&&navigator.serviceWorker.controller))try{navigator.serviceWorker.controller.postMessage({type:"CACHE_NOTIF",titolo:t.titolo,corpo:t.corpo||""})}catch{}}).catch(function(t){console.warn("[notifiche] _initBadgeNotifiche fetch fallito:",t)})}function Id(t){var e=String(t&&t.titolo||""),o=String(t&&t.corpo||""),i=(e+" "+o).replace(/\s+/g," ").trim();if(!i)return"";var n=i.match(/\bORD(?:INE)?\.?\s*[:#-]?\s*([A-Z0-9/-]{2,})/i);if(n&&n[1])return String(n[1]).trim();var a=i.match(/\b([A-Z]{2,}[A-Z0-9]*-[A-Z0-9-]{2,})\b/i);return a&&a[1]?String(a[1]).trim():""}async function Ad(t,e){try{var o=String(e||"").trim();if(!o){var i=JSON.parse(localStorage.getItem("_notificheArr")||"[]"),n=i[Number(t)];if(!n)return;o=Id(n)}if(Ga(),!o){g("Nessun riferimento ordine/codice trovato in questa notifica");return}typeof window.cambiaPagina=="function"&&await window.cambiaPagina("PROGRAMMA PRODUZIONE DEL MESE",null),setTimeout(function(){["universal-search","mobile-search"].forEach(function(a){var r=document.getElementById(a);r&&(r.value=o,r.dispatchEvent(new Event("input")))}),typeof window.filtraUniversale=="function"&&window.filtraUniversale()},280)}catch(a){console.warn("[notifiche] apriDettaglioNotifica errore:",a)}}function Ja(){window.apriPopupNotifiche=gd,window.chiudiPopupNotifiche=Ga,window.eliminaNotificaApp=bd,window.rispondiAccessoApp=vd,window.apriDettaglioNotifica=Ad}var ja,fd,Gt,Wa=V(()=>{lt();ut();dt();ja=7,fd=ja*24*60*60*1e3;Gt=y});function yo(){let t=document.getElementById("user-name-display"),e=document.getElementById("user-avatar-icon"),o=document.getElementById("account-ddrop-avatar"),i=document.getElementById("account-ddrop-name"),n=document.getElementById("account-ddrop-role"),a=document.getElementById("user-avatar-icon-mobile"),r=document.getElementById("account-ddrop-avatar-mob"),s=document.getElementById("account-ddrop-name-mob"),c=document.getElementById("account-ddrop-role-mob");if(h&&h.nome){let l=h.nome.charAt(0).toUpperCase(),d=h.nome.toUpperCase();t&&(t.innerText=d),e&&(e.innerText=l),o&&(o.innerText=l),i&&(i.innerText=d),n&&(n.innerText=(h.ruolo||"Utente").toUpperCase()),a&&(a.innerText=l),r&&(r.innerText=l),s&&(s.innerText=d),c&&(c.innerText=(h.ruolo||"Utente").toUpperCase())}Cd()}function Ka(t){try{let e=String(t||"").toUpperCase().trim();return bo[e]?bo[e]:localStorage.getItem("avatarColor_"+e)||"#374151"}catch{return"#374151"}}function Li(t){if(!t)return t;let e=String(t).trim().toUpperCase();return Za[e]?Za[e]:String(t).trim().toLowerCase().replace(/(?:^|\s|\.)\S/g,o=>o.toUpperCase())}function Cd(){if(!h||!h.nome)return;let t=Ka(h.nome);window._renderCustomSwatches&&window._renderCustomSwatches(),window._applyAvatarColorUI&&window._applyAvatarColorUI(t)}async function Pi(){try{let t=await fetch(I,{method:"POST",body:JSON.stringify({azione:"getAvatarColors"})});if(!t.ok)return;let e=await t.json();if(typeof e!="object"||Array.isArray(e))return;if(Object.entries(e).forEach(([o,i])=>{if(!i)return;let n=o.toUpperCase().trim();bo[n]=i;try{localStorage.setItem("avatarColor_"+n,i)}catch{}}),h?.nome){let o=e[h.nome.toUpperCase().trim()];o&&window._applyAvatarColorUI&&window._applyAvatarColorUI(o)}typeof window._repaintOpColors=="function"&&window._repaintOpColors()}catch(t){console.warn("_caricaColoriAvatarDaServer:",t)}}function Ya(){return!!(h&&h.nome)}function $d(){return h?String(h.ruolo||"").toUpperCase()==="COMMERCIALE":!1}function Td(){let t=new Date,e=t.getHours()*60+t.getMinutes();return e>=510&&e<1170}function zi(t){return sessionStorage.getItem("_accesso_extra_")==="1"||Ya()||Td()?(Qa(),!0):(t!==!1&&wo(),!1)}function wo(){if(document.getElementById("_lock-screen_"))return;let t=h&&h.nome?h.nome:"",e=document.createElement("div");e.id="_lock-screen_",e.style.cssText=["position:fixed","top:0","left:0","width:100%","height:100%","background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%)","z-index:99999","display:flex","flex-direction:column","align-items:center","justify-content:center","gap:16px","color:#e2e8f0","font-family:inherit"].join(";");let o=t?`<div style="margin-top:8px;font-size:0.82rem;color:#64748b">
        ...
        <div id="_lock-stato_" style="font-size:0.82rem;color:#64748b;min-height:1.2em;text-align:center;max-width:260px"></div>`,document.body.appendChild(e)}function Qa(){let t=document.getElementById("_lock-screen_");t&&t.remove(),kd()}function kd(){ki&&(clearInterval(ki),ki=null),Rd=null}function Xa(){window._normNome=Li,window._PREDEFINED_AVATAR_COLORS=Od,window._avatarColorsCache=bo,window._getOpColor=Ka,window._isUtenteEsente=Ya,window._isCommerciale=$d}var bo,Za,Od,Rd,ki,ts=V(()=>{lt();ut();bo={};Za={ALESSIO:"Alessio",RICCARDO:"Riccardo",FABIO:"Fabio T.","FABIO T":"Fabio T.","FABIO T.":"Fabio T.",NICCOLO:"Niccol\xF2","NICCOLO'":"Niccol\xF2","NICCOL\xD2'":"Niccol\xF2",RAYMOND:"Raymond",SIMONE:"Simone",GIACOMO:"Giacomo"};Od=["#8fe45e","#6366f1","#f59e0b","#ec4899","#06b6d4","#f87171","#a78bfa","#34d399"];Rd=null,ki=null;setInterval(function(){h&&h.nome&&zi(!0)},60*1e3)});var Zd=ws(()=>{lt();Tt();ut();wt();Vt();Zi();Jt();dt();un();yn();zn();Kn();Ta();qa();Ha();Wa();ts();try{let t=document.getElementById("critical-init");t&&t.remove()}catch{}"serviceWorker"in navigator&&navigator.serviceWorker.addEventListener("controllerchange",()=>{window.location.reload()});"serviceWorker"in navigator&&navigator.serviceWorker.addEventListener("message",function(t){if(t.data&&t.data.type==="NUOVE_NOTIFICHE"){ho(t.data.notifiche||[]);var e=t.data.notifiche||[];e.length>0&&e[0].titolo&&"caches"in window&&caches.open("prod-last-notif").then(function(i){i.put("last",new Response(JSON.stringify({titolo:e[0].titolo,corpo:e[0].corpo||""})))}).catch(function(){});return}if(t.data&&t.data.type==="OPEN_CSV_MODAL"){typeof window.cambiaPagina=="function"&&window.cambiaPagina("IMPOSTAZIONI",null).catch(function(){});return}if(t.data&&t.data.type==="OPEN_NOTIFICATION_TARGET")try{var o=String(t.data.target||"").trim();o&&typeof window.apriDettaglioNotifica=="function"&&window.apriDettaglioNotifica(-1,o)}catch{}});var es=!1,me=null,fe=null,So=0,Te=0,os=0,Mi=!1,Ld=0,is=0;function Pd(){let t=h?.sessionExpiresAt;if(!t)return 0;let e=Number(t);if(Number.isFinite(e)&&e>0)return e;let o=new Date(t).getTime();return Number.isFinite(o)?o:0}function ns(){let t=Pd();if(!t)return;let e=t-Date.now();if(e<=0||e>1440*60*1e3||Date.now()-is<180*1e3)return;is=Date.now();let o=Math.max(1,Math.floor(e/36e5));g("Sessione in scadenza tra circa "+o+" ore. Rientra per rinnovarla.","warning")}function Di(t){Ld=Number(t)||Date.now()}function Io(){return Hi()}function zd(){ji()}function ds(){if(es||typeof window.fetch!="function")return;let t=window.fetch.bind(window);function e(o){return zd(),o.clone().text().then(function(i){try{let n=JSON.parse(i);n&&n.status==="auth_error"&&ps(n.message||n.msg||"Sessione scaduta."),n&&n.status==="fuori_orario"&&typeof wo=="function"&&wo()}catch{}}).catch(function(){}),o}window.fetch=function(o,i){try{let n=Io(),a=typeof o=="string"?o:o&&o.url?o.url:"";if(!a||a.indexOf(I)!==0)return t(o,i);let r=String(i&&i.method||"GET").toUpperCase();if(r==="GET")return t(o,i).then(e);if(r==="POST"&&i&&typeof i.body=="string")try{let s=JSON.parse(i.body||"{}");if(n&&!s.sessionToken){s.sessionToken=n;let c=Object.assign({},i,{body:JSON.stringify(s)});return t(o,c).then(e)}}catch{}return t(o,i).then(e)}catch{}return t(o,i)},es=!0}ds();async function Ao(){let t=Io();if(!t)return!1;let e=h||null;if(!e)try{let o=localStorage.getItem("sessioneUtente")||sessionStorage.getItem("sessioneUtente");e=o?JSON.parse(o):null}catch{}try{let i=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"refreshSession",sessionToken:t,username:e&&e.nome?String(e.nome):"",email:e&&e.email?String(e.email):""})})).json();if(i&&i.status==="success"&&i.sessionToken){So=0,Te=0,h||Ct({}),h.sessionToken=i.sessionToken,h.sessionExpiresAt=i.sessionExpiresAt||"",h.expiresAt=Date.now()+2592e6,!h.nome&&i.nome&&(h.nome=i.nome),!h.email&&i.email&&(h.email=i.email),!h.ruolo&&i.ruolo&&(h.ruolo=i.ruolo);try{localStorage.setItem("sessioneUtente",JSON.stringify(h))}catch{}try{sessionStorage.setItem("sessioneUtente",JSON.stringify(h))}catch{}return!0}if(i&&i.status==="auth_error")return So++,So>=3&&(So=0,Re()),!1}catch{}return!1}function us(){me&&clearInterval(me),fe&&clearInterval(fe),me=setInterval(Ao,300*1e3),fe=setInterval(ns,60*1e3),ns()}window.addEventListener("storage",function(t){if(!(t.key!=="sessioneUtente"||!t.newValue))try{let e=JSON.parse(t.newValue);if(!e||!e.sessionToken)return;h||Ct({}),h.sessionToken=String(e.sessionToken),e.sessionExpiresAt&&(h.sessionExpiresAt=e.sessionExpiresAt)}catch{}});document.addEventListener("visibilitychange",function(){document.hidden||Ao()});var as=!1;async function ps(t){if(as)return;var e=document.getElementById("login-overlay");if(e&&e.style.display!=="none")return;let o=Date.now();if(o-os>3e4&&(Te=0),os=o,Te++,Te===1&&!Mi){Mi=!0;try{if(await Ao()){Te=0;return}}finally{Mi=!1}}as=!0,g(t||"Sessione scaduta. Effettua nuovamente il login.","error"),setTimeout(function(){Re()},2e3)}try{let t=localStorage.getItem("sessioneUtente")||sessionStorage.getItem("sessioneUtente");if(t){let e=JSON.parse(t);if(e&&e.sessionToken){let o=document.getElementById("login-overlay");o&&(o.style.display="none"),document.documentElement.classList.add("has-session")}}}catch{}var qi=null,Bi=!1;window.filtroRicercaArticoli=Bi;var Md=[];function Nd(){return[{nome:"PREPARARE",colore:"#94a3b8"},{nome:"PREPARARE PER LAVORAZIONE",colore:"#64748b"},{nome:"MANDA IN LAVORAZIONE",colore:"#475569"},{nome:"IN LAVORAZIONE",colore:"#f59e0b"},{nome:"TORNATO DALLA LAVORAZIONE",colore:"#7c3aed"},{nome:"IN PRODUZIONE",colore:"#242424"},{nome:"IMBALLATO",colore:"#22c55e"},{nome:"SPEDITO/CONSEGNATO",colore:"#06b6d4"}]}var Dd=[],ss=!1,qd=0,Ni=0,_o=null,rs=0,At=null,Ot=null,ms="BAHqp3uv56mQSAeTv_66-f4GYkzaESwuJNOP5DJCVMi197n-EKl9TW9XPrKeIIDpzBz0HTM42AcUCXWmOP5BSYI";async function fs(){if(!(!("serviceWorker"in navigator)||!("PushManager"in window)))try{let t=await navigator.serviceWorker.register("sw.js",{scope:"./"});await navigator.serviceWorker.ready,"caches"in window&&await(await caches.open("prod-auth")).put("username",new Response(h.nome.toUpperCase()));let e=await t.pushManager.getSubscription(),o=Notification.permission;if(!e&&o==="granted")try{e=await t.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:window._vapidB64ToUint8_?window._vapidB64ToUint8_(ms):null})}catch(a){console.warn("[Push] Auto-subscribe failed:",a);try{localStorage.setItem("_pushStato","errore-subscribe")}catch{}return}if(!e){try{localStorage.setItem("_pushStato","no-permesso")}catch{}return}let i=e.toJSON(),n=await gs({endpoint:i.endpoint,p256dh:i.keys?.p256dh,auth:i.keys?.auth});if(n&&(n.status==="saved"||n.status==="updated"))try{localStorage.setItem("_pushStato","ok")}catch{}else if(n&&n.status==="errore-verifica"){try{localStorage.setItem("_pushStato","errore-verifica")}catch{}g('\xE2\u0161\xA0\xEF\xB8\x8F Subscription creata ma NON confermata sul server. Riprova "Ri-registra subscription".',"error")}else try{localStorage.setItem("_pushStato","errore-salvataggio")}catch{}window._aggiornaUINotifiche&&window._aggiornaUINotifiche()}catch(t){console.warn("[Push] initPush:",t);try{localStorage.setItem("_pushStato","errore:"+t.message)}catch{}}}async function gs(t){try{let o=await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"salvaSottoscrizione",username:h.nome.toUpperCase(),endpoint:t.endpoint,p256dh:t.p256dh||"",auth:t.auth||""})})).json().catch(()=>({}));if(o&&(o.status==="saved"||o.status==="updated"))try{(await(await fetch(I,{method:"POST",body:JSON.stringify({azione:"verificaIscrizione",username:h.nome.toUpperCase(),endpoint:t.endpoint})})).json().catch(()=>({}))).found||(console.warn("[Push] verificaIscrizione: endpoint NON trovato nel foglio dopo il salvataggio!"),o.status="errore-verifica")}catch(i){console.warn("[Push] verificaIscrizione error:",i)}return o}catch(e){console.warn("[Push] _salvaSubVAPID_ error:",e)}}var xo=null,cs=null,Bd={card:"bg-white/90 border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow",cardGrid:"grid gap-3",label:"text-[10px] uppercase tracking-wide text-slate-500 font-semibold",value:"text-slate-900 font-semibold",btn:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-slate-200 bg-white hover:bg-slate-50 active:scale-[0.99] transition",btnPrimary:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 active:scale-[0.99] transition",btnSuccess:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 active:scale-[0.99] transition",btnWarning:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-orange-200 bg-orange-50 text-orange-700 hover:bg-orange-100 active:scale-[0.99] transition",btnDanger:"inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold border border-red-200 bg-red-50 text-red-600 hover:bg-red-100 active:scale-[0.99] transition",btnPrimaryLg:"inline-flex items-center gap-2 rounded-xl px-10 py-3.5 text-sm font-bold bg-slate-900 text-white hover:bg-slate-700 active:scale-[0.98] transition shadow-sm",pill:"inline-flex items-center justify-center rounded-full px-2 py-1 text-[10px] font-bold bg-slate-100 text-slate-600"};function Oo(){xo=document.querySelectorAll(".ordine-wrapper, .chat-card, .materiale-card, .manuale-card")}function vs(){if(typeof I>"u")return;U("_html__acq_ordini"),A.dashPromise=fetch(I,{method:"POST",body:JSON.stringify({azione:"getAllDashboard",includeArchivio:!1})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null}),A.rqPromise=fetch(I,{method:"POST",body:JSON.stringify({azione:"getAllRichieste"})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null}),A.matPromise=fetch(I,{method:"POST",body:JSON.stringify({pagina:"MATERIALE DA ORDINARE"})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null});let e=h?.nome?.toUpperCase().trim()==="ALESSIO"?"":h?.nome||"";A.ordiniPromise=fetch(I,{method:"POST",body:JSON.stringify({azione:"getOrdiniAcquisti",operatore:e})}).then(function(o){return o.ok?o.json():null}).catch(function(){return null}),A.dashPromise.then(function(o){o&&(A.dashBundle=o)}),A.rqPromise.then(function(o){o&&(A.rqBundle=o)}),A.matPromise.then(function(o){o&&(A.matBundle=o)}),A.ordiniPromise.then(function(o){o&&(A.ordiniBundle=o)})}function Ud(){let t=null;try{t=localStorage.getItem("ultimaPaginaProduzione")}catch{}(!t||t==="undefined"||t==="null")&&(t="PROGRAMMA PRODUZIONE DEL MESE"),delete F[t],Q[t]=0,U("_html_"+t)}function hs(){ds(),us(),At&&At.registerGlobals(),Ot&&Ot.registerGlobals(),Pn(),$a(),Da(),window.cambiaPagina=Eo,window.aggiornaListaFiltrabili=Oo,Qi({onRemoteChange:function(i){g("\u{1F504} "+i+" ha aggiornato i dati"),Di(Date.now()),Vi();try{D.clear()}catch{}switch(Object.keys(F).forEach(n=>{delete F[n]}),Object.keys(Q).forEach(n=>{Q[n]=0}),qi){case"PROGRAMMA PRODUZIONE DEL MESE":ge("PROGRAMMA_PRODUZIONE",uo,po,!0).catch(n=>console.warn("[RevisionPoller] refresh failed:",n));break;case"STORICO_RICHIESTE":Et();break;case"MATERIALE DA ORDINARE":Wt(null);break;case"MANUALI_PRODOTTI":Ee(null,null,!0);break;case"ARCHIVIO_ORDINI":typeof ue=="function"&&ue();break}},onUsersOnline:function(i){Gd(i)},getUtenteAttuale:function(){return h},getPaginaCorrente:function(){return qi}}),M.start(),Di(Date.now());let e=null;try{e=localStorage.getItem("ultimaPaginaProduzione")}catch{}(!e||e==="undefined"||e==="null")&&(e="PROGRAMMA PRODUZIONE DEL MESE");let o=document.querySelector(`.menu-item[data-page="${e}"]`);Eo(e,o).catch(i=>{i&&i.name!=="AbortError"&&console.warn("[init] cambiaPagina:",i)});try{if(new URLSearchParams(window.location.search).get("action")==="openCsvModal"){let a=window.location.pathname+window.location.hash;window.history.replaceState(null,"",a),setTimeout(function(){Eo("IMPOSTAZIONI",null).then(function(){setTimeout(function(){typeof window._apriCsvPendingModal_=="function"&&window._apriCsvPendingModal_()},300)}).catch(function(){})},400)}}catch{}}window.onload=async function(){if(ss)return;ss=!0;let t=document.getElementById("login-overlay"),e=null;try{e=localStorage.getItem("sessioneUtente")||sessionStorage.getItem("sessioneUtente")}catch{}if(e){if(Ct(JSON.parse(e)),h.expiresAt&&Date.now()>h.expiresAt){Ct(null);try{localStorage.removeItem("sessioneUtente"),sessionStorage.removeItem("sessioneUtente")}catch{}document.documentElement.classList.remove("has-session"),t&&(t.style.display="flex",t.style.opacity="1");let o=document.getElementById("login-error");o&&(o.innerText="Sessione scaduta. Effettua nuovamente il login.",o.style.color="#ef4444");return}if(h.ruolo!=="MASTER"&&!h.sessionToken){Ct(null);try{localStorage.removeItem("sessioneUtente"),sessionStorage.removeItem("sessioneUtente")}catch{}document.documentElement.classList.remove("has-session"),t&&(t.style.display="flex",t.style.opacity="1");let o=document.getElementById("login-error");o&&(o.innerText="Sessione non pi\xC3\xB9 valida. Effettua di nuovo il login.",o.style.color="#ef4444");return}vs(),yo(),fs(),Ri(),(window.requestIdleCallback||function(o){setTimeout(o,3e3)})(function(){Pi()}),t&&(t.style.display="none")}else document.documentElement.classList.remove("has-session"),t&&(t.style.display="flex",t.style.opacity="1");if(e&&typeof Oe=="function"&&await Oe().catch(o=>console.warn("[Boot] caricaDatiIniziali:",o)),e){if(h.ruolo!=="MASTER"&&!h.nome){document.documentElement.classList.remove("has-session");try{localStorage.removeItem("sessioneUtente"),sessionStorage.removeItem("sessioneUtente")}catch{}t&&(t.style.display="flex",t.style.opacity="1");return}hs()}};function Fd(){let t=document.getElementById("main-sidebar");if(!t)return;let e=t.classList.toggle("collapsed");document.body.classList.toggle("sidebar-collapsed",e);try{localStorage.setItem("sidebarCollapsed",e?"1":"0")}catch{}}function Hd(){try{let t=localStorage.getItem("sidebarCollapsed"),e=document.getElementById("main-sidebar");t==="1"&&(e&&e.classList.add("collapsed"),document.body.classList.add("sidebar-collapsed"))}catch{}}document.addEventListener("DOMContentLoaded",Hd);async function jd(){if(!zi(!0))return;h&&(h.expiresAt=Date.now()+2592e6);try{localStorage.setItem("sessioneUtente",JSON.stringify(h))}catch{}try{sessionStorage.setItem("sessioneUtente",JSON.stringify(h))}catch{}us(),Ao();let t=document.getElementById("login-overlay");t.style.transition="opacity 0.4s ease",t.style.opacity="0",vs(),U("_impostazioni_cache"),await Promise.all([Oe().catch(e=>console.warn("caricaDatiIniziali post-login:",e)),new Promise(e=>setTimeout(e,400))]),t.style.display="none",document.documentElement.classList.add("has-session"),typeof yo=="function"&&yo(),fs(),Ri(),(window.requestIdleCallback||function(e){setTimeout(e,3e3)})(function(){Pi()}),Ud(),hs()}function Re(){if(!Re._running){Re._running=!0,M.stop(),pe();try{let t=Io();t&&fetch(I,{method:"POST",body:JSON.stringify({azione:"logout",sessionToken:t})}).catch(function(){}),me&&(clearInterval(me),me=null),fe&&(clearInterval(fe),fe=null);try{D.clear()}catch{}let e={};for(let o=0;o<localStorage.length;o++){let i=localStorage.key(o);i&&(Wi.some(n=>i.startsWith(n))||Ji.includes(i))&&(e[i]=localStorage.getItem(i))}localStorage.clear(),sessionStorage.clear(),Object.entries(e).forEach(([o,i])=>{try{localStorage.setItem(o,i)}catch{}}),window.location.href=window.location.origin+window.location.pathname+"?logout="+Date.now()}catch(t){console.error("Errore durante il logout:",t),window.location.reload()}}}function Gd(t){var e=h&&h.nome?h.nome.toUpperCase():"",o=t.filter(function(c){return c.nome.toUpperCase()!==e}),i=document.getElementById("user-avatar-btn"),n=document.getElementById("user-avatar-btn-mobile");if(o.length===0){var a=document.getElementById("online-indicator");a&&a.remove();var r=document.getElementById("online-indicator-mob");r&&r.remove();return}var s="Online ora: "+o.map(function(c){return c.nome+(c.pagina?" ("+c.pagina+")":"")}).join(", ");[{parent:i,id:"online-indicator"},{parent:n,id:"online-indicator-mob"}].forEach(function(c){if(c.parent){var l=document.getElementById(c.id);l||(l=document.createElement("span"),l.id=c.id,c.parent.appendChild(l)),l.title=s}})}document.addEventListener("visibilitychange",function(){M._timer&&(document.hidden?M._schedule(M.INTERVAL_BG_MS):(M._check(),M._schedule(M.INTERVAL_FOCUS_MS)))});window.addEventListener("online",function(){M._timer&&M._check()});async function Eo(t,e){let o=Date.now();if(o-rs<300)return;if(rs=o,_o)try{_o.abort()}catch{}_o=new AbortController;let i=_o.signal,n=++qd;Ni=n,window._latestNavRequest=n,xo=null,t!=="PROGRAMMA PRODUZIONE DEL MESE"&&pe();let a=document.getElementById("universal-search");a&&(a.value="");let r=document.getElementById("desk-search-input");r&&(r.value=""),(!t||t==="undefined"||t==="null")&&(t="PROGRAMMA PRODUZIONE DEL MESE"),localStorage.setItem("ultimaPaginaProduzione",t),qi=t,window.paginaAttuale=t,Ii(t),document.body.classList.toggle("page-pip",t==="PIPISTRELLI"),t!=="PIPISTRELLI"&&At&&At.resetPipFetch(),t!=="KIT_PRODOTTI"&&Ot&&Ot.resetKitFetch();let s=document.getElementById("acq-tab-bar");s&&(s.style.display=t==="MATERIALE DA ORDINARE"?"flex":"none"),document.querySelectorAll(".menu-item").forEach(m=>m.classList.remove("active")),document.querySelectorAll(".tab-item").forEach(m=>m.classList.remove("active")),e||(e=document.querySelector(`.menu-item[data-page="${t}"]`)),e&&e.classList.add("active");let c=document.querySelector(`.tab-item[data-page="${t}"]`);c&&c.classList.add("active");let l={IMPOSTAZIONI:"Impostazioni Sistema",STORICO_RICHIESTE:"La mia Casella",ARCHIVIO_ORDINI:"Archivio Ordini","MATERIALE DA ORDINARE":"Gestione Acquisti",MANUALI_PRODOTTI:"Manuali Prodotti","PROGRAMMA PRODUZIONE DEL MESE":"Dashboard Produzione",PIPISTRELLI:"\xF0\u0178\xA6\u2021 Pipistrelli",KIT_PRODOTTI:"\xF0\u0178\xA7\xB0 Kit Prodotti"},d=document.getElementById("titolo-pagina");d&&(d.innerText=l[t]||t);let p=document.getElementById("page-title-desktop");p&&(p.innerText=l[t]||t);let f=document.getElementById("floating-cart-btn");if(f){let m=t==="MATERIALE DA ORDINARE";f.style.display=m?"flex":"none",!m&&typeof chiudiModalCarrello=="function"&&chiudiModalCarrello()}let u=document.getElementById("contenitore-dati");if(!F[t]){let m="_html_"+t,b=pt(m,3e5);if(b){F[t]=b;try{let w=localStorage.getItem(m),v=w?JSON.parse(w):null;Q[t]=v&&v.ts?v.ts:Date.now()-3e5-1e3}catch{Q[t]=Date.now()-3e5-1e3}}}if(F[t]?u.innerHTML="":u.innerHTML=`<div class="nav-skeleton">
        ...
        </div>`,["modalAiuto","modal-conferma","modal-gestione-articolo","modal-carrello"].forEach(m=>{let b=document.getElementById(m);if(b){if(m==="modal-carrello"){b.classList.remove("cart-open");return}if(m==="modal-gestione-articolo"){b.classList.remove("active"),setTimeout(()=>{b.classList.contains("active")||(b.style.display="none")},300);return}b.classList.remove("active"),setTimeout(()=>{b.classList.contains("active")||(b.style.display="none")},300)}}),t==="STORICO_RICHIESTE"){let m=document.getElementById("badge-richieste-count");m&&(m.style.display="none",m.classList.remove("badge-sollecito-attivo"));let b=document.getElementById("badge-mobile-notif");b&&(b.style.display="none");let w=document.getElementById("badge-bottom-richieste");w&&(w.style.display="none",w.classList.remove("badge-sollecito-attivo"))}if(F[t]){u.innerHTML=F[t],Di(Q[t]||Date.now()),z(u),Oo(),requestAnimationFrame(It),t==="PROGRAMMA PRODUZIONE DEL MESE"&&fo();let m=Date.now(),b=Q[t]||0;m-b>3e5&&(t==="PROGRAMMA PRODUZIONE DEL MESE"?mo(t,!0,n,i):t==="MATERIALE DA ORDINARE"?Wt(null,n,i,!0):t==="STORICO_RICHIESTE"?ge("STORICO_RICHIESTE",()=>te(i),we,!0).catch(()=>{}):t==="MANUALI_PRODOTTI"?Ee(n,i,!0).catch(()=>{}):t==="ARCHIVIO_ORDINI"&&ue());return}switch(t){case"IMPOSTAZIONI":Ht();break;case"STORICO_RICHIESTE":{let m=document.getElementById("contenitore-dati");m&&(m.innerHTML="<div class='centered-msg' id='_ric-loader'>Caricamento messaggi in corso...</div>"),ge("STORICO_RICHIESTE",te,we).catch(async b=>{if(b&&b.name==="AbortError")return;let w=null;try{w=await D.get("STORICO_RICHIESTE")}catch{}if(w){let v=new Date(w.timestamp),x=String(v.getHours()).padStart(2,"0"),O=String(v.getMinutes()).padStart(2,"0");g("Connessione assente \xE2\u20AC\u201D mostro dati salvati alle "+x+":"+O,"warning")}else{let v=document.getElementById("contenitore-dati");v&&(v.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('STORICO_RICHIESTE',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(v))}});break}case"ARCHIVIO_ORDINI":ue();break;case"MATERIALE DA ORDINARE":Wt(e?"catalogo":null,n,i);break;case"MANUALI_PRODOTTI":Ee(n,i,!1);break;case"ORDINI_ACQUISTI":Wt("ordini",n,i);return;case"PIPISTRELLI":try{At||(At=await import("./chunk-pipistrelli-YHWPXY32.js"),At.registerGlobals()),At.caricaPipistrelli()}catch(m){if(m&&m.name==="AbortError")return;console.warn("[PIPISTRELLI] Errore caricamento modulo:",m);let b=document.getElementById("contenitore-dati");b&&(b.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('PIPISTRELLI',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(b)),At=null}break;case"KIT_PRODOTTI":try{if(Ot||(Ot=await import("./chunk-kit-prodotti-NW57ALDA.js"),Ot.registerGlobals()),n!==Ni||window.paginaAttuale!=="KIT_PRODOTTI")return;Ot.caricaKitProdotti()}catch(m){if(m&&m.name==="AbortError"||n!==Ni||window.paginaAttuale!=="KIT_PRODOTTI")return;console.warn("[KIT_PRODOTTI] Errore caricamento modulo:",m);let b=document.getElementById("contenitore-dati");b&&(b.innerHTML=`<div class='centered-error-bold'>Errore nel caricamento. <button onclick="cambiaPagina('KIT_PRODOTTI',null)" style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">Riprova</button></div>`,z(b)),Ot=null}break;default:{let m=document.getElementById("contenitore-dati");m&&(m.innerHTML="<div class='inline-msg' id='_prod-loader'>Caricamento Dashboard...</div>",z(m)),ge("PROGRAMMA_PRODUZIONE",uo,po).catch(async b=>{if(b&&b.name==="AbortError")return;let w=null;try{w=await D.get("PROGRAMMA_PRODUZIONE")}catch{}if(w){let v=new Date(w.timestamp),x=String(v.getHours()).padStart(2,"0"),O=String(v.getMinutes()).padStart(2,"0");g("Connessione assente \xE2\u20AC\u201D mostro dati salvati alle "+x+":"+O,"warning")}else{let v=document.getElementById("contenitore-dati");v&&(v.innerHTML=`<div class='inline-error'>Errore nel caricamento dati.
                                <button onclick="cambiaPagina('PROGRAMMA PRODUZIONE DEL MESE', null)"
                                    style="margin-left:8px;padding:4px 12px;background:#242424;color:#fff;border:none;border-radius:6px;cursor:pointer">
                                    &#x21bb; Riprova</button></div>`,z(v))}})}}}function Vd(t){return t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}function Jd(t,e){return!e||t.trimStart().startsWith(e)?!0:t.split(/[\s(,;]+/).some(o=>o.toLowerCase().startsWith(e))}function Wd(){clearTimeout(cs),cs=setTimeout(function(){let t=document.getElementById("universal-search"),e=document.getElementById("mobile-search"),o=t&&t.value!==""?t.value:e&&e.value!==""?e.value:t?t.value:"",i=String(o||"").trim().toLowerCase();xo||Oo(),typeof window.filtroRicercaArticoli<"u"&&(Bi=!!window.filtroRicercaArticoli);let n=!!Bi,a=i?new RegExp(Vd(i),"i"):null;(xo||[]).forEach(function(s){if(!i){s.classList.remove("hidden-search");return}let c=(s.textContent||"").toLowerCase(),l=(s.getAttribute("data-codice")||"").toLowerCase(),d=!1;if(n)d=l?l.indexOf(i)!==-1:c.indexOf(i)!==-1;else{let p=(s.getAttribute("data-cliente")||"").toLowerCase(),f=(s.getAttribute("data-ordine")||s.getAttribute("data-codice")||"").toLowerCase(),u=(s.getAttribute("data-riferimento")||"").toLowerCase(),m=(s.getAttribute("data-codici")||"").toLowerCase(),b=c+" "+p+" "+f+" "+u+" "+m,w=Jd(b,i),v=a?a.test(b):!1;d=w||i.length>=2&&v}s.classList.toggle("hidden-search",!d)});let r=document.getElementById("sezione-archivio");r&&(r.style.display=i===""?"block":"none")},120)}Fa();Ja();Xa();Fi();dn();bn();Ln();Qn();Ca();Na();window.cambiaPagina=Eo;window.aggiornaListaFiltrabili=Oo;window.filtraUniversale=Wd;window.toggleSidebar=Fd;window.logout=Re;window.salvaEApriDashboard=jd;window.cacheContenuti=F;window.TW=Bd;window.listaStati=Dd;window.listaOperatori=Md;window._VAPID_PUBLIC_KEY=ms;window._salvaSubVAPID_=gs;window._gestisciAuthError_=ps;window._getSessionToken_=Io;window._defaultListaStati_=Nd});export default Zd();
