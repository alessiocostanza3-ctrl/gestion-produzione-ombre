...
<body>
    ...
    <div class="db-print-stage">
        <div class="db-print-page">
            ...
            <div class="db-print-meta-grid">
                ...
                <div class="db-print-meta-card">
                    <div class="db-print-meta-row"><div class="db-print-meta-label">Generato da</div><div class="db-print-meta-value">${r(P?.nome||"Sistema")}</div></div>
...
</html>`}function se(t){let{kits:i}=z(),n=i.find(a=>a.id===t);if(!n)return;let e=Q(n),o=wt(n,e);if(!o.totalePezzi||!o.totaleRighe){y("Componi prima un ordine per generare la distinta stampabile.","warning");return}R(e).documento||(L(t,function(a){ci(a)}),e=Q(n));let s=window.open("","_blank");if(!s){y("Popup bloccato: abilita l'anteprima di stampa per aprire il modello completo.","warning");return}s.document.open(),s.document.write(mi(n,o,e)),s.document.close(),s.focus()}function z(){try{let t=localStorage.getItem(Mt);if(!t)return{kits:[]};let i=JSON.parse(t);return{kits:Array.isArray(i?.kits)?i.kits.map(si):[]}}catch{return{kits:[]}}}function x(t){let i=Array.isArray(t)?t.map(si):[];try{localStorage.setItem(Mt,JSON.stringify({kits:i})),localStorage.setItem(gt,Date.now())}catch{}ae(i)}function ae(t){clearTimeout(Yt),Yt=setTimeout(function(){St({azione:"setKitData",kits:t}).catch(function(i){console.warn("[kit-prodotti] salvataggio remoto fallito:",i)})},1500)}function re(t){fetch(Ct,{method:"POST",body:JSON.stringify({azione:"getKitData"})}).then(i=>i.json()).then(i=>{if(i&&Array.isArray(i.kits)){let n=parseInt(i.ts||0),e=parseInt(localStorage.getItem(gt)||0);if(n>0&&n>e){try{localStorage.setItem(Mt,JSON.stringify({kits:i.kits}))}catch{}try{localStorage.setItem(gt,n)}catch{}t&&t(!0);return}}t&&t(!1)}).catch(()=>{t&&t(!1)})}function C(){return Math.random().toString(36).slice(2,10)+Date.now().toString(36)}function Lt(){if(!P||!P.nome)return!1;let t=String(P.nome).toUpperCase().trim();return t==="ALESSIO"||t==="0000"||P.ruolo==="MASTER"}function ce(t){let i={};for(let n of t.sezioni||[])for(let e of n.componenti||[]){if(B(e)){i[e.id]=0;continue}let o=0;for(let[s,a]of Object.entries(t.qtaDaProdurre||{}))o+=(Number.parseInt(a,10)||0)*T(e,s);i[e.id]=o}return i}function de(t){let i={};for(let n of t.sottoAssembly||[]){let e=Number.parseInt(t.pronti?.[n.id],10)||0;if(!e)continue;let o=n.varianteKey;for(let s of t.sezioni||[])for(let a of s.componenti||[]){if(B(a))continue;let d=T(a,o);d>0&&(i[a.id]=(i[a.id]||0)+e*d)}}return i}function ui(t,i){let n=M(t).find(e=>e.key===i);return n?r(n.nome):r(i)}function Pt(){return new Date().toLocaleString("it-IT",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"})}function st(){xt||(xt=!0,re(function(n){n&&st()}));let{kits:t}=z(),i=document.getElementById("contenitore-dati");if(i){i.innerHTML=`
    <div class="kit-page">
        ...
        <div id="kit-tab-bar" style="display:flex;gap:4px;padding:8px 0 0">
            <button class="acq-tab ${N==="kits"?"active":""}" data-tab="kits" onclick="_kitSwitchMainTab('kits')"><i class="fas fa-boxes-stacked"></i> Kits</button>
            <button class="acq-tab ${N==="anagrafiche"?"active":""}" data-tab="anagrafiche" onclick="_kitSwitchMainTab('anagrafiche')"><i class="fas fa-list"></i> Anagrafiche</button>
            <button class="acq-tab ${N==="distinte"?"active":""}" data-tab="distinte" onclick="_kitSwitchMainTab('distinte')"><i class="fas fa-file-alt"></i> Distinte</button>
        </div>
        <div id="kit-main-content" class="kit-main-content" style="border-top:1px solid #e2e8f0;padding-top:16px;margin-top:0"></div>
    </div>`,Z(N),Rt();try{if(window&&window._kitSuppressNextFade)try{delete window._kitSuppressNextFade}catch{}else rt(i)}catch{rt(i)}}}function fi(t,i){if(!i)return;let n=(Array.isArray(t)?t:[]).map(e=>{let s=M(e).length,a=(e.assiConfigurazione||[]).length,d=(e.sezioni||[]).reduce((c,l)=>c+(l.componenti||[]).length,0);return`
        ...
        ${t.length===0?`<div class="kit-empty-state">
               ...
               </div>`:`<div class="kit-grid">${n}</div>`}`}function Rt(){let t=document.getElementById("kit-page-actions");t&&(N==="kits"?t.innerHTML='<button type="button" class="btn-modal-send" style="font-size:0.8rem;padding:8px 14px" onclick="_kitNuovoKit()"><i class="fas fa-plus"></i> Nuovo Kit</button>':N==="anagrafiche"?t.innerHTML='<button type="button" class="btn-modal-send" style="font-size:0.8rem;padding:8px 14px" onclick="_kitOpenAnagraficaModal()"><i class="fas fa-plus"></i> Aggiungi</button>':t.innerHTML="")}function Z(t){N=t,document.querySelectorAll("#kit-tab-bar .acq-tab").forEach(e=>{e.classList.toggle("active",e.dataset.tab===t)});let{kits:i}=z(),n=document.getElementById("kit-main-content");n&&(t==="kits"?fi(i,n):t==="anagrafiche"?gi(i,n):t==="distinte"&&ki(i,n),Rt())}function gi(t,i){if(!i)return;let n=mt();if(!n.length){i.innerHTML=`
            ...
            </div>`;return}let e=n.reduce((s,a)=>{let d=a.categoria||"Senza categoria";return s[d]=s[d]||[],s[d].push(a),s},{}),o="";for(let[s,a]of Object.entries(e))o+=`<details class="ordine-group" open>
            ...
            <div class="ordine-items">`,o+=a.map(d=>`
                <div class="ordine-item" style="display:flex;justify-content:space-between;align-items:center">
                    <div style="flex:1;min-width:0">
                        <div style="font-weight:600;color:#1e293b">${r(d.nome)}${d.codice?` <span style="color:#94a3b8;font-size:.85rem;font-weight:400">\xB7 ${r(d.codice)}</span>`:""}</div>
                        ${d.descrizione?`<div style="color:#94a3b8;font-size:.82rem;margin-top:2px">${r(d.descrizione)}</div>`:""}
                    </div>
                    <div style="display:flex;gap:6px;flex-shrink:0;margin-left:12px">
                        <button type="button" class="btn-archive-action primary" onclick="_kitOpenAnagraficaModal('${r(d.id)}')"><i class="fas fa-pen"></i> Modifica</button>
                        <button type="button" class="btn-trash-modern" onclick="(function(){ if(confirm('Eliminare questo componente?')) _kitDeleteAnagrafica('${r(d.id)}') })()" title="Elimina"><i class="fas fa-trash"></i></button>
                    </div>
                </div>`).join(""),o+="</div></details>";i.innerHTML=o}function ki(t,i){if(!i)return;let n=W();if(!n.length){i.innerHTML='<div style="padding:24px 0;text-align:center"><p class="acquisti-subtitle">Nessuna distinta salvata.</p></div>';return}let e=n.map(o=>`
        ...
        </details>`).join("");i.innerHTML=e}function vi(t){let{kits:i}=z(),n=i.find(c=>c.id===t);if(!n){y("Kit non trovato \u26A0\uFE0F");return}let e=Q(n);R(e).documento||(L(t,function(c){ci(c)}),e=Q(n));let o=wt(n,e);if(!o.totalePezzi||!o.totaleRighe){y("Componi prima un ordine per generare la distinta stampabile.","warning");return}let s=W(),a=R(e),d={id:C(),kitId:n.id,kitNome:n.nome,nome:a.documento||`Distinta-${Date.now()}`,documento:a.documento||"",createdAt:Date.now(),createdBy:P?.nome||"Sistema",orderDraftSnapshot:e,distintaSnapshot:o};s.unshift(d),ht(s),y("Distinta salvata \u2713"),N==="distinte"&&Z("distinte")}function mt(){try{let t=localStorage.getItem(ti),i=t?JSON.parse(t):[];return Array.isArray(i)?i:[]}catch{return[]}}function Ht(t){try{localStorage.setItem(ti,JSON.stringify(t||[]));try{localStorage.setItem(Ri,Date.now())}catch{}}catch{}}function le(){if(document.getElementById("modal-kit-anagrafica-edit"))return;let t=document.createElement("div");t.innerHTML=`
    ...
    </div>`,document.body.appendChild(t.firstElementChild)}function pe(t){le();let i=document.getElementById("modal-kit-anagrafica-edit");if(!i)return;let n=document.getElementById("anag-componente"),e=document.getElementById("anag-codice"),o=document.getElementById("anag-categoria"),s=document.getElementById("anag-descrizione");if(t){let a=mt().find(d=>d.id===t);a&&(n&&(n.value=a.nome||""),e&&(e.value=a.codice||""),o&&(o.value=a.categoria||""),s&&(s.value=a.descrizione||""),i.dataset.editId=t)}else n&&(n.value=""),e&&(e.value=""),o&&(o.value=""),s&&(s.value=""),delete i.dataset.editId;i.classList.remove("active"),i.style.display="flex",requestAnimationFrame(()=>{requestAnimationFrame(()=>{i.classList.add("active")})}),setTimeout(()=>n&&n.focus(),120)}function bi(){let t=document.getElementById("modal-kit-anagrafica-edit");t&&(t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300))}function me(){let t=document.getElementById("modal-kit-anagrafica-edit");if(!t)return;let i=t.dataset.editId,n=(document.getElementById("anag-componente")?.value||"").trim();if(!n){y("Inserisci il nome del componente","warning");return}let e=(document.getElementById("anag-codice")?.value||"").trim(),o=(document.getElementById("anag-categoria")?.value||"").trim(),s=(document.getElementById("anag-descrizione")?.value||"").trim(),a=mt();if(i){let d=a.findIndex(c=>c.id===i);d!==-1?a[d]={...a[d],nome:n,codice:e,categoria:o,descrizione:s,updatedAt:Date.now()}:a.unshift({id:C(),nome:n,codice:e,categoria:o,descrizione:s,createdAt:Date.now(),createdBy:P?.nome||"Sistema"})}else a.unshift({id:C(),nome:n,codice:e,categoria:o,descrizione:s,createdAt:Date.now(),createdBy:P?.nome||"Sistema"});Ht(a),bi(),y("Componente salvato \u2713"),N==="anagrafiche"&&Z("anagrafiche")}function ue(t){let i=mt().filter(n=>n.id!==t);Ht(i),N==="anagrafiche"&&Z("anagrafiche"),y("Componente eliminato \u2713")}function fe(t){let i=W().find(o=>o.id===t);if(!i)return;let{kits:n}=z(),e=n.find(o=>o.id===i.kitId)||null;if(e){let o=window.open("","_blank");if(!o){y("Popup bloccato: abilita l'anteprima di stampa per aprire il modello completo.","warning");return}o.document.open();try{o.document.write(mi(e,i.distintaSnapshot,i.orderDraftSnapshot))}catch{o.document.write("<pre>"+r(JSON.stringify(i.distintaSnapshot,null,2))+"</pre>")}o.document.close(),o.focus()}else{let o=window.open("","_blank");if(!o){y("Popup bloccato","warning");return}o.document.open(),o.document.write("<pre>"+r(JSON.stringify(i.distintaSnapshot,null,2))+"</pre>"),o.document.close(),o.focus()}}function ge(t){let i=W().find(e=>e.id===t);if(!i)return;let n=Bt();n[i.kitId]=i.orderDraftSnapshot||{},ai(n),y("Bozza ordine ripristinata per il kit selezionato \u2713")}function ke(t){let i=W().filter(n=>n.id!==t);ht(i),N==="distinte"&&Z("distinte"),y("Distinta eliminata \u2713")}function ve(t){q=t,yi="ordine",D()}function D(){let{kits:t}=z(),i=t.find(g=>g.id===q);if(!i){st();return}let n=document.getElementById("contenitore-dati"),e=M(i),o=Q(i),s=R(o),a=wt(i,o),d=a.selectedVarianti.length?a.selectedVarianti.map(g=>`<span class="kit-meta-pill"><strong>${Y(o,g.key)}</strong> \xD7 ${r(g.nome)}</span>`).join(""):'<span class="kit-leg-item" style="color:#94a3b8">Nessuna configurazione selezionata.</span>',c=s.ordiniCliente.length?s.ordiniCliente.map(g=>`<span class="kit-order-ref-chip">${r(g)}
    ...
    </div>`,rt(n),di().catch(()=>{})}function be(){q=null,st()}function ye(t){yi=t,D()}function he(t){L(t,function(i,n){for(let e of M(n)){let o=document.getElementById("kit-qty-"+e.key);o&&(i[e.key]=Math.max(0,Number.parseInt(o.value,10)||0))}})}function ze(t,i,n){try{window._kitSuppressNextFade=!0}catch{}L(t,function(e){e[i]=Math.max(0,Number.parseInt(n,10)||0)})}function we(t,i,n){try{window._kitSuppressNextFade=!0}catch{}L(t,function(e){let o=Math.max(0,Number.parseInt(e[i],10)||0);e[i]=Math.max(0,o+n)})}function _e(t){L(t,function(i){for(let n of Object.keys(i))n!=="_meta"&&(i[n]=0);i._meta=pt({})})}function $e(t,i){L(t,function(n){n[i]=0})}function bt(t,i){let n=document.getElementById("kit-order-autocomplete-"+t);if(n){if(!i.length){n.style.display="none",n.innerHTML="";return}n.innerHTML=i.map(e=>`
    ...
    `).join(""),n.style.display="block"}}function Ce(t,i){let n=String(i||"").trim().toLowerCase();if(!n){bt(t,[]);return}di().then(function(e){let o=e.filter(s=>s.ordine.toLowerCase().includes(n)||s.cliente.toLowerCase().includes(n)).slice(0,8);bt(t,o)})}function Se(t){setTimeout(function(){bt(t,[])},140)}function Ie(t,i,n){let e=ot(i);if(!e)return;try{window._kitSuppressNextFade=!0}catch{}L(t,function(s){let a=R(s);a.ordiniCliente=[...new Set(a.ordiniCliente.concat(e))],a.cliente=li(a.ordiniCliente,{[e]:n}),zt(s,a)});let o=document.getElementById("kit-order-ref-input-"+t);o&&(o.value=""),bt(t,[])}function xe(t,i){let n=ot(i);try{window._kitSuppressNextFade=!0}catch{}L(t,function(e){let o=R(e);o.ordiniCliente=o.ordiniCliente.filter(s=>s!==n),o.cliente=li(o.ordiniCliente),zt(e,o)})}function Ae(t,i,n){let{kits:e}=z(),o=e.find(a=>a.id===t);if(!o)return;let s=Dt(o);if(s[i]=n,dt[t]=s,q===t){try{window._kitSuppressNextFade=!0}catch{}D()}}function Me(t){let{kits:i}=z(),n=i.find(a=>a.id===t);if(!n)return;let e=pi(n,Dt(n));if(!e){y("Completa prima le scelte elettroniche \u26A0\uFE0F");return}let o=Math.max(0,Number.parseInt(document.getElementById("kit-compose-qty-"+t)?.value,10)||0);if(!o){y("Inserisci una quantit\xE0 valida \u26A0\uFE0F");return}if(It[t])return;It[t]=Date.now(),setTimeout(function(){try{delete It[t]}catch{}},600);try{window._kitSuppressNextFade=!0}catch{}L(t,function(a){a[e.key]=Y(a,e.key)+o});let s=document.getElementById("kit-compose-qty-"+t);s&&(s.value=1)}function hi(t){let i=t.dataset.cid,n=t.dataset.sid,e=Math.max(0,Number.parseInt(t.value,10)||0),{kits:o}=z(),s=o.find(_=>_.id===q);if(!s)return;let a=(s.sezioni||[]).find(_=>_.id===n),d=a&&(a.componenti||[]).find(_=>_.id===i);if(!d||!Nt(d))return;d.caricato=e,x(o);let l=ce(s)[i]||0,m=Math.max(0,l-e),u=de(s)[i]||0,k=t.closest("tr");if(!k)return;let h=k.querySelector(".kit-ord-zero,.kit-ord-manca,.kit-ord-ok");h&&(h.textContent=l===0?"\u2014":m,h.className=l===0?"kit-ord-zero":m>0?"kit-ord-manca":"kit-ord-ok");let g=k.querySelector(".kit-car-liberi");g&&(u>0?(g.textContent=Math.max(0,e-u)+" lib.",g.style.display=""):g.style.display="none")}function Ee(t,i,n){let{kits:e}=z(),o=e.find(s=>s.id===t);o&&(o.pronti||(o.pronti={}),o.pronti[i]=Math.max(0,(Number.parseInt(o.pronti[i],10)||0)+n),x(e),q===t&&D())}function Oe(t,i,n){let{kits:e}=z(),o=e.find(a=>a.id===t);if(!o)return;o.pronti||(o.pronti={}),o.pronti[i]=Math.max(0,Number.parseInt(n,10)||0),x(e);let s=document.querySelector(`.kit-pronti-input[data-said="${i}"]`);s&&(s.value=o.pronti[i],s.classList.toggle("kit-pronti-val-on",o.pronti[i]>0))}function qe(t,i){let n=t.movimenti||[];return n.length?n.map(e=>{let o=i?`<button type="button" class="kit-mov-del" onclick="_kitEliminaMovimento('${r(t.id)}',${e.id})" title="Elimina">\u2715</button>`:'<span style="width:22px;flex-shrink:0"></span>',s=i&&(e.tipo==="carico"||e.tipo==="scarico")?`<button type="button" class="kit-mov-edit" onclick="_kitModificaMovimento('${r(t.id)}',${e.id})" title="Modifica">\u270E</button>`:'<span style="width:22px;flex-shrink:0"></span>';if(e.tipo==="spedizione"){let a=(e.righe||[]).reduce((l,m)=>l+m.qty,0),d=(e.righe||[]).map(l=>`<div class="kit-assemb-sub-row"><span class="kit-assemb-sub-mat" style="color:#94a3b8">${r(l.mat)}</span><span class="kit-mov-qty scarico">\u2212${l.qty}</span></div>`).join(""),c=(e.items||[]).map(l=>`<div class="kit-assemb-sub-row kit-sped-item-row"><span class="kit-assemb-sub-mat">${r(l.nome)}</span><span class="kit-mov-qty scarico">\xD7${l.qty}</span></div>`).join("");return`<details class="kit-mov-assemb-group">
            ...
            ${s}${o}
                ...
                </label>`}).join(""));let a=document.getElementById("kit-sped-nota-"+t),d=document.getElementById("kit-sped-modal-nota");d&&a&&(d.value=a.value||""),d&&!a&&(d.value=""),o.dataset.kitId=t,o.style.display="flex",o.offsetHeight,o.classList.add("active")}function $i(){let t=document.getElementById("modal-kit-sped");t&&(t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300))}function Pe(){let t=document.getElementById("modal-kit-sped");if(!t)return;let i=t.dataset.kitId;$i();let n=[...document.querySelectorAll(".kit-sped-chk:checked")].map(l=>l.dataset.said);if(!n.length)return;let{kits:e}=z(),o=e.find(l=>l.id===i);if(!o)return;let s=(document.getElementById("kit-sped-modal-nota")?.value||"").trim(),a=[],d=[];for(let l of n){let m=(o.sottoAssembly||[]).find(u=>u.id===l);if(!m)continue;let p=Number.parseInt(o.pronti?.[l],10)||0;if(p){a.push({saId:l,nome:m.nome,qty:p});for(let u of o.sezioni||[])for(let k of u.componenti||[]){if(B(k))continue;let h=T(k,m.varianteKey);if(!h)continue;let g=p*h;k.caricato=Math.max(0,(parseInt(k.caricato)||0)-g);let _=d.find(K=>K.cid===k.id);_?_.qty+=g:d.push({cid:k.id,mat:k.nome,qty:g})}o.pronti||(o.pronti={}),delete o.pronti[l]}}o.movimenti||(o.movimenti=[]),o.movimenti.unshift({id:Date.now(),tipo:"spedizione",items:a,righe:d,nota:s,ts:Pt()}),x(e);let c=a.reduce((l,m)=>l+m.qty,0);y(`Spedizione registrata: ${c} pz \u2713`),q===i&&D()}function Re(t){let{kits:i}=z(),n=i.find(a=>a.id===t);if(!n)return;let e=document.getElementById("modal-kit-reso");if(!e)return;let o=document.getElementById("kit-reso-items-list");if(o){let a=n.sottoAssembly||[];o.innerHTML=a.length===0?'<p style="color:#94a3b8;text-align:center">Configura prima i sub-assembly per registrare un reso.</p>':a.map(d=>{let c=ui(n,d.varianteKey);return`<div class="kit-reso-item-row">
        ...
        <div class="kit-cfg-section">
            ...
            ${k}
        </div>`,g=(i.sezioni||[]).map((v,$)=>{let f=(v.componenti||[]).map(b=>{let S=B(b),O=Tt(b,i),Ut=(e||[]).find(E=>E.id===O.asseId)||null,Mi=O.tipo==="gruppo"&&Ut?`<div class="kit-cfg-row">${(Ut.opzioni||[]).map(E=>`<label class="kit-meta-pill">
                ...
                <div class="kit-cfg-row">
                    ...
                    <select class="kit-cfg-select" style="max-width:120px"
                            ...
                            ${S?"disabled":""}>
                        ${qi.map(E=>`<option value="${r(E.value)}" ${Qt===E.value?"selected":""}>${r(E.label)}</option>`).join("")}
                    </select>
        ...
        <div class="kit-cfg-section">
            ...
            <div style="margin-top:8px">${V.length?V.map(v=>`<div class="kit-preset-row" style="display:flex;justify-content:space-between;align-items:center;padding:6px 0">
                <div style="flex:1">
                    ...
                    <div style="color:#94a3b8;font-size:0.85rem">${r(v.sourceKitId&&z().kits.find($=>$.id===v.sourceKitId)?.nome||"")}</div>
                </div>
    ...
    </div>`,rt(n)}function ln(t){if(t&&q===t){D();return}q=t,D()}function pn(t){F=t,j()}function I(t,i,n=!0){let{kits:e}=z(),o=e.find(s=>s.id===t);o&&(i(o),x(e),n&&j())}function mn(t,i){I(t,function(n){n.nome=i.trim()||"Kit senza nome"},!1)}function un(t){if(!confirm("Eliminare questo kit e tutti i suoi dati?"))return;let{kits:i}=z();x(i.filter(n=>n.id!==t)),Vt=null,q=null,st()}function fn(t){let{kits:i}=z(),n=i.find(o=>o.id===t);if(!n)return;let e={id:C(),nome:`Copia di ${n.nome}`,schemaVersion:Et,assiConfigurazione:[],varianti:[],sezioni:[],sottoAssembly:[],qtaDaProdurre:{},pronti:{},movimenti:[]};for(let o of n.assiConfigurazione||[])e.assiConfigurazione.push(oi(o,n,e));e.varianti=ei(e.assiConfigurazione);for(let o of n.sezioni||[])e.sezioni.push(kt(o,n,e));e.sottoAssembly=(n.sottoAssembly||[]).map(o=>({id:C(),nome:o.nome||"",varianteKey:o.varianteKey||"",noteConfig:o.noteConfig||""})),i.push(e),x(i),Ft(e.id),y(`Kit "${n.nome}" duplicato \u2713`)}function Ai(t){I(t,function(i){let n=(i.assiConfigurazione||[]).length+1;i.assiConfigurazione=i.assiConfigurazione||[],i.assiConfigurazione.push({id:C(),key:"asse"+n,nome:"Asse "+n,opzioni:[{id:C(),key:"opz1",nome:"Opzione 1"}]})})}function gn(t,i,n,e){I(t,function(o){let s=(o.assiConfigurazione||[]).find(a=>a.id===i);s&&(n==="key"?s.key=U(e,s.key||"asse"):s[n]=e.trim())})}function kn(t,i){I(t,function(n){n.assiConfigurazione=(n.assiConfigurazione||[]).filter(e=>e.id!==i)})}function vn(t,i){I(t,function(n){let e=(n.assiConfigurazione||[]).find(s=>s.id===i);if(!e)return;let o=(e.opzioni||[]).length+1;e.opzioni=e.opzioni||[],e.opzioni.push({id:C(),key:"opz"+o,nome:"Opzione "+o,codice:""})})}function bn(t,i,n,e,o){I(t,function(s){let a=(s.assiConfigurazione||[]).find(c=>c.id===i),d=a&&(a.opzioni||[]).find(c=>c.id===n);d&&(e==="key"?d.key=U(o,d.key||"opzione"):d[e]=o.trim())})}function yn(t,i,n){I(t,function(e){let o=(e.assiConfigurazione||[]).find(s=>s.id===i);o&&(o.opzioni=(o.opzioni||[]).filter(s=>s.id!==n))})}function hn(t){Ai(t)}function zn(t){I(t,function(i){i.sezioni=i.sezioni||[],i.sezioni.push({id:C(),nome:"Nuova sezione",componenti:[]})})}function wn(t){Si(t)}function _n(t,i,n,e){I(t,function(o){let s=(o.sezioni||[]).find(a=>a.id===i);s&&(s[n]=e.trim())},!1)}function $n(t,i){confirm("Eliminare questa sezione e tutti i suoi componenti?")&&I(t,function(n){n.sezioni=(n.sezioni||[]).filter(e=>e.id!==i)})}function Cn(t,i){I(t,function(n){let e=(n.sezioni||[]).find(o=>o.id===i);e&&(e.componenti=e.componenti||[],e.componenti.push({id:C(),nome:"Nuovo componente",codice:"",qtaPerVariante:{},caricato:0,modoComponente:"quantificato",tracciabile:!0,noteConfig:"",unitaMisura:"pz"}))})}function Sn(t,i,n,e,o,s){I(t,function(a){let d=(a.sezioni||[]).find(l=>l.id===i),c=d&&(d.componenti||[]).find(l=>l.id===n);if(c){if(e==="coeff"||e==="flag"){c.qtaPerVariante=c.qtaPerVariante||{},c.qtaPerVariante[o]=J(s);return}if(e==="modo"){c.modoComponente=s==="segnalazione"?"segnalazione":"quantificato",c.modoComponente==="segnalazione"?(c.tracciabile=!1,c.unitaMisura="flag"):c.unitaMisura==="flag"&&(c.unitaMisura="pz");return}if(e==="unitaMisura"){c.unitaMisura=c.modoComponente==="segnalazione"?"flag":yt(s,"pz");return}c[e]=s.trim()}},e!=="nome"&&e!=="noteConfig")}function In(t,i,n,e,o){I(t,function(s){let a=(s.sezioni||[]).find(l=>l.id===i),d=a&&(a.componenti||[]).find(l=>l.id===n);if(!d)return;let c=Tt(d,s);if(e==="tipo"){if(c.tipo=o==="gruppo"?"gruppo":"sempre",c.tipo==="gruppo"&&!c.asseId){c.asseId=s.assiConfigurazione?.[0]?.id||"";let l=(s.assiConfigurazione||[]).find(m=>m.id===c.asseId);c.opzioneIds=l?.opzioni?.length?[l.opzioni[0].id]:[]}}else if(e==="qtyBase")c.qtyBase=J(o);else if(e==="asseId"){c.asseId=String(o||"");let l=(s.assiConfigurazione||[]).find(m=>m.id===c.asseId);c.opzioneIds=l?.opzioni?.length?[l.opzioni[0].id]:[],c.tipo="gruppo"}d.applicazioneTipo=c.tipo,d.applicazioneAsseId=c.asseId,d.applicazioneOpzioneIds=c.opzioneIds,d.qtaBase=c.qtyBase,d.qtaPerVariante=Ot(d,s,c)})}function xn(t,i,n,e,o){I(t,function(s){let a=(s.sezioni||[]).find(m=>m.id===i),d=a&&(a.componenti||[]).find(m=>m.id===n);if(!d)return;let c=Tt(d,s),l=new Set(c.opzioneIds||[]);o?l.add(e):l.delete(e),c.tipo="gruppo",c.opzioneIds=[...l],d.applicazioneTipo=c.tipo,d.applicazioneAsseId=c.asseId,d.applicazioneOpzioneIds=c.opzioneIds,d.qtaBase=c.qtyBase,d.qtaPerVariante=Ot(d,s,c)})}function An(t,i,n,e){I(t,function(o){let s=(o.sezioni||[]).find(d=>d.id===i),a=s&&(s.componenti||[]).find(d=>d.id===n);!a||B(a)||(a.tracciabile=!!e)},!1)}function Mn(t,i,n){I(t,function(e){let o=(e.sezioni||[]).find(s=>s.id===i);o&&(o.componenti=(o.componenti||[]).filter(s=>s.id!==n))})}function En(t){I(t,function(i){i.sottoAssembly=i.sottoAssembly||[],i.sottoAssembly.push({id:C(),nome:"",varianteKey:M(i)[0]?.key||""})})}function On(t,i){I(t,function(n){n.sottoAssembly=n.sottoAssembly||[],n.sottoAssembly.push({id:C(),nome:"",varianteKey:i,noteConfig:""})})}function qn(t,i,n,e){I(t,function(o){o.sottoAssembly[i]&&(o.sottoAssembly[i][n]=e.trim())},!1)}function Nn(t,i){I(t,function(n){n.sottoAssembly.splice(i,1)})}function Tn(t){let i=document.getElementById("modal-kit-distinta-edit");if(!i){vi(t);return}let{kits:n}=z(),e=n.find(c=>c.id===t);if(!e)return;let o=Q(e),s=R(o),a=document.getElementById("distinta-edit-nome"),d=document.getElementById("distinta-edit-documento");a&&(a.value=s.documento||""),d&&(d.value=s.documento||""),i.dataset.kitId=t,i.style.display="flex",i.offsetHeight,i.classList.add("active"),setTimeout(()=>a&&a.focus(),80)}function At(){let t=document.getElementById("modal-kit-distinta-edit");t&&(t.classList.remove("active"),setTimeout(()=>{t.classList.contains("active")||(t.style.display="none")},300))}function Bn(){let t=document.getElementById("modal-kit-distinta-edit");if(!t)return;let i=t.dataset.kitId,n=(document.getElementById("distinta-edit-nome")?.value||"").trim(),e=(document.getElementById("distinta-edit-documento")?.value||"").trim();if(!n){y("Inserisci un nome per la distinta.","warning");return}L(i,function(m){let p=R(m);e?p.documento=e:p.documento||(p.documento=n),zt(m,p)});let{kits:o}=z(),s=o.find(m=>m.id===i);if(!s){At(),y("Kit non trovato \u26A0\uFE0F");return}let a=Q(s),d=wt(s,a);if(!d.totalePezzi||!d.totaleRighe){y("Componi prima un ordine per generare la distinta stampabile.","warning");return}let c=W(),l={id:C(),kitId:s.id,kitNome:s.nome,nome:n||a._meta?.documento||`Distinta-${Date.now()}`,documento:e||a._meta?.documento||"",createdAt:Date.now(),createdBy:P?.nome||"Sistema",orderDraftSnapshot:a,distintaSnapshot:d};c.unshift(l),ht(c),At(),y("Distinta salvata \u2713"),N==="distinte"&&Z("distinte")}function Vn(){window._kitOpenView=ve,window._kitOpenConfig=Ft,window._kitNuovoKit=Fe,window._kitBack=be,window._kitOpenPrintPreview=se,window._kitSwitchTab=ye,window._kitAggiornaQty=he,window._kitOrdineSet=ze,window._kitOrdineDelta=we,window._kitOrdineReset=_e,window._kitOrdineResetVoce=$e,window._kitOrderSearch=Ce,window._kitOrderHideSearch=Se,window._kitOrderPick=Ie,window._kitOrderRemoveRef=xe,window._kitComposeSelect=Ae,window._kitComposeAdd=Me,window._kitAggiornaCar=hi,window._kitAggiornaPronti=Ee,window._kitSetPronti=Oe,window._kitApriModalSped=Le,window._kitChiudiModalSped=$i,window._kitConfermaSpedizione=Pe,window._kitApriModalReso=Re,window._kitChiudiModalReso=Ci,window._kitResoQtyChange=He,window._kitResoAggiornaBOM=jt,window._kitConfermaReso=je,window._kitSalvaMovimento=Ne,window._kitEliminaMovimento=Te,window._kitModificaMovimento=Ke,window._kitChiudiModalEditMov=_i,window._kitConfermaModificaMov=De,window._kitChiudiModalDelMov=zi,window._kitConfermaEliminaMov=wi,window._kitSalvaManuale=Ve,window._kitElimina=un,window._kitDuplicaKit=fn,window._kitCfgBack=ln,window._kitCfgSwitchTab=pn,window._kitCfgSaveNome=mn,window._kitCfgAddVar=hn,window._kitCfgOpenImportModal=Si,window._kitCfgOpenImportAsseModal=Ue,window._kitCfgOpenCopySezModal=Qe,window._kitCfgCloseImportModal=et,window._kitCfgSetImportMode=Ge,window._kitCfgSetImportSearch=Je,window._kitCfgSelectImportSource=Ye,window._kitCfgSelectImportSection=We,window._kitCfgToggleImportTarget=Ze,window._kitCfgSelectAllImportTargets=Xe,window._kitCfgClearImportTargets=tn,window._kitCfgConfirmImport=en,window._kitOpenPresetsModal=nn,window._kitClosePresetsModal=Ii,window._kitSetPresetsSearch=on,window._kitSelectPreset=sn,window._kitCreatePresetFromSection=an,window._kitCreatePreset=xi,window._kitApplyPreset=rn,window._kitRenamePreset=cn,window._kitDeletePreset=dn,window._kitCfgAddAsse=Ai,window._kitCfgUpdateAsse=gn,window._kitCfgDelAsse=kn,window._kitCfgAddOpzione=vn,window._kitCfgUpdateOpzione=bn,window._kitCfgDelOpzione=yn,window._kitCfgAddSez=zn,window._kitCfgImportSez=wn,window._kitCfgUpdateSez=_n,window._kitCfgDelSez=$n,window._kitCfgAddComp=Cn,window._kitCfgUpdateComp=Sn,window._kitCfgUpdateCompRule=In,window._kitCfgToggleCompOption=xn,window._kitCfgToggleCompTracked=An,window._kitCfgDelComp=Mn,window._kitCfgAddSA=En,window._kitCfgAddSAForVariant=On,window._kitCfgUpdateSA=qn,window._kitCfgDelSA=Nn,window._kitSwitchMainTab=Z,window._kitRenderKitsGrid=fi,window._kitRenderAnagrafichePage=gi,window._kitRenderDistintePage=ki,window._kitLoadDistinte=W,window._kitSaveDistinte=ht,window._kitCreateDistintaFromDraft=vi,window._kitLoadAnagrafiche=mt,window._kitSaveAnagrafiche=Ht,window._kitOpenAnagraficaModal=pe,window._kitCloseAnagraficaModal=bi,window._kitConfirmSaveAnagrafica=me,window._kitDeleteAnagrafica=ue,window._kitRenderHeaderActions=Rt,window._kitOpenSaveDistintaModal=Tn,window._kitCloseSaveDistintaModal=At,window._kitConfirmSaveDistinta=Bn,window._kitDistintaOpenPrint=fe,window._kitDistintaApplyToDraft=ge,window._kitDistintaDelete=ke}var Mt,gt,Wt,Gt,Zt,Et,Li,Xt,Pi,ti,Ri,xt,G,ct,It,N,dt,Yt,q,yi,Vt,F,w,A,Fn,Kn=Ni(()=>{Ti();Ki();Di();Bi();Mt="_mlKitData",gt="_mlKitDataTs",Wt="_mlKitOrderDrafts",Gt="_mlKitOrderDraftSeq",Zt="_mlKitPresetSections",Et=2,Li=["pz","mt","cm","mm","kg","g","lt","ml"],Xt="_mlKitDistinte",Pi="_mlKitDistinteTs",ti="_mlKitAnagrafiche",Ri="_mlKitAnagraficheTs",xt=!1,G=[],ct=null,It={},N="kits";dt={};Yt=null;q=null,yi="ordine";Vt=null,F="info",w=null,A=null;Fn=st});Kn();export{st as caricaKitProdotti,Fn as default,Vn as registerGlobals,jn as resetKitFetch};
//# sourceMappingURL=chunk-kit-prodotti-YMTIDW66.js.map
