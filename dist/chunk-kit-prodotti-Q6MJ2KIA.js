...
<body>
...
</html>`}function Oi(t){let{kits:i}=w(),n=i.find(a=>a.id===t);if(!n)return;let e=ct(n),o=jt(n,e);if(!o.totalePezzi||!o.totaleRighe){A("Componi prima un ordine per generare la distinta stampabile.","warning");return}Y(e).documento||(R(t,function(a){wi(a)}),e=ct(n));let s=window.open("","_blank");if(!s){A("Popup bloccato: abilita l'anteprima di stampa per aprire il modello completo.","warning");return}s.document.open(),s.document.write(Mi(n,o,e)),s.document.close(),s.focus()}function w(){try{let t=localStorage.getItem(vt);if(!t)return{kits:[]};let i=JSON.parse(t);return{kits:Array.isArray(i?.kits)?i.kits.map(Lt):[]}}catch{return{kits:[]}}}function O(t){let i=Array.isArray(t)?t.map(Lt):[];try{localStorage.setItem(vt,JSON.stringify({kits:i})),localStorage.setItem(rt,Date.now())}catch{}qi(i)}function qi(t){clearTimeout(qt),qt=setTimeout(function(){ut({azione:"setKitData",kits:t}).catch(function(i){console.warn("[kit-prodotti] salvataggio remoto fallito:",i)})},1500)}function Ei(t){fetch(mt,{method:"POST",body:JSON.stringify({azione:"getKitData"})}).then(i=>i.json()).then(i=>{if(i&&Array.isArray(i.kits)){let n=parseInt(i.ts||0),e=parseInt(localStorage.getItem(rt)||0);if(n>0&&n>e){try{localStorage.setItem(vt,JSON.stringify({kits:i.kits}))}catch{}try{localStorage.setItem(rt,n)}catch{}t&&t(!0);return}}t&&t(!1)}).catch(()=>{t&&t(!1)})}function I(){return Math.random().toString(36).slice(2,10)+Date.now().toString(36)}function Ct(){if(!Q||!Q.nome)return!1;let t=String(Q.nome).toUpperCase().trim();return t==="ALESSIO"||t==="0000"||Q.ruolo==="MASTER"}function Ni(t){let i={};for(let n of t.sezioni||[])for(let e of n.componenti||[]){if(N(e)){i[e.id]=0;continue}let o=0;for(let[s,a]of Object.entries(t.qtaDaProdurre||{}))o+=(Number.parseInt(a,10)||0)*E(e,s);i[e.id]=o}return i}function Bi(t){let i={};for(let n of t.sottoAssembly||[]){let e=Number.parseInt(t.pronti?.[n.id],10)||0;if(!e)continue;let o=n.varianteKey;for(let s of t.sezioni||[])for(let a of s.componenti||[]){if(N(a))continue;let d=E(a,o);d>0&&(i[a.id]=(i[a.id]||0)+e*d)}}return i}function Ut(t,i){let n=S(t).find(e=>e.key===i);return n?r(n.nome):r(i)}function $t(){return new Date().toLocaleString("it-IT",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"})}function W(){gt||(gt=!0,Ei(function(e){e&&W()}));let{kits:t}=w(),i=document.getElementById("contenitore-dati"),n=t.map(e=>{let s=S(e).length,a=(e.assiConfigurazione||[]).length,d=(e.sezioni||[]).reduce((c,l)=>c+(l.componenti||[]).length,0);return`
    ...
    <div class="kit-page">
        ...
        ${t.length===0?`<div class="kit-empty-state">
    ...
    </div>`;try{if(window&&window._kitSuppressNextFade)try{delete window._kitSuppressNextFade}catch{}else X(i)}catch{X(i)}}function Ti(t){M=t,Ft="ordine",K()}function K(){let{kits:t}=w(),i=t.find(u=>u.id===M);if(!i){W();return}let n=document.getElementById("contenitore-dati"),e=S(i),o=ct(i),s=Y(o),a=jt(i,o),d=a.selectedVarianti.length?a.selectedVarianti.map(u=>`<span class="kit-meta-pill"><strong>${j(o,u.key)}</strong> \xD7 ${r(u.nome)}</span>`).join(""):'<span class="kit-leg-item" style="color:#94a3b8">Nessuna configurazione selezionata.</span>',c=s.ordiniCliente.length?s.ordiniCliente.map(u=>`<span class="kit-order-ref-chip">${r(u)}
            ...
            </span>`).join(""):'<div class="kit-order-meta-empty">Nessun ordine cliente collegato.</div>',l=wt(i),p=Ht(i,l),m=(i.assiConfigurazione||[]).length?(i.assiConfigurazione||[]).map(u=>`
            <div class="kit-compose-group">
                ...
                <div class="kit-order-line-main">
                    ...
                    <div class="kit-order-line-meta">${Array.isArray(u.selections)&&u.selections.length?u.selections.map(B=>r(B.opzioneNome)).join(" \xB7 "):r(u.key)}</div>
                </div>
    ...
    `).join(""),n.style.display="block"}}function ji(t,i){let n=String(i||"").trim().toLowerCase();if(!n){dt(t,[]);return}Dt().then(function(e){let o=e.filter(s=>s.ordine.toLowerCase().includes(n)||s.cliente.toLowerCase().includes(n)).slice(0,8);dt(t,o)})}function Ui(t){setTimeout(function(){dt(t,[])},140)}function Fi(t,i,n){let e=J(i);if(!e)return;try{window._kitSuppressNextFade=!0}catch{}R(t,function(s){let a=Y(s);a.ordiniCliente=[...new Set(a.ordiniCliente.concat(e))],a.cliente=Vt(a.ordiniCliente,{[e]:n}),zt(s,a)});let o=document.getElementById("kit-order-ref-input-"+t);o&&(o.value=""),dt(t,[])}function Qi(t,i){let n=J(i);try{window._kitSuppressNextFade=!0}catch{}R(t,function(e){let o=Y(e);o.ordiniCliente=o.ordiniCliente.filter(s=>s!==n),o.cliente=Vt(o.ordiniCliente),zt(e,o)})}function Gi(t,i,n){let{kits:e}=w(),o=e.find(a=>a.id===t);if(!o)return;let s=wt(o);if(s[i]=n,it[t]=s,M===t){try{window._kitSuppressNextFade=!0}catch{}K()}}function Ji(t){let{kits:i}=w(),n=i.find(a=>a.id===t);if(!n)return;let e=Ht(n,wt(n));if(!e){A("Completa prima le scelte elettroniche \u26A0\uFE0F");return}let o=Math.max(0,Number.parseInt(document.getElementById("kit-compose-qty-"+t)?.value,10)||0);if(!o){A("Inserisci una quantit\xE0 valida \u26A0\uFE0F");return}if(ft[t])return;ft[t]=Date.now(),setTimeout(function(){try{delete ft[t]}catch{}},600);try{window._kitSuppressNextFade=!0}catch{}R(t,function(a){a[e.key]=j(a,e.key)+o});let s=document.getElementById("kit-compose-qty-"+t);s&&(s.value=1)}function Qt(t){let i=t.dataset.cid,n=t.dataset.sid,e=Math.max(0,Number.parseInt(t.value,10)||0),{kits:o}=w(),s=o.find(z=>z.id===M);if(!s)return;let a=(s.sezioni||[]).find(z=>z.id===n),d=a&&(a.componenti||[]).find(z=>z.id===i);if(!d||!yt(d))return;d.caricato=e,O(o);let l=Ni(s)[i]||0,p=Math.max(0,l-e),k=Bi(s)[i]||0,g=t.closest("tr");if(!g)return;let b=g.querySelector(".kit-ord-zero,.kit-ord-manca,.kit-ord-ok");b&&(b.textContent=l===0?"\u2014":p,b.className=l===0?"kit-ord-zero":p>0?"kit-ord-manca":"kit-ord-ok");let u=g.querySelector(".kit-car-liberi");u&&(k>0?(u.textContent=Math.max(0,e-k)+" lib.",u.style.display=""):u.style.display="none")}function Yi(t,i,n){let{kits:e}=w(),o=e.find(s=>s.id===t);o&&(o.pronti||(o.pronti={}),o.pronti[i]=Math.max(0,(Number.parseInt(o.pronti[i],10)||0)+n),O(e),M===t&&K())}function Wi(t,i,n){let{kits:e}=w(),o=e.find(a=>a.id===t);if(!o)return;o.pronti||(o.pronti={}),o.pronti[i]=Math.max(0,Number.parseInt(n,10)||0),O(e);let s=document.querySelector(`.kit-pronti-input[data-said="${i}"]`);s&&(s.value=o.pronti[i],s.classList.toggle("kit-pronti-val-on",o.pronti[i]>0))}function Zi(t,i){let n=t.movimenti||[];return n.length?n.map(e=>{let o=i?`<button type="button" class="kit-mov-del" onclick="_kitEliminaMovimento('${r(t.id)}',${e.id})" title="Elimina">\u2715</button>`:'<span style="width:22px;flex-shrink:0"></span>',s=i&&(e.tipo==="carico"||e.tipo==="scarico")?`<button type="button" class="kit-mov-edit" onclick="_kitModificaMovimento('${r(t.id)}',${e.id})" title="Modifica">\u270E</button>`:'<span style="width:22px;flex-shrink:0"></span>';if(e.tipo==="spedizione"){let a=(e.righe||[]).reduce((l,p)=>l+p.qty,0),d=(e.righe||[]).map(l=>`<div class="kit-assemb-sub-row"><span class="kit-assemb-sub-mat" style="color:#94a3b8">${r(l.mat)}</span><span class="kit-mov-qty scarico">\u2212${l.qty}</span></div>`).join(""),c=(e.items||[]).map(l=>`<div class="kit-assemb-sub-row kit-sped-item-row"><span class="kit-assemb-sub-mat">${r(l.nome)}</span><span class="kit-mov-qty scarico">\xD7${l.qty}</span></div>`).join("");return`<details class="kit-mov-assemb-group">
        ...
        </div>`,at.innerHTML=p.length?p.map(f=>{let _=(e.targetKitIds||[]).includes(f.id),T=c?et(o,f):null,F=`${(f.sezioni||[]).length} sezioni`;return T&&(T.hasTargetVarianti?T.needsReview?F=`${T.exactMatches}/${T.targetCount} combinazioni allineate`:F=`${T.targetCount}/${T.targetCount} combinazioni allineate`:F="nessuna combinazione: rifinisci dopo"),`<label class="kit-import-option ${_?"kit-import-option--active":""}">
        ...
        <div class="kit-cfg-section">
            ...
            ${g}
        </div>`,u=(i.sezioni||[]).map((v,q)=>{let L=(v.componenti||[]).map(y=>{let C=N(y),f=ht(y,i),_=(e||[]).find(x=>x.id===f.asseId)||null,T=f.tipo==="gruppo"&&_?`<div class="kit-cfg-row">${(_.opzioni||[]).map(x=>`<label class="kit-meta-pill">
                   ...
                   </select>`:"",ei=f.tipo==="manuale"?'<div class="kit-cfg-warn">Questa parte usa ancora una configurazione avanzata precedente. Appena la modifichi verr\xE0 convertita nel nuovo schema semplice.</div>':"",xt=C?"flag":pt(y.unitaMisura,"pz"),ni=C?[{value:"flag",label:"Solo avviso"}]:[...new Set([xt,...di])].filter(Boolean).map(x=>({value:x,label:x}));return`<div class="kit-cfg-sa-group" style="padding:12px 14px">
        ...
        <div class="kit-cfg-section">
            ...
            </div>`}).join(""):B='<div class="kit-cfg-warn">\u26A0\uFE0F Aggiungi prima almeno un asse con opzioni nella tab <strong>Assi di configurazione</strong>.</div>';let at=`
        <div class="kit-cfg-section">
            <div class="kit-cfg-help">
            ...
            ${B}
        </div>`,Z={info:m,varianti:b,bom:z,sa:at},U=s.map(v=>`<button class="kit-tab ${D===v?"kit-tab--active":""}" onclick="_kitCfgSwitchTab('${v}')">${a[v]}</button>`).join("");n.innerHTML=`
    <div class="kit-page">
        <div class="kit-view-header">
